...
Public Class wSMPScanBarcodeSendSupl
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        _Str &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "]..TSMPTSendSuplToBranch AS M WITH (NOLOCK)"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " 	,''  AS FNHSysOperationId"
            _Qry &= vbCrLf & " 	,'' AS FNSeq"
            _Qry &= vbCrLf & " 	,''  AS FNHSysOperationIdTo"
            ...
            '_Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTLayCut AS B  WITH (NOLOCK) ON AA.FTLayCutNo = B.FTLayCutNo "
            ...
            '_Qry &= vbCrLf & " 	        INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTOrderProd_SendSupl AS SD WITH (NOLOCK)  ON A.FTSendSuplRef = SD.FTSendSuplRef AND ODP.FTOrderProdNo = SD.FTOrderProdNo and A.FNHSysPartId = SD.FNHSysPartId and A.FNSendSuplType = SD.FNSendSuplType "
            '_Qry &= vbCrLf & " 	        LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTMOperationByStyle AS OPS WITH (NOLOCK)  ON O.FNHSysStyleId = OPS.FNHSysStyleId AND SD.FNHSysOperationId = OPS.FNHSysOperationId  "
            _Qry &= vbCrLf & " 	        LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode AS B  WITH (NOLOCK) ON A.FTBarcodeSendSuplNo = B.FTBarcodeSendSuplNo "
            ...
            '_Qry &= vbCrLf & " 	 LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTOperationByOrderProd AS OPP WITH (NOLOCK)  ON ODP.FTOrderProdNo = OPP.FTOrderProdNo AND SD.FNHSysOperationId = OPP.FNHSysOperationId "
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'"
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch  WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "'  "
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode  WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "'  "
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'")
    ...
    Private Function CheckCreatePurchaseSendSuplier() As Boolean
        ...
        '_Qry &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTPurchaseSendSupl_DocSendRef AS A WITH(NOLOCK)"
...
#Region "MAIN PROC"
    ...
    Private Sub Proc_Delete(sender As System.Object, e As System.EventArgs) Handles ocmdelete.Click
        ...
        If Me.FTSendSuplNo.Text.Trim <> "" And Me.FTSendSuplNo.Properties.Tag.ToString <> "" Then
            ...
            _Qry &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl_Barcode AS A WITH(NOLOCK) INNER JOIN"
            _Qry &= vbCrLf & "      [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode AS B  WITH(NOLOCK)  ON A.FTBarcodeSendSuplNo = B.FTBarcodeSendSuplNo"
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTSendSuplNo.Text <> "" Then
            ...
            Dim cmd As String = ""
            cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.[SP_CREATE_TEMPSENDSUPL_SMP] '" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'"
            HI.Conn.SQLConn.ExecuteOnly(cmd, Conn.DB.DataBaseName.DB_SAMPLE)

            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "SampleRoom\"
                .ReportName = "SendSuplSlip_Smp.rpt"
                .Formular = "{TSMPTSendSupl.FTSendSuplNo}='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "' "
                .Preview()
            End With
        Else
...
#Region " Proc "
    ...
    Private Function SaveBarcode(Key As String) As Boolean
        ...
        Try
            ...
            _Str = " SELECT TOP 1 FTBarcodeSendSuplNo  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode WITH(NOLOCK) WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "' AND FTBarcodeSendSuplNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
            ...
            If _StateNew Then
                ...
                _Str = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode(FTInsUser, FDInsDate, FTInsTime, FTSendSuplNo, FTBarcodeSendSuplNo)  "
            ...
            Else
                ...
                _Str = " UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode "
    ...
    Private Function DeleteBarcode(BarcodeKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode  WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "' AND FTBarcodeSendSuplNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode  WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "' AND FTBarcodeSendSuplNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

            _Str = " DELETE A "
            _Str &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch  AS A "
            _Str &= vbCrLf & "  LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode  AS B ON A.FTSendSuplNo = B.FTSendSuplNo "
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " 	     RIGHT OUTER JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTBarcode_SendSupl AS A WITH (NOLOCK)  ON O.FTSMPOrderNo =  A.FTOrderProdNo  "
            _Qry &= vbCrLf & " 	     INNER JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTBundle AS BB WITH (NOLOCK)  ON A.FTBarcodeBundleNo = BB.FTBarcodeBundleNo "
            ''_Qry &= vbCrLf & " 	     INNER JOIN     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTOrderProd_SendSupl AS SD WITH (NOLOCK)  ON A.FTSendSuplRef = SD.FTSendSuplRef AND ODP.FTOrderProdNo = SD.FTOrderProdNo  "
            ''_Qry &= vbCrLf & " 	     LEFT OUTER JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTMOperationByStyle AS OPS WITH (NOLOCK)  ON O.FNHSysStyleId = OPS.FNHSysStyleId AND SD.FNHSysOperationId = OPS.FNHSysOperationId "
            _Qry &= vbCrLf & " 	     LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode AS B  WITH (NOLOCK) ON A.FTBarcodeSendSuplNo = B.FTBarcodeSendSuplNo "
            _Qry &= vbCrLf & " 	     LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS S WITH (NOLOCK) ON A.FNHSysSuplId = S.FNHSysSuplId"
            _Qry &= vbCrLf & " 		    "
            ...
            If (_StatePass) Then
                ...
                If SaveBarcode(Key) Then
                    ...
                    If Me.FTStateScanSendFinish.Checked Then
                        _Cmd = "Update TSMPTSendSupl Set FTStateScanSendFinish='0'  "
                    ...
                    If Me.FNHSysCmpIdTo.Text.Trim <> "" Then
                        ...
                        If Me.FTStateScanSendFinish.Checked Then
                            _Cmd = "Update TSMPTSendSuplToBranch Set FTStateScanSendFinish='0'  "
                            _Cmd &= vbCrLf & "Where FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "'"
                            HI.Conn.SQLConn.ExecuteOnly(_Cmd, Conn.DB.DataBaseName.DB_SAMPLE)
                        End If
                    End If
    ...
    Private Sub SaveSendSuplToBranch(DocKey As String, BarKey As String)
        ...
        If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_SAMPLE, "") = "" Then
            ...
            _Qry = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch("
            ...
            _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl AS A WITH(NOLOCK) "
            _Qry &= vbCrLf & " WHERE FTSendSuplNo='" & HI.UL.ULF.rpQuoted(DocKey) & "' "
        ...
        _Qry = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode("
...
#End Region
    ...
    Private Function VerrifyState() As Boolean
        Try
            Dim _Cmd As String = ""
            _Cmd = "Select Top 1 Isnull(FTStateScanSendFinish,'0') as FTStateScanSendFinish ,isnull( FTStateSendExcel ,'0') AS FTStateSendExcel  From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl with(nolock) "
            _Cmd &= vbCrLf & "where FTSendSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'"
            For Each R As DataRow In HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_SAMPLE).Rows
                ...
                If _Barcode <> "" Then
                    ...
                    If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_SAMPLE, "") = "" Then
                        ...
                        If _Barcode <> "" Then
                            ...
                            '' With New PROD
                            '_StyleCode = .GetStyleCodeByOrderNoSMP(_OderNo)
    ...
    Private Sub ocmFinish_Click(sender As Object, e As EventArgs) Handles ocmFinish.Click
        Try
            ...
            If Not (Me.FTStateScanSendFinish.Checked) Then
                ...
                _Cmd = "Update TSMPTSendSupl Set FTStateScanSendFinish='1'  "
                _Cmd &= vbCrLf & ",FDDateSendFinish=" & HI.UL.ULDate.FormatDateDB
                _Cmd &= vbCrLf & "Where FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "'"
                HI.Conn.SQLConn.ExecuteOnly(_Cmd, Conn.DB.DataBaseName.DB_SAMPLE)
                _Cmd = "Update TSMPTSendSuplToBranch Set FTStateScanSendFinish='1'  "
                _Cmd &= vbCrLf & ",FDDateSendFinish=" & HI.UL.ULDate.FormatDateDB
                _Cmd &= vbCrLf & "Where FTSendSuplNo='" & HI.UL.ULF.rpQuoted(FTSendSuplNo.Text) & "'"
                HI.Conn.SQLConn.ExecuteOnly(_Cmd, Conn.DB.DataBaseName.DB_SAMPLE)
    ...
    Private Sub updateProcess(Barcode As String)
        ...
        With CType(Me.ogcdetail.DataSource, DataTable)
            ...
            Try
                ...
                For Each R As DataRow In .Select("FNQuantity>0 and FTBarcodeSendSuplNo='" & Barcode & "'")
                    ...
                    If HI.Conn.SQLConn.GetDataTableOnbeginTrans(_Qry).Rows.Count <= 0 Then
                        ...
                        _Qry &= vbCrLf & " ,'" & HI.UL.ULDate.ConvertEnDB(Me.FDSendSuplDate.Text) & "'"
