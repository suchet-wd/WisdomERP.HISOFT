...
Public Class wMERSampleApprove

    Private _popupReason As wPopUpReason
    Sub New()
        ...
        _popupReason = New wPopUpReason()

        HI.TL.HandlerControl.AddHandlerObj(_popupReason)

        Dim oSysLang As New HI.ST.SysLanguage

        Try
            Call oSysLang.LoadObjectLanguage(HI.ST.SysInfo.ModuleID, _popupReason.Name.ToString.Trim, _popupReason)
...
#End Region
    ...
    Private Sub LoadOrderProdDetail()
        ...
        cmd &= vbCrLf & "    Select    FTStateApp as FTMerAppState , FTStateReject as FTRejectState ,  FTAppBy as FTMerAppBy , FDAppDate as FDMerAppDate , FTAppTime  as FTMerAppTime  ,FTRemark  as FTMerRemark  "
        ...
        If FTQCFinalStart.Text <> "" Or FTQCFinalEND.Text <> "" Then
            ...
            cmd &= vbCrLf & "    From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPSampleQC As X  With(NOLOCK)"
            cmd &= vbCrLf & "  WHERE 1=1 --FNSampleState=11 -- AND ISNULL(FTTeam,'') <> '' "

            If FTQCFinalStart.Text <> "" Then
                cmd &= vbCrLf & " And X.FTDate >= '" & HI.UL.ULDate.ConvertEnDB(FTQCFinalStart.Text) & "' "
            End If

            If FTQCFinalEND.Text <> "" Then
                cmd &= vbCrLf & " And X.FTDate <= '" & HI.UL.ULDate.ConvertEnDB(FTQCFinalEND.Text) & "' "
            End If
    ...
    Private Sub ocmapprove_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        Try
            ...
            If _odt.Select("FTSelect  = '1'  and  FTMerAppState = '1' ").Length > 0 Then
                HI.MG.ShowMsg.mInfo("คุณเลือกข้อมูลที่มีการอนุมัติแล้ว กรุณาตรวจสอบ  ....", 23005040855, Me.Text)
                Exit Sub
            End If
            ...
            Dim _RemarkMer As String = ""
            HI.TL.HandlerControl.ClearControl(_popupReason)
            With _popupReason
                .StartPosition = FormStartPosition.CenterScreen
                .FTRemark.Text = ""
                .ShowDialog()
                If (.proc) Then
                    _RemarkMer = .FTRemark.Text
            ...
            Try
                ...
                For Each R As DataRow In _odt.Select("FTSelect = '1'")
                    _Qry = "Update  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPSampleQCMER "
                    _Qry &= vbCrLf & " SET   FTStateApp='1' , FTStateReject='0' "
                    _Qry &= vbCrLf & ",FTAppBy='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                    _Qry &= vbCrLf & ",FDAppDate=" & HI.UL.ULDate.FormatDateDB & " "
                    _Qry &= vbCrLf & ",FTAppTime=" & HI.UL.ULDate.FormatTimeDB & " "
                    _Qry &= vbCrLf & ",FTRemark='" & HI.UL.ULF.rpQuoted(_RemarkMer) & "'"
                    _Qry &= vbCrLf & " where FTTeam ='" & HI.UL.ULF.rpQuoted(R!FTTeam) & "' "

                    If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then


                        _Qry = " insert into  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPSampleQCMER  "
                        _Qry &= vbCrLf & " (FTInsUser, FDInsDate, FTInsTime,  FTSMPOrderNo, FTTeam, FTDate, FNSeq, FTSizeBreakDown, FTColorway, FNQuantity,   FTRemark , FTStateApp, FTStateReject, FTAppBy, FDAppDate, FTAppTime) "
                        ...
                        _Qry &= vbCrLf & ", '" & HI.UL.ULF.rpQuoted(_RemarkMer) & "' "
        ...
        Try
            ...
            <tr style='height:28.4pt'>
                ...
                <td width=154 valign=top
                    ...
                    <p class=MsoNormal align=center style='text-align:center'><b>Date Mer Reject<o:p></o:p></b></p>
                </td>
                <td width=142 valign=top
                    style='width:3.0cm;border:solid windowtext 1.0pt;border-left:none;padding:0cm 5.4pt 0cm 5.4pt;height:28.4pt'>
                    <p class=MsoNormal align=center style='text-align:center'><b>Reject By<o:p></o:p></b></p>
                </td>
                <td width=142 valign=top
                    style='width:3.0cm;border:solid windowtext 1.0pt;border-left:none;padding:0cm 5.4pt 0cm 5.4pt;height:28.4pt'>
                    <p class=MsoNormal align=center style='text-align:center'><b>Reject Reason<o:p></o:p></b></p>
                </td>
    ...
    Private Sub ocmreject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        Try
            ...
            Try
                ...
                For Each R As DataRow In _odt.Select("FTSelect = '1'")
                    _Qry = "Update  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPSampleQCMER "
                    _Qry &= vbCrLf & " SET   FTStateApp='0' , FTStateReject='1' "
                    _Qry &= vbCrLf & ",FTAppBy='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                    _Qry &= vbCrLf & ",FDAppDate=" & HI.UL.ULDate.FormatDateDB & " "
                    _Qry &= vbCrLf & ",FTAppTime=" & HI.UL.ULDate.FormatTimeDB & " "
                    _Qry &= vbCrLf & ",FTRemark='" & HI.UL.ULF.rpQuoted(_RemarkMer) & "'"
                ...
                <td width=142 valign=top
                    style='width:3.0cm;border-top:none;border-left:none;border-bottom:solid windowtext 1.0pt;border-right:solid windowtext 1.0pt;padding:0cm 5.4pt 0cm 5.4pt;height:13.85pt'>
                    <p class=MsoNormal>
                       <o:p>" & HI.UL.ULF.rpQuoted(_RemarkMer) & "</o:p>
                    </p>
                </td>
                ...
                With mailItem
                    .Display()
                    .Subject = "MER REJECT SAMPLE ROOM  STYLE "
                    .To = _EmailTo
                    .CC = ""
                    mySignature = .HTMLBody
                    .HTMLBody = "Hi, Sample Room Team   " & _msg & mySignature
                End With
