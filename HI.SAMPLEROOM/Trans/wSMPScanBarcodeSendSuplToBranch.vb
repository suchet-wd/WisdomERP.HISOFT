...
Public Class wSMPScanBarcodeSendSuplToBranch
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        _Str &= vbCrLf & "    FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch AS A  WITH(NOLOCK)"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " 	,  '' as FTOperationCode --Mpp.FTOperationCode "
            ...
            _Qry &= vbCrLf & " 	 FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch AS A WITH (NOLOCK)   INNER JOIN"
            _Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode AS B  WITH (NOLOCK) ON A.FTSendSuplNo = B.FTSendSuplNo INNER JOIN"
            _Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder AS O WITH (NOLOCK) ON B.FTOrderNo=O.FTSMPOrderNo INNER JOIN "
            ...
            ''  _Qry &= vbCrLf & " 	 LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTOperationByOrderProd AS OPP WITH (NOLOCK)  ON B.FTOrderProdNo = OPP.FTOrderProdNo AND B.FNHSysOperationId = OPP.FNHSysOperationId "
...
#End Region
    ...
    Private Sub ocmapprove_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        ...
        With DirectCast(Me.ogcdetail.DataSource, DataTable)
            ...
            For Each R As DataRow In .Select("FTSelect='1'")
                _Strsql = "UPDate  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode "
        ...
        _Strsql = "UPDate  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch "
        _Strsql &= vbCrLf & " SET FTStateBranchAccept='1'"
        _Strsql &= vbCrLf & " ,FTStateBranchAcceptBy='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
        _Strsql &= vbCrLf & " ,FTStateBranchAcceptDate=" & HI.UL.ULDate.FormatDateDB & " "
        _Strsql &= vbCrLf & " ,FTStateBranchAcceptTime=" & HI.UL.ULDate.FormatTimeDB & " "
        ...
        _Strsql = "Select top 1  * From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl With(nolock) where FTSendSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'   "
        If HI.Conn.SQLConn.GetDataTable(_Strsql, Conn.DB.DataBaseName.DB_SAMPLE).Rows.Count <= 0 Then
            _Strsql = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl "
            ...
            _Strsql &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch  "
            ...
            HI.Conn.SQLConn.ExecuteOnly(_Strsql, Conn.DB.DataBaseName.DB_SAMPLE)
        End If
        _Strsql = "Select top 1  * From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode With(nolock) where FTSendSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTSendSuplNo.Text) & "'   "
        If HI.Conn.SQLConn.GetDataTable(_Strsql, Conn.DB.DataBaseName.DB_SAMPLE).Rows.Count <= 0 Then
            _Strsql = "insert into [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode (FTInsUser, FDInsDate, FTInsTime, FTUpdUser, FDUpdDate, FTUpdTime, FTSendSuplNo, FTBarcodeSendSuplNo)"
            _Strsql &= vbCrLf & "Select   FTInsUser, FDInsDate, FTInsTime, FTUpdUser, FDUpdDate, FTUpdTime, FTSendSuplNo, FTBarcodeSendSuplNo"
            _Strsql &= vbCrLf & "From TSMPTSendSuplToBranch_Barcode"
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            Me.FTSendSuplNo.Text = HI.Conn.SQLConn.GetField("Select Top 1 FTSendSuplNo From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSuplToBranch_Barcode Where FTBarcodeSendSuplNo='" & Key.ToString & "'", Conn.DB.DataBaseName.DB_SAMPLE, "")
