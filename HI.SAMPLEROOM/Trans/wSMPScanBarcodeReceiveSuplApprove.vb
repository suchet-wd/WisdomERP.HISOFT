...
Public Class wSMPScanBarcodeReceiveSuplApprove
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl As M WITH(NOLOCK)  "
    ...
    Private Sub LoadStateApp(Key As String)
        Try
            ...
            _Cmd &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "]..TSMPTReceiveSuplToBranch WITH(NOLOCK) "
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        Try
            ...
            '_Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTOrderProd AS ODP WITH (NOLOCK)  ON O.FTOrderNo = ODP.FTOrderNo RIGHT OUTER JOIN"
            '_Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TPRODBarcode_SendSupl AS A WITH (NOLOCK)  ON  ODP.FTOrderProdNo =  A.FTOrderProdNo  INNER JOIN"
            ...
            '_Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTOrderProd_SendSupl AS SD WITH (NOLOCK)  ON A.FTSendSuplRef = SD.FTSendSuplRef AND ODP.FTOrderProdNo = SD.FTOrderProdNo and A.FNHSysPartId = SD.FNHSysPartId and A.FNSendSuplType = SD.FNSendSuplType LEFT OUTER JOIN"
            '_Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TPRODMOperationByStyle AS OPS WITH (NOLOCK)  ON O.FNHSysStyleId = OPS.FNHSysStyleId AND SD.FNHSysOperationIdTo = OPS.FNHSysOperationId LEFT OUTER JOIN"
            '_Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl_Barcode AS B  WITH (NOLOCK) ON A.FTBarcodeSendSuplNo = B.FTBarcodeSendSuplNo LEFT OUTER JOIN"
    ...
    Private Function SaveDataSendBranch(ByVal _Key As String) As Boolean
        Try
            ...
            '_Cmd = " SELECT  TOP (1) S.FNHSysCmpId  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl AS S WITH (NOLOCK) INNER JOIN"
            '_Cmd &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTSendSupl_Barcode AS B WITH (NOLOCK) ON S.FTSendSuplNo = B.FTSendSuplNo"
    ...
    Private Function SaveBarcode(Key As String) As Boolean
        ...
        Try
            ...
            If _StateNew Then
                ...
                _Str = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl_Barcode(FTInsUser, FDInsDate, FTInsTime, FTRcvSuplNo, FTBarcodeSendSuplNo)  "
...
#End Region
        ...
        Try
            ...
            With DirectCast(Me.ogcdetail.DataSource, DataTable)
                ...
                For Each R As DataRow In .Select("FTSelect = '1'")
                    ...
                    _Cmd = "Update  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSuplToBranch_Barcode "
            ...
            _MailToUser = "Select Top 1  FTRcvSuplBy FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSuplToBranch  WHERE FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(_Key) & "'"

            If _StateApp = "1" Then
                _Cmd = "Select Top 1 FTRcvSuplNo From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl With(nolock) where FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(_Key) & "'"
                If HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_SAMPLE, "") = "" Then
                    _Cmd = " INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl"
                    ...
                    _Cmd &= vbCrLf & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSuplToBranch "
                    _Cmd &= vbCrLf & " WHERE FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(_Key) & "'"
                    ...
                    _Cmd = " INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl_Barcode"
                    ...
                    _Cmd &= vbCrLf & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSuplToBranch_Barcode "
                    _Cmd &= vbCrLf & " WHERE FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(_Key) & "'"
                    If HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                        'HI.Conn.SQLConn.Tran.Rollback()
                        'HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
            ...
            Else
                ...
                '_Cmd = " Delete From   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl"
                '_Cmd &= vbCrLf & " WHERE FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(_Key) & "'"
                'HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

                '_Cmd = " Delete From   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSupl_Barcode"
                '_Cmd &= vbCrLf & " WHERE FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(_Key) & "'"
                'HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            End If
        ...
        Try
            With Me.ogvdetail
                ...
                Else
                    ...
                    _Edit = True
                End If
                '    _Strsql = "UPDate  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSuplToBranch_Barcode "
                ...
                'HI.Conn.SQLConn.ExecuteOnly(_Strsql, Conn.DB.DataBaseName.DB_SAMPLE)
            End With
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            Me.FTRcvSuplNo.Text = HI.Conn.SQLConn.GetField("Select Top 1 FTRcvSuplNo From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTReceiveSuplToBranch_Barcode  where FTBarcodeSendSuplNo='" & Key.ToString & "'", Conn.DB.DataBaseName.DB_SAMPLE, "")
            ...
            '                _Qry = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.SP_CheckOperationBefore '" & HI.UL.ULF.rpQuoted(FTOrderProdNo.Text) & "'," & Integer.Parse(Val(dt.Rows(0) !FNHSysOperationIdTo.ToString)) & ",'" & HI.UL.ULF.rpQuoted(FTBarcodeNo.Text) & "'"
