...
#End Region
    ...
    Private Sub LoadOrderProdDetail()
        ...
        '    cmd &= "SELECT '" & HI.ST.SysInfo.CmpID & "' AS 'FNHSysCmpID' , '" & HI.ST.SysInfo.CmpID & "'  AS FNHSysCmpID_Hide "
        ...
        '    cmd &= vbCrLf & ", A.OrderStatus_Hide AS 'OrderStatus_Hide', C.FTCmpCode, '-' AS FTVenderPramCode"

        '    ' Start As A
        '    'OD.FNSeq,
        '    cmd &= vbCrLf & vbCrLf
        '    cmd &= vbCrLf & "FROM (Select A.FTSMPOrderNo, A.FDSMPOrderDate, A.FNSMPOrderType, A.FNSMPPrototypeNo, A.FTSMPOrderBy, A.FTPgmName, A.FNOrderSampleType, A.FTCustomerTeam "
        '    cmd &= vbCrLf & ", A.FNHSysCmpId, ISNULL(A.FNSMPOrderStatus,0) AS 'OrderStatus_Hide' "
        '    cmd &= vbCrLf & ", CASE WHEN ISNULL(A.FTRemark,'') <> '' THEN ISNULL(A.FTRemark,'') + char(30) ELSE  '' END AS FTOrderRemark  "
        '    cmd &= vbCrLf & ", MST.FTStyleCode, MSS.FTSeasonCode , MCT.FTCustCode, MCT.FTCustNameTH, MCT.FTCustNameEN, MMT.FTMerTeamCode,  OD.FTSizeBreakDown  "
        '    cmd &= vbCrLf & ", B.FTBuyCode "
        '    cmd &= vbCrLf & ", OD.FNQuantity, OD.FTRemark , MST.FTStyleNameEN AS 'FTStyleName', GD.FTGenderCode, GD.FTGenderNameEN  AS 'FTGenderName' "
        '    cmd &= vbCrLf & ", OD.FTPatternDate, OD.FTFabricDate, OD.FTAccessoryDate , OD.FTGACDate "
        '    cmd &= vbCrLf
        ...
        '    cmd &= vbCrLf & "where B.FTOrderNo = A.FTOrderNo  FOR XML PATH('')),1,1,'') AS FTSize "

        '    cmd &= vbCrLf & ", ISNULL(SUM(SB.FNQuantity),0) AS 'FNQuantity' "
        '    cmd &= vbCrLf & ", A.FTRemark AS 'FTRemark', A.FNJobState AS 'OrderStatus_Hide', C.FTCmpCode, v.FTVenderPramCode "
        ...
        '    cmd &= vbCrLf
        '    cmd &= vbCrLf & "OUTER APPLY (SELECT v.FTVenderPramCode FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMVenderPram AS v WITH (NOLOCK) "
        '    cmd &= vbCrLf & "WHERE v.FNHSysVenderPramId = A.FNHSysVenderPramId) AS v "
        '    cmd &= vbCrLf
        ...
        '    cmd &= vbCrLf & ", MCT.FTCustCode, MMT.FTMerTeamCode, A.FNJobState, A.FTStateBy, C.FTCmpCode, v.FTVenderPramCode "
        ...
        If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
            cmd &= ", '" & ST.Lang.eLang.TH & "' "
        Else
            cmd &= ", '" & ST.Lang.eLang.EN & "' "
        End If

        If (FTPayYear.Text <> "") Then
            cmd &= ", '" + (HI.ST.SysInfo.CmpID).ToString + "', '" + FTPayYear.Text + "/01/01' , '" + FTPayYear.Text + "/12/31' "
        End If

        If (FNHSysCustId.Text <> "") Then
            cmd &= ", '" & Val(FNHSysCustId.Properties.Tag.ToString) & "' "
        Else
            cmd &= ", ''"
        End If

        If (FNHSysStyleId.Text <> "") Then
            cmd &= ", '" & Val(FNHSysStyleId.Properties.Tag.ToString) & "' "
        Else
            cmd &= ", ''"
        End If

        If (FNHSysSeasonId.Text <> "") Then
            cmd &= ", '" & Val(FNHSysSeasonId.Properties.Tag.ToString) & "' "
        Else
            cmd &= ", ''"
        End If

        If (FNHSysMerTeamId.Text <> "") Then
            cmd &= ", '" & Val(FNHSysMerTeamId.Properties.Tag.ToString) & "' "
        Else
            cmd &= ", '' "
        End If

        If (FTStartOrderDate.Text <> "" And FTEndOrderDate.Text <> "") Then
            cmd &= ", '" & HI.UL.ULDate.ConvertEnDB(FTStartOrderDate.Text)
            cmd &= "', '" & HI.UL.ULDate.ConvertEnDB(FTEndOrderDate.Text) & "' "
        Else
            cmd &= ", '' ,'' "
        End If

        If (FNHSysBuyIdFrom.Text <> "" And FNHSysBuyIdTo.Text <> "") Then
            cmd &= ", '" & FNHSysBuyIdFrom.Text & "', '" & FNHSysBuyIdTo.Text & "' "
        Else
            cmd &= ", '' ,'' "
        End If
