...
Public Class wDailyQAReport
        ...
        Try
            ...
            For Each Rx As DataRow In dt.Rows
                If Rx!FPStyleImage1.ToString <> "" Then
                    Try
                        Me.FTImage.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "_0.JPG")
                ...
                If Rx!FPStyleImage2.ToString <> "" Then
                    Try
                        Me.FTImage2.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "_1.JPG")
                ...
                '    Try
                '        Me.FTImage3.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "_2.JPG")
    ...
    Private Sub LoadImageStyle(_StyleCode As String, ByRef img As DevExpress.XtraEditors.PictureEdit, ByRef img2 As DevExpress.XtraEditors.PictureEdit)
        Try
            ...
            For Each Rx As DataRow In dt.Rows
                If Rx!FPStyleImage1.ToString <> "" Then
                    Try
                        Images.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(_StyleCode, 6).ToString() & "\" & Microsoft.VisualBasic.Left(_StyleCode, 6).ToString() & "_0.JPG")
                ...
                If Rx!FPStyleImage2.ToString <> "" Then
                    Try
                        Images2.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(_StyleCode, 6).ToString() & "\" & Microsoft.VisualBasic.Left(_StyleCode, 6).ToString() & "_1.JPG")
            ...
            If dt.Rows.Count <= 0 Then
                Try
                    Images.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "\" & Microsoft.VisualBasic.Left(_StyleCode, 6).ToString() & "_0.JPG")
                Catch ex As Exception
                    Images.Image = Nothing
                End Try

                Try
                    Images2.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\MODEL\" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString() & "\" & Microsoft.VisualBasic.Left(_StyleCode, 6).ToString() & "_1.JPG")
                Catch ex As Exception
                    Images2.Image = Nothing
                End Try
            End If
    ...
    Private Sub LoadDrawRectangleRectangle(ByVal _Type As Integer, ByVal img As DevExpress.XtraEditors.PictureEdit, ByVal _StyleCode As String)
        ...
        _Qry &= vbCrLf & "FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTStylePoint AS A WITH(NOLOCK) LEFT OUTER JOIN "
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMStyle AS B WITH(NOLOCK) ON A.FNHSysStyleId = B.FNHSysStyleId "
        _Qry &= vbCrLf & "Where LEFT(B.FTStyleCode,6)='" & Microsoft.VisualBasic.Left(_StyleCode, 6) & "' and ( A.FNPointX > 0  and A.FNPointY > 0 and A.FNPicHeight > 0  and A.FNPicWidth > 0 ) "
        _Qry &= vbCrLf & "AND A.FTPicType=" & CInt(_Type)
        _oDt = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_PROD)
        For Each R As DataRow In _oDt.Rows
    ...
    Private Function GetDefectQty(ByVal _Seq As Integer) As String
        Try
            ...
            _Cmd &= vbCrLf & "and LEFT(S.FTStyleCode,6)   = B.FTStyleCode"
            _Cmd &= vbCrLf & "   LEFT OUTER JOIN"
            ...
            _Cmd &= vbCrLf & "AND (A.FNHSysUnitSectId = " & Me.FNHSysUnitSectId.Properties.Tag & ") AND (LEFT(S.FTStyleCode,6) = '" & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6) & "') AND (A.FTOrderNo = '" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "')"
    ...
    Private Function GetPoint() As Integer
        Try
            ...
            _Cmd &= vbCrLf & "Where M.FTStyleCode like '" & HI.UL.ULF.rpQuoted(Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6)) & "%'"
