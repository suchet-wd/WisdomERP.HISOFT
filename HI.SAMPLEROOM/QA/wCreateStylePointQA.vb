...
#End Region
        ...
        Try
            ...
            If e.Button = System.Windows.Forms.MouseButtons.Left Then
                ...
                If CType(sender, ZBobb.AlphaBlendTextBox).Name.ToString <> "ZBobbAlphaBlendTextBox" Then
                    ...
                    _Qry &= vbCrLf & "FROM   TSMPTStylePoint AS A WITH(NOLOCK) LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMStyle AS B WITH(NOLOCK) ON A.FNHSysStyleId = B.FNHSysStyleId  "
                    ...
                    For Each R As DataRow In _oDt.Rows
                        ...
                        Me.FTImagePoint.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\POINTMODELSAMPLE\" & Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", "") & "\" & R!FTPicName.ToString)
    ...
    Private Function GetSeq() As Integer
        Try
            ...
            Dim _Qry As String = ""
            _Qry = "Select max(FNSeq)  From TSMPTStylePoint with(nolock) "
    ...
    Private Function SaveData() As Boolean
        Try
            ...
            If (My.Computer.FileSystem.DirectoryExists(_Path + "\Images\POINTMODELSAMPLE\" & Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", "")) = False) Then   ' & Microsoft.VisualBasic.Left(Me.FNHSysStyleId.Text, 6).ToString) 
                My.Computer.FileSystem.CreateDirectory(_Path + "\Images\POINTMODELSAMPLE\" & Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", ""))
            End If
            HI.Conn.DB.ConnectionString(Conn.DB.DataBaseName.DB_SAMPLE)
            HI.Conn.SQLConn.SqlConnectionOpen()
            HI.Conn.SQLConn.Cmd = HI.Conn.SQLConn.Cnn.CreateCommand
            HI.Conn.SQLConn.Tran = HI.Conn.SQLConn.Cnn.BeginTransaction
            _Qry = "Update [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTStylePoint "
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                _Seq = HI.Conn.SQLConn.GetField("Select Max(FNSeq) AS FNSeq From TSMPTStylePoint WHERE FNHSysStyleId=" & CInt(Me.FNHSysStyleId.Properties.Tag), Conn.DB.DataBaseName.DB_SAMPLE, "0")
                _Seq += +1
                _Qry = "INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTStylePoint( FTInsUser, FDInsDate, FTInsTime, FNHSysStyleId, FTPointName, FNSeq, FNPointX, FNPointY, FTPicType, FNPicHeight, FNPicWidth, FTPicName,FTRemark,FTStyleCode)"
                ...
                _Qry &= vbCrLf & ",'" & HI.UL.ULImage.SaveImage(HI.UL.ULImage.ResizeImage(FTImagePoint.Image, New Size(700, 450)), Me.FTPointName.Text, _Path & "\Images\POINTMODELSAMPLE\" & Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", "")) & "'"
    ...
    Private Sub LoadData()
        ...
        _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPTStylePoint AS A WITH(NOLOCK) "
        ...
        Me.ogcdetail.DataSource = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_SAMPLE)
    End Sub

    Private Function DeletingData(ByVal _FNStylyNo As Integer, ByVal _FTPointName As String, ByVal Seq As Integer) As Boolean
        Try
            Dim _Qry As String = ""
            _Qry = "Delete From TSMPTStylePoint"
            _Qry &= vbCrLf & "WHERE FTStyleCode='" & HI.UL.ULF.rpQuoted(Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", "")) & "'"
    ...
    Private Sub LoadDrawRectangleRectangle(ByVal _Type As Integer)
        ...
        _Qry &= vbCrLf & "FROM   TSMPTStylePoint AS A WITH(NOLOCK) LEFT OUTER JOIN "
    ...
    Private Sub ogvdetail_DoubleClick(sender As Object, e As EventArgs) Handles ogvdetail.DoubleClick
        Try
            With ogvdetail
                ...
                Me.FTImagePoint.Image = HI.UL.ULImage.LoadImage(_Path & "\Images\POINTMODELSAMPLE\" & Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", "") & "\" & .GetRowCellValue(.FocusedRowHandle, "FTPicName").ToString)
    ...
    Private Sub ocmNewImg_Click(sender As Object, e As EventArgs) Handles ocmAddNewImg.Click
        Try
            ...
            Dim _NPath As String = _Path & "\Images\MODELSAMPLE\" & Replace(Replace(Me.FNHSysStyleId.Text, "/", ""), "*", "")
