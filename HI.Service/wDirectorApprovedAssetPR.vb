...
Public Class wDirectorApprovedAssetPR
            ...
            Else
                Select Case tCmdName.ToString.ToUpper
                    Case "FuncExit".ToUpper
                        ClsService.StateAssetPRDirectorShow = False
                        Me.Close()
                End Select
            End If
        Catch ex As Exception
...
1:      ' Me.Enabled = True
    ...
    Private Sub ValidateApp()
        ...
        If FindComputerName(Environment.MachineName.ToString()) Then
            ...
            'Else
            DTPRPurchaseAssetNoDirector = LoadogcTPURTPurchaseDi()
            'End If
            ...
            If _CountApp > 0 Then
                ...
                If Not (DTPRPurchaseAssetNoDirector Is Nothing) Then


                    ogSupervisorApproved.DataSource = DTPRPurchaseAssetNoDirector
    ...
    '        '_str &= Environment.NewLine & " isnull(A.FNPRState,'') as FNPRState,"
    ...
    '        '' _str &= Environment.NewLine & " L6.FTDeliveryCode,l7.FTCurCode,L1.FTDeliveryCode,"ISNULL( A.FTContactPerson,'') as FTContactPerson 
    ...
    '        '_str &= Environment.NewLine & " ISNULL(A.FNNetAmt,0) as FNNetAmt, ISNULL(A.FNQuantity,0) as FNQuantity," 'ISNULL(A.FNVatAmt,0) as FNVatAmt,"
    '        ' '' _str &= Environment.NewLine & "   ISNULL  (A.FNPOGrandAmt,0) as FNPOGrandAmt,"
    '        ''_str &= Environment.NewLine & " l8.FTTeamGrpCode,"ISNULL (A.FNSurcharge,0) as FNSurcharge,
    ...
    '        '_str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEUserLogin as B	ON a.FTPRPurchaseBy = b.FTUserName INNER JOIN"
    '        '_str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMTeamGrp as C ON B.FNHSysTeamGrpId = C.FNHSysTeamGrpId "
    ...
    '        '_str &= Environment.NewLine & " AND (a.FTStateSendApp = '1') AND (a.FTStateApp = '0')"
    ...
    '        _str &= Environment.NewLine & " isnull(A.FNPRState,'') as FNPRState,"
    '        _str &= Environment.NewLine & " ISNULL(A.FTRemark,'') as FTRemark,"
    '        _str &= Environment.NewLine & " ISNULL(A.FNNetAmt,0) as FNNetAmt, ISNULL(A.FNQuantity,0) as FNQuantity,"
    '        _str &= Environment.NewLine & " ISNULL(C.FTUserName,'') as FTUserName, A.FNFixedAssetType"
    '        _str &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.V_TFIXEDTPurchase_Request AS A   INNER JOIN "
    '        _str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEUserLogin as B	ON a.FTPRPurchaseBy = b.FTUserName INNER JOIN"
    '        _str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMTeamGrp as C ON B.FNHSysTeamGrpId = C.FNHSysTeamGrpId "
    '        _str &= Environment.NewLine & " WHERE (C.FTUserName = '" & HI.ST.UserInfo.UserName & "')"
    ...
    Public Shared Function LoadogcTPURTPurchaseDi() As DataTable
        Try
            ...
            _str &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.V_TFIXEDTPurchase_Request AS A   INNER JOIN "
            '_str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEUserLogin as B	ON a.FTPRPurchaseBy = b.FTUserName INNER JOIN"
            '_str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo. TCNMDirectorGrpUser as DU ON B.FNHSysDirectorGrpId=DU.FNHSysDirectorGrpId INNER JOIN"
    ...
    Private Sub BindGrid()
        Try
            ...
            Call Set_HeadGrid()
            'Dim _App1 As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateApp,'0')as FTStateApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request As P where P.FTStateApp = '1' ", Conn.DB.DataBaseName.DB_SYSTEM, "")
            'Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateApp,'0')as FTStateApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request As P where P.FTStateApp = '0'", Conn.DB.DataBaseName.DB_SYSTEM, "")
            'Dim _MApp As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateManagerApp,'0')as FTStateApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request As P where P.FTStateManagerApp = '0' ", Conn.DB.DataBaseName.DB_SYSTEM, "")
            '  HI.ST.SysInfo.StateDirector = True     ' true ทดสอบ Super     false ทดสอบ Manager
            ' กลับมาเรียก Function ของตัวเอง
            'Dim _dt As DataTable
    ...
    Private Sub sBtnSave_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        Try
            ...
            '        If _App = "0" Then
            '            Call _ClsService.Update_SupervisorApprovedAssetPR(ogvSupervisorApproved, 1)
            '        Else
