...
#Region "MAIN PROC"
    ...
    Private Sub ProcessPreview(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmpreview.Click
        ...
        '            Call SumLeaveReport(sumBusinessLeave, sumSickLeave, sumVacationLeave, sumAbsent, sumLate, LeaveVacation)
...
#Region " Procedure "
    ...
    Private Function SaveData() As Boolean
        Try
            ...
            _Qry = "SELECT Top 1  Isnull(T.FTStateMngApp,'0')  as FTStateMngApp "
...
#End Region
    ...
    Public Function LoadKey() As Boolean
        Try
            Me.FNHSysEmpId.Text = _KeyCode
        Catch ex As Exception
    ...
    Private Sub wLeave_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ' Me.FNHSysEmpId.Text = _KeyCode
        If Me.FNLeaveType_param.Text <> "" And Me.FTStartDate_param.Text <> "" Then
            Me.FNHSysEmpId.Text = _KeyCode
            Me.FTStateNotMergeHoliday.Checked = Me.FTStateNotMergeHoliday_param.Checked
            Me.FNLeaveType.SelectedIndex = HI.TL.CboList.GetIndexByValue(Me.FNLeaveType.Properties.Tag.ToString, "" & Me.FNLeaveType_param.Text)



            Me.FTSTime.Text = Me.FTSTime_param.Text
            Me.FTETime.Text = Me.FTETime_param.Text

            Me.FTStartDate.DateTime = Me.FTStartDate_param.Text
    ...
    Private Sub LoadText()
        Dim _Qry As String = ""
        Dim _Str_FNLeaveDay = ""
        Dim _Str_FNLeaveType = ""
        If HI.ST.Lang.Language = HI.ST.SysInfo.LanguageLocal Then
            _Qry = vbCrLf & " SELECT FTNameTH  AS  'FTName' "
            _Qry &= vbCrLf & " From HITECH_SYSTEM.dbo.HSysListData "
            _Qry &= vbCrLf & " WHERE FTListName ='FNLEAVETYPE' AND FNListIndex = " & Val(Me.FNLeaveType_param.Text)
        Else
            _Qry = vbCrLf & " SELECT FTNameEN  AS  'FTName' "
            _Qry &= vbCrLf & " From HITECH_SYSTEM.dbo.HSysListData "
            _Qry &= vbCrLf & " WHERE FTListName ='FNLEAVETYPE' AND FNListIndex = " & Val(Me.FNLeaveType_param.Text)
        End If
        _Str_FNLeaveType = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_SYSTEM, "")

        Me.FNLeaveType_Text.Text = _Str_FNLeaveType

        If HI.ST.Lang.Language = HI.ST.SysInfo.LanguageLocal Then
            _Qry = vbCrLf & " SELECT FTNameTH  AS  'FTName' "
            _Qry &= vbCrLf & " From HITECH_SYSTEM.dbo.HSysListData "
            _Qry &= vbCrLf & " WHERE FTListName ='FNLEAVEDAY' AND FNListIndex = " & Val(Me.FNLeaveDay_param.Text)
        Else
            _Qry = vbCrLf & " SELECT FTNameEN  AS  'FTName' "
            _Qry &= vbCrLf & " From HITECH_SYSTEM.dbo.HSysListData "
            _Qry &= vbCrLf & " WHERE FTListName ='FNLEAVEDAY' AND FNListIndex = " & Val(Me.FNLeaveDay_param.Text)
        End If
        _Str_FNLeaveDay = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_SYSTEM, "")
        Me.FNLeaveDay_Text.Text = _Str_FNLeaveDay
    End Sub
    ...
    Private Sub Load_PDF()
        ...
        For Each R As DataRow In _oDt.Rows
            Dim FBFile As String = R!FBFile.ToString
            Me.oGrpdetail.Controls.Clear()
            Select Case FBFile
                Case "PDF"
                    Dim _Pdfv As New PdfViewer
