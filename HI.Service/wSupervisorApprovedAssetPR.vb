...
Public Class wSupervisorApprovedAssetPR
...
#End Region
    ...
    Private Sub ValidateApp()
        ...
        If FindComputerName(Environment.MachineName.ToString()) Then
            ...
            DTPRPurchaseAssetNo = LoadogcTPURTPurchase()
            'Else
            ...
            If _CountAppAssetPR > 0 Then
                ...
                If Not (DTPRPurchaseAssetNo Is Nothing) Then


                    ogSupervisorApproved.DataSource = DTPRPurchaseAssetNo
    ...
    Public Shared Function LoadogcTPURTPurchase() As DataTable
        ...
        Try
            ...
            _str &= Environment.NewLine & " ISNULL(Convert(numeric(18,2),A.FNPRGrandAmt),0) as FNNetAmt, ISNULL(Convert(numeric(18,2),A.FNQuantity),0) as FNQuantity,"
            ...
            With CType(ogSupervisorApproved.DataSource, DataTable)
                ...
                For Each R As DataRow In dt.Select("FTPRPurchaseNo <> '' ", "FTStateApproved")
                    If R!FTStateApproved.ToString = "1" Then
                        'Dim _App As Double = HI.Conn.SQLConn.GetField("Select (Convert(numeric(18, 0),FN ,FNNetAmt)) as FNNetAmt from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail As P where P.FTPRPurchaseNo = '" & R!FTPRPurchaseNo.ToString & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
                        ...
                        Dim _ByPR As String = HI.Conn.SQLConn.GetField("SELECT R.FTPRPurchaseBy FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request AS R  where R.FTPRPurchaseNo ='" & R!FTPRPurchaseNo.ToString & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
                        Dim _UPR As String = HI.Conn.SQLConn.GetField("SELECT U.FTManagerUserName FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEUserLogin AS U where U.FTUserName = '" & _ByPR & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
                        Dim _UPRD As String = HI.Conn.SQLConn.GetField("Select Top 1 C.FTUserName  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request AS R  LEFT OUTER JOIN [HITECH_FIXEDASSET].dbo.TFIXEDTPurchase_Request_Detail AS RD ON R.FTPRPurchaseNo=RD.FTPRPurchaseNo LEFT OUTER JOIN [HITECH_MASTER].dbo.TASMAssetConfigLevel AS C ON RD.FNFixedAssetType=C.FNFixedAssetType AND R.FNHSysCmpId=C.FNHSysCmpId AND R.FNPRAmt>=C.FNStartQty AND R.FNPRAmt<=C.FNEndQty  where C.FTStateDirector='1' AND  R.FTPRPurchaseNo = '" & R!FTPRPurchaseNo.ToString & "' AND C.FTUserName='" & HI.ST.UserInfo.UserName & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
