...
Public Class wOrderCostApproved
    ...
    Sub New()
        ...
        _Splash = New HI.TL.SplashScreen("HI SOFT SYSTEM", "Loading Approve Order Costing ..." & "  ")
...
#End Region
    ...
    Private Sub MainRibbon_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Try
            ClsService.StateOrderCost = False
    ...
    Private Sub MainRibbon_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'If Not Application.OpenForms.OfType(Of wOrderCostApproved).Any Then
        Call BindGrid()
        'End If

        Try
            Call HI.UL.AppRegistry.LoadLayoutGridFromRegistry(Me, Me.ogvdocument)
        Catch ex As Exception
        End Try
    ...
    Public Shared Function LoadDcoumentation() As DataTable
        Try
            Dim _Cmd As String = ""
            ...
            _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.SP_GET_ORDERCOST_APP '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULF.rpQuoted(_LangDisPlay) & "'"
            _oDt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT)

            If _oDt.Select("FTStateApp <>'4'").Length > 0 Then
                Return _oDt.Select("FTStateApp <>'4'").CopyToDataTable
            Else
                Return Nothing
            End If
        Catch ex As Exception
    ...
    Private Sub BindGrid()
        Try
            Call Set_HeadGrid()
            DTICM = LoadDcoumentation()
            If Not (DTICM Is Nothing) Then
                Me.FTSelectAllOrderCosting.Checked = False
                '  Me.ogcdocument.DataSource = DTICM
                Me.ogdtime.DataSource = DTICM
        ...
        Try
            Select Case Me.otbmain.SelectedTabPage.Name
                Case otpappcminv.Name
                    ...
                    'Try
                    '    Call _ClsService.Update_OrderCostApprove(ogvdocument, "1", "")

                    'Catch ex As Exception
                    'End Try
                    If ogdtime.Views(0).RowCount = 0 Then Exit Sub
                    With CType(ogdtime.DataSource, DataTable)
                        .AcceptChanges()
                        If .Select("FTSelect='1'").Length <= 0 Then Exit Sub
                    End With
                    Dim _Spls As New HI.TL.SplashScreen("Approving Order Costing...,Pease wait...")
                    Try
                        Call _ClsService.Update_OrderCostApprove(ogdtime.Views(0), "1", "")
    ...
    Private Sub sBtnReject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        ...
        Try
            ...
            '        Try
            '            Call _ClsService.Update_OrderCostApprove(ogvdocument, "0", wShowRejectDC.Data_Reason)
            '        Catch ex As Exception
            '        End Try
            '        _Spls.Close()
            'End Select
            ...
            Select Case Me.otbmain.SelectedTabPage.Name
                Case otpappcminv.Name
                    Dim _Spls As New HI.TL.SplashScreen("Rejecting Order Costing...,Pease wait.")
                    Try
                        Call _ClsService.Update_OrderCostApprove(ogdtime.Views(0), "0", wShowRejectDC.Data_Reason)
                    Catch ex As Exception
                    End Try
                    _Spls.Close()
            End Select
    ...
    Private Sub otmchkpo_Tick(sender As Object, e As EventArgs) Handles otmchkpo.Tick
        If Not Application.OpenForms.OfType(Of wOrderCostApproved).Any Or ClsService.StateShow = False Then
            Call BindGrid()
        End If
        ...
        ocmreject.Enabled = True
        'ocmpreview.Enabled = True
        If (Me.ogdtime.DataSource Is Nothing) And Application.OpenForms.OfType(Of wOrderCostApproved).Any Or ClsService.StateShow = False Then
            Call BindGrid()
        End If

        If Me.ogdtime.DataSource Is Nothing Then Exit Sub
        If DirectCast(Me.ogdtime.DataSource, DataTable).Rows.Count > 0 Then
            If Not Application.OpenForms.OfType(Of wOrderCostApproved).Any Or ClsService.StateShow = False Then
                Call HI.Service.ClsService.ValidateAppOrderCost()

            Else
                If ClsService.StateShow Then
                    Me.Show()
                Else
                    BindGrid()
                End If
    ...
    Private Sub ocmsavelayout_Click(sender As Object, e As EventArgs) Handles ocmsavelayout.Click
        HI.UL.AppRegistry.SaveLayoutGridToRegistry(Me, Me.ogvdocument)
        HI.MG.ShowMsg.mInfo("Save Layout Grid Complete...", 1404240001, Me.Text, , System.Windows.Forms.MessageBoxIcon.Information)
    End Sub

    Private Sub ogvcminv_RowCountChanged(sender As Object, e As EventArgs)
        Try
            Me.otpappcminv.PageVisible = (ogvdocument.RowCount > 0)
        Catch ex As Exception
    ...
    Private Sub FTSelectAllCMInv_CheckedChanged(sender As Object, e As EventArgs) Handles FTSelectAllOrderCosting.CheckedChanged
        Try
            Dim _State As String = "0"
            If Me.FTSelectAllOrderCosting.Checked Then
                _State = "1"
            End If
            With ogdtime
                If Not (.DataSource Is Nothing) And oAdvBandedGridView.RowCount > 0 Then
    ...
    Private Sub oAdvBandedGridView_DoubleClick(sender As Object, e As EventArgs) Handles oAdvBandedGridView.DoubleClick
        Try
            With Me.oAdvBandedGridView
                ...
                Dim _Month As String = Microsoft.VisualBasic.Left(HI.UL.ULDate.ConvertEnDB("" & .GetRowCellValue(.FocusedRowHandle, "FDInvoiceDate").ToString()), 7)
                Dim _Cmd As String = "" & .GetRowCellValue(.FocusedRowHandle, "FTCmpName").ToString()

                Dim _wJobCost As New HI.ACC.wAccJobCustSummaryBranch(_Month, _Cmd)

                Dim _TmpMenu As String = HI.ST.SysInfo.MenuName
                HI.ST.SysInfo.MenuName = "MnuJobCost"
                Dim _WShow As New wShowData(_wJobCost, _OrderNo)
    ...
    'Private Sub ogvdocument_DoubleClick(sender As Object, e As EventArgs) Handles ogvdocument.DoubleClick
    '    Try
    '        With Me.ogvdocument
    '            If .FocusedRowHandle < 0 Or .FocusedRowHandle > .RowCount Then Exit Sub
    '            Dim _OrderNo As String = "" & .GetRowCellValue(.FocusedRowHandle, "FTOrderNo").ToString()
    '            Dim _wJobCost As New HI.ACC.wJobCostTransaction
