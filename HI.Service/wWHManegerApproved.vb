...
Public Class wWHManegerApproved
...
#End Region
    ...
    Public Shared Function LoadCommercialInvoice() As DataTable
        Try
            Dim _Cmd As String = ""
            Dim _oDt As DataTable
            '_Cmd = "SELECT    '0' AS FTSelect , I.FTCustomerPO, I.FTInvoiceNo, I.FNHsysCmpID, I.FTStateSendApp, I.FTStateApp, I.FTStateWHApp, sum(Isnull(D.FNQuantity,0)) AS FNQuantity , (Isnull(S.FNNetCM,0)) AS FNCM , S.FTStyleCode, C.FTCmpCode  "
            ...
            '_Cmd &= vbCrLf & "WHERE ISNULL(I.FTStateApp,'0')  = '1' and (ISNULL(I.FTStateWHApp,'0') = '0') and (ISNULL(I.FTStateWHReject,'0') = '0') "
            '_Cmd &= vbCrLf & "group by I.FTCustomerPO, I.FTInvoiceNo, I.FDInvoiceDate, I.FNHsysCmpID, I.FTStateSendApp, I.FTStateApp, I.FTStateWHApp,  S.FTStyleCode, C.FTCmpCode ,I.FTStateSendBy,I.FTStateAppBy, I.FTStateRejectBy ,I.FTStateWHRejectBy,Isnull(S.FNNetCM,0)"
            ...
            _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.SP_GET_WAREHOUSE_APP_MI '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULF.rpQuoted(_LangDisPlay) & "'"
            _oDt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT)
    ...
    Public Shared Function LoadogcTPURTPurchase() As DataTable
        Try
            ...
            _str = ""
            _str = " SELECT * FROM (SELECT  isnull(A.FTStateManagerApp,0) as FTStateApproved, A.FTPurchaseNo,"
        ...
        Try
            Select Case Me.otbmain.SelectedTabPage.Name
                Case otpappcminv.Name
                    ...
                    End With
                    Dim _Spls As New HI.TL.SplashScreen("Approving WareHouse Commercial Invoice...,Pease wait...")
                    Try
                        Call _ClsService.Update_CMInvApprove(ogvcminv, "1", "", "1")
                    Catch ex As Exception
                    End Try
                    _Spls.Close()
            End Select
    ...
    Private Sub sBtnReject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        ...
        Try
            ...
            Select Case Me.otbmain.SelectedTabPage.Name
                Case otpappcminv.Name
                    Dim _Spls As New HI.TL.SplashScreen("Rejecting Ware House  Commercial Invoice...,Pease wait.")
                    Try
                        Call _ClsService.Update_CMInvApprove(ogvcminv, "0", wShowReject.Data_Reason, "1")
                    Catch ex As Exception
                    End Try
                    _Spls.Close()
            End Select
    ...
    Private Sub ogvcminv_RowCountChanged(sender As Object, e As EventArgs) Handles ogvcminv.RowCountChanged
        Try
            Me.otpappcminv.PageVisible = (ogvcminv.RowCount > 0)
        Catch ex As Exception
