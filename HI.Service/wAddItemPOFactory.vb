...
Public Class wAddItemPOFactory
            ...
            Else
                ...
                If Integer.Parse(Val(FNHSysRawMatId.Properties.Tag.ToString())) > 0 Then
                    ...
                    If FTOrderNo.Properties.ReadOnly = False Then
                        ...
                        _Qry = "SELECT TOP 1 A.FTPurchaseNo, A.FNHSysRawMatId, A.FNPrice, B.FTUnitCode,ISNULL(A.FNSurchangeAmt,0) AS FNSurchangeAmt "
                        _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo AS A WITH(NOLOCK) "
                        ...
                        Else
                            ...
                            _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase AS P WITH(NOLOCK) INNER JOIN "
                            _Qry &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo AS A WITH(NOLOCK) "
                            _Qry &= vbCrLf & " INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS B WITH(NOLOCK) ON A.FNHSysUnitId = B.FNHSysUnitId "
