...
Public Class wDCFactoryApproved
    ...
    Sub New()
        ...
        _Splash = New HI.TL.SplashScreen("HI SOFT SYSTEM", "Loading Approve Factory Order..." & "  ")
...
#End Region
    ...
    Public Shared Function LoadDcoumentation() As DataTable
        Try
            ...
            _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_DOC) & "].dbo.SP_GET_MNGDEP_APP_DOCUMENT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULF.rpQuoted(_LangDisPlay) & "'"
            _oDt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_DOC)

            _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_DOC) & "].dbo.SP_GET_DIRECTOR_APP_DOCUMENT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULF.rpQuoted(_LangDisPlay) & "'"
    ...
    Private Sub BindGrid()
        Try
            ...
            Else
                Me.ogcdocument.DataSource = Nothing
            End If
        ...
        Try
            Select Case Me.otbmain.SelectedTabPage.Name
                Case otpappcminv.Name
                    ...
                    End With
                    Dim _Spls As New HI.TL.SplashScreen("Approving Documentation...,Pease wait...")
                    Try
                        Dim _Cmd As String = ""

                        Call _ClsService.Update_SectApprove(ogvdocument, "1", "")
                        ...
                        '    Call _ClsService.Update_SectApprove(ogvdocument, "1", "")
                        'Else
                        '    Call _ClsService.Update_DocApprove(ogvdocument, "1", "")
                        'End If
                    Catch ex As Exception
                    End Try
                    _Spls.Close()
    ...
    Private Sub sBtnReject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        ...
        Try
            ...
            Select Case Me.otbmain.SelectedTabPage.Name
                Case otpappcminv.Name
                    Dim _Spls As New HI.TL.SplashScreen("Rejecting Documnetation...,Pease wait.")
                    Try
                        Dim _Cmd As String = ""
                        _Cmd = "Select FTUserName From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "]..TCNMSect WITH(NOLOCK) "
                        _Cmd &= vbCrLf & "Where FTUserName='" & HI.ST.UserInfo.UserName & "'"
                        If HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_MASTER, "") <> "" Then
                            Call _ClsService.Update_SectApprove(ogvdocument, "0", wShowRejectDC.Data_Reason)
                        Else
                            Call _ClsService.Update_DocApprove(ogvdocument, "0", wShowRejectDC.Data_Reason)
                        End If
                    Catch ex As Exception
                    End Try
                    _Spls.Close()
    ...
    Private Sub otmchkpo_Tick(sender As Object, e As EventArgs) Handles otmchkpo.Tick
        ...
        ocmreject.Enabled = True
        'ocmpreview.Enabled = True
        If ogvdocument.RowCount > 0 Then
            Me.Show()
