...
Public Class wMerChandiserManagerApproved
    ...
    Sub New()
        ...
        _Splash = New HI.TL.SplashScreen("Wisdom System", "Loading Approve Transaction Value Work Sheet..." & "  ")
...
#End Region
    ...
    Private Sub MainRibbon_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        Try
            ClsService.StateMerChandiseManegerShow = False
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub MainRibbon_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Call BindGrid()
        Try
            Call HI.UL.AppRegistry.LoadLayoutGridFromRegistry(Me, Me.ogvcminv)
        Catch ex As Exception
    ...
    Public Shared Function LoadCommercialInvoice(Optional State As Boolean = False) As DataTable
        Try
            ...
            '_Cmd &= vbCrLf & "  ,A.FTCmpName"
            '_Cmd &= vbCrLf & "  ,A.FNHSysCustId"
            '_Cmd &= vbCrLf & "  ,CO.FTCustCode "

            'If HI.ST.Lang.Language = Lang.eLang.TH Then
            '    _Cmd &= vbCrLf & "  ,  CO.FTCustNameTH  AS  FTCustName  "
            'Else
            '    _Cmd &= vbCrLf & "  , CO.FTCustNameEN  AS  FTCustName  "
            'End If
            ...
            '_Cmd &= vbCrLf & " ,Convert(numeric(18,2),A.FNQuantity * A.FNNetCM) AS FNNetCMAmt"
            '_Cmd &= vbCrLf & " ,A.FNFirstPrice"
            '_Cmd &= vbCrLf & " ,Convert(numeric(18,2),A.FNQuantity * A.FNFirstPrice) AS FNFirstPriceAmt"

            '_Cmd &= vbCrLf & " FROM("
            '_Cmd &= vbCrLf & " SELECT '0' AS FTSelect, I.FTCustomerPO, I.FTInvoiceNo,  SUM(ISNULL(D.FNQuantity, 0)) AS FNQuantity,  S.FTStyleCode, "
            ...
            '_Cmd &= vbCrLf & "  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTTransactionValueWorksheet AS XCA WITH(NOLOCK) "
            ...
            '_Cmd &= vbCrLf & "   ,S.FNNetCM"
            '_Cmd &= vbCrLf & "	 ) AS A "
            '_Cmd &= vbCrLf & "	 LEFT OUTER JOIN HITECH_MASTER.dbo.TCNMCustomer AS CO ON A.FNHSysCustId =CO.FNHSysCustId "
    ...
    Private Sub BindGrid()
        Try
            ...
            If Not (DTICM Is Nothing) Then
                ...
                Try
                    If ogvcminv.FocusedRowHandle >= 0 Then
                        _FocusRowInDex = ogvcminv.FocusedRowHandle
                        _FTPurchaseNo = ogvcminv.GetRowCellValue(ogvcminv.FocusedRowHandle, "FTInvoiceNo").ToString
                    End If
                Catch ex As Exception
                End Try
                ...
                view = ogccminv.Views(0)
                view.OptionsView.ShowAutoFilterRow = True
                Me.ogccminv = view.GridControl
                Me.ogccminv.Refresh()
                _FocusRowInDex = -1
                With Me.ogvcminv
                    If .RowCount > 0 And _FTPurchaseNo <> "" Then
                        Try
                            _FocusRowInDex = .LocateByValue("FTInvoiceNo", _FTPurchaseNo)
                        Catch ex As Exception
                        End Try
                        Try
    ...
    Private Sub sBtnSave_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        ...
        Try
            ...
            End With
            Dim _Spls As New HI.TL.SplashScreen("Approving Transaction value worksheet...,Pease wait...")
            Try
                Call _ClsService.Update_TVWApprove(ogvcminv, 1)
            Catch ex As Exception
            End Try
            _Spls.Close()
    ...
    Private Sub sBtnReject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        ...
        Try
            ...
            Try
                Dim _Spls As New HI.TL.SplashScreen("Rejecting  Transaction value worksheet...,Pease wait.")
                Call _ClsService.Update_TVWRejected(ogvcminv, "0", wShowReject.Data_Reason)
                _Spls.Close()
            Catch ex As Exception
            End Try
    ...
    Private Sub ocmsavelayout_Click(sender As Object, e As EventArgs) Handles ocmsavelayout.Click
        HI.UL.AppRegistry.SaveLayoutGridToRegistry(Me, Me.ogvcminv)
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        ...
        With Me.ogvcminv
            ...
            Dim _Fm As String = ""
            _Fm = "{TACCTXMLCreateInvoice.FTCustomerPO}='" & HI.UL.ULF.rpQuoted("" & .GetFocusedRowCellValue("FTCustomerPO").ToString) & "' "
            _Fm &= " And {TACCTXMLCreateInvoice.FTInvoiceNo}='" & HI.UL.ULF.rpQuoted("" & .GetFocusedRowCellValue("FTInvoiceNo").ToString) & "' "
