...
Public Class wSupervisorApprovedAsset
            ...
            Else
                Select Case tCmdName.ToString.ToUpper
                    Case "FuncExit".ToUpper
                        ClsService.StateAssetShow = False
                        Me.Close()
                End Select
            End If
        Catch ex As Exception
...
#End Region
    ...
    Private Sub ValidateApp()
        ...
        If FindComputerName(Environment.MachineName.ToString()) Then
            ...
            'Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTStateSuperVisorApp = '1'", Conn.DB.DataBaseName.DB_SYSTEM, "")

            DTPurchaseAssetNo = Nothing
            'If _App = "" Then
            DTPurchaseAssetNo = LoadogcTPURTPurchase()
            'Else
            ...
            '        Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTPurchaseNo = '" & R!FTPurchaseNo.ToString & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
            ...
            If _CountAppAsset > 0 Then
                ...
                If Not (DTPurchaseAssetNo Is Nothing) Then


                    ogSupervisorApproved.DataSource = DTPurchaseAssetNo
    ...
    Public Shared Function LoadogcTPURTPurchase() As DataTable
        Try
            ...
            _str &= Environment.NewLine & "  A.FNFixedAssetType" ',AA.FTAssetCode" ',A.FTStateFree"
            ...
            '_str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAsset AS AA with (nolock) ON PD.FNHSysFixedAssetId=AA.FNHSysFixedAssetId LEFT JOIN "
    ...
    End Function
    'Public Shared Function LoadogcTPURTPurchaseDi() As DataTable
    '    Try
    ...
    '        _str &= Environment.NewLine & "  SUBSTRING(A.FDPurchaseDate,9,2) + '/'+ SUBSTRING(A.FDPurchaseDate,6,2) + '/' + SUBSTRING(A.FDPurchaseDate,1,4) as FDPurchaseDate,"
    '        _str &= Environment.NewLine & " ISNULL( A.FTPurchaseBy,'') as FTPurchaseBy, "
    '        _str &= Environment.NewLine & " ISNULL( A.FTManagerName,'') as FTManagerName, "
    '        _str &= Environment.NewLine & " isnull(A.FTPurchaseState,'') as FTPurchaseState,"
    '        _str &= Environment.NewLine & " ISNULL( l2.FTCmpRunCode,'') as FTCmpRunCode,"
    '        _str &= Environment.NewLine & " L3.FTSuplCode,"
    '        _str &= Environment.NewLine & " SUBSTRING(A.FDDeliveryDate,9,2) + '/'+ SUBSTRING(A.FDDeliveryDate,6,2) + '/' + SUBSTRING(A.FDDeliveryDate,1,4) as FDDeliveryDate,"
    ...
    '        _str &= Environment.NewLine & "  A.FNFixedAssetType" ',A.FTStateFree"
    '        '  _str &= Environment.NewLine & " FROM TPURTPurchase as A with(nolock) INNER JOIN "
    '        _str &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.V_TFIXEDTPurchase AS A   INNER JOIN "
    ...
    '        _str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TFINMCurrency as L7 with (nolock)  on a.FNHSysCurId = L7.FNHSysCurId left join"
    '        _str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TCNMTeamGrp as L8 with (nolock) on b.FNHSysTeamGrpId = L8.FNHSysTeamGrpId left join"
    '        _str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMDirectorGrp as D ON C.FNHSysDirectorGrpId=D.FNHSysDirectorGrpId left join "
    '        _str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo. TCNMDirectorGrpUser as DU ON D.FNHSysDirectorGrpId=DU.FNHSysDirectorGrpId"
    '        '_str &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TPURMPURGrp as L9 with (nolock) on a.FNHSysPurGrpId = L9.FNHSysPurGrpId "
    '        _str &= Environment.NewLine & " WHERE (DU.FTUserName  = '" & HI.ST.UserInfo.UserName & "')"
    '        _str &= Environment.NewLine & " AND (a.FTStateSendApp = '1') AND (a.FTStateSuperVisorApp = '1')  AND (a.FTStateManagerApp = '0')"
    '        _str &= Environment.NewLine & " Order by A.FTPurchaseBy, a.FDPurchaseDate"
    ...
    Private Sub BindGrid()
        Try
            ...
            Call Set_HeadGrid()
            'Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTStateSuperVisorApp = '" & ColFTPurchaseNo.ToString & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
            ''  HI.ST.SysInfo.StateDirector = True     ' true ทดสอบ Super     false ทดสอบ Manager
            '' กลับมาเรียก Function ของตัวเอง
            'Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTStateSuperVisorApp = '0'", Conn.DB.DataBaseName.DB_SYSTEM, "")
            'Dim _App1 As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTStateSuperVisorApp = '1' ", Conn.DB.DataBaseName.DB_SYSTEM, "")
            'Dim _MApp As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateManagerApp,'0')as FTStateManagerApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTStateManagerApp = '0' ", Conn.DB.DataBaseName.DB_SYSTEM, "")
            ...
            '    .AcceptChanges()
            '    _dt = .Copy
            'For Each R As DataRow In _dt.Select("FTPRPurchaseNo <> '' ", "FTStateApproved")
            '    Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTPurchaseNo = '" & R!FTPurchaseNo.ToString & "'", Conn.DB.DataBaseName.DB_SYSTEM, "")
    ...
    Private Sub sBtnSave_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        Try
            ...
            'If _App = "" Then
            Call _ClsService.Update_SupervisorApprovedAssetPO(ogvSupervisorApproved, 1)
            'Else
            '    Call _ClsService.Update_SupervisorApprovedAssetPODi(ogvSupervisorApproved, 1)
            'End If
    ...
    Private Sub sBtnReject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        ...
        Try
            ...
            'Dim _App As String = HI.Conn.SQLConn.GetField("Select isnull(FTStateSuperVisorApp,'0')as FTStateSuperVisorApp  from [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase As P where P.FTStateSuperVisorApp = '1'", Conn.DB.DataBaseName.DB_SYSTEM, "")
            'If _App = "" Then
            Call _ClsService.Update_SupervisorApprovedAssetPO(ogvSupervisorApproved, 2, wShowReject.Data_Reason)
            'Else
            '    Call _ClsService.Update_SupervisorApprovedAssetPODi(ogvSupervisorApproved, 2, wShowReject.Data_Reason)
            'End If
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        With Me.ogvSupervisorApproved
            ...
            _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase AS A WITH(NOLOCK)"
            ...
            With New HI.RP.Report
                ...
                .Formular = "{TFIXEDTPurchase.FTPurchaseNo}='" & HI.UL.ULF.rpQuoted(_PoNo) & "'"
                .Preview()
