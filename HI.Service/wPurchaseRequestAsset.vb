...
Public Class wPurchaseRequestAsset
...
#Region "Procedure Grid Detail"
    ...
    Private Sub LoadGridDetail()
        ...
        Dim _dt As DataTable
        'With _AddPopup
        '    Dim _FIXTYPE As String = .FNFixedAssetType.Text

        '    If _FIXTYPE = "อะไหล่จักร" Then
        ...
        _Qry = "SELECT * FROM ("
        _Qry &= vbCrLf & "select PRD.FNSeq,PRD.FTPRPurchaseNo,P.FTAssetPartCode AS FTAssetCode,PRD.FNHSysFixedAssetId,P.FTProductCode AS FTProductCode,PRD.FTRemark,isnull(PRD.FTDescription,'-')as FTDescription "
        If ST.Lang.Language = ST.Lang.eLang.TH Then
            _Qry &= vbCrLf & ",P.FTAssetPartNameTH  as FTAssetName,isnull(B.FTAssetBrandNameTH,'-') as FTAssetBrandName,'-' as FTAssetModelName,L.FTNameTH AS FNFixedAssetType"
        Else
            _Qry &= vbCrLf & ",P.FTAssetPartNameEN  as FTAssetName,B.FTAssetBrandNameEN as FTAssetBrandName,'-' as FTAssetModelName,L.FTNameEN AS FNFixedAssetType"
        End If
        _Qry &= vbCrLf & ",U.FTUnitAssetCode AS FTUnitCode,PRD.FNQuantity,ISNULL(Convert(numeric(18,2),PRD.FNPrice),0) as FNPrice,FNDisPer,ISNULL(Convert(numeric(18,2),FNDisAmt),0) as FNDisAmt,ISNULL(Convert(numeric(18,2),PRD.FNNetAmt),0) as FNNetAmt  from"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail AS PRD WItH(NOLOCK) LeFT OUTER JOIN"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request AS PR WItH(NOLOCK) ON PRD.FTPRPurchaseNo=PR.FTPRPurchaseNo LEFT OUTER JOIN"
        ...
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetBrand AS B WITH(NOLOCK) ON P.FNHSysAssetBrandId=B.FNHSysAssetBrandId LEFT OUTER JOIN"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitAsset AS U WITH(NOLOCK) ON PRD.FNHSysUnitId=U.FNHSysUnitAssetId LEFT OUTER JOIN"
        _Qry &= vbCrLf & "(SELECT L.FTNameTH ,L.FTNameEN,L.FNListIndex FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L WHERE L.FTListName ='FNFixedAssetType')AS L   ON PRD.FNFixedAssetType=L.FNListIndex"
        _Qry &= vbCrLf & "where P.FTAssetPartCode<>'' AND PRD.FTPRPurchaseNo='" & Me.FTPRPurchaseNo.Text & "' "
        _Qry &= vbCrLf & ")AS AP UNION ALL"
        _Qry &= vbCrLf & "SELECT * FROM ("
        _Qry &= vbCrLf & "select PRD.FNSeq,PRD.FTPRPurchaseNo,A.FTAssetCode AS FTAssetCode, PRD.FNHSysFixedAssetId,A.FTProductCode AS FTProductCode,PRD.FTRemark,isnull(PRD.FTDescription,'-')as FTDescription "
        If ST.Lang.Language = ST.Lang.eLang.TH Then
            _Qry &= vbCrLf & ",A.FTAssetNameTH as FTAssetName,isnull(B.FTAssetBrandNameTH,'-') as FTAssetBrandName,isnull(M.FTAssetModelNameTH,'-') as FTAssetModelName,L.FTNameTH AS FNFixedAssetType"
        Else
            _Qry &= vbCrLf & ",A.FTAssetNameEN  as FTAssetName,B.FTAssetBrandNameEN as FTAssetBrandName,M.FTAssetModelNameEN as FTAssetModelName,L.FTNameEN AS FNFixedAssetType"
        End If
        _Qry &= vbCrLf & ",U.FTUnitAssetCode AS FTUnitCode,PRD.FNQuantity,ISNULL(Convert(numeric(18,2),PRD.FNPrice),0) as FNPrice,FNDisPer,ISNULL(Convert(numeric(18,2),FNDisAmt),0) as FNDisAmt,ISNULL(Convert(numeric(18,2),PRD.FNNetAmt),0) as FNNetAmt from"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail AS PRD WItH(NOLOCK) LeFT OUTER JOIN"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request AS PR WItH(NOLOCK) ON PRD.FTPRPurchaseNo=PR.FTPRPurchaseNo LEFT OUTER JOIN"
        ...
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitAsset AS U WITH(NOLOCK) ON PRD.FNHSysUnitId=U.FNHSysUnitAssetId LEFT OUTER JOIN"
        _Qry &= vbCrLf & "(SELECT L.FTNameTH ,L.FTNameEN,L.FNListIndex FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L WHERE L.FTListName ='FNFixedAssetType')AS L   ON PRD.FNFixedAssetType=L.FNListIndex"
        _Qry &= vbCrLf & "where A.FTAssetCode<>'' AND PRD.FTPRPurchaseNo='" & Me.FTPRPurchaseNo.Text & "'"
        _Qry &= vbCrLf & ")AS AC"
...
#Region " function "
    ...
    Private Function SaveData() As Boolean
        ...
        If (_StateNew) Then
            ...
            _Key = HI.TL.Document.GetDocumentNo(Me.SysDBName, Me.SysTableName, "", False, _CmpH & "PR" & _STA & "-" & _Year & _Month & _Day).ToString
        ...
        Try
            ...
            'Dim _Cmd As String = ""
            '_Cmd = "Delete From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_File"
            ...
            '_Date = HI.Conn.SQLConn.GetFieldOnBeginTrans("Select " & HI.UL.ULDate.FormatDateDB, Conn.DB.DataBaseName.DB_FIXED, "")
            '_Time = HI.Conn.SQLConn.GetFieldOnBeginTrans("Select " & HI.UL.ULDate.FormatTimeDB, Conn.DB.DataBaseName.DB_FIXED, "")
            ...
            '    _Cmd = "insert into  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TFIXEDTPurchase_Request_File (FTInsUser , FDInsDate ,FTInsTime ,FTPRPurchaseNo ,FNSeq ,FBFile , FNFileType ,FTFileName)"
            '    _Cmd &= " Values( @FTInsUser , @FDInsDate ,@FTInsTime ,@FTPRPurchaseNo  ,@FNSeq ,@FBFile ,@FNFileType , @FTFileName)"
            ...
            For Each t As DevExpress.XtraTab.XtraTabPage In Me.oTabcAttachedDetail.TabPages
                ...
                _Cmd = "insert into  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_File (FTInsUser , FDInsDate ,FTInsTime ,FTPRPurchaseNo ,FNSeq ,FBFile , FNFileType ,FTFileName)"
                _Cmd &= " Values( @FTInsUser , @FDInsDate ,@FTInsTime ,@FTPRPurchaseNo  ,@FNSeq ,@FBFile ,@FNFileType , @FTFileName)"
                ...
                Dim p2 As New SqlParameter("@FTInsUser", SqlDbType.NVarChar)
                p2.Value = HI.ST.UserInfo.UserName
                Dim p3 As New SqlParameter("@FDInsDate", SqlDbType.VarChar)
                p3.Value = _Date
                Dim p4 As New SqlParameter("@FTInsTime", SqlDbType.VarChar)
                p4.Value = _Time
                Dim p6 As New SqlParameter("@FNFileType", SqlDbType.Int)
                p6.Value = IIf(_FileType = ".pdf", 0, 1)
                Dim p7 As New SqlParameter("@FTFileName", SqlDbType.NVarChar)
                p7.Value = _FileName
...
#End Region
    ...
    Private Sub ocmadd_Click(sender As Object, e As EventArgs) Handles ocmadd.Click
            ...
            With _AddPopup
                ...
                If (.AddMat) Then
                    ...
                    Try
                        ...
                        _Str = "insert into [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail (FTInsUser, FDInsDate, FTInsTime"
                        _Str &= vbCrLf & ",FTPRPurchaseNo, FNSeq, FNHSysFixedAssetId, FNHSysUnitId, FNPrice, FNDisPer, FNDisAmt, FNQuantity, FNNetAmt,FTRemark,FNFixedAssetType)"
                        _Str &= vbCrLf & "select '" & ST.UserInfo.UserName & "'," & UL.ULDate.FormatDateDB & "," & UL.ULDate.FormatTimeDB & ""
                        ...
                        _Str &= vbCrLf & "," & .FNPrice.Value & "," & .FNDisPer.Value & "," & .FNDisAmt.Value & "," & .FNQuantity.Value & "," & .FNNetAmt.Value & ",'" & .FTRemark.Text & "'," & _Type & ""
                        ...
                        _Str = "select isnull(sum(convert(numeric(18,2),PD.FNNetAmt)),0) AS FNNetAmt"
                        _Str &= vbCrLf & "from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail AS PD WiTH(NOLOCK) where PD.FTPRPurchaseNo='" & FTPRPurchaseNo.Text & "'"
                        Me.FNPRAmt.Value = Val(HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_FIXED, "0"))
                        Me.SaveData()
                    Catch ex As Exception
                        HI.Conn.SQLConn.Tran.Rollback()
                        HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
            ...
            With _AddPopup1
                ...
                If (.AddMat) Then
                    ...
                    Try
                        ...
                        _Str = "insert into [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail (FTInsUser, FDInsDate, FTInsTime"
                        _Str &= vbCrLf & ",FTPRPurchaseNo, FNSeq, FNHSysFixedAssetId, FNHSysUnitId, FNPrice, FNDisPer, FNDisAmt, FNQuantity, FNNetAmt,FTRemark,FTDescription,FNFixedAssetType)"
                        _Str &= vbCrLf & "select '" & ST.UserInfo.UserName & "'," & UL.ULDate.FormatDateDB & "," & UL.ULDate.FormatTimeDB & ""
                        _Str &= vbCrLf & ",'" & UL.ULF.rpQuoted(Me.FTPRPurchaseNo.Text) & "'," & Val(_FNSeq) & ",'" & .FTAssetCode.Properties.Tag & "','" & .FNHSysUnitAssetId.Properties.Tag & "'"
                        _Str &= vbCrLf & "," & .FNPrice.Value & "," & .FNDisPer.Value & "," & .FNDisAmt.Value & "," & .FNQuantity.Value & "," & .FNAmount.Value & ",'" & .FTNote.Text & "','" & .FTDescription.Text & "'," & _Type & ""
        ...
        End If
        Dim _PRSta As String = ""

        If FNPRState.Text = "สั่งจ้าง" Then


            _PRSta = HI.Conn.SQLConn.GetField("SELECT S.FNTax FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS S  WITH(NOLOCK) WHERE  S.FTSuplCode='" & Me.FNHSysSuplId.Text & "'", Conn.DB.DataBaseName.DB_MASTER, "")
            FNVatPer.Text = _PRSta
            FNTaxPer.Text = "3.00"
        Else
            _PRSta = HI.Conn.SQLConn.GetField("SELECT S.FNTax FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS S  WITH(NOLOCK) WHERE  S.FTSuplCode='" & Me.FNHSysSuplId.Text & "'", Conn.DB.DataBaseName.DB_MASTER, "")
            FNVatPer.Text = _PRSta
    ...
    Private Sub ocmclear_Click(sender As Object, e As EventArgs) Handles ocmclear.Click
        ...
        Me.FNPRAmt.Value = "0" And
        Me.FNDisCountPer.Value = "0" And
        Me.FNDisCountAmt.Value = "0" And
        Me.FNAmt.Value = "0" And
        Me.FNAmtBeforeVat.Value = "0" And
        Me.FNTaxPer.Value = "0" And
        Me.FNTaxAmt.Value = "0" And
        Me.FNPRNetAmt.Value = "0" And
        Me.FNVatPer.Value = "0" And
        Me.FNVatAmt.Value = "0" And
        Me.FNSurcharge.Value = "0" And
        Me.FNPRGrandAmt.Value = "0"
    ...
    Private Sub ocmsendpoapprove_Click(sender As Object, e As EventArgs) Handles ocmsendpoapprove.Click
        ...
        If Me.FTPRPurchaseNo.Text <> "" And Me.FTPRPurchaseNo.Properties.Tag.ToString <> "" Then
            ...
            If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_FIXED, "") <> "1" Then
                ...
                Dim _SAF As String = HI.Conn.SQLConn.GetField("Select s.FNHSysAssetTyped   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_Detail AS D  LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetSafety AS S ON D.FNFixedAssetType=S.FNFixedAssetType AND  D.FNHSysFixedAssetId=S.FNHSysAssetTyped  WHERE D.FTPRPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPRPurchaseNo.Text) & "'  AND S.FNHSysAssetTyped<>''", Conn.DB.DataBaseName.DB_FIXED, "")
                If _SAF <> "" Then
                    ...
                    _Qry &= vbCrLf & "  ,FTStateSafety='0' "
                    _Qry &= vbCrLf & "  ,FTSafetyName='' "
    ...
    Private Sub LoadFile()
        Try
            ...
            _Cmd = "Select   FTPRPurchaseNo, FNSeq, FBFile , FNFileType ,FTFileName  "
            _Cmd &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request_File  WITH(NOLOCK)"
            _Cmd &= vbCrLf & " WHERE FTPRPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPRPurchaseNo.Text) & "'"
            _Cmd &= vbCrLf & " Order by FNSeq asc"
            _oDT = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_FIXED,, False)
            Me.oTabcAttachedDetail.TabPages.Clear()
    ...
    '    End Try
    'End Sub
    'Private Sub Calculate(sender As System.Object, e As System.EventArgs) Handles FNDisCountAmt.EditValueChanged,
    '                                                                                FNDisCountPer.EditValueChanged,
    ...
    '    '_Proc = True
    '    'Dim _POAmt As Double = FNPRAmt.Value
    ...
    '    ''If FNDisCountAmt.Value <> "0" Then
    '    ''    _Amt = _POAmt - FNDisCountAmt.Value
    '    ''End If


    '    'Dim _VAT As String = HI.Conn.SQLConn.GetField("Select R.FNVatType   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request AS R    WHERE R.FTPRPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPRPurchaseNo.Text) & "' ", Conn.DB.DataBaseName.DB_FIXED, "")

    '    ''If _VAT = "0" Then
    '    ''    Me.FNAmtBeforeVat.Value = _Amt
    '    ''Else
    '    ''    Me.FNAmtBeforeVat.Value = (_Amt - (_Amt * 7 / 107))
    '    ''End If
    ...
    '    'Dim _FNAmtVat As Double = FNAmtBeforeVat.Value
    ...
    '    ''        _VatPer = FNVatPer.Value
    '    ''        _VatAmt = Format((_FNAmtVat * _VatPer) / 100, HI.ST.Config.AmtFormat)
    '    ''        FNVatAmt.Value = _VatAmt
    ...
    '    '        '    If (_POAmt - _DisAmt) > 0 Then
    '    '        '        _VatPer = Format((_VatAmt * 100) / _FNAmtVat, HI.ST.Config.PercentFormat)
    '    '        '    Else
    '    '        '        _VatPer = 0
    '    '        '    End If
    '    '        '    FNVatPer.Value = _VatPer
    ...
    '    'If _VAT = "0" Then
    '    '    Me.FNAmtBeforeVat.Value = FNAmt.Value
    '    'Else
    '    '    Me.FNAmtBeforeVat.Value = (FNAmt.Value - (FNAmt.Value * 7 / 107))
    '    'End If


    '    ''If Me.FNDisCountPer.Text = "0.00000" Then
    '    ''    Me.FNPRNetAmt.Value = (_POAmt - _TaxAmt)
    '    ''Else
    '    ''    Me.FNPRNetAmt.Value = (_FNAmt - _TaxAmt)
    '    ''End If
    '    'Me.FNPRNetAmt.Value = (_FNAmtVat - _TaxAmt)
    ...
    '    'FNPRGrandAmt.Value = Format(Me.FNPRNetAmt.Value + FNVatAmt.Value + _SurAmt, HI.ST.Config.AmtFormat)
    '    'Me.FTPRGrandAmtEN.Text = HI.UL.ULF.Convert_Bath_EN(FNPRGrandAmt.Value)
    '    'Me.FTPRGrandAmtTH.Text = HI.UL.ULF.Convert_Bath_TH(FNPRGrandAmt.Value)
    '    '_Proc = False
    '    '' End If
    ...
    '    If _VAT = "0" Then
    '        Me.FNAmtBeforeVat.Value = _FNAmt
    '    Else
    '        Me.FNAmtBeforeVat.Value = (_FNAmt - (_FNAmt * 7 / 107))
    '    End If
    ...
    '    '        End If
    '    'End Select
    '    Select Case Me.FNDisCountPer.EditValue
    '        Case 0
    '            _DisPer = FNDisCountPer.Value
    '            FNDisCountAmt.Value = _DisAmt
    '        Case Else
    ...
    '    Select Case Me.FNDisCountAmt.EditValue
    '        Case 0
    '            _DisAmt = FNDisCountAmt.Value
    '            FNDisCountPer.Value = _DisPer
    '        Case Else
    ...
    Private Sub Calculate(sender As System.Object, e As System.EventArgs) Handles FNDisCountPer.EditValueChanged,
        ...
        If Not (_Proc) And Not (_ProcLoad) Then
            _Proc = True
            Dim _PRAmt As Double = FNPRAmt.Value
            ...
            Dim _TaxPer As Double = FNTaxPer.Value
            Dim _TaxAmt As Double = FNTaxAmt.Value
            Dim _PRNet As Double = FNPRNetAmt.Value
            Dim _Amt As Double = 0

            Select Case sender.Name.ToString.ToUpper
                Case "FNDisCountPer".ToUpper
                    _DisPer = FNDisCountPer.Value
                    _DisAmt = Format((_PRAmt * _DisPer) / 100, HI.ST.Config.AmtFormat)
                    FNDisCountAmt.Value = _DisAmt
                Case "FNDisCountAmt".ToUpper
                    _DisAmt = FNDisCountAmt.Value

                    If _PRAmt > 0 Then
                        _DisPer = Format((_DisAmt * 100) / _PRAmt, HI.ST.Config.PercentFormat)
                    Else
                        _DisPer = 0
                    End If
                    FNDisCountPer.Value = _DisPer
                    _VatPer = FNVatPer.Value
                    _Amt = ((_PRAmt - _DisAmt) - (((_PRAmt - _DisAmt) * 7) / 107))
                ...
                Case "FNVatPer".ToUpper
                    _VatPer = FNVatPer.Value
                    If FNVatType.Text = "ไม่รวมภาษีมูลค่าเพิ่ม" Then
                        _Amt = ((_PRAmt - _DisAmt) - (((_PRAmt - _DisAmt) * 7) / 107))
                        _VatAmt = Format((_Amt * _VatPer) / 100, HI.ST.Config.AmtFormat)
                    Else
                        _Amt = _PRAmt
                ...
                Case "FNVatAmt".ToUpper
                    ...
                    If (_PRAmt - _DisAmt) > 0 Then
                        _VatPer = Format((_VatAmt * 100) / _Amt, HI.ST.Config.PercentFormat)
                    Else
                        _VatPer = 0
                    End If
                    FNVatPer.Value = _VatPer
                Case Else
                    ...
                    _VatPer = FNVatPer.Value
                    _VatAmt = Format(((_PRAmt - _DisAmt) * _VatPer) / 100, HI.ST.Config.AmtFormat)
                    FNVatAmt.Value = _VatAmt
            End Select

            Me.FNAmt.Value = (_PRAmt - _DisAmt)

            Dim _VAT As String = HI.Conn.SQLConn.GetField("Select R.FNVatType   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Request AS R    WHERE R.FTPRPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPRPurchaseNo.Text) & "' ", Conn.DB.DataBaseName.DB_FIXED, "")

            If _VAT = "0" Then
                Me.FNAmtBeforeVat.Value = Me.FNAmt.Value
            Else
                Me.FNAmtBeforeVat.Value = (Me.FNAmt.Value - ((Me.FNAmt.Value * 7) / 107))
            End If

            Me.FNPRNetAmt.Value = Format(Me.FNAmtBeforeVat.Value + FNVatAmt.Value + _SurAmt, HI.ST.Config.AmtFormat)
            ...
            FNPRGrandAmt.Value = Format(Me.FNPRNetAmt.Value - _TaxAmt, HI.ST.Config.AmtFormat)
            Me.FTPRGrandAmtEN.Text = HI.UL.ULF.Convert_Bath_EN(FNPRGrandAmt.Value)
            Me.FTPRGrandAmtTH.Text = HI.UL.ULF.Convert_Bath_TH(FNPRGrandAmt.Value)
            ...
            If _PRAmt <> 0 Then
                ...
                If _DisAmt <> 0 Then
                    Me.SaveData()
                ElseIf _TaxAmt <> 0 Then
                    Me.SaveData()
                ElseIf _VatAmt <> 0 Then
                    Me.SaveData()
                End If
