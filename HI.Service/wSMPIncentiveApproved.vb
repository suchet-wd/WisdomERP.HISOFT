...
Public Class wSMPIncentiveApproved
...
#End Region
    ...
    Public Shared Function LoadogcTPURTPurchase() As DataTable
        Try
            ...
            _Qry = " SELECT 1 AS FNDataType,'0' AS FTSelect,FTStateFinishDate AS FTStateFinishDateOrg"
            ...
            _Qry &= vbCrLf & ",ISNULL(PX.FNAmount,0) AS FNAmount"

            _Qry &= vbCrLf & " 	,'' AS FNHSysEmpID"
            ...
            _Qry &= vbCrLf & " SELECT 2 AS FNDataType,'0' AS FTSelect,FTCalculateDate AS FTStateFinishDateOrg"
            _Qry &= vbCrLf & " 	,CASE WHEN ISDATE(A.FTCalculateDate) = 1 THEN  Convert(varchar(10),convert(Datetime,A.FTCalculateDate) ,103) ELSE '' END AS FTStateFinishDate"
            _Qry &= vbCrLf & " 	,'CUT TEAM' FTSMPOrderNo, '' FTTeam ,'0' FTStateFinish "
            _Qry &= vbCrLf & ", '1' AS FTStateCal"
            _Qry &= vbCrLf & " 	,CASE WHEN ISDATE(A.FTCalculateDate) = 1 THEN  Convert(varchar(10),convert(Datetime,A.FTCalculateDate) ,103) ELSE '' END AS FTStateCalDate"
            _Qry &= vbCrLf & " 	, A.FTInsUser AS FTStateCalBy"
            _Qry &= vbCrLf & " 	,ISNULL(Emp.FTEmpName,'') AS FTEmpName"
            _Qry &= vbCrLf & ",0 AS FNQuantity"
            _Qry &= vbCrLf & ",ISNULL(A.FNCutAmount,0) AS FNCutAmount"
            ...
            _Qry &= vbCrLf & "  Select  STUFF((Select  ', ' + FTEmpName  "
            _Qry &= vbCrLf & "	From (SELECT Convert(nvarchar(10),Row_number() Over(Order By X1.FNSeq)) + '.'  + X2.FTEmpNameTH + ' ' + FTEmpSurnameTH   AS FTEmpName"
            _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculateCut_Emp As X1 INNER JOIN"
            _Qry &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee AS X2 ON X1.FNHSysEmpID = X2.FNHSysEmpID"
            _Qry &= vbCrLf & "   WHERE X1.FTCalculateDate = A.FTCalculateDate "
            _Qry &= vbCrLf & "  ) As T "
            _Qry &= vbCrLf & "	FOR XML PATH(''), TYPE ).value('.', 'NVARCHAR(MAX)'),1,1,'') AS FTEmpName"
            _Qry &= vbCrLf & " ) As Emp	"
    ...
    Friend Function Update_ManagerApproved(ByVal TempGrid As DevExpress.XtraGrid.Views.Grid.GridView, ByVal TempStatus As String) As Boolean
        ...
        Try
            ...
            For i = 0 To TempGrid.RowCount - 1
                ...
                If TempGrid.GetRowCellValue(i, "FTSelect").ToString() = 1 Then
                    _Str = ""
        ...
        Try
            ...
            With ogvSupervisorApproved
                ...
                For i = 0 To .RowCount - 1
                    ...
                    'If .GetRowCellValue(i, "FTSelect").ToString() = 1 Then


                    '    If .GetRowCellValue(i, "FNDataType").ToString() = 2 Then
                    '        Dim dataDate As String = "" & .GetRowCellValue(i, "FTStateFinishDateOrg").ToString
                    ...
                    '        _Qry &= vbCrLf & " WHERE FTCalculateDate='" & dataDate & "' "

                    '        HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_SAMPLE)
                    '    Else
                    '        Dim dataSMPOrderNo As String = "" & .GetRowCellValue(i, "FTSMPOrderNo").ToString()
                    '        Dim dataTeam As String = "" & .GetRowCellValue(i, "FTTeam").ToString
                    ...
                    '        _Qry &= vbCrLf & " WHERE FTSMPOrderNo='" & HI.UL.ULF.rpQuoted(dataSMPOrderNo) & "' AND FTTeam='" & HI.UL.ULF.rpQuoted(dataTeam) & "' "
                    ...
                    If .GetRowCellValue(i, "FTSelect").ToString() = 1 Then
                        ...
                        If .GetRowCellValue(i, "FNDataType").ToString() = 2 Then
                            ...
                            _Qry = " Update [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculateCut "
                            _Qry &= vbCrLf & " SET FTStateManagerApprove='1'"
                            _Qry &= vbCrLf & ",FTManagerApproveBy='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                            _Qry &= vbCrLf & " ,FTManagerApproveDate=" & HI.UL.ULDate.FormatDateDB & ""
                            _Qry &= vbCrLf & " ,FTManagerApproveTime=" & HI.UL.ULDate.FormatTimeDB & ""
                            _Qry &= vbCrLf & " , FTStateApprove='1'"
                            _Qry &= vbCrLf & ",FTApproveBy='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                            _Qry &= vbCrLf & " ,FTApproveDate=" & HI.UL.ULDate.FormatDateDB & ""
                            _Qry &= vbCrLf & " ,FTApproveTime=" & HI.UL.ULDate.FormatTimeDB & ""
                            _Qry &= vbCrLf & " WHERE FTCalculateDate='" & dataDate & "' "
                            ...
                            _Qry = "DELETE X "
                            _Qry &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTWageDaily AS X"
                            _Qry &= vbCrLf & " INNER JOIN ( Select FNHSysEmpID,  FTCalculateDate "
                            _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculateCut_Emp AS X WITH(NOLOCK) "
                            _Qry &= vbCrLf & " WHERE FTCalculateDate='" & dataDate & "' "
                            _Qry &= vbCrLf & " GROUP BY FNHSysEmpID,  FTCalculateDate "
                            _Qry &= vbCrLf & " ) AS B ON X.FNHSysEmpID=B.FNHSysEmpID And X.FTDateTrans=B.FTCalculateDate "
                            _Qry &= vbCrLf & "  WHERE  FTStateSampleRoom='1' "


                            HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_SAMPLE)

                            _Qry = "Insert into [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTWageDaily("
                            _Qry &= vbCrLf & "  FTInsUser, FTInsDate, FTInsTime,FNHSysEmpID, FTDateTrans, FNAmtNormal, FNAmtOT, FNNetAmt, FNProNormal, FNProOT, FNProOther, FNNetProAmt, FTStateSampleRoom "
                            _Qry &= vbCrLf & " ) "
                            _Qry &= vbCrLf & " Select  FTInsUser, FDInsDate, FTInsTime,FNHSysEmpID,  FTCalculateDate, FNAmtNormal, FNAmtOT, FNNetAmt,FNNetAmt+ FNIncAmount,0,0,FNNetAmt+FNIncAmount,'1' AS FTStateSampleRoom"
                            _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculateCut_Emp AS X WITH(NOLOCK) "
                            _Qry &= vbCrLf & " WHERE FTCalculateDate='" & dataDate & "' "
                        ...
                        Else
                            ...
                            _Qry &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTWageDaily AS X"
                            _Qry &= vbCrLf & " INNER JOIN ( Select FNHSysEmpID,  FTCalculateDate "
                            _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculate_Emp AS X WITH(NOLOCK) "
                            _Qry &= vbCrLf & " WHERE FTSMPOrderNo='" & HI.UL.ULF.rpQuoted(dataSMPOrderNo) & "' AND FTTeam='" & HI.UL.ULF.rpQuoted(dataTeam) & "' "
                            _Qry &= vbCrLf & " GROUP BY FNHSysEmpID,  FTCalculateDate "
                            _Qry &= vbCrLf & " ) AS B ON X.FNHSysEmpID=B.FNHSysEmpID And X.FTDateTrans=B.FTCalculateDate "
                            _Qry &= vbCrLf & "  WHERE  FTStateSampleRoom='1' "
                            ...
                            _Qry &= vbCrLf & "  FTInsUser, FTInsDate, FTInsTime,FNHSysEmpID, FTDateTrans, FNAmtNormal, FNAmtOT, FNNetAmt, FNProNormal, FNProOT, FNProOther, FNNetProAmt, FTStateSampleRoom "
                            _Qry &= vbCrLf & " ) "
                            _Qry &= vbCrLf & " Select  MAX(X.FTInsUser) AS FTInsUser"
                            _Qry &= vbCrLf & " , MAX(X.FDInsDate) AS FDInsDate"
                            _Qry &= vbCrLf & ", MAX(X.FTInsTime) AS FTInsTime "
                            _Qry &= vbCrLf & " ,X.FNHSysEmpID,  X.FTCalculateDate"
                            _Qry &= vbCrLf & " , MAX(X.FNAmtNormal) AS FNAmtNormal"
                            _Qry &= vbCrLf & ", MAX(X.FNAmtOT) AS FNAmtOT"
                            _Qry &= vbCrLf & ", MAX(X.FNNetAmt) AS FNNetAmt"
                            _Qry &= vbCrLf & ",MAX(X.FNNetAmt)+ SUM(FNIncAmount)"
                            _Qry &= vbCrLf & ",0,0"
                            _Qry &= vbCrLf & ",MAX(X.FNNetAmt)+ SUM(X.FNIncAmount)"
                            _Qry &= vbCrLf & ",'1' AS FTStateSampleRoom"
                            _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculate_Emp AS X WITH(NOLOCK) "
                            _Qry &= vbCrLf & " INNER JOIN ( Select FNHSysEmpID,  FTCalculateDate "
                            _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPCalculate_Emp AS X WITH(NOLOCK) "
                            _Qry &= vbCrLf & " WHERE FTSMPOrderNo='" & HI.UL.ULF.rpQuoted(dataSMPOrderNo) & "' AND FTTeam='" & HI.UL.ULF.rpQuoted(dataTeam) & "' "
                            _Qry &= vbCrLf & " GROUP BY FNHSysEmpID,  FTCalculateDate "
                            _Qry &= vbCrLf & " ) AS B ON X.FNHSysEmpID=B.FNHSysEmpID And X.FTCalculateDate=B.FTCalculateDate "
                            _Qry &= vbCrLf & " GROUP BY X.FNHSysEmpID,  X.FTCalculateDate"
