...
#Region "Command Button"
    ...
    Private Sub ocmclear_Click(sender As Object, e As EventArgs) Handles ocmclear.Click
        Try
            ...
            Me.FNSelectQty_lbl.Text = 0
            Me.FNSelectCartonQty_lbl.Text = 0
...
#Region "Processing"
    ...
    Private Sub LoadData1()
        ...
        Try
            ...
            _Cmd = "Exec    " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FG) & ".dbo.SP_GET_ONHAND_FG_CD '" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdFG.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdFGTo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNoTo.Text) & "'"
            ...
            _Cmd = "Exec    " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FG) & ".dbo.SP_GET_ONHAND_FG_SUM '" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdFG.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdFGTo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNoTo.Text) & "'"
            _Cmd &= ",'" & HI.UL.ULF.rpQuoted(Me.FTCustomerPO.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTCustomerPOTo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTTransferFGNo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTTransferFGNoTo.Text) & "'"

            _oDt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_FG)
            Me.ogcsum.DataSource = _oDt
...
#End Region
    ...
    Private Function Auto() As Boolean
        ...
        Try
            ...
            With CType(Me.ogcdetail.DataSource, DataTable)
                ...
                _Qry &= vbCrLf & " ("
                _Qry &= vbCrLf & "FTInsUser, FDInsDate, FTInsTime, FTTransferFGNo, FDDateTransferFG, FTTransferFGBy, FNHSysWHIdFG, FNHSysWHIdFGTo,  FNHSysCmpId ,FTRemark "
                _Qry &= vbCrLf & "  )"
                ...
                _Qry &= vbCrLf & "," & Integer.Parse(HI.ST.SysInfo.CmpID)
                _Qry &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(_AutoTransferToWH.FTRemark.Text) & "' "
                ...
                For Each R As DataRow In .Select("FTSelect='1'")
                    _Qry = "insert into " & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & ".dbo.TFGTransferFG_Detail"
            ...
            '_Qry = "UPDATE " & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & ".dbo.TFGTransferFG"
        ...
        With ogvdetail
            ...
            For Each i As Integer In .GetSelectedRows()
                ...
                Try
                    ...
                    HI.Conn.SQLConn.Cmd = HI.Conn.SQLConn.Cnn.CreateCommand
                    HI.Conn.SQLConn.Tran = HI.Conn.SQLConn.Cnn.BeginTransaction
                    _Qry = "Delete From " & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & ".dbo.TFGTransferFG_Detail WHERE FTBarCodeCarton='" & HI.UL.ULF.rpQuoted(_Barcode.ToString) & "' AND FTTransferFGNo='" & HI.UL.ULF.rpQuoted(_FGNo.ToString) & "'"
                    If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                        HI.Conn.SQLConn.Tran.Rollback()
                        HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                        HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
    ...
    Private Function CheckStageApprove() As Boolean
        Dim _Qry As String = ""
        _Qry = "SELECT TOP 1 ISNULL(F.FTStateApprove,'') FROM" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & ".dbo.TFGTransferFG AS F"
                ...
                If Not (.DataSource Is Nothing) And ogvdetail.RowCount > 0 Then
                    ...
                    With ogvdetail
                        For I As Integer = 0 To .RowCount - 1
                            ...
                            If _State = "1" Then
                                Me.FNSelectCartonQty_lbl.Text = Format(_Qty + Double.Parse("0" & Me.FNSelectCartonQty_lbl.Text), "#,##0.00")
                                Me.FNSelectQty_lbl.Text = Format(_select + Double.Parse("0" & Me.FNSelectQty_lbl.Text), "#,##0")
                            Else
                                Me.FNSelectCartonQty_lbl.Text = Format(Double.Parse("0" & Me.FNSelectCartonQty_lbl.Text) - _Qty, "#,##0.00")
                                Me.FNSelectQty_lbl.Text = Format(Double.Parse("0" & Me.FNSelectQty_lbl.Text) - _select, "#,##0")
                            End If
                        Next
                    End With
    ...
    Private Sub XtraTabControl1_SelectedPageChanged(sender As Object, e As DevExpress.XtraTab.TabPageChangedEventArgs) Handles XtraTabControl1.SelectedPageChanged
        Try
            If e.Page.Name.ToString = "XtraTabPage1" Then
                Dim _oDt As DataTable
                With DirectCast(Me.ogcsum.DataSource, DataTable)
                    .AcceptChanges()
                    _oDt = .Copy
                End With
                Dim _PORef As String = "" : Dim _Cmd As String = ""
                Dim _LineItem As String = ""
                For Each R As DataRow In _oDt.Select("FTSelect = '1'")
                    If _PORef <> "" Then _PORef &= ","
                    _PORef &= "" & HI.UL.ULF.rpQuoted(R!FTPOref.ToString)

                    If _LineItem <> "" Then _LineItem &= ","
                    _LineItem &= "" & HI.UL.ULF.rpQuoted(R!FTNikePOLineItem.ToString)

                Next


                _Cmd = "Exec    " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FG) & ".dbo.SP_GET_ONHAND_FG_CD '" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdFG.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdFGTo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNoTo.Text) & "'"
                _Cmd &= ",'" & _PORef & "','" & HI.UL.ULF.rpQuoted(Me.FTCustomerPOTo.Text) & "','" & _LineItem & "','" & HI.UL.ULF.rpQuoted(Me.FTTransferFGNoTo.Text) & "'"
    ...
    Private Sub ogvdetail_RowCountChanged(sender As Object, e As EventArgs) 'Handles ogvdetail.RowCountChanged
        Try
            ...
            With DirectCast(Me.ogcdetail.DataSource, DataTable)
                ...
                _oDt = .Copy()
            End With
            Me.FNTalalCBM.Value = Double.Parse(Val(_oDt.Compute("sum(FNCMB)", "CBM>0").ToString))
        Catch ex As Exception
            Me.FNTalalCBM.Value = 0
        End Try
    End Sub
    ...
    Private Sub RepositoryItemSumSelect_EditValueChanging(sender As Object, e As ChangingEventArgs) Handles RepositoryItemSumSelect.EditValueChanging
        Try
            Dim _oDt As DataTable
            ...
            End With
            Try
                Me.FNSelectQty_lbl.Text = Format(Double.Parse(Val(_oDt.Compute("sum(FNTotalQuantityPCS)", "FTSelect ='1' and FNCartonCURENTQty > 0").ToString)), "##,##0")
            Catch ex As Exception
                Me.FNSelectQty_lbl.Text = 0
            End Try
            Try
                Me.FNSelectCartonQty_lbl.Text = Format(Double.Parse(Val(_oDt.Compute("sum(FNCartonCURENTQty)", "FTSelect ='1' and FNCartonCURENTQty > 0").ToString)), "##,##0")
            Catch ex As Exception
                Me.FNSelectCartonQty_lbl.Text = 0
            End Try
            Try
                Me.FNTalalCBM.Value = Double.Parse(Val(_oDt.Compute("sum(FNCMB)", "FTSelect ='1' and FNCMB>0").ToString))
