...
Public Class wFGAdjust
    ...
    Private Sub wFGAdjust_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            Me.FTBarcodeCust.EnterMoveNextControl = False
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        Call LoadDetail(Me.FTAdjustFGNo.Text)
        _ProcLoad = False
    End Sub
    ...
    Private Function SaveDetail(key As String) As Boolean
        Try
            ...
            For Each R As DataRow In _oDt.Rows
                _Cmd = "UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG_Detail "
                ...
                _Cmd &= vbCrLf & "WHERE FTAdjustFGNo='" & HI.UL.ULF.rpQuoted(key) & "'"
                _Cmd &= vbCrLf & "And FTCustBarcodeNo='" & HI.UL.ULF.rpQuoted(R!FTCustBarcodeNo.ToString) & "'"
                _Cmd &= vbCrLf & "And FTOrderNo='" & HI.UL.ULF.rpQuoted(R!FTOrderNo.ToString) & "'"
                If HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    _Cmd = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG_Detail (FTInsUser, FDInsDate, FTInsTime,  FTAdjustFGNo, FTCustBarcodeNo, FNQuantity,FTOrderNo)"
                    _Cmd &= vbCrLf & "Select '" & HI.ST.UserInfo.UserName & "'"
                    _Cmd &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB
                    _Cmd &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB
                    _Cmd &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(key) & "'"
                    _Cmd &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(R!FTCustBarcodeNo.ToString) & "'"
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG WHERE FTAdjustFGNo='" & HI.UL.ULF.rpQuoted(Me.FTAdjustFGNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG_Detail WHERE FTAdjustFGNo='" & HI.UL.ULF.rpQuoted(Me.FTAdjustFGNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG WHERE FTAdjustFGNo='" & HI.UL.ULF.rpQuoted(Me.FTAdjustFGNo.Text) & "'")
...
#Region "Command"
    ...
    Private Sub ocmdelete_Click(sender As Object, e As EventArgs) Handles ocmdelete.Click
        Try
            If HI.MG.ShowMsg.mConfirmProcessDefaultNo(MG.ShowMsg.ProcessType.mDelete, Me.FTAdjustFGNo.Text, Me.Text) = False Then
                Exit Sub
            End If
...
#End Region
    ...
    Private Sub LoadDetail(key As String)
        Try
            ...
            End If
            _Cmd &= vbCrLf & ", VA.FTColorway, VA.FTSizeBreakDown, AJ.FTCustBarcodeNo, AJ.FNQuantity"
            _Cmd &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG_Detail AS AJ WITH (NOLOCK) LEFT OUTER JOIN"
            _Cmd &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FG) & "].dbo.V_GetDetailAdj AS VA   ON AJ.FTCustBarcodeNo = VA.FTCustBarcodeNo and AJ.FTOrderNo = VA.FTOrderNo"
            _Cmd &= vbCrLf & " LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH(NOLOCK) ON VA.FTOrderNo = O.FTOrderNo "
            _Cmd &= vbCrLf & " WHERE AJ.FTAdjustFGNo='" & HI.UL.ULF.rpQuoted(key) & "'"
    ...
    Private Sub FTBarcodeCust_KeyDown(sender As Object, e As KeyEventArgs) Handles FTBarcodeCust.KeyDown, FNQuantity.KeyDown
        Try
            If Me.VerrifyData Then
                Me.SaveData()
            End If
            If e.KeyCode = Keys.Enter Then
                ...
                If Me.FNQuantity.Value <= 0 Then
                    HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.SelectData, Me.FNQuantity_lbl.Text)
                    Me.FNQuantity.Focus()
                    Exit Sub
                End If

                Dim _Cmd As String = ""
                Dim _dt As DataTable = GetDetailBarcodeCust(Me.FTBarcodeCust.Text, Me.FTOrderNo.Text)
    ...
    Private Function GetDetailBarcodeCust(BarCode As String, OrderNo As String) As DataTable
        Try
            Dim _Cmd As String = ""
            _Cmd = "SELECT   Top 1    FTPORef, FTOrderNo, FTColorway, FTSizeBreakDown, FTCustBarcodeNo, FTProdTypeCode, FNHSysStyleId, FTStyleCode"
            If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
        ...
        Try
            ...
            If e.KeyCode = Keys.Delete Then
                With ogvdetail
                    ...
                    _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FG) & "].dbo.TFGAdjustFG_Detail WHERE FTAdjustFGNo='" & HI.UL.ULF.rpQuoted(Me.FTAdjustFGNo.Text) & "'"
                    _Str &= vbCrLf & " And FTCustBarcodeNo='" & HI.UL.ULF.rpQuoted(.GetRowCellValue(.FocusedRowHandle, "FTCustBarcodeNo").ToString) & "'"
                    HI.Conn.SQLConn.ExecuteNonQuery(_Str, Conn.DB.DataBaseName.DB_FG)
    ...
    Private Sub FTBarcodeCust_EditValueChanged(sender As Object, e As EventArgs) Handles FTBarcodeCust.EditValueChanged
