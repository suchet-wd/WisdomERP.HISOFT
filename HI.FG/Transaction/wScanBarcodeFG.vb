...
Public Class wScanBarcodeFG
    ...
    Private Function SaveData(_FTProductBarcodeNo As String, state As Boolean, Optional ByVal _FTOrderNo As String = "", Optional ByVal _FTColorway As String = "", Optional ByVal _FTSizeBreakDown As String = "") As Boolean
        Try
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                _Cmd = "INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBarcodeScanFG (FTInsUser, FDInsDate, FTInsTime, FTBarCodeCarton, FNHSysWHFGId, FTColorWay, FTSizeBreakDown, FTOrderNo, FNQuantity , FTPackNo , FNCartonNo ,FTSubOrderNo , FNHSysWHLocId ,FNHSysCartonId , FTCustomerPO ,FTPOLine)"
                ...
                _Cmd &= vbCrLf & " ,T.FTSubOrderNo"
                _Cmd &= vbCrLf & "," & CInt(Me.FNHSysWHLocId.Properties.Tag)
                _Cmd &= vbCrLf & " ,T.FNHSysCartonId"
                _Cmd &= vbCrLf & " ,T.FTCustomerPO"
                _Cmd &= vbCrLf & " ,T.FTPOLine"
                ...
                _Cmd &= vbCrLf & ",D.FNHSysCartonId"
                _Cmd &= vbCrLf & ",PD.FTCustomerPO"
                _Cmd &= vbCrLf & ",D.FTPOLine"
                ...
                _Cmd &= vbCrLf & "  LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Detail AS D WITH (NOLOCK) ON B.FTPackNo = D.FTPackNo AND B.FNCartonNo = D.FNCartonNo AND B.FTColorWay = D.FTColorWay AND B.FTSizeBreakDown = D.FTSizeBreakDown "
                _Cmd &= vbCrLf & "  LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack AS PD WITH (NOLOCK) ON B.FTPackNo = PD.FTPackNo  "
                _Cmd &= vbCrLf & " WHERE (C.FTBarCodeCarton='" & _FTProductBarcodeNo & "'"
                _Cmd &= vbCrLf & " OR   B.FTBarcodeNo='" & _FTProductBarcodeNo & "')"
                If (state) Then
                    _Cmd &= vbCrLf & " And  B.FTOrderNo='" & _FTOrderNo & "'"
                ...
                _Cmd &= vbCrLf & " SELECT     FTBarCodeCarton +'|'+FTOrderNo +'|'+FTColorWay +'|'+FTSizeBreakDown+'|'+FTPackNo+'|'+convert(nvarchar(18) , FNCartonNo)  "
                _Cmd &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBarcodeScanFG WITH(NOLOCK) )"
                _Cmd &= vbCrLf & "Group by  P.FTPackNo , P.FNCartonNo,   B.FTOrderNo, B.FTColorway, B.FTSizeBreakDown,Isnull(C.FTBarCodeCarton , B.FTBarcodeNo)"
                _Cmd &= vbCrLf & ",D.FTSubOrderNo"
                _Cmd &= vbCrLf & ",D.FNHSysCartonId"
                _Cmd &= vbCrLf & ",PD.FTCustomerPO"
                _Cmd &= vbCrLf & ",D.FTPOLine"
