...
Public Class wCustomerOrderValueTracking
...
#Region "MAIN PROC"
    ...
    Private Sub PROC_SAVE(sender As Object, e As EventArgs)
...
#Region "Sub Procedure And Sub Function"
    ...
    Private Function W_PRCbLoadFONoSubBreakdown(Optional StateClearFilter As Boolean = True) As Boolean
        ...
        Try
            ...
            cmdstring = " SELECT A.FNHSysStyleId, C.FTOrderNo, C.FTSubOrderNo,CASE WHEN ISNULL(C.FNOrderSetType,0) =1 THEN 'เสื้อ' WHEN ISNULL(C.FNOrderSetType,0) =2 THEN 'กางเกง' ELSE 'NONE' END AS FTOrderSetType, C.FTSubOrderNoRef"
            cmdstring &= vbCrLf & "     , D.FNHSysMatColorId, D.FTMatColorCode "
            cmdstring &= vbCrLf & "       , E.FNHSysMatSizeId, E.FTMatSizeCode "


            If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                cmdstring &= vbCrLf & "  ,D.FTMatColorNameTH AS  FTMatColorName "
                cmdstring &= vbCrLf & "  ,E.FTMatSizeNameTH AS  FTMatSizeName "
            Else
                cmdstring &= vbCrLf & "  ,D.FTMatColorNameEN AS  FTMatColorName "
                cmdstring &= vbCrLf & "  ,E.FTMatSizeNameEN AS  FTMatSizeName "
            End If

            cmdstring &= vbCrLf & " ,  C.FNQuantity"
            cmdstring &= vbCrLf & " , C.FNPrice,ISNULL(C.FNPriceOrg,C.FNPrice) As FNPriceOrg"
            cmdstring &= vbCrLf & " ,C.FNPrice -ISNULL(C.FNPriceOrg,C.FNPrice) As FNPriceDiff"
            cmdstring &= vbCrLf & " ,  C.FNAmt"
            cmdstring &= vbCrLf & " , C.FNExtraQty"
            cmdstring &= vbCrLf & " , C.FNQuantityExtra"
            cmdstring &= vbCrLf & " , C.FNGrandQuantity"
            cmdstring &= vbCrLf & " , C.FNAmntExtra"
            cmdstring &= vbCrLf & " , C.FNGrandAmnt"
            cmdstring &= vbCrLf & " , C.FNGarmentQtyTest"
            cmdstring &= vbCrLf & " ,CU.FTCurCode"
            cmdstring &= vbCrLf & " ,C.FTStateHold ,C.FTNikePOLineItem,A.FNHSysSeasonId ,SSA.FTSeasonCode,CASE WHEN ISNULL(C.FTPORef,'') ='' THEN A.FTPORef ELSE ISNULL(C.FTPORef,'')   END AS FTCustomerPO"
            cmdstring &= vbCrLf & " ,ISNULL(C.FNOperateFee,0) AS FNOperateFee ,ISNULL(C.FNOperateFeeAmt,0) AS FNOperateFeeAmt ,ISNULL(FNNetFOB,0) AS FNNetFOB  "
            cmdstring &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS A WITH(NOLOCK) "
            cmdstring &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS B WITH(NOLOCK) ON A.FTOrderNo = B.FTOrderNo"
            cmdstring &= vbCrLf & " 	                                              "
            cmdstring &= vbCrLf & "      LEFT JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_AllDivert As C  On B.FTOrderNo = C.FTOrderNo"
            cmdstring &= vbCrLf & "      And B.FTSubOrderNo = C.FTSubOrderNoRef"

            cmdstring &= vbCrLf & " 	 LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.[TMERMSeason] As SSA With(NOLOCK) On A.FNHSysSeasonId = SSA.FNHSysSeasonId"
            cmdstring &= vbCrLf & "      INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatColor AS D WITH(NOLOCK) ON C.FTColorway = D.FTMatColorCode"
            cmdstring &= vbCrLf & "      INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize As E With(NOLOCK) On C.FTSizeBreakDown = E.FTMatSizeCode LEFT OUTER JOIN HITECh_MASTER..TFINMCurrency As CU With(NOLOCK) On B.FNHSysCurId = CU.FNHSysCurId"

            cmdstring &= vbCrLf & "   WHERE  ISNULL(A.FTOrderNo,'') <> '' AND (A.FNJobState = 0 OR A.FNJobState = 1 OR A.FNJobState IS NULL)  "

            If FNHSysCmpId.Text <> "" Then
                cmdstring &= vbCrLf & "   AND  (A.FNHSysCmpId = " & Val(FNHSysCmpId.Properties.Tag.ToString) & ")  "
            End If

            If FNHSysPOID.Text <> "" Then
                cmdstring &= vbCrLf & "   AND  (CASE WHEN ISNULL(C.FTPORef,'') ='' THEN A.FTPORef ELSE ISNULL(C.FTPORef,'')  END ='" & HI.UL.ULF.rpQuoted(FNHSysPOID.Text) & "')  "
            End If

            If FNHSysStyleId.Text <> "" Then
                cmdstring &= vbCrLf & "   AND  (A.FNHSysStyleId = " & Val(FNHSysStyleId.Properties.Tag.ToString) & ")  "
            End If

            If FNHSysCustId.Text <> "" Then
                cmdstring &= vbCrLf & "   AND  (A.FNHSysCustId = " & Val(FNHSysCustId.Properties.Tag.ToString) & ")  "
            End If
            ...
            If FNHSysSeasonId.Text <> "" Then
                cmdstring &= vbCrLf & "   AND  (A.FNHSysSeasonId = " & Val(FNHSysSeasonId.Properties.Tag.ToString) & ")  "
            End If

            cmdstring &= vbCrLf & "   ORDER BY C.FTOrderNo ASC, C.FTSubOrderNo ASC, D.FNMatColorSeq ASC, E.FNMatSizeSeq ASC  "

            oDBdtFactorySubOrderNoBreakdown = HI.Conn.SQLConn.GetDataTable(cmdstring, Conn.DB.DataBaseName.DB_MERCHAN)
...
#End Region
    ...
    Private Sub ocmSaveFOB_Click(sender As Object, e As EventArgs)
        Try
            PDataTableTmp = New DataTable
            With PDataTableTmp.Columns
                .Add("FNHSysStyleId", GetType(Integer))
