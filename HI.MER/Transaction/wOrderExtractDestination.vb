...
Public Class wOrderExtractDestination

    Private _FormHeader As New List(Of HI.TL.DynamicForm)()
    Private oGridViewBreakdownDivertSrc As DevExpress.XtraGrid.Views.Grid.GridView
    Private _wDivertOrderDistination As wDivertOrderDistination
...
#End Region
    ...
    Public Sub LoadDetailSubDivertBreakDown()
        ...
        _Qry = "SELECT AA.FNDivertSeq,AA.oFTSubOrderNo,AA.ooFDShipDate"
        _Qry &= vbCrLf & ",AA.oFTContinetName,AA.oFTCountryName"
        _Qry &= vbCrLf & ",AA.oFTProvinceName,AA.oFTShipModenName,AA.oFTShipPortName"
        _Qry &= vbCrLf & ",AA.TotalQuantity,AA.FTRemark,AA.FTPORef,AA.FTPOTrading"
        _Qry &= vbCrLf & "FROM"
        _Qry &= vbCrLf & "(SELECT sd.FTSubOrderNo AS oFTSubOrderNo,SD.FTPORef,sd.FNDivertSeq AS FNDivertSeq,convert(varchar(10),convert(Datetime,sd.FDShipDate),103) AS ooFDShipDate,sd.FTRemark AS FTRemark,Max(SD.FTPOTrading) As FTPOTrading"

        If HI.ST.Lang.Language = HI.ST.SysInfo.LanguageLocal Then
            _Qry &= vbCrLf & ",conti.FTContinentNameTH AS oFTContinetName,cun.FTCountryNameTH AS oFTCountryName"
        ...
        Else
            _Qry &= vbCrLf & ",conti.FTContinentNameEN AS oFTContinetName,cun.FTCountryNameEN AS oFTCountryName"
        ...
        _Qry &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert AS SD inner join"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_BreakDown AS SDB with(NOLOCK) on sd.FTSubOrderNo=sdb.FTSubOrderNo AND sd.FTOrderNo=sdb.FTOrderNo AND sd.FNDivertSeq=sdb.FNDivertSeq LEFT OUTER JOIN"
        ...
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TCNMShipPort AS Shp with(NOLOCK) on sd.FNHSysShipPortId=shp.FNHSysShipPortId"
        _Qry &= vbCrLf & "where sd.FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"

        If HI.ST.Lang.Language = HI.ST.SysInfo.LanguageLocal Then
            _Qry &= vbCrLf & "GROUP BY sd.FTSubOrderNo,sd.FNDivertSeq,sd.FDShipDate,sd.FTRemark"
            _Qry &= vbCrLf & ",conti.FTContinentNameTH,cun.FTCountryNameTH"
            _Qry &= vbCrLf & ",pro.FTProvinceNameTH,sh.FTShipModenNameTH,shp.FTShipPortNameTH,SD.FTPORef) AS AA"
        Else
            _Qry &= vbCrLf & "GROUP BY sd.FTSubOrderNo,sd.FNDivertSeq,sd.FDShipDate,sd.FTRemark"
            _Qry &= vbCrLf & ",conti.FTContinentNameEN,cun.FTCountryNameEN"
            _Qry &= vbCrLf & ",pro.FTProvinceNameEN,sh.FTShipModeNameEN,shp.FTShipPortNameEN,SD.FTPORef) AS AA"
        End If
    ...
    Private Function DeleteDivert(Spls As HI.TL.SplashScreen) As Boolean
        Try
            ...
            '_QryChkSDB = "SELECT TOP 1 MAX(FNDivertSeq) AS MAXFNDivertSeq FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_BreakDown "
            'MaxSeqSDB = HI.Conn.SQLConn.GetField(_QryChkSDB, HI.Conn.DB.DataBaseName.DB_MERCHAN)
            '_QryChkSD = "SELECT TOP 1 MAX(FNDivertSeq) AS MAXFNDivertSeq FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert"
            'MaxSeqSD = HI.Conn.SQLConn.GetField(_QryChkSD, HI.Conn.DB.DataBaseName.DB_MERCHAN)
            ...
            _Qry = "Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_BreakDown"
            _Qry &= vbCrLf & "WHERE FNDivertSeq=" & Val(SeqSDB) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "Delete FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert"
            _Qry &= vbCrLf & "WHERE FNDivertSeq=" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "Delete FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_Sew"
            _Qry &= vbCrLf & "WHERE FNDivertSeq=" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "Delete FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_Pack"
            _Qry &= vbCrLf & "WHERE FNDivertSeq=" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "Delete FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_Bundle"
            _Qry &= vbCrLf & "WHERE FNDivertSeq=" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "Delete FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_SizeSpec"
            _Qry &= vbCrLf & "WHERE FNDivertSeq=" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub SET FDUpdDate=" & HI.UL.ULDate.FormatDateDB & " "
            _Qry &= vbCrLf & " WHERE FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            ...
            _Qry = "UPDATE A SET  FNDivertSeq=FNDivertSeq-1   From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_BreakDown AS A "
            _Qry &= vbCrLf & "WHERE FNDivertSeq>" & Val(SeqSDB) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE B SET FNDivertSeq=FNDivertSeq-1 FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert AS B "
            _Qry &= vbCrLf & "WHERE FNDivertSeq>" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE C SET FNDivertSeq=FNDivertSeq-1 FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_Sew AS C "
            _Qry &= vbCrLf & "WHERE FNDivertSeq>" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE D SET FNDivertSeq=FNDivertSeq-1 FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_Pack AS D "
            _Qry &= vbCrLf & "WHERE FNDivertSeq>" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE E SET FNDivertSeq=FNDivertSeq-1 FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_Bundle AS E "
            _Qry &= vbCrLf & "WHERE FNDivertSeq>" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE F SET FNDivertSeq=FNDivertSeq-1 FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_SizeSpec AS F "
            _Qry &= vbCrLf & "WHERE FNDivertSeq>" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
    ...
    Private Sub ocmDivert_Click(sender As Object, e As EventArgs) Handles ocmDivert.Click
        ...
        If Me.FTStateCutting.Checked = True Then

            If HI.MG.ShowMsg.mConfirmProcess("พบรายการ กำลังผลิต FO No.Sub นี้", 1507201525, Me.FTOrderNo.Text & "|" & Me.FTSubOrderNo.Text) = False Then
                Exit Sub
            End If

        Else

            HI.MG.ShowMsg.mInvalidData("ไม่พบรายการ กำลังผลิต FO No.Sub นี้", 1507201544, Me.FTOrderNo.Text)
            Me.FTOrderNo.Focus()
            Exit Sub

        End If

        If CreateDocumentMI(Me.FTOrderNo.Text, FTSubOrderNo.Text, True) Then
            Exit Sub
        End If

        If Me.FTOrderNo.Text.Trim <> "" Then

            _wDivertOrderDistination = New wDivertOrderDistination()
    ...
    Private Function CreateDocumentMI(_OrderNo As String, Optional SubOrderNo As String = "", Optional Add As Boolean = False) As Boolean
        Dim _Qry As String = ""

        If Add Then

            _Qry = " SELECT TOP 1 A.FTOrderNo  "
            _Qry &= vbCrLf & " FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS A "
            '_Qry &= vbCrLf & "         INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice AS B WITH(NOLOCK) ON A.FTPOref = B.FTCustomerPO AND A.FNHSysContinentId = B.FNHSysContinentId AND A.FNHSysCountryId = B.FNHSysCountryId AND "
            ...
            _Qry &= vbCrLf & " LEFT OUTER JOIN (SELECT A.FTOrderNo, A.FTSubOrderNo, A.FTColorway, A.FTSizeBreakDown, A.FTPOref, SUM(B.FNQuantity) AS FNQuantity  "
            _Qry &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "]..TACCTFactoryCMInvoice_D AS B WITH(NOLOCK)  "
            _Qry &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS A ON B.FTColorway = A.FTColorway AND B.FTSizeBreakDown = A.FTSizeBreakDown AND B.FTCustomerPO = A.FTPOref AND B.FTPOLineItem = A.FTNikePOLineItem "
            _Qry &= vbCrLf & "  GROUP BY A.FTOrderNo, A.FTSubOrderNo, A.FTColorway, A.FTSizeBreakDown, A.FTPOref ) As B "
            _Qry &= vbCrLf & "  ON A.FTOrderNo =B.FTOrderNo "
            ...
            _Qry &= vbCrLf & "  AND (A.FNGrandQuantity - ISNULL(B.FNQuantity,0)) > 0 "

        Else

            _Qry = " SELECT TOP 1 A.FTOrderNo  "
            _Qry &= vbCrLf & " FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS A "
            _Qry &= vbCrLf & "  INNER JOIN (SELECT A.FTOrderNo, A.FTSubOrderNo, A.FTColorway, A.FTSizeBreakDown, A.FTPOref,B.FTPOLineItem, SUM(B.FNQuantity) AS FNQuantity  "
            _Qry &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "]..TACCTFactoryCMInvoice_D AS B WITH(NOLOCK)  "
            _Qry &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS A ON B.FTColorway = A.FTColorway AND B.FTSizeBreakDown = A.FTSizeBreakDown AND B.FTCustomerPO = A.FTPOref AND B.FTPOLineItem = A.FTNikePOLineItem "
            _Qry &= vbCrLf & "  GROUP BY A.FTOrderNo, A.FTSubOrderNo, A.FTColorway, A.FTSizeBreakDown, A.FTPOref,B.FTPOLineItem ) As B "
            _Qry &= vbCrLf & "  ON A.FTOrderNo =B.FTOrderNo "
            _Qry &= vbCrLf & "  AND A.FTSubOrderNo =B.FTSubOrderNo "
            _Qry &= vbCrLf & "  AND  A.FTColorway=B.FTColorway "
            _Qry &= vbCrLf & "  AND A.FTSizeBreakDown=B.FTSizeBreakDown  "
            _Qry &= vbCrLf & "  AND A.FTPOref=B.FTPOref  "
            _Qry &= vbCrLf & "  AND A.FTNikePOLineItem=B.FTPOLineItem  "
            _Qry &= vbCrLf & "  WHERE A.FTOrderNo='" & HI.UL.ULF.rpQuoted(_OrderNo) & "'"
    ...
    Private Sub ocmDelete_Click(sender As Object, e As EventArgs) Handles ocmDelete.Click
        ...
        Else
            If Me.FTSubOrderNo.Text <> "" Then
                ...
                If CreateDocumentMI(Me.FTOrderNo.Text, FTSubOrderNo.Text & "-D" & MaxSeqSD.ToString) Then
                    Exit Sub
                End If
    ...
    Private Sub PrepareDetailBreakDown()
        Dim _Qry As String
        Dim _dt As DataTable
        ...
        Try
            ...
            _Qry = "Exec[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "]..SP_GET_DivertBreakDown '" & HI.UL.ULF.rpQuoted(SubOrder.ToString) & "', " & FNseq & ""
            _dt = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN)
    ...
    Private Sub ogvDivertSrc_Click(sender As Object, e As EventArgs) Handles ogvDivertSrc.Click
        ...
        With ogvDivertSrc
            ...
            FNseq = HI.UL.ULF.rpQuoted("" & .GetRowCellValue(.FocusedRowHandle, "FNDivertSeq").ToString)
