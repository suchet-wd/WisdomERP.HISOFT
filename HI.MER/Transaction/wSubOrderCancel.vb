...
Public Class wSubOrderCancel

    Private _FormHeader As New List(Of HI.TL.DynamicForm)()
    Private oGridViewBreakdownDivertSrc As DevExpress.XtraGrid.Views.Grid.GridView
    Private _wDivertOrderDistination As wSubOrderCancelBreakdown
    ...
    Private Sub BrowseSubOrderData()
        ...
        _Qry &= vbCrLf & ",ISNULL(pmt.FTPlantCode,'') AS FTPlantCode"
...
#End Region
    ...
    Public Sub LoadDetailSubDivertBreakDown()
        ...
        _Qry = "SELECT AA.FNCancelSeq,AA.oFTSubOrderNo"
        _Qry &= vbCrLf & ",AA.TotalQuantity,AA.FTRemark"
        _Qry &= vbCrLf & "FROM"
        _Qry &= vbCrLf & "(SELECT sd.FTSubOrderNo AS oFTSubOrderNo,sd.FNCancelSeq AS FNCancelSeq,sd.FTRemark AS FTRemark"
        _Qry &= vbCrLf & ",sum(sdb.FNQuantity) AS TotalQuantity"
        _Qry &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Cancel AS SD inner join"
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Cancel_BreakDown AS SDB with(NOLOCK) on sd.FTSubOrderNo=sdb.FTSubOrderNo AND sd.FTOrderNo=sdb.FTOrderNo AND sd.FNCancelSeq=sdb.FNCancelSeq "
        _Qry &= vbCrLf & "where sd.FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
        _Qry &= vbCrLf & "GROUP BY sd.FTSubOrderNo,sd.FNCancelSeq,sd.FTRemark"
        _Qry &= vbCrLf & "  ) AS AA ORDER BY FNCancelSeq "
    ...
    Private Function DeleteDivert(Spls As HI.TL.SplashScreen) As Boolean
        Try
            ...
            With Me.ogvDivertSrc
                MaxSeqSD = Integer.Parse(Val("" & .GetFocusedRowCellValue("FNCancelSeq").ToString))
            End With

            '_QryChkSDB = "SELECT TOP 1 MAX(FNCancelSeq) AS MAXFNCancelSeq FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert_BreakDown "
            'MaxSeqSDB = HI.Conn.SQLConn.GetField(_QryChkSDB, HI.Conn.DB.DataBaseName.DB_MERCHAN)
            '_QryChkSD = "SELECT TOP 1 MAX(FNCancelSeq) AS MAXFNCancelSeq FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Divert"
            ...
            _Qry = "Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Cancel_BreakDown"
            _Qry &= vbCrLf & "WHERE FNCancelSeq=" & Val(SeqSDB) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "Delete FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Cancel"
            _Qry &= vbCrLf & "WHERE FNCancelSeq=" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            ...
            _Qry = "UPDATE A SET  FNCancelSeq=FNCancelSeq-1   From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Cancel_BreakDown AS A "
            _Qry &= vbCrLf & "WHERE FNCancelSeq>" & Val(SeqSDB) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
            _Qry &= vbCrLf & "UPDATE B SET FNCancelSeq=FNCancelSeq-1 FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_Cancel AS B "
            _Qry &= vbCrLf & "WHERE FNCancelSeq>" & Val(SeqSD) & ""
            _Qry &= vbCrLf & "AND FTSubOrderNo='" & Me.FTSubOrderNo.Text & "'"
    ...
    Private Sub ocmDivert_Click(sender As Object, e As EventArgs) Handles ocmcancel.Click
        ...
        If Me.FTOrderNo.Text.Trim <> "" Then
            ...
            _wDivertOrderDistination = New wSubOrderCancelBreakdown()
    ...
    Private Sub PrepareDetailBreakDown()
        ...
        Try
            ...
            _Qry = "Exec[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "]..SP_GET_CancelBreakDown '" & HI.UL.ULF.rpQuoted(SubOrder.ToString) & "', " & FNseq & ""
            _dt = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN)
            ...
            With Me.ogvDetailBreakDown
                ...
                For I As Integer = .Columns.Count - 1 To 0 Step -1
                    ...
                    Select Case .Columns(I).FieldName.ToString.ToUpper
                        ...
                        Case "FNCancelSeq".ToUpper, "FTSubOrderNo".ToUpper, "FTColorway".ToUpper, "FTNikePOLineItem".ToUpper
    ...
    Private Sub ogvDivertSrc_Click(sender As Object, e As EventArgs) Handles ogvDivertSrc.Click
        ...
        With ogvDivertSrc
            ...
            FNseq = HI.UL.ULF.rpQuoted("" & .GetRowCellValue(.FocusedRowHandle, "FNCancelSeq").ToString)
