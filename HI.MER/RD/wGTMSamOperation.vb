...
Public Class wGTMSamOperation

    Private _Copy As wCopyGTMSamOperation
    Sub New()
        ...
        _Copy = New wCopyGTMSamOperation
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(styleid As Integer, seasonid As Integer, cmpid As Integer)
        ...
        _Qry &= vbCrLf & "  FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam_Detail AS A WITH(NOLOCK) INNER JOIN"
...
#Region "General"
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            _Qry = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam SET "
            ...
            _Qry &= vbCrLf & " ,FNGESam=" & FNGESam.Value & " "
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                ...
                _Qry = "  INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam "
                _Qry &= vbCrLf & " (FTInsUser, FDInsDate, FTInsTime,  FNHSysStyleId, FNHSysSeasonId, FTRemark, FNSam,FNOperater, FNCost,FNMinuteHour,FNProdPersonPerDay,FNWorkingTimeMinuteDay,FNTargetPerDay,FTStateSendApp,FNSamCut,FNCostCut,FNNetCostCut,FNSamPack,FNCostPack,FNNetCostPack,FNGESam,FNHSysCmpId) "
            ...
            For Each R As DataRow In dtoper.Select("FTRDOperationCode<>'' AND FTRDMachineTypeCode<>''", "FNSeq")
                ...
                _Qry = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam_Detail SET "
                _Qry &= vbCrLf & " FTUpdUser= '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                _Qry &= vbCrLf & " ,FDUpdDate=" & HI.UL.ULDate.FormatDateDB & " "
                _Qry &= vbCrLf & " ,FTUpdTime=" & HI.UL.ULDate.FormatTimeDB & " "
                ...
                If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    _Qry = "Insert INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam_Detail"
            ...
            _Qry = " DELETE FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam_Detail  "
            _Qry &= vbCrLf & " WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & ""
            _Qry &= vbCrLf & "  AND FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
            _Qry &= vbCrLf & "     AND FNSeq > " & _FNSeq & " "
...
#End Region
    ...
    Private Sub LoadSMPInfo(styleid As Integer, seasonid As Integer, cmpid As Integer)
        ...
        For Each R As DataRow In dt.Rows
            ...
            FNGESam.Value = Val(R!FNGESam.ToString())
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            With New HI.RP.Report
                ...
                .Formular = " {TRDTGTMSam.FNHSysStyleId} =" & Val(FNHSysStyleId.Properties.Tag.ToString) & " And {TRDTGTMSam.FNHSysStyleId} =" & Val(FNHSysSeasonId.Properties.Tag.ToString) & "  And {TRDTGTMSam.FNHSysCmpId} =" & Val(FNHSysCmpId.Properties.Tag.ToString) & "  "
                .Preview()
            End With
        Else
    ...
    Private Sub ocmsendpoapprove_Click(sender As Object, e As EventArgs) Handles ocmsendpoapprove.Click
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            ...
            If SaveData() = False Then
                ...
                Exit Sub
            End If
            If FNSam.Value > FNGESam.Value Then
    ...
    Private Sub ocmapprove_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            ...
            If FTStateApp.Checked = False Then
                ...
                cmd = "select top 1 FN1stProdSam  From  " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & ".dbo.TRDMSamStyle  AS x with(nolock) where  (FNHSysStyleId = " & styleid & ")   "
                ...
                _Cmd = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam SET "
                ...
                _Cmd &= vbCrLf & "  INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam ( "
                ...
                _Cmd &= vbCrLf & " ,FTStateFromRDDate, FTStateFromRDTime, FNGTMSam, FNSamCut, FNCostCut, FNNetCostCut, FNSamPack, FNCostPack, FNNetCostPack "
                _Cmd &= vbCrLf & "  ) "
                _Cmd &= vbCrLf & "  SELECT   FNHSysCmpId, FNHSysStyleId, FNHSysSeasonId, FTRemark, FNSam, FNOperater, FNCost, FNMinuteHour, FNProdPersonPerDay, FNWorkingTimeMinuteDay, FNTargetPerDay, FTStateNewFromRD, FTStateFromRDBy "
                _Cmd &= vbCrLf & " ,FTStateFromRDDate, FTStateFromRDTime, FNSam, FNSamCut, FNCostCut, FNNetCostCut, FNSamPack, FNCostPack, FNNetCostPack "
                _Cmd &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam "
                _Cmd &= vbCrLf & " WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  AND FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  AND FNHSysCmpId=" & Val(FNHSysCmpId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam_Detail  ( "
                ...
                _Cmd &= vbCrLf & "  ) "
                _Cmd &= vbCrLf & "  SELECT FNHSysCmpId, FNHSysStyleId, FNHSysSeasonId, FNSeq, FNHSysRDOperationId, FNHSysRDMachineTypeId, FNSam, FNOperater, FNCost, FNOutputPer1Hour, FNOutputPer8Hour, FTRemark, FTStateNoSew, FNHSysRDPositionPartId, FTLenght, FTAdditionalInfo, FTAttachment,FNSam"
                _Cmd &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam_Detail "
                _Cmd &= vbCrLf & " WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  AND FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  AND FNHSysCmpId=" & Val(FNHSysCmpId.Properties.Tag.ToString()) & ""
                ...
                If StateSamStyle Then
                    ...
                    _Cmd &= vbCrLf & "  Update  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDMSamStyle SET FN1stProdSam =" & FNSam.Value & " where  (FNHSysStyleId = " & styleid & ")  "
