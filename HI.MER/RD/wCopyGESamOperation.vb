...
Public Class wCopyGESamOperation
    ...
    Private Function SaveCopy() As Boolean
        Try
            ...
            Try
                ...
                _Cmd = " delete from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail where FNHSysStyleId=" & Val(StyleId) & " AND FNHSysSeasonId=" & Val(SeasonId) & " "
                ...
                _Cmd &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam As A  "
                _Cmd &= vbCrLf & " OUTER APPLY ( Select TOP 1 *  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam As B With(NOLOCK)   where FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString) & " And FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString) & "   ) As B"
                _Cmd &= vbCrLf & "  WHERE  A.FNHSysStyleId=" & Val(StyleId) & " And A.FNHSysSeasonId=" & Val(SeasonId) & "  "
                _Cmd &= vbCrLf & " insert into [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail( FTInsUser, FDInsDate, FTInsTime, FNHSysStyleId, FNHSysSeasonId, FNSeq, FNHSysRDOperationId, FNHSysRDMachineTypeId, FNSam, FNOperater, FNCost, FNOutputPer1Hour,FNOutputPer8Hour, FTRemark, FTStateNoSew, FNHSysRDPositionPartId) "
                _Cmd &= vbCrLf & " select  "
                ...
                _Cmd &= vbCrLf & " from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail AS X WITH(NOLOCK) "
                _Cmd &= vbCrLf & "  where FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString) & " And FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString) & "  "
...
#Region "Event Handle"
    ...
    Private Sub ocmok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmok.Click
        ...
        If FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" Then
            ...
            Dim cmd As String = " Select SUM(1) As CountRec  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail As X With(NOLOCK) WHERE  (FNHSysStyleId = " & Val(FNHSysStyleId.Properties.Tag.ToString()) & ")  AND  (FNHSysSeasonId = " & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ")    "
