...
Public Class wCopySMVSamOperation
...
#Region "Procedure And Function"
    ...
    Private Function SaveCopy() As Boolean
        Try
            ...
            _Cmd = " Select TOP 1 FNHSysStyleId  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam As A  "
            ...
            If HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_PLANNING, "") = "" Then
                _Cmd = " insert into [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam ( FNHSysCmpId, FNHSysStyleId, FNHSysSeasonId, FTRemark, FNSam, FNOperater, FNCost, FNMinuteHour, FNProdPersonPerDay,"
                ...
                _Cmd &= vbCrLf & "   FNWorkingTimeMinuteDay, FNTargetPerDay, FTStateSendApp, FTStateSendAppBy, FTStateSendAppDate, FTStateSendAppTime, FTStateApp, FTStateAppBy, FTStateAppDate, FTStateAppTime, FTStateNewFromRD, "
                _Cmd &= vbCrLf & "   FTStateFromRDBy, FTStateFromRDDate, FTStateFromRDTime, FNGTMSam, FNSamCut, FNCostCut, FNNetCostCut, FNSamPack, FNCostPack, FNNetCostPack "
                _Cmd &= vbCrLf & "    FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam As A  "
                _Cmd &= vbCrLf & "  WHERE  A.FNHSysStyleId=" & Val(StyleId) & " And A.FNHSysSeasonId=" & Val(SeasonId) & "  "
            ...
            Try
                ...
                _Cmd = " delete from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam_Detail where FNHSysStyleId=" & Val(StyleId) & " And FNHSysSeasonId=" & Val(SeasonId) & " And FNHSysCmpId=" & HI.ST.SysInfo.CmpID & " "
                ...
                _Cmd &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam As A  "
                _Cmd &= vbCrLf & " OUTER APPLY ( Select TOP 1 *  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam As B With(NOLOCK)   where FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString) & " And FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString) & "  AND FNHSysCmpId=" & HI.ST.SysInfo.CmpID & "   ) As B"
                _Cmd &= vbCrLf & "  WHERE  A.FNHSysStyleId=" & Val(StyleId) & " And A.FNHSysSeasonId=" & Val(SeasonId) & "  "
                _Cmd &= vbCrLf & "  AND A.FNHSysCmpId=" & HI.ST.SysInfo.CmpID & " "

                _Cmd &= vbCrLf & " insert into [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam_Detail( FTInsUser, FDInsDate, FTInsTime, FNHSysStyleId, FNHSysSeasonId, FNSeq, FNHSysRDOperationId, FNHSysRDMachineTypeId, FNSam, FNOperater, FNCost, FNOutputPer1Hour,FNOutputPer8Hour, FTRemark, FTStateNoSew, FNHSysRDPositionPartId) "
                _Cmd &= vbCrLf & " select  "
                ...
                _Cmd &= vbCrLf & " from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTSMVSam_Detail AS X WITH(NOLOCK) "
