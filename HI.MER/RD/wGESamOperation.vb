...
Public Class wGESamOperation

    Private _Copy As wCopyGESamOperation
    Sub New()
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(styleid As Integer, seasonid As Integer)
        ...
        _Qry &= vbCrLf & "  FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail AS A WITH(NOLOCK) INNER JOIN"
...
#Region "General"
    ...
    Private Sub GetStateApp()
        Dim dt As New DataTable
        Dim _Qry As String = "SELECT TOP 1  ISNULL(FTStateApp,'')  as FTStateApp,ISNULL(FTStateSendApp,'')  as FTStateSendApp FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam As X WITH(NOLOCK)   WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & "  And FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            _Qry = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam SET "
            ...
            _Qry &= vbCrLf & " ,FNRDSam=" & FNRDSam.Value & " "
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                ...
                _Qry = "  INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam "
                _Qry &= vbCrLf & " (FTInsUser, FDInsDate, FTInsTime,  FNHSysStyleId, FNHSysSeasonId, FTRemark, FNSam,FNOperater, FNCost,FNMinuteHour,FNProdPersonPerDay,FNWorkingTimeMinuteDay,FNTargetPerDay,FTStateSendApp,FNSamCut,FNCostCut,FNNetCostCut,FNSamPack,FNCostPack,FNNetCostPack,FNRDSam) "
            ...
            For Each R As DataRow In dtoper.Select("FTRDOperationCode<>'' AND FTRDMachineTypeCode<>''", "FNSeq")
                ...
                _Qry = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail SET "
                _Qry &= vbCrLf & " FTUpdUser= '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                _Qry &= vbCrLf & " ,FDUpdDate=" & HI.UL.ULDate.FormatDateDB & " "
                _Qry &= vbCrLf & " ,FTUpdTime=" & HI.UL.ULDate.FormatTimeDB & " "
                ...
                If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    _Qry = "Insert INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail"
                    _Qry &= vbCrLf & "(FTInsUser, FDInsDate, FTInsTime,   FNHSysStyleId, FNHSysSeasonId, FNSeq, FNHSysRDOperationId,FNHSysRDMachineTypeId, FNSam,FNOperater,FNCost,FNOutputPer1Hour,FNOutputPer8Hour,FTRemark,FTStateNoSew,FNHSysRDPositionPartId,FNSamBF)"
                    _Qry &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                    _Qry &= vbCrLf & " ," & HI.UL.ULDate.FormatDateDB & " "
                    _Qry &= vbCrLf & " ," & HI.UL.ULDate.FormatTimeDB & " "
                    _Qry &= vbCrLf & " ," & Val(FNHSysStyleId.Properties.Tag.ToString()) & " "
            ...
            _Qry = " DELETE FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail  "
            _Qry &= vbCrLf & " WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & ""
            _Qry &= vbCrLf & "  AND FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
            _Qry &= vbCrLf & "     AND FNSeq > " & _FNSeq & " "
    ...
    Private Sub ocmsave_Click(sender As Object, e As EventArgs) Handles ocmsave.Click
        ...
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            ...
            If FTStateApp.Checked = False Then
                ...
                If SaveData() Then
                    Call LoadSMPInfo(Val(FNHSysStyleId.Properties.Tag.ToString()), Val(FNHSysSeasonId.Properties.Tag.ToString()))
...
#End Region
    ...
    Private Sub LoadSMPInfo(styleid As Integer, seasonid As Integer)
        Dim cmd As String = ""
        Dim dt As New DataTable
        ...
        For Each R As DataRow In dt.Rows
            ...
            FNRDSam.Value = Val(R!FNRDSam.ToString())
        ...
        Call LoadDataInfo(styleid, seasonid)
    End Sub
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            With New HI.RP.Report
                ...
                .ReportName = "GESamReport.rpt"
                .Formular = " {TRDTGESam.FNHSysStyleId} =" & Val(FNHSysStyleId.Properties.Tag.ToString) & " AND {TRDTGESam.FNHSysStyleId} =" & Val(FNHSysSeasonId.Properties.Tag.ToString) & " "
                .Preview()
            End With
        Else
    ...
    Private Sub ocmsendpoapprove_Click(sender As Object, e As EventArgs) Handles ocmsendpoapprove.Click
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            ...
            If SaveData() = False Then
                ...
                Exit Sub
            End If
            If FNSam.Value > FNRDSam.Value Then
    ...
    Private Sub ocmapprove_Click(sender As Object, e As EventArgs) Handles ocmapprove.Click
        If FNHSysStyleId.Properties.Tag.ToString <> "" And FNHSysStyleId.Text <> "" And FNHSysSeasonId.Text <> "" And FNHSysSeasonId.Properties.Tag.ToString <> "" Then
            ...
            If FTStateApp.Checked = False Then
                ...
                Dim StateSamStyleCut As Boolean = False
                ...
                cmd = "select top 1 FNGTMSam  From  " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & ".dbo.TRDMSamStyle  AS x with(nolock) where  (FNHSysStyleId = " & styleid & ")   "
                StateSamStyle = (Val(HI.Conn.SQLConn.GetField(cmd, Conn.DB.DataBaseName.DB_PLANNING, "0")) = 0)


                cmd = "select top 1 FNHSysStyleId  From  " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & ".dbo.TPPCTSamByStyle  AS x with(nolock) where  (FNHSysStyleId = " & styleid & ")   "
                StateSamStyleCut = (HI.Conn.SQLConn.GetField(cmd, Conn.DB.DataBaseName.DB_PLANNING, "") = "")


                _Cmd = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam SET "
                ...
                _Cmd &= vbCrLf & "  INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam ( "
                ...
                _Cmd &= vbCrLf & " ,FTStateFromRDDate, FTStateFromRDTime, FNGESam, FNSamCut, FNCostCut, FNNetCostCut, FNSamPack, FNCostPack, FNNetCostPack "
                _Cmd &= vbCrLf & "  ) "
                ...
                _Cmd &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam "
                _Cmd &= vbCrLf & " WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  AND FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGTMSam_Detail  ( "
                _Cmd &= vbCrLf & "  FNHSysCmpId, FNHSysStyleId, FNHSysSeasonId, FNSeq, FNHSysRDOperationId, FNHSysRDMachineTypeId, FNSam, FNOperater, FNCost, FNOutputPer1Hour, FNOutputPer8Hour, FTRemark, FTStateNoSew, FNHSysRDPositionPartId, FTLenght, FTAdditionalInfo, FTAttachment,FNSamBF"
                _Cmd &= vbCrLf & "  ) "
                _Cmd &= vbCrLf & "  SELECT 0 AS FNHSysCmpId, FNHSysStyleId, FNHSysSeasonId, FNSeq, FNHSysRDOperationId, FNHSysRDMachineTypeId, FNSam, FNOperater, FNCost, FNOutputPer1Hour, FNOutputPer8Hour, FTRemark, FTStateNoSew, FNHSysRDPositionPartId, FTLenght, FTAdditionalInfo, FTAttachment,FNSam"
                _Cmd &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDTGESam_Detail "
                _Cmd &= vbCrLf & " WHERE FNHSysStyleId=" & Val(FNHSysStyleId.Properties.Tag.ToString()) & ""
                _Cmd &= vbCrLf & "  AND FNHSysSeasonId=" & Val(FNHSysSeasonId.Properties.Tag.ToString()) & ""
                ...
                If StateSamStyle Then
                    ...
                    _Cmd &= vbCrLf & "  Update  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TRDMSamStyle SET FNGTMSam =" & FNSam.Value & " where  (FNHSysStyleId = " & styleid & ")  "

                End If


                If StateSamStyleCut Then

                    _Cmd &= vbCrLf & "  INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PLANNING) & "].dbo.TPPCTSamByStyle ( "
                    _Cmd &= vbCrLf & "  FTInsUser, FDInsDate, FTInsTime, FNHSysStyleId, FNSam, FNCostPerMin, FNPrice, FNMultiple, FNWringPrice, FNSamEmb, FNSamCut, FNCostCut, FNNetCostCut, FNSamPack, FNCostPack, FNNetCostPack "
                    _Cmd &= vbCrLf & "  ) "
                    _Cmd &= vbCrLf & "  SELECT   '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'," & HI.UL.ULDate.FormatDateDB & "," & HI.UL.ULDate.FormatTimeDB & ", " & styleid & ",  " & FNSam.Value & " "
                    _Cmd &= vbCrLf & " ,0.71,Convert(numeric(18,2)," & FNSam.Value & " * 0.711),1,0,0," & FNSamCut.Value & "," & FNCostCut.Value & "," & FNNetCostCut.Value & "," & FNSamPack.Value & "," & FNCostPack.Value & "," & FNNetCostPack.Value & " "
