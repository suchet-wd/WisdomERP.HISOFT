        ...
        Select Case e.Button.Index
            Case 0
                Try
                    ...
                    opFileDialog.Filter = "Excel Files(*.xls;*.xlsx;*.xlsm;*.csv)|*.xls;*.xlsx;*.xlsm;*.csv"
        ...
        If Me.FTFilePath.Text <> "" Then
            ...
            If Me.FTStyleCode.Text.Trim <> "" And Me.FTSeasonCode.Text.Trim <> "" Then
                ...
                Try
                    ...
                    Dim _nFNHSysSeasonId As Integer = 0
                    ...
                    If _FNHSysSeasonId <= 0 Then
                        ...
                        _nFNHSysSeasonId = Val(HI.TL.RunID.GetRunNoID("TMERMSeason", "FNHSysSeasonId", Conn.DB.DataBaseName.DB_MASTER).ToString())

                        _Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..[TMERMSeason]("
                        _Qry &= vbCrLf & " FTInsUser, FDInsDate, FTInsTime, FNHSysSeasonId, FTSeasonCode, FTSeasonNameTH, FTSeasonNameEN, FTRemark, FTStateActive, FNUSExchangeRate, FNEURExchangeRate"
                        _Qry &= vbCrLf & " ) "
                        _Qry &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                        _Qry &= vbCrLf & "      ," & HI.UL.ULDate.FormatDateDB & ""
                        _Qry &= vbCrLf & "      ," & HI.UL.ULDate.FormatTimeDB & ""
                        _Qry &= vbCrLf & "      ," & _nFNHSysSeasonId & ""
                        ...
                        _FNHSysSeasonId = _nFNHSysSeasonId
                    ...
                    _Qry &= vbCrLf & ", FNCMPOrg=CASE WHEN " & FNCmpOrg.Value & ">0 THEN CASE WHEN " & FNCmpOrg.Value & "< FNCMPOrg THEN " & FNCmpOrg.Value & "  ELSE FNCMPOrg END ELSE FNCMPOrg END"
                    ...
                    If HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT) = False Then
                        ...
                        _Qry &= vbCrLf & "( FTInsUser, FDInsDate, FTInsTime,  FTStyleCode, FTSeason, FNFabricAmt, FNAccessoryAmt, FNImportFabricAmt, FNImportAccessoryAmt,FNImportCMP,FNCMPOrg)"
                        _Qry &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                        _Qry &= vbCrLf & " ," & HI.UL.ULDate.FormatDateDB & ""
                        _Qry &= vbCrLf & " ," & HI.UL.ULDate.FormatTimeDB & ""
                        _Qry &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(Me.FTStyleCode.Text.Trim) & "'"
                        ...
                        If _dt.Select("F12='CMT'").Length > 0 Or _dt.Select("F13='CMT'").Length Then
                            ...
                            If _dt.Select("F12='CMT'").Length > 0 Then
                                ...
                                For Each R As DataRow In _dt.Select("F12='CMT'")
                                    _FNCmpOrg = Val(R!F16.ToString.Replace("$", "").Trim())
                                    _FNCmp = _FNCmp + Val(R!F16.ToString.Replace("$", "").Trim())
