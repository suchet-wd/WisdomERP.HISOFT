            ...
            If Not oDBdtSrc Is Nothing And oDBdtSrc.Rows.Count > 0 And oDBdtSrc.Rows.Count > _nRowHeader Then
                ...
                If bAppendSrcHeader = True Then
                    ...
                    For nLoopOrder = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        Dim tFieldVender As String = "F1"
                        Dim tFieldCategory As String = "F11"
                        Dim tFieldSubCategory As String = "F12"
                        Dim tFieldSilhouette As String = "F19"
                        ...
                        tVender = CStr(oDBdtSrc.Rows(nLoopOrder).Item(tFieldVender))
                        tCategory = CStr(oDBdtSrc.Rows(nLoopOrder).Item(tFieldCategory))
                        tSubCategory = CStr(oDBdtSrc.Rows(nLoopOrder).Item(tFieldSubCategory))
                        tSilhouette = CStr(oDBdtSrc.Rows(nLoopOrder).Item(tFieldSilhouette))
    ...
    Private Function W_PRCbImportFactoryOrder(_Spls) As Boolean
        ...
        Try
            ...
            If Not oDBdtImport Is Nothing AndAlso oDBdtImport.Rows.Count > 0 Then
                ...
                tSql &= Environment.NewLine & "                  );"
                'tSql &= Environment.NewLine & "  UPDATE STM"
                'tSql &= Environment.NewLine & "   SET FNCMDisAmt=XSX.FNCMDisAmt"
                'tSql &= Environment.NewLine & "      ,FNNetCM = (FNCM - XSX.FNCMDisAmt ) "
                'tSql &= Environment.NewLine & "  FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..[TMERMStyle] AS STM "
                'tSql &= Environment.NewLine & "      INNER JOIN (SELECT B2.FNHSysStyleId,MAX(BB.FNCMDisAmt) AS FNCMDisAmt"
                'tSql &= Environment.NewLine & "      FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTImportOrderTemp] AS AA WITH(NOLOCK)"
                'tSql &= Environment.NewLine & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS B WITH(NOLOCK) ON AA.FTGenerateOrderNo = B.FTOrderNo"
                'tSql &= Environment.NewLine & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS B2 WITH(NOLOCK) ON B.FNHSysStyleId = B2.FNHSysStyleId"
                ...
                'tSql &= Environment.NewLine & "  INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTSeasonCMPrice]  "
                ...
                'tSql &= Environment.NewLine & "      FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTImportOrderTemp] AS AA WITH(NOLOCK)"
                'tSql &= Environment.NewLine & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS B WITH(NOLOCK) ON AA.FTGenerateOrderNo = B.FTOrderNo"
                'tSql &= Environment.NewLine & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS B2 WITH(NOLOCK) ON B.FNHSysStyleId = B2.FNHSysStyleId  AND B.FNHSysSeasonId = B2.FNHSysSeasonId "
                'tSql &= Environment.NewLine & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrderSub] AS B4 WITH(NOLOCK) ON B2.FTOrderNo = B4.FTOrderNo"
                'tSql &= Environment.NewLine & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrderSub_BreakDown] AS BB WITH(NOLOCK) ON B4.FTOrderNo = BB.FTOrderNo AND B4.FTSubOrderNo=BB.FTSubOrderNo "
            ...
            If _bImportComplete = True Then
                ...
                ''tSql &= Environment.NewLine & "                      WHERE L2.FTOrderNo = M3.FTOrderNo"
                ''tSql &= Environment.NewLine & "                            AND L2.FTSubOrderNo = M3.FTSubOrderNo)"
                            ...
                            If Not oDBdtExcel Is Nothing And oDBdtExcel.Rows.Count > 0 Then
                                ...
                                Else
                                    ...
                                    If W_PRCbSaveListOrderToTemp(oDBdtExcel) = True Then
                                        ...
                                        'tSql &= vbCrLf & "  ,FNExtraType=CASE WHEN (XA.Plant ='1025' OR  XA.Plant ='1027'  OR  XA.Plant ='1039' ) AND (XA.FNHSysBuyGrpId = 1405080001 OR XA.FNHSysBuyGrpId = 1566580001) THEN 99 "
                                        'tSql &= vbCrLf & "                  ELSE   "
                                        'tSql &= vbCrLf & "                  CASE WHEN XA.Vender='HIG' THEN 22  ELSE  "
                                        'tSql &= vbCrLf & "                  CASE WHEN XA.Vender='HIT' AND XA.Category='FOOTBALL/SOCCER LICENSED' AND XA.SubCategory='CLUB TEAMS' AND XA.SubCategory='CLUB TEAMS' AND XA.Silhouete LIKE 'SHORT%'  THEN 22  ELSE 44  "
                                        'tSql &= vbCrLf & "                    "
                                        'tSql &= vbCrLf & "                  END END  END"

                                        'tSql &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTImportOrderTemp As XA INNER JOIN"
                                        'tSql &= vbCrLf & " 	("
                                        'tSql &= vbCrLf & "   Select  A.* "
                                        'tSql &= vbCrLf & "	,ROW_NUMBER() Over (Partition by A.FTPONo,A.FTStyle Order by A.FTPONo,A.FTStyle,A.FDShipDate , A.FNHSysBuyGrpId,A.FNHSysPlantId) As FNGrpSeq "
                                        'tSql &= vbCrLf & "FROM ( "

                                        'tSql &= vbCrLf & " Select A.FTPONo,A.FTStyle,A.FDShipDate , A.FNHSysBuyGrpId,A.FNHSysPlantId "
                                        'tSql &= vbCrLf & "  FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTImportOrderTemp As A With (NOLOCK) "

                                        'tSql &= vbCrLf & " WHERE A.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"

                                        'tSql &= vbCrLf & " GROUP BY A.FTPONo,A.FTStyle,A.FDShipDate , A.FNHSysBuyGrpId,A.FNHSysPlantId ) AS A "


                                        'tSql &= vbCrLf & "  ) As XX On XA.FTPONo = XX.FTPONo  "
                                        'tSql &= vbCrLf & " And  XA.FNHSysBuyGrpId = XX.FNHSysBuyGrpId "
                                        'tSql &= vbCrLf & " And XA.FDShipDate = XX.FDShipDate "
                                        'tSql &= vbCrLf & "  And XA.FTStyle = XX.FTStyle"
                                        'tSql &= vbCrLf & " And  XA.FNHSysPlantId = XX.FNHSysPlantId "
