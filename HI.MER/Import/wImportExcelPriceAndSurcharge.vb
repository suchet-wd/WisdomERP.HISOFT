...
Public Class wImportExcelPriceAndSurcharge
...
#Region "Variable Declaration"
    ...
    Private Const _nStartColImportSizeValExcel As Integer = 52
    Private Const _nStartRowImportCSV As Integer = 3
    Private Const _nStartColImporSizeValCSV As Integer = 52
...
#Region "Procedure And Function"
            ...
            If Not oDBdtSrc Is Nothing And oDBdtSrc.Rows.Count > 0 And oDBdtSrc.Rows.Count > _nRowHeader Then
                ...
                If bAppendSrcHeader = True Then
                    ...
                    For nLoopOrder = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        tStyle = Microsoft.VisualBasic.Left(oDBdtSrc.Rows(nLoopOrder).Item("F18"), 6)
                        tMaterialStyle = oDBdtSrc.Rows(nLoopOrder).Item("F19")
                        tPlanSeasonStyle = oDBdtSrc.Rows(nLoopOrder).Item("F28")

                        If Not DBNull.Value.Equals(oDBdtSrc.Rows(nLoopOrder).Item("F29")) Then
                            If Len(oDBdtSrc.Rows(nLoopOrder).Item("F29")) = 4 Then
                                tYearStyle = Microsoft.VisualBasic.Right(oDBdtSrc.Rows(nLoopOrder).Item("F29"), 2)
                            ElseIf Len(oDBdtSrc.Rows(nLoopOrder).Item("F29")) = 2 Then
                                tYearStyle = oDBdtSrc.Rows(nLoopOrder).Item("F29")
                            End If
                        Else
                        ...
                        tDateF8 = CStr(oDBdtSrc.Rows(nLoopOrder).Item("F13"))

                        Dim tDateF9 As String

                        tDateF9 = CStr(oDBdtSrc.Rows(nLoopOrder).Item("F14"))
                        Dim tDateF41 As String

                        tDateF41 = CStr(oDBdtSrc.Rows(nLoopOrder).Item("F44"))
                        Dim tDateF42 As String

                        tDateF42 = CStr(oDBdtSrc.Rows(nLoopOrder).Item("F45"))
                        ...
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "'" & HI.UL.ULF.rpQuoted(oDBdtSrc.Rows(nLoopOrder).Item("F11")) & "'"))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "'" & HI.UL.ULF.rpQuoted(oDBdtSrc.Rows(nLoopOrder).Item("F12")) & "'"))
                        ...
                        oStrBuilder.AppendLine(String.Format("      ,{0}", 0))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "''"))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "'" & HI.UL.ULF.rpQuoted(tMaterialStyle) & "'"))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "''"))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", 0))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "''"))
                ...
                If bAppendSrcDetial = True Then
                    ...
                    Dim tColPONo As String = "F10"
                    Dim tColPOTrading As String = "F11"
                    Dim tColPOItem As String = "F12"

                    Dim tColMatColor As String = "F19"
                    Dim tColMatColorDesc As String = "F20"
                    Dim tColShipMode As String = "F49"
                    Dim tColUom As String = "F50"
                    ...
                    For nLoopSizeBreakdown = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        Dim tDateF42 As String
                        tDateF42 = CStr(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F45"))
                        ...
                        If nPosHyphen > 0 Then
                            ...
                            Do While tMatchSize <> "" And UCase(tMatchSize) <> UCase(_tTextMatchSizeTerminate) And (nLoopColMatchSize < nCntColMatchSize)
                                ...
                                Dim tFOBPrice As Double
                                Dim tFOBPriceSizeUp As Double
                                Dim tSurchargePrice As Double
                                Dim tSurchargePriceSizeUp As Double

                                tFOBPrice = (Val(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F5").ToString))
                                tFOBPriceSizeUp = (Val(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F6").ToString))
                                tSurchargePrice = (Val(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F7").ToString))
                                tSurchargePriceSizeUp = (Val(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F8").ToString))

                                tStyle = Microsoft.VisualBasic.Left(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F18"), 6)
                                tMaterialStyle = oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F19")
                                tPlanSeasonStyle = oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F28")

                                If Not DBNull.Value.Equals(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F29")) Then
                                    If Len(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F29")) = 4 Then
                                        tYearStyle = Microsoft.VisualBasic.Right(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F29"), 2)
                                    ElseIf Len(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F29")) = 2 Then
                                        tYearStyle = oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F29")
                                    End If
                                Else
                                    tYearStyle = CStr(Now.Year)
                                End If
                                ...
                                If HI.Conn.SQLConn.GetField(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN, "") <> "" Then
                                    ...
                                    If nFNQuantity > 0 Then
                                        ...
                                        oStrBuilder.AppendLine(String.Format("   ,A.[FNFOB] = {0}", "N'" & HI.UL.ULF.rpQuoted(tFOBPrice) & "'"))
                                        oStrBuilder.AppendLine(String.Format("   ,A.[FNFOBSizeup] = {0}", "N'" & HI.UL.ULF.rpQuoted(tFOBPriceSizeUp) & "'"))
                                        oStrBuilder.AppendLine(String.Format("   ,A.[FNSurcharge] = {0}", "N'" & HI.UL.ULF.rpQuoted(tSurchargePrice) & "'"))
                                        oStrBuilder.AppendLine(String.Format("   ,A.[FNSurchargeSizeup] = {0}", "N'" & HI.UL.ULF.rpQuoted(tSurchargePriceSizeUp) & "'"))
                                ...
                                Else
                                    ...
                                    oStrBuilder.AppendLine("           ,[FTTradingCoGrossUnitPrice],[FTShipDate],FNFOB, FNFOBSizeup, FNSurcharge, FNSurchargeSizeup)")
                                    oStrBuilder.AppendLine("     VALUES(")
                                    ...
                                    oStrBuilder.AppendLine(String.Format("           ,{0}", tFOBPrice))
                                    oStrBuilder.AppendLine(String.Format("           ,{0}", tFOBPriceSizeUp))
                                    oStrBuilder.AppendLine(String.Format("           ,{0}", tSurchargePrice))
                                    oStrBuilder.AppendLine(String.Format("           ,{0})", tSurchargePriceSizeUp))
    ...
    Private Function W_PRCbShowBrowseDataImportFactoryOrder() As Boolean
        ...
        Try
            ...
            tSql = ""
            tSql = "SELECT A.FNRowImport, A.FTPONo, A.FTPOTrading, A.FTPOItem,B.FNFOB,B.FNFOBSizeup,B.FNSurcharge,B.FNSurchargeSizeup"
            ...
            tSql &= Environment.NewLine & "                                                                             , A.FTStyle, A.FTColorwayCode, A.FTColorwayDesc, A.FTMode, A.FTUom,A.FNFOB,A.FNFOBSizeup,A.FNSurcharge,A.FNSurchargeSizeup"
    ...
    Private Function W_GETdtReadExcelData_REM_20140605_AM0830(ptFilePath As String, _extension As String) As DataTable
        ...
        Dim tSheetName As String = "Sheet1$"
        ...
        Try
            ...
            Dim tSheetNameHeader As String = "Sheet1$"
            If Not tSheetName.EndsWith("_") Then
    ...
    Private Function W_PRCbImportFactoryOrder(_Spls) As Boolean
        ...
        Try
            '---------------------- Start FOB -------------------------
            _Qry = "   UPDATE OB SET FNPrice = A.FNFOB,FNPriceOrg= A.FNFOB,FNCMDisAmt= Convert(numeric(18,4),(A.FNFOB * ISNULL(ST.FNCMDisPer,0)) /100.00)"
            _Qry &= vbCrLf & "  ,  FNOperateFee = (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )"
            _Qry &= vbCrLf & ", FNOperateFeeAmt =  Convert(numeric(18,2),(A.FNFOB * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "
            _Qry &= vbCrLf & ", FNNetFOB =A.FNFOB - Convert(numeric(18,2),(A.FNFOB * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "
            _Qry &= vbCrLf & ",FNAmt = Convert(numeric(18,2), ISNULL(OB.FNQuantity,0) * A.FNFOB )"
            _Qry &= vbCrLf & ",FNAmntExtra = Convert(numeric(18,2), ISNULL(OB.FNQuantityExtra,0) * A.FNFOB )"
            _Qry &= vbCrLf & ",FNAmntQtyTest = Convert(numeric(18,2), ISNULL(OB.FNGarmentQtyTest,0) * A.FNFOB )"
            _Qry &= vbCrLf & ",FNGrandAmnt = Convert(numeric(18,2), ISNULL(OB.FNGrandQuantity,0) * A.FNFOB )"


            _Qry &= vbCrLf & "  ,[FNNetPriceOperateFee] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) ELSE OB.[FNNetPriceOperateFee] END "
            _Qry &= vbCrLf & "   ,[FNNetPriceOperateFeeAmt] =  CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN Convert(numeric(18,2),(A.FNFOB * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) ELSE OB.[FNNetPriceOperateFeeAmt] END "
            _Qry &= vbCrLf & "   ,[FNNetNetPrice] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN A.FNFOB - Convert(numeric(18,2),(A.FNFOB * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00)  ELSE OB.[FNNetNetPrice] END "
            ...
            _Qry &= vbCrLf & " AND MCY.FNMatSizeSeq< ISNULL((SELECT TOP 1 FNMatSizeSeq FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize AS MMCX WITH(NOLOCK) WHERE FTMatSizeCode='" & HI.UL.ULF.rpQuoted(_SizeUp) & "' ),999)"
            _Qry &= vbCrLf & " AND ISNULL(A.FNFOB,0) > 0 AND ISNULL(ODS.FNOrderSetType,0) <=1"
            ...
            _Qry = "   UPDATE OB SET FNPrice = A.FNFOBSizeup,FNPriceOrg= A.FNFOBSizeup,FNCMDisAmt= Convert(numeric(18,4),(A.FNFOBSizeup * ISNULL(ST.FNCMDisPer,0)) /100.00)"
            _Qry &= vbCrLf & "  ,  FNOperateFee = (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) "
            _Qry &= vbCrLf & " , FNOperateFeeAmt =  Convert(numeric(18,2),(A.FNFOBSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "
            _Qry &= vbCrLf & " , FNNetFOB =A.FNFOBSizeup - Convert(numeric(18,2),(A.FNFOBSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "
            _Qry &= vbCrLf & " ,FNAmt = Convert(numeric(18,2), ISNULL(OB.FNQuantity,0) * A.FNFOBSizeup )"
            _Qry &= vbCrLf & " ,FNAmntExtra = Convert(numeric(18,2), ISNULL(OB.FNQuantityExtra,0) * A.FNFOBSizeup )"
            _Qry &= vbCrLf & " ,FNAmntQtyTest = Convert(numeric(18,2), ISNULL(OB.FNGarmentQtyTest,0) * A.FNFOBSizeup )"
            _Qry &= vbCrLf & " ,FNGrandAmnt = Convert(numeric(18,2), ISNULL(OB.FNGrandQuantity,0) * A.FNFOBSizeup )"

            _Qry &= vbCrLf & "  ,[FNNetPriceOperateFee] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) ELSE OB.[FNNetPriceOperateFee] END "
            _Qry &= vbCrLf & "   ,[FNNetPriceOperateFeeAmt] =  CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN Convert(numeric(18,2),(A.FNFOBSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) ELSE OB.[FNNetPriceOperateFeeAmt] END "
            _Qry &= vbCrLf & "   ,[FNNetNetPrice] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN A.FNFOBSizeup - Convert(numeric(18,2),(A.FNFOBSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00)  ELSE OB.[FNNetNetPrice] END "
            ...
            _Qry &= vbCrLf & " WHERE  (A.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "') AND ISNULL(ODS.FTPORef,'') ='' AND XM.FTCustomerPO IS NULL"
            _Qry &= vbCrLf & " AND MCY.FNMatSizeSeq >= ISNULL((SELECT TOP 1 FNMatSizeSeq FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize AS MMCX WITH(NOLOCK) WHERE FTMatSizeCode='" & HI.UL.ULF.rpQuoted(_SizeUp) & "' ),999)"
            _Qry &= vbCrLf & " AND ISNULL(A.FNFOBSizeup,0) > 0 AND ISNULL(ODS.FNOrderSetType,0) <=1 "
            ...
            '----- Update Free
            _Qry = "  UPDATE STM"
            _Qry &= vbCrLf & "   SET FNCMDisAmt=XSX.FNCMDisAmt"
            _Qry &= vbCrLf & "      ,FNNetCM = (STM.FNCM - XSX.FNCMDisAmt ) "
            _Qry &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTSeasonCMPrice] AS STM "
            _Qry &= vbCrLf & "     INNER JOIN (SELECT B2.FNHSysStyleId,B2.FNHSysSeasonId,MAX(BB.FNCMDisAmt) AS FNCMDisAmt"
            _Qry &= vbCrLf & "      FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS B WITH(NOLOCK) "
            _Qry &= vbCrLf & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS B2 WITH(NOLOCK) ON B.FNHSysStyleId = B2.FNHSysStyleId  AND B.FNHSysSeasonId = B2.FNHSysSeasonId "
            _Qry &= vbCrLf & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrderSub] AS B4 WITH(NOLOCK) ON B2.FTOrderNo = B4.FTOrderNo"
            _Qry &= vbCrLf & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrderSub_BreakDown] AS BB WITH(NOLOCK) ON B4.FTOrderNo = BB.FTOrderNo AND B4.FTSubOrderNo=BB.FTSubOrderNo "
            _Qry &= vbCrLf & "           INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..[TMERMStyle] AS ST  WITH(NOLOCK) ON B2.FNHSysStyleId = ST.FNHSysStyleId "
            _Qry &= vbCrLf & "     WHERE AA.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
            _Qry &= vbCrLf & " AND  B4.FNHSysCurId IN (SELECT FNHSysCurId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TFINMCurrency AS C WITH(NOLOCK) WHERE ISNULL(FTStateLocal,'0') <>'1')"
            _Qry &= vbCrLf & " AND  Convert(nvarchar(50),B2.FNHSysStyleId) + Convert(nvarchar(50),B2.FNHSysSeasonId) IN ("
            _Qry &= vbCrLf & " SELECT DISTINCT Convert(nvarchar(50),B.FNHSysStyleId) + Convert(nvarchar(50),B.FNHSysSeasonId) AS FTRef"
            _Qry &= vbCrLf & "  FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMStyle AS ST INNER JOIN"
            _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS B ON ST.FNHSysStyleId = B.FNHSysStyleId INNER JOIN"
            _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert AS ODS ON B.FTOrderNo = ODS.FTOrderNo INNER JOIN"
            _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_BreakDown AS OB ON ODS.FTOrderNo = OB.FTOrderNo AND  ODS.FTSubOrderNo = OB.FTSubOrderNo AND   ODS.FNDivertSeq = OB.FNDivertSeq INNER JOIN"
            ...
            _Qry &= vbCrLf & ") "
            _Qry &= vbCrLf & "   GROUP BY B2.FNHSysStyleId,B2.FNHSysSeasonId) AS XSX"
            _Qry &= vbCrLf & "   ON  STM.FNHSysStyleId=XSX.FNHSysStyleId AND STM.FNHSysSeasonId=XSX.FNHSysSeasonId"
            _Qry &= vbCrLf & "  LEFT OUTER JOIN ("
            _Qry &= vbCrLf & "SELECT C.FNHSysStyleId, C.FNHSysSeasonId"
            _Qry &= vbCrLf & " FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice AS A WITH(NOLOCK)  INNER JOIN"
            _Qry &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS B ON A.FTCustomerPO = B.FTPOref INNER JOIN"
            _Qry &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS C WITH(NOLOCK) ON B.FTOrderNo = C.FTOrderNo"
            _Qry &= vbCrLf & " GROUP BY C.FNHSysStyleId, C.FNHSysSeasonId"
            _Qry &= vbCrLf & ""
            _Qry &= vbCrLf & ") AS XXXXT ON  STM.FNHSysStyleId=XXXXT.FNHSysStyleId AND STM.FNHSysSeasonId=XXXXT.FNHSysSeasonId "
            _Qry &= vbCrLf & " WHERE XXXXT.FNHSysStyleId IS NULL"
            ...
            '---------------------- Start Surcharge -------------------------
            _Qry = "   UPDATE OB SET FNPrice = A.FNSurcharge"
            _Qry &= vbCrLf & "  ,  FNOperateFee = (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) "
            _Qry &= vbCrLf & " , FNOperateFeeAmt =  Convert(numeric(18,2),(A.FNSurcharge * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "
            _Qry &= vbCrLf & " , FNNetFOB =A.FNSurcharge - Convert(numeric(18,2),(A.FNSurcharge * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) ) / 100.00) "


            _Qry &= vbCrLf & " ,FNAmt = Convert(numeric(18,2), ISNULL(OB.FNQuantity,0) * A.FNSurcharge )"
            _Qry &= vbCrLf & " ,FNAmntExtra = Convert(numeric(18,2), ISNULL(OB.FNQuantityExtra,0) * A.FNSurcharge )"
            _Qry &= vbCrLf & " ,FNAmntQtyTest = Convert(numeric(18,2), ISNULL(OB.FNGarmentQtyTest,0) * A.FNSurcharge )"
            _Qry &= vbCrLf & " ,FNGrandAmnt = Convert(numeric(18,2), ISNULL(OB.FNGrandQuantity,0) * A.FNSurcharge )"

            _Qry &= vbCrLf & "  ,[FNNetPriceOperateFee] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) ELSE OB.[FNNetPriceOperateFee] END "
            _Qry &= vbCrLf & "   ,[FNNetPriceOperateFeeAmt] =  CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN Convert(numeric(18,2),(A.FNSurcharge * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) ELSE OB.[FNNetPriceOperateFeeAmt] END "
            _Qry &= vbCrLf & "   ,[FNNetNetPrice] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN A.FNSurcharge - Convert(numeric(18,2),(A.FNSurcharge * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00)  ELSE OB.[FNNetNetPrice] END "
            ...
            _Qry &= vbCrLf & " WHERE  (A.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "') AND ISNULL(ODS.FTPORef,'') ='' " 'AND XM.FTCustomerPO IS NULL"
            _Qry &= vbCrLf & " AND MCY.FNMatSizeSeq< ISNULL((SELECT TOP 1 FNMatSizeSeq FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize AS MMCX WITH(NOLOCK) WHERE FTMatSizeCode='" & HI.UL.ULF.rpQuoted(_SizeUp) & "' ),999)"
            _Qry &= vbCrLf & " AND ISNULL(A.FNSurcharge,0) > 0 AND ISNULL(ODS.FNOrderSetType,0) <=1 "
            ...
            _Qry = "   UPDATE OB SET FNPrice = A.FNSurchargeSizeup"
            _Qry &= vbCrLf & "  ,  FNOperateFee = (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) "
            _Qry &= vbCrLf & " , FNOperateFeeAmt =  Convert(numeric(18,2),(A.FNSurchargeSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "
            _Qry &= vbCrLf & " , FNNetFOB =A.FNSurchargeSizeup - Convert(numeric(18,2),(A.FNSurchargeSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) "

            _Qry &= vbCrLf & " ,FNAmt = Convert(numeric(18,2), ISNULL(OB.FNQuantity,0) * A.FNSurchargeSizeup )"
            _Qry &= vbCrLf & " ,FNAmntExtra = Convert(numeric(18,2), ISNULL(OB.FNQuantityExtra,0) * A.FNSurchargeSizeup )"
            _Qry &= vbCrLf & " ,FNAmntQtyTest = Convert(numeric(18,2), ISNULL(OB.FNGarmentQtyTest,0) * A.FNSurchargeSizeup )"
            _Qry &= vbCrLf & " ,FNGrandAmnt = Convert(numeric(18,2), ISNULL(OB.FNGrandQuantity,0) * A.FNSurchargeSizeup )"

            _Qry &= vbCrLf & "  ,[FNNetPriceOperateFee] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END ) ELSE OB.[FNNetPriceOperateFee] END "
            _Qry &= vbCrLf & "   ,[FNNetPriceOperateFeeAmt] =  CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN Convert(numeric(18,2),(A.FNSurchargeSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00) ELSE OB.[FNNetPriceOperateFeeAmt] END "
            _Qry &= vbCrLf & "   ,[FNNetNetPrice] = CASE WHEN ISNULL(OB.FTStateImportNetPrice,'') <> '1' THEN A.FNSurchargeSizeup - Convert(numeric(18,2),(A.FNSurchargeSizeup * (CASE WHEN ISNULL(Cmpc.FNCmpBranchState,0) <=0 THEN ISNULL(Cus.FNOperateFee,0) ELSE ISNULL(Cus.FNOperateFeeForeign,0) END )) / 100.00)  ELSE OB.[FNNetNetPrice] END "
            ...
            _Qry &= vbCrLf & " WHERE  (A.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "') AND ISNULL(ODS.FTPORef,'') ='' " 'AND XM.FTCustomerPO IS NULL"
            _Qry &= vbCrLf & " AND MCY.FNMatSizeSeq >= ISNULL((SELECT TOP 1 FNMatSizeSeq FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize AS MMCX WITH(NOLOCK) WHERE FTMatSizeCode='" & HI.UL.ULF.rpQuoted(_SizeUp) & "' ),999)"
            _Qry &= vbCrLf & " AND ISNULL(A.FNSurchargeSizeup,0) > 0 AND ISNULL(ODS.FNOrderSetType,0) <=1 "
        ...
        Try
            ...
            If Me.FTFilePath.Text.Trim() <> "" Then
                ...
                If IO.File.Exists(Me.FTFilePath.Text.Trim().ToString()) Then
                    ...
                    Try
                        If oDBdtExcel.Rows(2)!F5.ToString.ToUpper <> "FOB".ToUpper Then
                            HI.MG.ShowMsg.mInfo("รูปแบบ File ไม่ถูกต้องกรุณาทำการตรวจสอบ File !!!", 1501318881, Me.Text, "Work sheet name !!!", MessageBoxIcon.Warning)
                            Exit Sub
                        End If


                        If oDBdtExcel.Rows(2)!F7.ToString.ToUpper <> "Surcharge".ToUpper Then
                            HI.MG.ShowMsg.mInfo("รูปแบบ File ไม่ถูกต้องกรุณาทำการตรวจสอบ File !!!", 1501318881, Me.Text, "Work sheet name !!!", MessageBoxIcon.Warning)
                            Exit Sub
                        End If
        ...
        Try
            If Me.ogvImportOrder.RowCount > 0 Then
                ...
                If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_MERCHAN, "") = "" Then
                    HI.MG.ShowMsg.mInfo("ไม่พบข้อมูลที่สามารถทำการ Import ได้ เนื่องจากข้อมูล ไม่ตรงกับ ข้อมูล FO !!!", 1507188785, Me.Text, , MessageBoxIcon.Warning)
                    Exit Sub
                End If

                If HI.MG.ShowMsg.mConfirmProcess("คุณต้องการทำการ Import Factory Order No FOB AND Surcharge Price  ใช่หรือไม่ ?", 1507178833, "Confirm") = True Then

                    If HI.Conn.SQLConn.ExecuteNonQuery(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN) = True Then
                    End If

                    Dim _Spls As New HI.TL.SplashScreen("Importing FOB AND Surcharge Price.....Please Wait")
                    ...
                    If W_PRCbImportFactoryOrder(_Spls) = True Then
                        ...
                        _Spls.UpdateInformation("Finishing Import FOB AND Surcharge Price.....Please Wait")
                        ...
                        HI.MG.ShowMsg.mInfo("Import Factory Order No. Net Price Complete...", 1507178836, Me.Text, , MessageBoxIcon.Information)
                    Else
                        ...
                        HI.MG.ShowMsg.mInfo("พบปัญหาในการบันทึกรายการนำเข้าข้อมูล Net Price !!!", 1507178834, Me.Text, , MessageBoxIcon.Warning)
                    End If
