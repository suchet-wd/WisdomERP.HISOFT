...
Public Class wImportExcelNetPrice
...
#Region "Variable Declaration"
    ...
    Private Const _nStartColImportSizeValExcel As Integer = 46
    Private Const _nStartColImportSize As Integer = 1
    Private Const _nStartRowImportCSV As Integer = 3
    Private Const _nStartColImporSizeValCSV As Integer = 46
    ...
    Private oDBdtMapSize1 As System.Data.DataTable
    Private tmpDTMapSize As System.Data.DataTable
...
#Region "Procedure And Function"
    ...
    Private Function W_PRCbLoadMapSizeBreak() As System.Data.DataTable
        Dim tmpDTMapSizeMaster1 As System.Data.DataTable


        Try
            tSql = ""
            tSql = "SELECT DISTINCT S.FTSizeBreakDown AS FTMapSize,S.FTSizeBreakDown AS FTMapSizeExtension"
            tSql &= Environment.NewLine & "FROM(SELECT DISTINCT A.FTSizeBreakDown"
            tSql &= Environment.NewLine & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS A WITH (NOLOCK) INNER JOIN"
            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS M WITH (NOLOCK) ON A.FTOrderNo = M.FTOrderNo AND A.FTSubOrderNo = M.FTSubOrderNo INNER JOIN"
            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH (NOLOCK) ON M.FTOrderNo = O.FTOrderNo"
            tSql &= Environment.NewLine & "  UNION ALL"
            tSql &= Environment.NewLine & "SELECT DISTINCT A2.FTSizeBreakDown"
            tSql &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_BreakDown AS A2 WITH (NOLOCK) INNER JOIN"
            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert AS M2 WITH (NOLOCK) ON A2.FTOrderNo = M2.FTOrderNo AND A2.FTSubOrderNo = M2.FTSubOrderNo AND A2.FNDivertSeq = M2.FNDivertSeq INNER JOIN"
            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O2 WITH (NOLOCK) ON M2.FTOrderNo = O2.FTOrderNo"
            tSql &= Environment.NewLine & "UNION ALL"
            tSql &= Environment.NewLine & "SELECT DISTINCT A2.FTSizeBreakDown"
            tSql &= Environment.NewLine & "FROM            [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder_Breakdown AS A2 WITH (NOLOCK) INNER JOIN"
            tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder AS O2 WITH (NOLOCK) ON A2.FTSMPOrderNo = O2.FTSMPOrderNo )AS S"
            tSql &= Environment.NewLine & "ORDER BY S.FTSizeBreakDown ASC;"
            tmpDTMapSizeMaster1 = HI.Conn.SQLConn.GetDataTable(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN)

        Catch ex As Exception
            Throw New Exception(ex.Message().ToString() & Environment.NewLine & ex.StackTrace().ToString())
            tmpDTMapSizeMaster1 = Nothing
        End Try

        oDBdtMapSize1 = tmpDTMapSizeMaster1.Copy()

        tmpDTMapSizeMaster1.Dispose()

        Return oDBdtMapSize1
            ...
            If Not oDBdtSrc Is Nothing And oDBdtSrc.Rows.Count > 0 And oDBdtSrc.Rows.Count > _nRowHeader Then
                ...
                If bAppendSrcHeader = True Then
                    ...
                    For nLoopOrder = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        tFTBuyer = ""
                        If Not DBNull.Value.Equals(oDBdtSrc.Rows(nLoopOrder).Item("F33")) Then
                            If oDBdtSrc.Rows(nLoopOrder).Item("F33").ToString() <> "" Then
                                tFTBuyer = oDBdtSrc.Rows(nLoopOrder).Item("F33").ToString().Trim()
                            End If
                        End If

                        tStyle = Microsoft.VisualBasic.Left(oDBdtSrc.Rows(nLoopOrder).Item("F13"), 6)
                        tMaterialStyle = oDBdtSrc.Rows(nLoopOrder).Item("F14")
                        tPlanSeasonStyle = oDBdtSrc.Rows(nLoopOrder).Item("F22")

                        If Not DBNull.Value.Equals(oDBdtSrc.Rows(nLoopOrder).Item("F23")) Then
                            If Len(oDBdtSrc.Rows(nLoopOrder).Item("F23")) = 4 Then
                                tYearStyle = Microsoft.VisualBasic.Right(oDBdtSrc.Rows(nLoopOrder).Item("F23"), 2)
                            ElseIf Len(oDBdtSrc.Rows(nLoopOrder).Item("F23")) = 2 Then
                                tYearStyle = oDBdtSrc.Rows(nLoopOrder).Item("F23")
                            End If
                        Else
                        ...
                        tDateF41 = CStr(oDBdtSrc.Rows(nLoopOrder).Item("F38"))
                        Dim tDateF42 As String

                        tDateF42 = CStr(oDBdtSrc.Rows(nLoopOrder).Item("F39"))
                        ...
                        oStrBuilder.AppendLine("                                                         ,[FNHSysProdTypeId],[FTCurrency])")
                        ...
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "'" & HI.UL.ULF.rpQuoted(tPlanSeasonStyle) & "'"))
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "'" & HI.UL.ULF.rpQuoted(tYearStyle) & "'"))
                        ...
                        oStrBuilder.AppendLine(String.Format("      ,{0}", "(SELECT ISNULL((SELECT TOP 1 L1.FNHSysCountryId " &
                ...
                If bAppendSrcDetial = True Then
                    ...
                    Dim tColMatColor As String = "F13"
                    Dim tColMatColorDesc As String = "F20"
                    Dim tColShipMode As String = "F43"
                    Dim tColUom As String = "F44"
                    ...
                    For nLoopSizeBreakdown = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        Dim tDateF42 As String
                        tDateF42 = CStr(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F39"))
                        ...
                        If nPosHyphen > 0 Then
                            ...
                            '_Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS OB ON B.FTOrderNo = OB.FTOrderNo "
                            ''_Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTImportOrderSizeBreakdownTemp AS A ON OB.FTPORef = A.FTPONo AND OB.FTColorway = A.FTColorwayCode"
                            ...
                            '_Qry &= vbCrLf & " WHERE  OB.FTPOref= N'" & HI.UL.ULF.rpQuoted(oDBdtSrc.Rows(nLoopSizeBreakdown).Item(tColPONo)) & "'"
                            '_ORDER = HI.Conn.SQLConn.GetField(_Qry, HI.Conn.DB.DataBaseName.DB_MERCHAN)

                            tSql = ""
                            tSql = "SELECT DISTINCT S.FTSizeBreakDown AS FTMatSize " ' ,S.FTOrderNo,S.FTColorway,S.FTPORef"
                            tSql &= Environment.NewLine & "FROM(SELECT DISTINCT A.FTSizeBreakDown, A.FTOrderNo,A.FTColorway,O.FTPORef"
                            tSql &= Environment.NewLine & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS A WITH (NOLOCK) INNER JOIN"
                            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS M WITH (NOLOCK) ON A.FTOrderNo = M.FTOrderNo AND A.FTSubOrderNo = M.FTSubOrderNo INNER JOIN"
                            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH (NOLOCK) ON M.FTOrderNo = O.FTOrderNo"
                            tSql &= Environment.NewLine & "  UNION ALL"
                            tSql &= Environment.NewLine & "SELECT DISTINCT A.FTSizeBreakDown, A.FTOrderNo,A.FTColorway,M.FTPORef"
                            tSql &= Environment.NewLine & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS A WITH (NOLOCK) INNER JOIN"
                            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS M WITH (NOLOCK) ON A.FTOrderNo = M.FTOrderNo AND A.FTSubOrderNo = M.FTSubOrderNo INNER JOIN"
                            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH (NOLOCK) ON M.FTOrderNo = O.FTOrderNo"
                            tSql &= Environment.NewLine & "  UNION ALL"
                            tSql &= Environment.NewLine & "SELECT DISTINCT A2.FTSizeBreakDown,M2.FTOrderNo,A2.FTColorway,O2.FTPORef"
                            tSql &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_BreakDown AS A2 WITH (NOLOCK) INNER JOIN"
                            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert AS M2 WITH (NOLOCK) ON A2.FTOrderNo = M2.FTOrderNo AND A2.FTSubOrderNo = M2.FTSubOrderNo AND A2.FNDivertSeq = M2.FNDivertSeq INNER JOIN"
                            tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O2 WITH (NOLOCK) ON M2.FTOrderNo = O2.FTOrderNo"
                            tSql &= Environment.NewLine & "UNION ALL"
                            tSql &= Environment.NewLine & "SELECT DISTINCT A2.FTSizeBreakDown,O2.FTSMPOrderNo AS FTOrderNo,A2.FTColorway,'' AS FTPORef"
                            tSql &= Environment.NewLine & "FROM            [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder_Breakdown AS A2 WITH (NOLOCK) INNER JOIN"
                            tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder AS O2 WITH (NOLOCK) ON A2.FTSMPOrderNo = O2.FTSMPOrderNo )AS S" ' INNER JOIN"

                            tSql &= Environment.NewLine & " Left OUTER JOIN("
                            tSql &= Environment.NewLine & " Select  DISTINCT  OB.FTSizeBreakDown,B.FTMatSizeCode,M.FTMapSize,M.FTMapSizeExtension,L1.FTPONo,OB.FTColorway"
                            tSql &= Environment.NewLine & "From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTImportOrderTemp] AS L1"
                            tSql &= Environment.NewLine & "INNER Join [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrder] AS O WITH(NOLOCK) ON  L1.FTPONo=O.FTPORef"
                            tSql &= Environment.NewLine & "INNER Join [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTOrderSub_BreakDown] AS OB WITH(NOLOCK) ON  O.FTOrderNo=OB.FTOrderNo"
                            tSql &= Environment.NewLine & "INNER Join [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..[TMERMMatSize] AS B WITH(NOLOCK) ON OB.FTSizeBreakDown = B.FTMatSizeCode"
                            tSql &= Environment.NewLine & "INNER Join [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERMMapSize] AS M WITH(NOLOCK) ON OB.FTSizeBreakDown= M.FTMapSize Or OB.FTSizeBreakDown= M.FTMapSizeExtension"
                            tSql &= Environment.NewLine & ")AS M ON S.FTPORef=M.FTPONo And S.FTColorway=M.FTColorway And S.FTSizeBreakDown=M.FTMapSizeExtension"

                            tSql &= Environment.NewLine & "WHERE  S.FTPORef = N'" & HI.UL.ULF.rpQuoted(oDBdtSrc.Rows(nLoopSizeBreakdown).Item(tColPONo)) & "'"
                          '  tSql &= Environment.NewLine & "AND  S.FTOrderNo = N'" & HI.UL.ULF.rpQuoted(_ORDER) & "'"
                            tSql &= Environment.NewLine & "      AND S.FTColorway = N'" & HI.UL.ULF.rpQuoted(tMatColor) & "'"

                            '  tSql &= Environment.NewLine & "      AND  (S.FTSizeBreakDown = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "' OR M.FTMapSize = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "')"

                            tSql &= Environment.NewLine & "ORDER BY S.FTSizeBreakDown ASC;"
                            '_Map = HI.Conn.SQLConn.GetField(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN)
                            _dt = HI.Conn.SQLConn.GetDataTable(tSql, Conn.DB.DataBaseName.DB_MERCHAN)
                            '_Map = _dt.Rows(0)!FTMatSize.ToString
                            '_Map = _Map.Trim()
                            '---------------------------------------------------
                            ...
                            Do While tMatchSize <> "" And UCase(tMatchSize) <> UCase(_tTextMatchSizeTerminate) And (nLoopColMatchSize < nCntColMatchSize)
                                ...
                                tStyle = Microsoft.VisualBasic.Left(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F13"), 6)
                                tMaterialStyle = oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F14")
                                tPlanSeasonStyle = oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F22")

                                tYearStyle = Microsoft.VisualBasic.Right$(CStr(Now.Year), 4)
                                If Not DBNull.Value.Equals(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F23")) Then
                                    If Len(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F23")) = 4 Then
                                        tYearStyle = Microsoft.VisualBasic.Right(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F23"), 2)
                                    ElseIf Len(oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F23")) = 2 Then
                                        tYearStyle = oDBdtSrc.Rows(nLoopSizeBreakdown).Item("F23")
                                    End If
                                Else
                                ...
                                ''_Qry &= vbCrLf & "           AND OB.FTSizeBreakDown = A.FTSizeBreakdownCode AND LEFT(ST.FTStyleCode,len(A.FTStyle)) = A.FTStyle AND OB.FTNikePOLineItem=A.FTPOItem"
                                '_Qry &= vbCrLf & " WHERE  OB.FTPOref= N'" & HI.UL.ULF.rpQuoted(oDBdtSrc.Rows(nLoopSizeBreakdown).Item(tColPONo)) & "'"
                                '_ORDER1 = HI.Conn.SQLConn.GetField(_Qry, HI.Conn.DB.DataBaseName.DB_MERCHAN)
                                ...
                                'tSql &= Environment.NewLine & "      AND  (S.FTSizeBreakDown = N'" & HI.UL.ULF.rpQuoted(_dt) & "' OR M.FTMapSize = N'" & HI.UL.ULF.rpQuoted(_Map) & "')"

                                'tSql &= Environment.NewLine & "ORDER BY S.FTSizeBreakDown ASC;"
                                '_Map1 = HI.Conn.SQLConn.GetField(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN)
                                ...
                                If _dt.Rows.Count > 0 Then
                                    ...
                                    For Each Row As DataRow In _dt.Rows
                                        ...
                                        tSql &= Environment.NewLine & "      AND A.FTSizeBreakdownCode = N'" & HI.UL.ULF.rpQuoted(_Map) & "';"
                                        If HI.Conn.SQLConn.GetField(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN, "") <> "" Then
                                            ...
                                            If nFNQuantity > 0 Then
                                                ...
                                                oStrBuilder.AppendLine(String.Format("      AND A.[FTSizeBreakdownCode] = {0}", "N'" & HI.UL.ULF.rpQuoted(_Map) & "'"))
                                            End If
                                        ...
                                        Else
                                            ...
                                            'oStrBuilder.AppendLine(String.Format("           ,{0}", "N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "'"))
                                            oStrBuilder.AppendLine(String.Format("           ,{0}", "N'" & HI.UL.ULF.rpQuoted(_Map) & "'"))
                                            oStrBuilder.AppendLine(String.Format("           ,{0}", "N'" & HI.UL.ULF.rpQuoted(_Map) & "'"))
...
#End Region
        ...
        Try
            ...
            If Me.FTFilePath.Text.Trim() <> "" Then
                ...
                If IO.File.Exists(Me.FTFilePath.Text.Trim().ToString()) Then
                    ...
                    End With
                    'If CheckMapSize(oDBdtExcel) = True Then

                    '    Call W_PRCbLoadMapSizeBreak()
                    'Else
                    '    Call W_PRCbLoadMapSize()
                    'End If
                    'If CheckMapSize1(oDBdtExcel) = True Then
                    '    'Call CheckMapSize1(oDBdtExcel)
                    '    Call W_PRCbLoadMapSizeBreak()
                    'Else
                    Call W_PRCbLoadMapSize()
                    'End If
                    ...
                    'If CheckMapSize2(oDBdtExcel) = True Then

                    '    Call W_PRCbLoadMapSizeBreakDown(oDBdtExcel)
                    '    ' Call W_PRCbLoadMapSizeBreak(oDBdtExcel)
                    '    ' Call W_PRCbLoadMapSizeBreak()
                        ...
                        If Not oDBdtExcel Is Nothing And oDBdtExcel.Rows.Count > 0 Then
                            ...
                            If W_PRCbSaveListOrderToTemp(oDBdtExcel) = True Then
                                ...
                                If CheckOrderQtyZero() = False Then
                                    ...
                                    _Qry = "    SELECT Distinct A.FTPONo AS FTCustomerPO"
                                    _Qry &= vbCrLf & " ,A.FTPOItem"
                                    _Qry &= vbCrLf & " ,A.FTColorwayCode"
                                    _Qry &= vbCrLf & " ,A.FTSizeBreakdownCode"
                                    _Qry &= vbCrLf & " FROM ("
                                    _Qry &= vbCrLf & "SELECT FTPONo, FTPOItem, FTStyle, FTColorwayCode, FTSizeBreakdownCode, FNPrice"
                                    _Qry &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTImportOrderSizeBreakdownTemp AS X WITH(NOLOCK)"
                                    ...
                                    _Qry &= vbCrLf & "	[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS SBD ON A.FTPONo = SBD.FTPOref "
                                    _Qry &= vbCrLf & "	AND A.FTColorwayCode = SBD.FTColorway"
                                    _Qry &= vbCrLf & "	AND A.FTSizeBreakdownCode = SBD.FTSizeBreakDown"
                                    _Qry &= vbCrLf & "	AND A.FTPOItem = SBD.FTNikePOLineItem "

                                    _Qry &= vbCrLf & " INNER JOIN ( SELECT A.FTCustomerPO, B.FTColorway, B.FTSizeBreakDown, B.FTPOLineItem"
                                    _Qry &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTXMLCreateInvoice AS A WITH(NOLOCK) INNER JOIN"
                                    _Qry &= vbCrLf & "       [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTXMLCreateInvoice_D AS B WITH(NOLOCK) ON A.FTCustomerPO = B.FTCustomerPO AND A.FTInvoiceNo = B.FTInvoiceNo"
                                    _Qry &= vbCrLf & " WHERE ISNULL(A.FTPostUser,'') <>'' "
                                    _Qry &= vbCrLf & " ) AS XM"
                                    ...
                                    _dtPoNetPriceDifferent = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN)
    ...
    Private Sub ocmImportOrder_Click(sender As Object, e As EventArgs) Handles ocmImportnetprice.Click
        ...
        Try
            If Me.ogvImportOrder.RowCount > 0 Then
                ...
                _Qry &= vbCrLf & "  FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMStyle AS ST INNER JOIN"
                _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS B ON ST.FNHSysStyleId = B.FNHSysStyleId INNER JOIN"
                _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS OB ON B.FTOrderNo = OB.FTOrderNo INNER JOIN"
                _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTImportOrderSizeBreakdownTemp AS A ON OB.FTPORef = A.FTPONo AND OB.FTColorway = A.FTColorwayCode"
                _Qry &= vbCrLf & "           AND OB.FTSizeBreakDown = A.FTSizeBreakdownCode AND LEFT(ST.FTStyleCode,len(A.FTStyle)) = A.FTStyle AND OB.FTNikePOLineItem=A.FTPOItem"
                _Qry &= vbCrLf & " WHERE  (A.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "')"

                If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_MERCHAN, "") = "" Then
                    HI.MG.ShowMsg.mInfo("ไม่พบข้อมูลที่สามารถทำการ Import ได้ เนื่องจากข้อมูล ไม่ตรงกับ ข้อมูล FO !!!", 1507180785, Me.Text, , MessageBoxIcon.Warning)
                    Exit Sub
                End If

                If HI.MG.ShowMsg.mConfirmProcess("คุณต้องการทำการ Import Factory Order No Net Price ใช่หรือไม่ ?", 1507170033, "Confirm") = True Then
                    ...
                    Dim _Spls As New HI.TL.SplashScreen("Importing Factory Order No Net Price .....Please Wait")
                    ...
                    If W_PRCbImportFactoryOrder(_Spls) = True Then
                        ...
                        Cmdstring &= Environment.NewLine & "    From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_BreakDown_ShipDestination As A WITH(NOLOCK) INNER Join "
                        Cmdstring &= Environment.NewLine & "         [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTImportOrderSizeBreakdownTemp As B WITH(NOLOCK) On A.FTPOref = B.FTPONo "
                        ...
                        _Spls.UpdateInformation("Finishing Import Factory Order No Net Price .....Please Wait")
                        ...
                        HI.MG.ShowMsg.mInfo("Import Factory Order No. Net Price Complete...", 1507170036, Me.Text, , MessageBoxIcon.Information)
                    Else
                        ...
                        HI.MG.ShowMsg.mInfo("พบปัญหาในการบันทึกรายการนำเข้าข้อมูล Net Price !!!", 1507170034, Me.Text, , MessageBoxIcon.Warning)
                    End If
    ...
    Private Function CheckMapSize() As Boolean
        'Dim bAppendSrcDetial As Boolean = True
        ...
        '    For nLoopSizeBreakdown = _nStartRowImportExcel To oDBdt.Rows.Count - 1
        '        Application.DoEvents()
        ...
        '        Dim tMatColor As String
        '        tMatColor = oDBdt.Rows(nLoopSizeBreakdown).Item(tColMatColor)
        ...
        '                        If oDataRow(0)("FTMatSize") <> "" Then
        '                            tMatchSize = oDataRow(0)("FTMatSize").ToString()
        '                        End If
        ...
        '                            Dim oDataRowMapSizeManual As System.Data.DataRow()
        '                            Try
        '                                oDataRowMapSizeManual = _oDBdtUserImportMapSizeManual.Select("FTMatSize = '" & HI.UL.ULF.rpQuoted(tMatchSize) & "'")
        '                                If oDataRowMapSizeManual.Length > 0 Then

        '                                    If oDataRowMapSizeManual(0)("FTMatSize") <> "" Then
        '                                        tMatchSize = oDataRowMapSizeManual(0)("FTMatSize").ToString()
        '                                    End If
        ...
        _FilePath = Me.FTFilePath.Text.Trim()
        Dim oDBdtSrc As New System.Data.DataTable
        oDBdtSrc = HI.UL.ReadExcel.Read(_FilePath, "Sheet1", -1)

        ''tSql = ""
        ''tSql = "SELECT DISTINCT S.FTSizeBreakDown AS FTMatSize" ' ,S.FTOrderNo,S.FTColorway,S.FTPORef"
        ''tSql &= Environment.NewLine & "FROM(SELECT DISTINCT A.FTSizeBreakDown, A.FTOrderNo,A.FTColorway,M.FTPORef"
        ''tSql &= Environment.NewLine & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS A WITH (NOLOCK) INNER JOIN"
        ''tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS M WITH (NOLOCK) ON A.FTOrderNo = M.FTOrderNo AND A.FTSubOrderNo = M.FTSubOrderNo INNER JOIN"
        ''tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH (NOLOCK) ON M.FTOrderNo = O.FTOrderNo"
        ''tSql &= Environment.NewLine & "  UNION ALL"
        ''tSql &= Environment.NewLine & "SELECT DISTINCT A2.FTSizeBreakDown,M2.FTOrderNo,A2.FTColorway,M2.FTPORef"
        ''tSql &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_BreakDown AS A2 WITH (NOLOCK) INNER JOIN"
        ''tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert AS M2 WITH (NOLOCK) ON A2.FTOrderNo = M2.FTOrderNo AND A2.FTSubOrderNo = M2.FTSubOrderNo AND A2.FNDivertSeq = M2.FNDivertSeq INNER JOIN"
        ''tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O2 WITH (NOLOCK) ON M2.FTOrderNo = O2.FTOrderNo"
        ''tSql &= Environment.NewLine & "UNION ALL"
        ...
        ''tSql &= Environment.NewLine & "FROM            [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder_Breakdown AS A2 WITH (NOLOCK) INNER JOIN"
        ''tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder AS O2 WITH (NOLOCK) ON A2.FTSMPOrderNo = O2.FTSMPOrderNo )AS S" ' INNER JOIN"
        '  tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTImportOrderSizeBreakdownTemp AS A2 ON S.FTPORef = A2.FTPONo AND S.FTColorway = A2.FTColorwayCode"
        'tSql &= Environment.NewLine & "WHERE " '  (A2.FTUserLogin = N'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "')"

        ' tSql &= Environment.NewLine & "      AND A.FTColorwayCode = N'" & HI.UL.ULF.rpQuoted(tMatColor) & "'"
        'tSql &= Environment.NewLine & "       S.FTSizeBreakDown = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "';"


        ''tSql &= Environment.NewLine & "ORDER BY S.FTSizeBreakDown ASC;"

        ''Return (HI.Conn.SQLConn.GetField(tSql, Conn.DB.DataBaseName.DB_MERCHAN, "") <> "")
        ...
        _Qry &= vbCrLf & "   FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTImportOrderSizeBreakdownTemp WITH (NOLOCK) "
        _Qry &= vbCrLf & "  WHERE  (FTUserLogin ='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "') "
        _Qry &= vbCrLf & "   GROUP BY  FTPONo ) AS A  "

        _PO = HI.Conn.SQLConn.GetField(_Qry, HI.Conn.DB.DataBaseName.DB_MERCHAN)
        ...
        tSql &= Environment.NewLine & "WHERE  S.FTPORef = N'" & HI.UL.ULF.rpQuoted(_PO) & "'"
        'tSql &= Environment.NewLine & "      AND S.FTColorway = N'" & HI.UL.ULF.rpQuoted(tMatColor) & "'"

        ' tSql &= Environment.NewLine & "      AND  S.FTSizeBreakDown = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "'"

        tSql &= Environment.NewLine & "ORDER BY S.FTSizeBreakDown ASC;"

        Return (HI.Conn.SQLConn.GetField(tSql, Conn.DB.DataBaseName.DB_MERCHAN, "") <> "")
        'Dim dtstss As DataTable
        'dtstss = HI.Conn.SQLConn.GetDataTable(tSql, HI.Conn.DB.DataBaseName.DB_MERCHAN)
    ...
    Private Function CheckMapSize1(ByVal oDBdtSrc As System.Data.DataTable) As Boolean
                    ...
                    For nLoopSizeBreakdown = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        If nPosHyphen > 0 Then
                            ...
                            Do While tMatchSize <> "" And UCase(tMatchSize) <> UCase(_tTextMatchSizeTerminate) And (nLoopColMatchSize < nCntColMatchSize)
                                ...
                                ''   tSql &= Environment.NewLine & "WHERE  S.FTPORef = N'" & HI.UL.ULF.rpQuoted(oDBdtSrc.Rows(nLoopSizeBreakdown).Item(tColPONo)) & "'"
                                'tSql &= Environment.NewLine & "WHERE  S.FTOrderNo = N'" & HI.UL.ULF.rpQuoted(_ORDER) & "'"
                                'tSql &= Environment.NewLine & "      AND S.FTColorway = N'" & HI.UL.ULF.rpQuoted(tMatColor) & "'"
                                ...
                                tSql = " Select  DISTINCT S.FTMatSize"
                                tSql &= Environment.NewLine & " From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O "
                                tSql &= Environment.NewLine & "INNER Join[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_OrderSub_BreakDown_ShipDestination AS T ON O.FTOrderNo=T.FTOrderNo"
                                tSql &= Environment.NewLine & "INNER Join(SELECT DISTINCT S.FTSizeBreakDown AS FTMatSize"
                                tSql &= Environment.NewLine & "FROM(SELECT DISTINCT A.FTSizeBreakDown, A.FTOrderNo ,A.FTColorway,M.FTPORef"
                                tSql &= Environment.NewLine & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS A WITH (NOLOCK) INNER Join"
                                tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS M WITH (NOLOCK) ON A.FTOrderNo = M.FTOrderNo And A.FTSubOrderNo = M.FTSubOrderNo INNER Join"
                                tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH (NOLOCK) ON M.FTOrderNo = O.FTOrderNo"
                                tSql &= Environment.NewLine & "  UNION ALL"
                                tSql &= Environment.NewLine & "Select  DISTINCT A2.FTSizeBreakDown,M2.FTOrderNo,A2.FTColorway,M2.FTPORef"
                                tSql &= Environment.NewLine & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_BreakDown AS A2 WITH (NOLOCK) INNER Join"
                                tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert AS M2 WITH (NOLOCK) ON A2.FTOrderNo = M2.FTOrderNo And A2.FTSubOrderNo = M2.FTSubOrderNo And A2.FNDivertSeq = M2.FNDivertSeq INNER Join"
                                tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O2 WITH (NOLOCK) ON M2.FTOrderNo = O2.FTOrderNo"
                                tSql &= Environment.NewLine & "UNION ALL"
                                tSql &= Environment.NewLine & "Select  DISTINCT A2.FTSizeBreakDown,O2.FTSMPOrderNo As FTOrderNo,A2.FTColorway,'' AS FTPORef"
                                tSql &= Environment.NewLine & "From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder_Breakdown AS A2 WITH (NOLOCK) INNER Join"
                                tSql &= Environment.NewLine & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPOrder AS O2 WITH (NOLOCK) ON A2.FTSMPOrderNo = O2.FTSMPOrderNo )AS S"
                                tSql &= Environment.NewLine & "UNION ALL"
                                tSql &= Environment.NewLine & "Select  M.FTMapSizeExtension As  FTMapSize"
                                tSql &= Environment.NewLine & "FROM(SELECT M.FTMapSize, M.FTMapSizeExtension"
                                tSql &= Environment.NewLine & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERMMapSize AS M)AS M)AS S ON T.FTSizeBreakDown=S.FTMatSize"
                                tSql &= Environment.NewLine & "WHERE  O.FTOrderNo = N'" & HI.UL.ULF.rpQuoted(_ORDER) & "'"
                                tSql &= Environment.NewLine & "      AND T.FTColorway = N'" & HI.UL.ULF.rpQuoted(tMatColor) & "'"
                                tSql &= Environment.NewLine & "      AND  S.FTMatSize = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "'"
                                tSql &= Environment.NewLine & "ORDER BY S.FTMatSize ASC;"
    ...
    End Function
    Private Function CheckMapSize2(ByVal oDBdtSrc As System.Data.DataTable) As Boolean
                    ...
                    For nLoopSizeBreakdown = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        If nPosHyphen > 0 Then
                            ...
                            Do While tMatchSize <> "" And UCase(tMatchSize) <> UCase(_tTextMatchSizeTerminate) And (nLoopColMatchSize < nCntColMatchSize)
                                ...
                                tSql &= Environment.NewLine & " Left OUTER JOIN("
                                tSql &= Environment.NewLine & "Select  DISTINCT L1.FTSizeBreakdownCode,B.FTMatSizeCode,M.FTMapSize,M.FTMapSizeExtension,L1.FTPONo,L1.FTColorwayCode"
                                tSql &= Environment.NewLine & "From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERTImportOrderSizeBreakdownTemp] AS L1"
                                tSql &= Environment.NewLine & "INNER Join [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..[TMERMMatSize] AS B WITH(NOLOCK) ON L1.FTSizeBreakdownCode = B.FTMatSizeCode"
                                tSql &= Environment.NewLine & "INNER Join [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..[TMERMMapSize] AS M WITH(NOLOCK) ON L1.FTSizeBreakdownCode = M.FTMapSize"
                                tSql &= Environment.NewLine & ")AS M ON S.FTPORef=M.FTPONo And S.FTColorway=M.FTColorwayCode And S.FTSizeBreakDown=M.FTMapSizeExtension"
                                ...
                                '  tSql &= Environment.NewLine & "      AND  (S.FTSizeBreakDown = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "' OR M.FTMatSizeCode = N'" & HI.UL.ULF.rpQuoted(tMatchSize) & "')"
    ...
    End Function
    Private Function W_PRCbLoadMapSizeBreakDown(ByVal oDBdtSrc As System.Data.DataTable) As Boolean
        '...catch raw data before confirm import factory order no.
        Dim bAppendSrcHeader As Boolean = True
        Dim bAppendSrcDetial As Boolean = True
        Dim bRet As Boolean = False
                    ...
                    For nLoopSizeBreakdown = _nStartRowImportExcel To oDBdtSrc.Rows.Count - 1
                        ...
                        If nPosHyphen > 0 Then
                            ...
                            Do While tMatchSize <> "" And UCase(tMatchSize) <> UCase(_tTextMatchSizeTerminate) And (nLoopColMatchSize < nCntColMatchSize)
                                ...
                                tSql = ""
                                tSql = "SELECT DISTINCT S.FTSizeBreakDown AS FTMatSize ,S.FTSizeBreakDown AS FTMapSizeExtension" ' ,S.FTOrderNo,S.FTColorway,S.FTPORef"
                                tSql &= Environment.NewLine & "FROM(SELECT DISTINCT A.FTSizeBreakDown, A.FTOrderNo,A.FTColorway,O.FTPORef"
                                tSql &= Environment.NewLine & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS A WITH (NOLOCK) INNER JOIN"
                                tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub AS M WITH (NOLOCK) ON A.FTOrderNo = M.FTOrderNo AND A.FTSubOrderNo = M.FTSubOrderNo INNER JOIN"
                                tSql &= Environment.NewLine & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH (NOLOCK) ON M.FTOrderNo = O.FTOrderNo"
    ...
    Private Function CheckMap(ByVal oDBdtSrc As System.Data.DataTable) As Boolean
    ...
    Private Sub ocmReadExcel_AutoSizeChanged(sender As Object, e As EventArgs) Handles ocmReadExcel.AutoSizeChanged
