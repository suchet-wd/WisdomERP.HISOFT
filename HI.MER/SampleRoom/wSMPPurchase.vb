...
#Region "Grid"
    Private Sub InitGridControl()
        ...
        With ogvdetail
            ...
            .Columns("FNSurcharge").Summary.Add(DevExpress.Data.SummaryItemType.Sum, "FNSurcharge")
            .Columns("FNSurcharge").SummaryItem.DisplayFormat = "{0:n2}"
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        Dim dt As DataTable
        _Str = "  Select TOP 1 FTStateSendApp, FTStateSuperVisorApp, FTStateManagerApp, FTPurchaseNo From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase AS X WITH(NOLOCK) WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Key.ToString) & "' "
        dt = HI.Conn.SQLConn.GetDataTable(_Str, Conn.DB.DataBaseName.DB_SAMPLE)
        ...
        For Each R As DataRow In dt.Rows
            ...
            FTStateSuperVisorApp.Checked = (R!FTStateSuperVisorApp.ToString = "1")
            FTStateManagerApp.Checked = (R!FTStateManagerApp.ToString = "1")
            Exit For
        Next
        dt.Dispose()
        'Me.FNHSysCurId.Text = ""
...
#End Region
    ...
    Private Sub LoadDetail(ByVal _DocRefNo As String)
        Try
            ...
            _Qry = " SELECT  P.FNSeq, P.FTSMPOrderNo, P.FTMatCode,P.FTMatColorCode,P.FTMatSizeCode,P.FTDescription,P.FTColorName, P.FNQuantity, P.FNPrice, P.FNAmount,P.FTNote,ISNULL(U.FTUnitCode,'') AS FTUnitCode,ISNULL(P.FNSurcharge,0) AS FNSurcharge"
            _Qry &= vbCrLf & " ,ISNULL(RCV.FTStateRcv,'0') FTStateRcv "
            ...
            _Qry &= vbCrLf & " OUTER APPLY (SELECT TOP 1  '1' AS FTStateRcv "
            _Qry &= vbCrLf & "	 FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReceive AS RCVH  WITH(NOLOCK) "
            _Qry &= vbCrLf & "	 INNER JOIN     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReceive_Detail AS RCVD  WITH(NOLOCK) ON RCVH.FTReceiveNo=RCVD.FTReceiveNo "
            _Qry &= vbCrLf & " WHERE RCVH.FTPurchaseNo = P.FTPurchaseNo "
            _Qry &= vbCrLf & " AND RCVD.FTSMPOrderNo = P.FTSMPOrderNo "
            _Qry &= vbCrLf & " AND RCVD.FTMatCode = P.FTMatCode "
            _Qry &= vbCrLf & " AND RCVD.FTMatColorCode = P.FTMatColorCode "
            _Qry &= vbCrLf & " AND RCVD.FTMatSizeCode = P.FTMatSizeCode "
            _Qry &= vbCrLf & " AND RCVD.FNSeqRef = P.FNSeq "
            _Qry &= vbCrLf & " ) As RCV "
            ...
            _Qry = "SELECT ROW_NUMBER() Over (Order By A.FNSeq) AS FNSeq , FTMatCode,FTMatColorCode,FTMatSizeCode,FTDescription,FNQuantity,FTColorName,FNPrice,Convert(numeric(18,2),FNQuantity * FNPrice ) AS FNAmount,FNSurcharge"
            _Qry &= vbCrLf & " FROM ( SELECT  MIN(P.FNSeq) AS FNSeq,  P.FTMatCode,P.FTMatColorCode,P.FTMatSizeCode,MAX(P.FTDescription) AS FTDescription,MAX(P.FTColorName) As FTColorName, SUM(P.FNQuantity) AS FNQuantity, MAX(P.FNPrice) AS FNPrice ,MAX(P.FTNote) AS FTNote,MAX(ISNULL(U.FTUnitCode,'')) AS FTUnitCode,MAX(ISNULL(P.FNSurcharge,0)) AS FNSurcharge"
            _Qry &= vbCrLf & "	 FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail AS P WITH(NOLOCK) "
            _Qry &= vbCrLf & "	INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS U WITH(NOLOCK) ON P.FNHSysUnitId=U.FNHSysUnitId "
            _Qry &= vbCrLf & "	 WHERE P.FTPurchaseNo ='" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "'"
        ...
        With _AddItem
            ...
            If (.AddMat) Then
                ...
                Try
                    ...
                    _Str &= vbCrLf & " ,FTColorName=N'" & HI.UL.ULF.rpQuoted(.FTSMPMatColorCode_None.Text.Trim) & "' "
                    ...
                    _Str &= vbCrLf & " ,FNSurcharge=" & Val(.FNSurchangeAmt.Value) & " "
                    _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                    ...
                    If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                        ...
                        _Str = "Insert into  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail(FTInsUser, FDInsDate, FTInsTime"
                        _Str &= vbCrLf & " , FTPurchaseNo, FNSeq, FTSMPOrderNo,FTMatCode,FTMatColorCode,FTMatSizeCode, FTDescription,FNHSysUnitId, FNQuantity, FNPrice, FNAmount, FTNote,FTColorName,FNSurcharge)"
                        _Str &= vbCrLf & "  SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                        _Str &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB & ""
                        _Str &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB & ""
                        _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                        ...
                        _Str &= vbCrLf & "," & .FNAmount.Value & " "
                        _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(.FTNote.Text.Trim) & "' "
                        _Str &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(.FTSMPMatColorCode_None.Text.Trim) & "' "
                        _Str &= vbCrLf & " ," & Val(.FNSurchangeAmt.Value) & " "
                    ...
                    _Str &= vbCrLf & ",FNSurchargePerUnit = CASE WHEN " & .FNSurchangeAmt.Value & " <= 0 THEN 0.0000 ELSE  Convert(numeric(18,5)," & .FNSurchangeAmt.Value & " / ISNULL(( SELECT SUM(FNQuantity) AS FNQuantity "
                    _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail  "
                    _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                    _Str &= vbCrLf & "  AND FTMatCode=N'" & HI.UL.ULF.rpQuoted(.FTSMPMatCode.Text.Trim) & "' "
                    _Str &= vbCrLf & "  AND FTMatColorCode=N'" & HI.UL.ULF.rpQuoted(.FTSMPMatColorCode.Text.Trim) & "' "
                    _Str &= vbCrLf & "  AND FTMatSizeCode=N'" & HI.UL.ULF.rpQuoted(.FTSMPMatSizeCode.Text.Trim) & "' "
                    ...
                    _Str &= vbCrLf & " ,FNNetPrice=  " & .FNPrice.Value & " "
                    _Str &= vbCrLf & " +  CASE WHEN " & .FNSurchangeAmt.Value & " <= 0 THEN 0.0000 ELSE  Convert(numeric(18,5)," & .FNSurchangeAmt.Value & " / ISNULL(( SELECT SUM(FNQuantity) AS FNQuantity "
                    _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail  "
                    _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                    _Str &= vbCrLf & "  AND FTMatCode=N'" & HI.UL.ULF.rpQuoted(.FTSMPMatCode.Text.Trim) & "' "
                    _Str &= vbCrLf & "  AND FTMatColorCode=N'" & HI.UL.ULF.rpQuoted(.FTSMPMatColorCode.Text.Trim) & "' "
                    ...
                    _Str &= vbCrLf & "  UPDATE  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase "
                    ...
                    _Str = "      Select SUM((Convert(numeric(18, 2), FNQuantity * (FNPrice ))) + FNSurcharge ) AS NETAMT"
                    _Str &= vbCrLf & "    FROM"
                    _Str &= vbCrLf & " ("
                    _Str &= vbCrLf & " SELECT       FTMatCode,FTMatColorCode,FTMatSizeCode,  FNPrice, SUM(FNQuantity) AS  FNQuantity,Max(ISNULL(FNSurcharge,0)) AS FNSurcharge"
    ...
    Private Sub ocmremove_Click(sender As Object, e As EventArgs) Handles ocmRemoveDT.Click
        ...
        With ogvdetail
            ...
            Dim _MatCode As String = "" & ogvdetail.GetFocusedRowCellValue("FTMatCode").ToString
            Dim _MatColor As String = "" & ogvdetail.GetFocusedRowCellValue("FTMatColorCode").ToString
            Dim _MatSize As String = "" & ogvdetail.GetFocusedRowCellValue("FTMatSizeCode").ToString
            ...
            If HI.Conn.SQLConn.ExecuteOnly(_Str, _DBEnum) = True Then
                ...
                _Str = "Update [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail SET  "
                _Str &= vbCrLf & " FNNetPrice= FNPrice  "
                _Str &= vbCrLf & " +  CASE WHEN  FNSurcharge <= 0 THEN 0.0000 ELSE  Convert(numeric(18,5),FNSurcharge / ISNULL(( SELECT SUM(FNQuantity) AS FNQuantity "
                _Str &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail "
                _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                _Str &= vbCrLf & "  AND FTMatCode=N'" & HI.UL.ULF.rpQuoted(_MatCode) & "' "
                _Str &= vbCrLf & "  AND FTMatColorCode=N'" & HI.UL.ULF.rpQuoted(_MatColor) & "' "
                _Str &= vbCrLf & "  AND FTMatSizeCode=N'" & HI.UL.ULF.rpQuoted(_MatSize) & "' "
                _Str &= vbCrLf & " ),1))  END "
                _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                _Str &= vbCrLf & "  AND FTMatCode=N'" & HI.UL.ULF.rpQuoted(_MatCode) & "' "
                _Str &= vbCrLf & "  AND FTMatColorCode=N'" & HI.UL.ULF.rpQuoted(_MatColor) & "' "
                _Str &= vbCrLf & "  AND FTMatSizeCode=N'" & HI.UL.ULF.rpQuoted(_MatSize) & "' "
                _Str &= vbCrLf & "    Select SUM((Convert(numeric(18, 2), FNQuantity * (FNPrice ))) + FNSurcharge ) AS NETAMT"
                _Str &= vbCrLf & "    FROM"
                _Str &= vbCrLf & " ("
                _Str &= vbCrLf & " SELECT       FTMatCode,FTMatColorCode,FTMatSizeCode,  FNPrice, SUM(FNQuantity) AS  FNQuantity,Max(ISNULL(FNSurcharge,0)) AS FNSurcharge"
                _Str &= vbCrLf & " FROM            [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPPurchase_Detail AS A  WITH(NOLOCK)"
        ...
        Try
            With ogvdetail
                ...
                With _AddItem
                    ...
                    .FTSMPMatColorCode_None.Text = "" & ogvdetail.GetFocusedRowCellValue("FTColorName").ToString
                    .FNSurchangeAmt.Value = Val("" & ogvdetail.GetFocusedRowCellValue("FNSurcharge").ToString)
                    ...
                    If (.AddMat) Then
                        ...
                        Try
                            ...
                            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                                ...
                                _Str &= vbCrLf & "," & .FNAmount.Value & " "
                                _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(.FTNote.Text.Trim) & "' "
                                _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(.FTSMPMatColorCode_None.Text) & "' "
                                _Str &= vbCrLf & " ," & Val(.FNSurchangeAmt.Value) & " "
