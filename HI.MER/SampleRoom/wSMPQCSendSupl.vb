...
Public Class wSMPQCSendSupl
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " 	  INNER JOIN " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & ".dbo.TPRODTBundle AS BD WITH(NOLOCK) ON BS.FTBarcodeBundleNo = BD.FTBarcodeBundleNo"
    ...
    Private Function _SaveData() As Boolean
        Try
            ...
            If HI.Conn.SQLConn.ExecuteNonQuery(_Cmd, Conn.DB.DataBaseName.DB_PROD) = False Then
                _Cmd = "INSERT INTO " & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & ".dbo.TPROSendSuplDefect ( FTInsUser, FDInsDate, FTInsTime,   FTBarcodeSendSuplNo, FNDefectQty, FTRemark  )"
    ...
    Private Function LoadOnhandFG() As DataTable
        Try
            ...
            _Cmd &= vbCrLf & "FROM " & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & ".dbo.TPRODTBundle As B With (NOLOCK) RIGHT OUTER JOIN"
