...
Public Class wSMPRetSupl
    ...
    Private _AddItemPopup As wSMPRetSuplItem
    ...
    Sub New()
        ...
        _AddItemPopup = New wSMPRetSuplItem
...
#Region "Procedure"
    ...
    Private Sub LoadRcvDetail(PoKey As String)
        ...
        _Str &= vbCrLf & " FROM             [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl_Detail AS D WITH (NOLOCK)  "
        ...
        _Str &= vbCrLf & " WHERE        (D.FTRetSuplNo = N'" & HI.UL.ULF.rpQuoted(PoKey) & "')"
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl WHERE FTRetSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl_Detail WHERE FTRetSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl WHERE FTRetSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "'")
            Return True
        Catch ex As Exception
            HI.Conn.SQLConn.Tran.Rollback()
            HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
...
#Region "MAIN PROC"
    ...
    Private Function CheckOwner() As Boolean
        If (HI.ST.UserInfo.UserName.ToUpper = FTRetSuplBy.Text.ToUpper) Or (HI.ST.SysInfo.Admin) Then
            Return True
        Else
            ...
            _Qry &= vbCrLf & "   WHERE  FTUserName = '" & HI.UL.ULF.rpQuoted(FTRetSuplBy.Text) & "' "
    ...
    Private Function CheckDocRef() As Boolean
        ...
        _Str = "Select TOP 1 FTIssueRefNo FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail As R WITH(NOLOCK) WHERE FTIssueRefNo='" & HI.UL.ULF.rpQuoted(FTRetSuplNo.Text) & "'  "

        If HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_SAMPLE, "") <> "" Then

            HI.MG.ShowMsg.mProcessError(1303154331, "พบข้อมูลการคืน สต๊อก แล้ว ไม่สามารถทำการลบหรือแก้ไขรายการได้ !!!", Me.Text, System.Windows.Forms.MessageBoxIcon.Information)
    ...
    Private Sub Proc_Delete(sender As System.Object, e As System.EventArgs) Handles ocmdelete.Click
        ...
        If (CheckDocRef() = False) Then Exit Sub
        If HI.MG.ShowMsg.mConfirmProcessDefaultNo(MG.ShowMsg.ProcessType.mDelete, Me.FTRetSuplNo.Text, Me.Text) = False Then
            Exit Sub
        End If
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTRetSuplNo.Text <> "" Then
            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "Inventrory\"
                .ReportName = "SMPIssuelip.rpt"
                .Formular = "{TSMPRetSupl.FTRetSuplNo}='" & HI.UL.ULF.rpQuoted(FTRetSuplNo.Text) & "' "
                .Preview()
            End With


        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.SelectData, Me.Text, Me.FTRetSuplNo_lbl.Text)
            FTRetSuplNo.Focus()
        End If
    End Sub
...
#End Region
    ...
    Private Sub FTReceiveNo_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles FTRetSuplNo.EditValueChanged
    End Sub
    ...
    Private Sub ocmadd_Click(sender As System.Object, e As System.EventArgs) Handles ocmadd.Click
        If CheckOwner() = False Then Exit Sub
        If (CheckDocRef() = False) Then Exit Sub
        If FTRetSuplNo.Properties.Tag.ToString = "" Then
            If Me.VerrifyData() Then
                If Me.SaveData Then
                Else
                    Exit Sub
            ...
            End If
        Else
            If Me.FTRetSuplNo.Text = "" Or Me.FTRetSuplNo.Properties.Tag.ToString = "" Then
                Exit Sub
            Else
        ...
        _Str = " EXEC  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.SP_GET_ONHAND_FOR_RET_SUPL " & Val(FNHSysWHId.Properties.Tag.ToString()) & ",'" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "','" & HI.UL.ULF.rpQuoted(FTRetSuplNo.Text) & "' "
            ...
            If .ProcessProc Then
                ...
                If _dt.Select(" FNIssueQuantity > 0 ").Length > 0 Then
                    ...
                    Try
                        ...
                        _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl_Detail "
                        _Str &= vbCrLf & " WHERE   FTRetSuplNo ='" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "' "
                        ...
                        For Each R As DataRow In _dt.Select("FNIssueQuantity > 0")
                            ...
                            _Str = " INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl_Detail ( FTInsUser, FDInsDate, FTInsTime, FTRetSuplNo, FNSeq, FTMatCode, FTMatColorCode,FTMatSizeCode,FTDescription, FNQuantity, FNPrice, FNAmount, FTPurchaseNoRef, FTDocReferNo,FNHSysUnitId,FTSMPOrderNo) "
                            _Str &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                            _Str &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB & " "
                            _Str &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB & " "
                            _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "' "
                    ...
                    Me.LoadRcvDetail(Me.FTRetSuplNo.Text)
    ...
    Private Sub ocmremove_Click(sender As System.Object, e As System.EventArgs) Handles ocmremove.Click
        With ogvdetail
            ...
            Try
                ...
                _Str = " Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl_Detail   WHERE FTRetSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "' AND  FNSeq=" & Val(_MatID) & " "
                If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    HI.Conn.SQLConn.Tran.Rollback()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
                ...
                HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPRetSupl_Detail   WHERE FTRetSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTRetSuplNo.Text) & "' AND  FNSeq=" & Val(_MatID) & " ")
    ...
    Private Sub ocmrefresh_Click(sender As Object, e As EventArgs) Handles ocmrefresh.Click
        Call LoadDataInfo(Me.FTRetSuplNo.Text)
    End Sub


    Private Sub FDReceiveDate_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles FDRetSuplDate.EditValueChanging
        'Try
        '    If FDRetSuplDate.Properties.ReadOnly = False Then
    ...
    Private Sub LoadPOInfo(PoKey As String, Optional LoadRcv As Boolean = False)
        ...
        _Str = " SELECT   TOP 1      H.FTPurchaseNo, H.FNExchangeRate, S.FTSuplCode, C.FTCurCode"
