...
Public Class wSMPReturnStock
    ...
    Private _AddItemPopup As wSMPReturnStockItem
    ...
    Sub New()
        ...
        _AddItemPopup = New wSMPReturnStockItem
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        FDReturnDate.Properties.Buttons(0).Visible = False
        FDReturnDate.Properties.ReadOnly = True
    ...
    Private Sub LoadRcvDetail(PoKey As String)
        ...
        _Str &= vbCrLf & " FROM             [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail AS D WITH (NOLOCK)  "
        ...
        _Str &= vbCrLf & " WHERE        (D.FTReturnNo = N'" & HI.UL.ULF.rpQuoted(PoKey) & "')"
    ...
    Private Function SaveData() As Boolean
        ...
        If (_StateNew) Then
            ...
            If Me.FDReturnDate.Properties.ReadOnly = False Then
                _Key = HI.TL.Document.GetDocumentNo(Me.SysDBName, Me.SysTableName, "", False, _CmpH, HI.UL.ULDate.ConvertEnDB(Me.FDReturnDate.Text)).ToString
            Else
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn WHERE FTReturnNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail WHERE FTReturnNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn WHERE FTReturnNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "'")
            Return True
        Catch ex As Exception
            HI.Conn.SQLConn.Tran.Rollback()
            HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
...
#Region "MAIN PROC"
    ...
    Private Function CheckOwner() As Boolean
        If (HI.ST.UserInfo.UserName.ToUpper = FTReturnBy.Text.ToUpper) Or (HI.ST.SysInfo.Admin) Then
            Return True
        Else
            ...
            _Qry &= vbCrLf & "   WHERE  FTUserName = '" & HI.UL.ULF.rpQuoted(FTReturnBy.Text) & "' "
    ...
    Private Function CheckDocRef() As Boolean
        ...
        _Str = "Select TOP 1 FTDocReferNo FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPIssue_Detail As R WITH(NOLOCK) WHERE FTDocReferNo='" & HI.UL.ULF.rpQuoted(FTReturnNo.Text) & "'  "

        If HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_SAMPLE, "") <> "" Then

            HI.MG.ShowMsg.mProcessError(1303157331, "พบข้อมูลการจ่าย แล้ว ไม่สามารถทำการลบหรือแก้ไขรายการได้ !!!", Me.Text, System.Windows.Forms.MessageBoxIcon.Information)
    ...
    Private Sub Proc_Delete(sender As System.Object, e As System.EventArgs) Handles ocmdelete.Click
        ...
        If (CheckDocRef() = False) Then Exit Sub
        If HI.MG.ShowMsg.mConfirmProcessDefaultNo(MG.ShowMsg.ProcessType.mDelete, Me.FTReturnNo.Text, Me.Text) = False Then
            Exit Sub
        End If
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTReturnNo.Text <> "" Then
            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "Inventrory\"
                .ReportName = "SMPReturnToStockSlip.rpt"
                .Formular = "{TSMPReturn.FTReturnNo}='" & HI.UL.ULF.rpQuoted(FTReturnNo.Text) & "' "
                .Preview()
            End With


        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.SelectData, Me.Text, Me.FTReturnNo_lbl.Text)
            FTReturnNo.Focus()
        End If
    End Sub
...
#End Region
    ...
    Private Sub FTReceiveNo_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles FTReturnNo.EditValueChanged
    End Sub
    ...
    Private Sub ocmadd_Click(sender As System.Object, e As System.EventArgs) Handles ocmadd.Click
        If CheckOwner() = False Then Exit Sub
        If (CheckDocRef() = False) Then Exit Sub
        If FTReturnNo.Properties.Tag.ToString = "" Then
            If Me.VerrifyData() Then
                If Me.SaveData Then
                Else
                    Exit Sub
        ...
        Else
            ...
            If Me.FTReturnNo.Text = "" Or Me.FTReturnNo.Properties.Tag.ToString = "" Then
                Exit Sub
            Else
        ...
        _Str = " EXEC  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.SP_GET_ONHAND_FOR_RET '" & HI.UL.ULF.rpQuoted(FTSMPOrderNo.Text) & "','" & HI.UL.ULF.rpQuoted(FTReturnNo.Text) & "' "
            ...
            If .ProcessProc Then
                ...
                If _dt.Select(" FNIssueQuantity > 0 ").Length > 0 Then
                    ...
                    Try
                        ...
                        _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail "
                        _Str &= vbCrLf & " WHERE   FTReturnNo ='" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "' "
                        ...
                        For Each R As DataRow In _dt.Select("FNIssueQuantity > 0")
                            ...
                            _Str = " INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail ( FTInsUser, FDInsDate, FTInsTime, FTReturnNo, FNSeq, FTMatCode,FTMatColorCode,FTMatSizeCode, FTDescription, FNQuantity, FNPrice, FNAmount, FTPurchaseNoRef, FTIssueRefNo,FNHSysUnitId) "
                            _Str &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                            _Str &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB & " "
                            _Str &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB & " "
                            _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "' "
                    ...
                    Me.LoadRcvDetail(Me.FTReturnNo.Text)
    ...
    Private Sub ocmremove_Click(sender As System.Object, e As System.EventArgs) Handles ocmremove.Click
        With ogvdetail
            ...
            Try
                ...
                _Str = " Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail   WHERE FTReturnNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "' AND  FNSeq=" & Val(_MatID) & " "
                If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    HI.Conn.SQLConn.Tran.Rollback()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
                ...
                HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SAMPLE) & "].dbo.TSMPReturn_Detail   WHERE FTReturnNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnNo.Text) & "' AND  FNSeq=" & Val(_MatID) & " ")
    ...
    Private Sub ocmrefresh_Click(sender As Object, e As EventArgs) Handles ocmrefresh.Click
        Call LoadDataInfo(Me.FTReturnNo.Text)
    End Sub
    ...
    Private Sub FDReceiveDate_EditValueChanged(sender As Object, e As EventArgs) Handles FDReturnDate.EditValueChanged

    End Sub

    Private Sub FDReceiveDate_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles FDReturnDate.EditValueChanging
        Try
            If FDReturnDate.Properties.ReadOnly = False Then
