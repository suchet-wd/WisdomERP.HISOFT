...
#Region "Handler Control"
    ...
    Sub New()
        ...
        Try
            Call oSysLang.LoadObjectLanguage(HI.ST.SysInfo.ModuleID, _CompareBom.Name.ToString.Trim, _CompareBom)
        Catch ex As Exception
        Finally
        End Try
...
#Region "Form Load"
    ...
    Private Sub wGenerateStyle_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            RemoveHandler FNHSysParentBOMId.EditValueChanged, AddressOf HI.TL.HandlerControl.DynamicButtonedit_EditValueChanged
            RemoveHandler FNHSysParentBOMId.Leave, AddressOf HI.TL.HandlerControl.DynamicButtonedit_LeaveOnly
...
#Region "MAIN PROC"
    ...
    Private Sub LoadStyleInfo(ByVal _FNHSysStyleDevId As String, Optional _DefaultTab As Boolean = False)
        ...
        If (FNHSysStyleDevId_Hide.Text <> "") Then
            ...
            _Str &= vbCrLf & ", CONVERT(VARCHAR(10), CONVERT(DATETIME, MS.FDInsDate, 120), 103) AS FDInsDate"
            ...
            _Str &= vbCrLf
            _Str &= vbCrLf & "WHERE(MS.FNHSysStyleDevId = " & Val(FNHSysStyleDevId_Hide.Text) & ")"
            '_Str &= vbCrLf & "And ISNULL(MS.FTBOMState,0) =  1"
            _Str &= vbCrLf
            ...
            If _dt.Rows.Count > 0 Then
                For Each R As DataRow In _dt.Rows
                    ...
                    FDImportDate.Text = R!FDInsDate.ToString
                    ...
                    'FNHSysParentBOMId.SelectedIndex = Val(R!sss.ToString)
    ...
    Private Sub LoadStyleDetail(ByVal FNHSysStyleDevId As String)
        ...
        _Str &= vbCrLf & ", T2.FNHSysCOFOId, T2.FTNikeSuplCode "
        _Str &= vbCrLf
    ...
    Private Sub ImportSize()
        ...
        Try
            ...
            StrSql = " EXEC [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) &
                "].[dbo].[SP_GET_STYLE_SIZEBREAKDOWN_DEVELOP] " & Val(FNHSysStyleDevId_Hide.Text) &
            ...
            StrSql = " EXEC [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[SP_GET_STYLE_SIZEBREAKDOWNINFO_DEVELOP_V2] " & Val(FNHSysStyleDevId_Hide.Text) & ",'" & HI.ST.Lang.Language.ToString() & "' "
    ...
    Private Function SaveData(ByVal _FNHSysStyleDevId As Integer) As Boolean
        Dim ret As Boolean
        Dim _Str As String = ""
        ...
        Try
            ...
            FNHSysStyleDevId_Hide.Text = _FNHSysStyleDevId
            ...
            Try
                _Str = "SELECT TOP 1 FNHSysStyleDevId FROM " &
                    "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[TMERTDevelopStyle] WHERE FNHSysStyleDevId = " & Val(FNHSysStyleDevId_Hide.Text) & " "
                If HI.Conn.SQLConn.ExecuteTran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    If cnt = 0 Then
                        ...
                        _Str &= vbCrLf & ", FTDimension , FTSeniordeveloper , FTProgram , FTProductDev ) "
                        ...
                        _Str &= vbCrLf & "," & Val(FNVersion.Text) & ""
                        ...
                        _Str &= vbCrLf & ", N'" & HI.UL.ULF.rpQuoted(FTDimension.Text.Trim) & "'"
                        _Str &= vbCrLf & ", N'" & HI.UL.ULF.rpQuoted(FTStyleSeniorDev.Text.Trim) & "'"
                        _Str &= vbCrLf & ", N'" & HI.UL.ULF.rpQuoted(FTProgram.Text.Trim) & "'"
                        _Str &= vbCrLf & ", N'" & HI.UL.ULF.rpQuoted(FTProductDev.Text.Trim) & "'"
                    ...
                    Else
                        ...
                        ",FNVersion =" & Val(FNVersion.Text) & "" & vbCrLf &
                        ",FTMSCCode = '" & HI.UL.ULF.rpQuoted(FNHSysMSCId.Text) & "'" & vbCrLf &
                        ",FTMSCLevel1 = '" & HI.UL.ULF.rpQuoted(FTMSCLevel1.Text) & "' " & vbCrLf &
                        ",FTMSCLevel2 = '" & HI.UL.ULF.rpQuoted(FTMSCLevel2.Text) & "' " & vbCrLf &
                        ",FTMSCLevel3 = '" & HI.UL.ULF.rpQuoted(FTMSCLevel3.Text) & "' " & vbCrLf &
                        ",FTSilhouette = '" & HI.UL.ULF.rpQuoted(FTSilhouette.Text.Trim) & "' " & vbCrLf &
                        ",FTDimension = '" & HI.UL.ULF.rpQuoted(FTDimension.Text.Trim) & "' " & vbCrLf &
                        ",FTSeniordeveloper = '" & HI.UL.ULF.rpQuoted(FTStyleSeniorDev.Text.Trim) & "' " & vbCrLf &
                        ",FTProgram = '" & HI.UL.ULF.rpQuoted(FTProgram.Text.Trim) & "' " & vbCrLf &
                        ",FTProductDev = '" & HI.UL.ULF.rpQuoted(FTProductDev.Text.Trim) & "' " & vbCrLf &
...
#Region "Delete Process"
    ...
    Private Function DeleteAllData(ByVal objspl As HI.TL.SplashScreen) As Boolean
        Try
            ...
            _Str = " EXEC " & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & ".[dbo].[SP_DELETE_BOMDEV] "
            _Str &= vbCrLf & " @FNHSysStyleDevId = '" & Val(FNHSysStyleDevId_Hide.Text) & "'"
            _Str &= vbCrLf & ", @UserName = '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
...
#End Region
    ...
    Private Sub LoadImangeStyle()
        Try
            ...
            _Qry &= vbCrLf & " OUTER APPLY(SELECT TOP 1 * FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle AS b WITH (NOLOCK) "
            _Qry &= vbCrLf & " WHERE S.FTStyleCode = b.FTStyleDevCode ) AS b "
            _Qry &= vbCrLf & "WHERE S.FTStyleCode = '" & FNHSysStyleDevId.Text.Trim() & "' "
            '_Qry &= vbCrLf & "WHERE b.FNHSysStyleDevId=" & Val(FNHSysStyleDevId_Hide.Text.Trim())
            '_Qry &= vbCrLf & " WHERE FNHSysStyleId=" & _FNHSysStyleId & ""
    ...
    Private Sub ocmManualImport_Click(sender As Object, e As EventArgs) Handles ocmManualImport.Click
        ...
        With _CreateBomDevImport
            ...
            If (.ProcComplete) Then
                ...
                '    _Qry = "SELECT TOP 1  ISNULL(MAX(b.FNVersion),0) + 1  "
                '    _Qry &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle AS b WITH(NOLOCK) "
                '    _Qry &= vbCrLf & "WHERE b.FTStyleDevCode = '" & .oFTStyle.Text & "' "
                '    _Qry &= vbCrLf & "AND b.FTSeason = '" & .oFTSeason.Text & "' "
                '    _Qry &= vbCrLf & "AND b.FNBomDevType = '" & .oFNBomDevType.Text & "'"
