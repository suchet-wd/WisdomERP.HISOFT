...
Public Class wGenerateStyleDevelop
...
#Region "MAIN PROC"
        ...
        Try
            ...
            Else
                If FNHSysStyleDevId.Text <> "" Then
                    Dim _Str As String = "SELECT TOP 1 FNHSysStyleDevId FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle  WITH(NOLOCK) WHERE FTStyleDevCode ='" & HI.UL.ULF.rpQuoted(FNHSysStyleDevId.Text) & "' AND FTSeason='" & HI.UL.ULF.rpQuoted(FTSeason.Text) & "' AND ISNULL(FNVersion,0)=" & FNVersion.Value & " "
    ...
    Private Function VerifyMasterData() As Boolean
        ...
        _Qry &= vbCrLf & "  FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_Mat As A  INNER JOIN"
        ...
        _Qry &= vbCrLf & "   LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS C WITH(NOLOCK) ON A.FTUnitCode = C.FTUnitCode LEFT OUTER JOIN"
        _Qry &= vbCrLf & "       [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS B WITH(NOLOCK) ON A.FTSuplCode = B.FTSuplCode"
    ...
    Private Function CheckColorWay() As Boolean
        ...
        Try
            ...
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_ColorWay AS A WITH(NOLOCK) LEFT OUTER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatColor AS MC WITH(NOLOCK)  ON A.FTColorWay = MC.FTMatColorCode"
    ...
    Private Function CheckRawMatColor() As Boolean
        ...
        Try
            ...
            _Qry &= vbCrLf & ",'1' AS FTStateActive,''"
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_ColorWay AS A WITH(NOLOCK) LEFT OUTER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMatColor AS MC WITH(NOLOCK)  ON  A.FTColorCode = MC.FTRawMatColorCode "
    ...
    Private Function CheckSizeBeakDown() As Boolean
        ...
        Try
            ...
            _Qry &= vbCrLf & ",'' AS FTRemark"
            _Qry &= vbCrLf & ",'1' AS FTStateActive"
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_SizeBreakDown AS A WITH(NOLOCK) LEFT OUTER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize AS MC WITH(NOLOCK)  ON A.FTSizeBreakDown = MC.FTMatSizeCode"
    ...
    Private Function CheckRawMatSize() As Boolean
        ...
        Try
            ...
            _Qry &= vbCrLf & ",'1' AS FTStateActive"
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_SizeBreakDown AS A WITH(NOLOCK) LEFT OUTER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMatSize AS MC WITH(NOLOCK)  ON A.FTSizeCode = MC.FTRawMatSizeCode"
            ...
            If _FTStyleCode <> "" Then
                ...
                For Each R As DataRow In _dtmat.Rows
                    ...
                    If _MaxFNSeq < 0 Then
                        ...
                        _Qry &= vbCrLf & "  FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_Mat AS A WITH(NOLOCK) LEFT OUTER JOIN"
                        _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS U  WITH(NOLOCK)   ON A.FTUnitCode = U.FTUnitCode LEFT OUTER JOIN"
                        _Qry &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS S  WITH(NOLOCK)   ON A.FTSuplCode = S.FTSuplCode"
                        _Qry &= vbCrLf & " WHERE  (A.FNHSysStyleDevId  =" & Val(_FNHSysStyleDevId) & ")"
                    ...
                    _Qry &= vbCrLf & "  FROM      [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_ColorWay AS A WITH(NOLOCK) INNER JOIN"
                    ...
                    _Qry &= vbCrLf & " FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle_SizeBreakDown AS A  WITH(NOLOCK) INNER JOIN"
                    _Qry &= vbCrLf & "          [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMatSize AS MS  WITH(NOLOCK)  ON A.FTSizeBreakDown = MS.FTMatSizeCode LEFT OUTER JOIN"
                    _Qry &= vbCrLf & "          [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMatSize AS MRZ  WITH(NOLOCK) ON A.FTSizeCode = MRZ.FTRawMatSizeCode"
                    _Qry &= vbCrLf & " WHERE  (A.FNHSysStyleDevId  =" & Val(_FNHSysStyleDevId) & ")"
                    _Qry &= vbCrLf & " AND  A.FNSeq =" & _FNSeq & ""
                    _Qry &= vbCrLf & " AND  A.FNMerMatSeq =" & _FNMerMatSeq & ""
    ...
    Private Sub LoadStylePostInfo()
        ...
        _Str &= vbCrLf & "    WHERE FTStyleDevCode ='" & HI.UL.ULF.rpQuoted(FNHSysStyleDevId.Text) & "' AND FTSeason='" & HI.UL.ULF.rpQuoted(FTSeason.Text) & "'   AND ISNULL(FNVersion,0)=" & FNVersion.Value & " AND FNBomDevType =" & FNBomDevType.SelectedIndex & "  "
    ...
    Private Sub LoadStyleInfo(ByVal _FNHSysStyleDevId As String, Optional _DefaultTab As Boolean = False)
        ...
        Dim _dt As DataTable
        Dim _Str As String = ""
        _Str = "SELECT MS.FNHSysStyleDevId, MS.FTStyleDevCode, MS.FTStyleDevNameTH, MS.FTStyleDevNameEN, MS.FNHSysCustId,  MS.FTUpdUser, CONVERT(VARCHAR(10), CONVERT(DATETIME, MS.FDUpdDate, 120), 103) AS FDUpdDate, MS.FTUpdTime,msc.FTMSCCode "
        _Str &= vbCrLf & ", T8.FTCustCode, T8.FTCustNameEN,MS.FTDevelopDate,MS.FTNote,MS.FTNikeDeveloperName,MS.FNVersion,MS.FTMSCLevel1,MS.FTMSCLevel2,MS.FTMSCLevel3,MS.FTSilhouette,MS.FNBomDevType"
        _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle AS MS WITH(NOLOCK)  "
        ...
        If _dt.Rows.Count > 0 Then
            For Each R As DataRow In _dt.Rows
                ...
                FNVersion.Value = Val(R!FNVersion.ToString)
        ...
        Else
            ...
            _Str = "SELECT TOP 1 MS.FNHSysStyleDevId, MS.FTStyleDevCode, MS.FTStyleDevNameTH, MS.FTStyleDevNameEN, MS.FNHSysCustId,  MS.FTUpdUser, CONVERT(VARCHAR(10), CONVERT(DATETIME, MS.FDUpdDate, 120), 103) AS FDUpdDate, MS.FTUpdTime,msc.FTMSCCode"
            ...
            If _dt.Rows.Count > 0 Then
                ...
                For Each R As DataRow In _dt.Rows
                    ...
                    FNHSysMSCId.Text = R!FTMSCCode.ToString
                    FNVersion.Value = 0 'Val(R!FNVersion.ToString)
    ...
    Private Sub LoadStyleDetail(ByVal FNHSysStyleDevId As String)
        ...
        _Str &= vbCrLf & " FROM         [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[TMERTDevelopStyle] AS T1 WITH(NOLOCK) INNER JOIN"
        _Str &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[TMERTDevelopStyle_Mat] AS T2 WITH(NOLOCK) ON T1.FNHSysStyleDevId = T2.FNHSysStyleDevId LEFT OUTER JOIN"
        _Str &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TFINMCurrency AS T6 WITH(NOLOCK) ON T2.FNHSysCurId = T6.FNHSysCurId "
                ...
                Select Case InitGrid.Columns(Idx).FieldName.ToString.ToLower
                    Case "FNHSysStyleDevId".ToLower, "FNSeq".ToLower, "FNMerMatSeq".ToLower, "FTItemNo".ToLower,
                        "FTMainMatName".ToLower, "FNPart".ToLower, "FTPositionPartName".ToLower, "FNConSmp".ToLower, "FNConSmpPlus".ToLower,
                         "FNColorWaySeq".ToLower, "FTRunColor".ToLower, "FTItemREfNo".ToLower
                    Case Else
...
#End Region
    ...
    Private Sub FTSeason_EditValueChanged(sender As Object, e As EventArgs) Handles FTSeason.EditValueChanged
        ...
        Try
            ...
            Else
                ...
                Dim _Str As String = "SELECT TOP 1 FNHSysStyleDevId FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTDevelopStyle  WITH(NOLOCK) WHERE FTStyleDevCode ='" & HI.UL.ULF.rpQuoted(FNHSysStyleDevId.Text) & "' AND FTSeason='" & HI.UL.ULF.rpQuoted(FTSeason.Text) & "'  AND ISNULL(FNVersion,0)=" & FNVersion.Value & "  AND FNBomDevType =" & FNBomDevType.SelectedIndex & " "
                FNHSysStyleDevId.Properties.Tag = HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_MERCHAN, "")
                Call LoadStyleInfo(FNHSysStyleDevId.Properties.Tag.ToString, True)
    ...
    Private Sub otx_Click(sender As Object, e As EventArgs) Handles otx.Click
    ...
    Private Sub RepositoryItemCalFNPackPerCarton_EditValueChanged(sender As Object, e As EventArgs) Handles RepositoryItemCalFNPackPerCarton.EditValueChanged

    End Sub

    Private Sub RepositoryConsPlusCalcEdit_EditValueChanged(sender As Object, e As EventArgs) Handles RepositoryConsPlusCalcEdit.EditValueChanged
