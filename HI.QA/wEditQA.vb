Public Class wEditQA
    ...
    Private Sub Load_Grid(ByVal _UnitSectId As Integer, ByVal _QADate As String, ByVal _StyleId As Integer, ByVal _OrderNo As String)
        Try
            ...
            _Cmd = "Select '0' AS FTSelect , FNHSysStyleId, FNHSysUnitSectId, FTOrderNo, FDQADate,Left(FNHourNo,2)+':'+Right(FNHourNo,2) AS FNHourNo , FNQAInQty, FNQAAqlQty, FNQAActualQty, Isnull(FNAndon,0) AS FNAndon"
            ...
            _Cmd &= vbCrLf & "  From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "]..TPRODTQA_SubDetail AS D WITH(NOLOCK) LEFT OUTER JOIN "
            ...
            _Cmd &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTQA AS A WITH(NOLOCK)"
            ...
            _Cmd = "  SELECT   '0' AS FTSelect,  A.FNHSysStyleId, A.FNHSysUnitSectId, A.FTOrderNo, A.FDQADate,  Left(A.FNHourNo,2)+':'+Right(A.FNHourNo,2) AS FNHourNo  , A.FNSeq, A.FTPointSubName, A.FNHSysQADetailId ,B.FTQADetailCode   "
            ...
            End If
            _Cmd &= vbCrLf & "FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTQA_SubDetail AS A LEFT OUTER JOIN"
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            For Each R As DataRow In _oDt.Select("FTSelect = '1'")
                ...
                _Cmd = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTQA "
                _Cmd &= vbCrLf & " WHERE FNHSysUnitSectId=" & CInt(Me.FNHSysUnitSectId.Properties.Tag)
                _Cmd &= vbCrLf & " AND FDQADate='" & HI.UL.ULDate.ConvertEnDB(Me.FDDate.Text) & "'"
                _Cmd &= vbCrLf & " AND FNHSysStyleId =" & CInt(Me.FNHSysStyleId.Properties.Tag)
                _Cmd &= vbCrLf & " AND FTOrderNo = '" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "'"
            ...
            For Each L As DataRow In _oDt2.Select("FTSelect = '1'")
                ...
                _Cmd = "Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTQA_SubDetail"
                _Cmd &= vbCrLf & "WHERE FNHSysStyleId=" & CInt(Me.FNHSysStyleId.Properties.Tag)
                _Cmd &= vbCrLf & "AND FDQADate='" & HI.UL.ULDate.ConvertEnDB(Me.FDDate.Text) & "'"
                _Cmd &= vbCrLf & "AND FNHSysUnitSectId=" & CInt(Me.FNHSysUnitSectId.Properties.Tag)
                _Cmd &= vbCrLf & "AND FTOrderNo = '" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "'"
