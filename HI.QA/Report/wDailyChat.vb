...
Public Class wDailyChat
    ...
    Private _wQADailyDetail As wQADailyDetail
    Sub New()
        ...
        _wQADailyDetail = New wQADailyDetail
    ...
    Private Sub LoadChart(ByVal _oDt As DataTable, ByVal _oDtd As DataTable)
        Try
            ...
            For Each R As DataRow In _oDt.Rows
                ...
                _Where = ""
                If _UnitSectCodeBefore <> "" Then _Where = " AND FTUnitSectCode > '" & HI.UL.ULF.rpQuoted(_UnitSectCodeBefore) & "'"
...
#Region "Procedure"
    Private Sub LoadData()
        ...
        Try
            ...
            _Cmd &= vbCrLf & "   , ISNULL    ((SELECT     count(FTStateReject) AS Expr1    "
            _Cmd &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTQA_Detail WITH(NOLOCK)   "
            _Cmd &= vbCrLf & "   WHERE  FTStateReject = '1' and  A.FNHSysStyleId = FNHSysStyleId and A.FNHSysUnitSectId = FNHSysUnitSectId and A.FTOrderNo = FTOrderNo and A.FNHourNo = FNHourNo  and A.FDQADate = FDQADate  ), 0) AS FTTotalDefect "

            _Cmd &= vbCrLf & "   , convert(numeric(18,2) , ISNULL    ((SELECT     count(FTStateReject) AS Expr1    "
            _Cmd &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTQA_Detail WITH(NOLOCK)   "
            _Cmd &= vbCrLf & "   WHERE  FTStateReject = '1' and  A.FNHSysStyleId = FNHSysStyleId and A.FNHSysUnitSectId = FNHSysUnitSectId and A.FTOrderNo = FTOrderNo and A.FNHourNo = FNHourNo   and A.FDQADate = FDQADate ), 0))  / SUM(A.FNQAActualQty) AS FTRejectPer "
            ...
            _Cmd &= vbCrLf & ""
            _Cmd &= vbCrLf & ""
            _Cmd &= vbCrLf & " group by  A.FNHSysUnitSectId , B.FTUnitSectCode , A.FNHSysStyleId , A.FTOrderNo , A.FNHourNo , A.FDQADate  "
    ...
    Private Sub ogvMain_RowCellClick(sender As Object, e As DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs) Handles ogvMain.RowCellClick
        Try
            If e.Column.Name.ToString = "cFTTotalDefect" Then
                ...
                Dim _wPopupDefect As wPopupDefect
                _wPopupDefect = New wPopupDefect
                Dim oSysLang As New ST.SysLanguage
                Try
