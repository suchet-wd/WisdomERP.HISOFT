...
Public Class wFabricflowsTrackig
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        Dim pSuplCode As String = FTSupl.Text
        ...
        Select Case FNFabricFlowsListType.SelectedIndex
            Case 0
                Me.ogdpo.DataSource = _dt.Copy
            Case 1
                Me.ogdwpo.DataSource = _dt.Copy
            Case 2
                Me.ogdpopayment.DataSource = _dt.Copy
        End Select
...
#End Region
    ...
    Private Sub FNFabricFlowsListType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles FNFabricFlowsListType.SelectedIndexChanged
        ...
        Try
            ...
            Me.otpnew.PageVisible = (FNFabricFlowsListType.SelectedIndex = 0)
            Me.otpworking.PageVisible = (FNFabricFlowsListType.SelectedIndex = 1)
            Me.otppayment.PageVisible = (FNFabricFlowsListType.SelectedIndex = 2)
        Catch ex As Exception
            Me.otpnew.PageVisible = True
    ...
    Private Sub RepositoryItemPopupOrder_QueryPopUp(sender As Object, e As CancelEventArgs) Handles RepositoryItemPopupSupplier.QueryPopUp
        Try
            Me.ogdlistsupplier.DataSource = Nothing
            ockselectorderall.Checked = False

            Dim cmdstring As String = ""

            cmdstring = " EXEC   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.USP_GETFABRICFLOWS_SUPPLIER '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"

            Dim dt As DataTable = HI.Conn.SQLConn.GetDataTable(cmdstring, Conn.DB.DataBaseName.DB_MERCHAN)

            ogvlistsupplier.ClearColumnsFilter()
            ogvlistsupplier.ActiveFilter.Clear()
            ogvlistsupplier.OptionsView.ShowAutoFilterRow = True

            Dim pSuplCode As String = FTSupl.Text

            If pSuplCode <> "" Then

                For Each scode As String In pSuplCode.Split(",")

                    For Each R As DataRow In dt.Select("FTSupplierCode='" & HI.UL.ULF.rpQuoted(scode) & "'")
                        R!FTSelect = "1"
                    Next
                Next
            End If
            ...
            Me.ogdlistsupplier.DataSource = dt.Copy
    ...
    Private Sub RepositoryItemPopupOrder_QueryCloseUp(sender As Object, e As CancelEventArgs) Handles RepositoryItemPopupSupplier.QueryCloseUp


        Try
            Dim pSuplCode As String = ""
            Dim RIdx As Integer = 0
            With CType(Me.ogdlistsupplier.DataSource, DataTable)
                .AcceptChanges()
                For Each R As DataRow In .Select("FTSelect='1'")

                    If RIdx = 0 Then
                        pSuplCode = R!FTSupplierCode.ToString
                    Else
                        pSuplCode = pSuplCode & "," & R!FTSupplierCode.ToString
                    End If
                    RIdx = RIdx + 1
                Next

            End With
            FTSupl.Text = pSuplCode
            RepositoryItemPopupSupplier.EditValue = pSuplCode
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub ockselectorderall_CheckedChanged(sender As Object, e As EventArgs) Handles ockselectorderall.CheckedChanged
        Try
            ...
            If Not Me.ogdlistsupplier.DataSource Is Nothing Then

                If ogvlistsupplier.RowCount > 0 Then
    ...
    Private Sub RepositoryItemPopupSupplier_QueryResultValue(sender As Object, e As QueryResultValueEventArgs) Handles RepositoryItemPopupSupplier.QueryResultValue
        Try
            Dim pSuplCode As String = ""
            Dim RIdx As Integer = 0
    ...
    Private Sub RepositoryItemCheckEdit3_EditValueChanging(sender As Object, e As ChangingEventArgs) Handles RepositoryItemSelectsupl.EditValueChanging

        Try
            Dim pSate As String = "0"
            If e.NewValue.ToString = "1" Then
                pSate = "1"
            End If

            ogvlistsupplier.SetFocusedRowCellValue("FTSelect", pSate)
        Catch ex As Exception
    ...
    Private Sub ocmdownloadpopdf_Click(sender As Object, e As EventArgs) Handles ocmdownloadpopdf.Click
        Try
                ...
                If Op.FileName <> "" Then
                    ...
                    If dtpo IsNot Nothing Then
                        ...
                        For Each pPoNo As String In grp
                            ...
                            If RIndx = 1 Then
                                cmdstring = " select top 1 FPFile from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.[TPURTPurchase_PDF]  WITH(NOLOCK)   WHERE FTPurchaseNo='" + HI.UL.ULF.rpQuoted(pPoNo) + "' AND FTStateFIle='1' "
                            Else
                                cmdstring += Constants.vbCrLf + " UNION "
                                cmdstring += Constants.vbCrLf + " select top 1 FPFile from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.[TPURTPurchase_PDF]  WITH(NOLOCK)   WHERE FTPurchaseNo='" + HI.UL.ULF.rpQuoted(pPoNo) + "' AND FTStateFIle='1' "
                            End If
