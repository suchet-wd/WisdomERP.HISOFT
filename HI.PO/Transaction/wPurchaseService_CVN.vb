...
Public Class wPurchaseService_CVN
    ...
    Private _AddItem As wPurchaseServiceAddItem
    ...
    Sub New()
        ...
        _AddItem = New wPurchaseServiceAddItem
...
#Region "Procedure"
    ...
    Private Sub InitFormControl()
        ...
        _Str &= vbCrLf & " WHERE FTDynamicFormName=N'" & HI.UL.ULF.rpQuoted("wPurchaseService") & "' "
    ...
    Private Function SaveData() As Boolean
        ...
        If (_StateNew) Then
            ...
            If (_PORunDocVat) Then
                ...
                _Key = HI.TL.Document.GetDocumentNo(Me.SysDBName, Me.SysTableName, "", False, "POS-").ToString
...
#End Region
    ...
    Private Sub ocmadd_Click(sender As Object, e As EventArgs) Handles ocmAddDT.Click
        ...
        With _AddItem
            ...
            If (.AddMat) Then
                ...
                _Str = "SELECT TOP 1 FNSeq +1  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService_Detail WITH(NOLOCK) "
                _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                _Str &= vbCrLf & " ORDER BY FNSeq DESC  "

                _FNSeq = Integer.Parse(Val(HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_PUR, "1")))

                _Str = "Insert into  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService_Detail(FTInsUser, FDInsDate, FTInsTime"
                _Str &= vbCrLf & " , FTPurchaseNo, FNSeq, FTOrderNo, FTDescription, FNQuantity, FNPrice, FNAmount, FTNote)"
                ...
                _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(.FTDescription.Text.Trim) & "' "
                ...
                Try
                    ...
                    _Str = " UPDATE  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService "
                    ...
                    _Str &= vbCrLf & "    FROM"
                    _Str &= vbCrLf & " ("
                    _Str &= vbCrLf & " SELECT        FTPurchaseNo,FTDescription,  FNPrice, SUM(FNQuantity) AS  FNQuantity"
                    _Str &= vbCrLf & " FROM            [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService_Detail AS A  WITH(NOLOCK)"
                    _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                    _Str &= vbCrLf & " GROUP BY  FTPurchaseNo,FTDescription,  FNPrice "
                    _Str &= vbCrLf & " ) AS A"
    ...
    Private Sub ocmremove_Click(sender As Object, e As EventArgs) Handles ocmRemoveDT.Click
        ...
        With ogvdetail
            ...
            Dim _Str As String = "Delete From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService_Detail  WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' AND FNSeq=" & Val(_FNSeq) & " "
        ...
        Try
            With ogvdetail
                ...
                With _AddItem
                    ...
                    .FTDescription.Text = "" & ogvdetail.GetFocusedRowCellValue("FTDescription").ToString
                    ...
                    If (.AddMat) Then
                        ...
                        _Str = "Update [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService_Detail SET  FTUpdUser='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                        ...
                        _Str &= vbCrLf & " ,FTDescription=N'" & HI.UL.ULF.rpQuoted(.FTDescription.Text.Trim) & "' "
                        _Str &= vbCrLf & " WHERE FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' "
                        _Str &= vbCrLf & " AND FNSeq=" & Val(_FNSeq) & " "
