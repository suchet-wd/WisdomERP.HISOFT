...
Public Class wAutoGeneratePOFactory
...
#Region "Query"
    ...
    Private Function QueryJOb() As String
        ...
        _Str &= vbCrLf & " ,ISNULL(Cmp.FTCmpCode,'') AS FTCmpCode "
        _Str &= vbCrLf & "    FROM "
        _Str &= vbCrLf & "  (SELECT        S.FTOrderNo"
        ...
        _Str &= vbCrLf & "  INNER Join [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS B ON S.FTOrderNo=B.FTOrderNo "
        _Str &= vbCrLf & " INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmpRun AS CR WITH(NOLOCK)   ON  B.FNHSysCmpRunId = CR.FNHSysCmpRunId  "
        _Str &= vbCrLf & "  WHERE        (FTPurchaseNo = N'')   AND ISNULL(FNHSysUnitId,0) <> 0  AND FNTotalPurchaseQuantity>0  "
        _Str &= vbCrLf & " AND  (B.FNHSysCmpId =" & HI.ST.SysInfo.CmpID & ")"
        _Str &= vbCrLf & " AND  (CR.FTStateHire ='1')"
        _Str &= vbCrLf & " AND  ISNULL(B.FNHSysStyleIdPull,0) <=0 "

        If Not (HI.ST.SysInfo.Admin) Then
            _Str &= vbCrLf & "   AND S.FTInsUser   IN (SELECT FTUserName FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.FT_GetOrderHirePermission_PurchseFacTeam('" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "') ) "
        End If

        _Str &= vbCrLf & "  GROUP BY S.FTOrderNo"
    ...
    Private Function QuerySupplier() As String
        ...
        _Str &= vbCrLf & "  INSERT INTO #TAB(FTOrderNo)	 "
        _Str &= vbCrLf & " Select FTOrderNo FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS B  With(NOLOCK)"
        _Str &= vbCrLf & " INNER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmpRun AS CR WITH(NOLOCK)   ON  B.FNHSysCmpRunId = CR.FNHSysCmpRunId  "
        _Str &= vbCrLf & "  WHERE (B.FNHSysCmpId =" & HI.ST.SysInfo.CmpID & ")"
        _Str &= vbCrLf & " AND  (CR.FTStateHire ='1')"
        _Str &= vbCrLf & " AND  ISNULL(B.FNHSysStyleIdPull,0) <=0 "
    ...
    Private Function QueryPOItem()
        ...
        If Not (HI.ST.SysInfo.Admin) Then
            _Str &= vbCrLf & "   AND SC.FTInsUser   IN (SELECT FTUserName FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.FT_GetOrderHirePermission_PurchseFacTeam('" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "') ) "
        End If
...
#Region "Procedure"
            ...
            Case 0 'Generate By Supplier All Item Code / PO
                ...
                If _PoDocNo <> "" Then
                    _Complete = GeneratePO(SuplKey, _PoDocNo, POTypeKey, POGrpText, POGrpKey, DeliDateKey,
                                                                      ...
                                                                      Vat, SurCharge, Credit, CreditDay, PaymentTerm, ContactPerson, _MaxItem, _Filter, SuplCode, SuplName, cmprunpo)
    ...
    Private Function GeneratePO(SuplKey As String, PoNoKey As String, POTypeKey As Integer, POGrpText As String, POGrpKey As Integer, DeliDateKey As String,
                                                              ...
                                                              _MaxItem As Integer, Filter As String, SuplCode As String, SuplName As String, cmprunpo As String) As Boolean
