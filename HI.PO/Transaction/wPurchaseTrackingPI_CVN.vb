...
Public Class wPurchaseTrackingPI_CVN
    ...
    Private wAddPI As wPurchaseTrackingPIAddPI_CVN
    ...
    Sub New()
        ...
        wAddPI = New wPurchaseTrackingPIAddPI_CVN
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        _Qry = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.USP_GETDATAPO_TRACKING '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'," & FNListDocumentTrackPIData.SelectedIndex.ToString() & ",'" & HI.UL.ULDate.ConvertEnDB(FTStartPurchaseDate.Text) & "','" & HI.UL.ULDate.ConvertEnDB(FTEndPurchaseDate.Text) & "','" & HI.UL.ULDate.ConvertEnDB(FTStartDelivery.Text) & "','" & HI.UL.ULDate.ConvertEnDB(FTEndDelivery.Text) & "','',''," & Val(FNHSysSuplId.Properties.Tag.ToString) & ""
...
#End Region
            ...
            If .AddMat Then
                ...
                For Each R As DataRow In dtpoList.Rows
                    ...
                    For Each Rxm As DataRow In dtsurcharge.Select("FTDescription<>''", "FNSeq")
                        ...
                        cmdstring &= vbCrLf & ", FTPINo, FNSeq, FTDescription, FNSurchargeAmt, FTRemark,FNHSysSuplId,FNHSysAccountId)  "
                        cmdstring &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                        cmdstring &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB & " "
                        cmdstring &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB & " "
                        cmdstring &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(pPINo) & "' "
                        ...
                        cmdstring &= vbCrLf & " ," & SuplID & " "
                        cmdstring &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(Rxm!FNHSysAccountId.ToString) & "' "
                    Next
                ...
                _Spls.Close()
                ' Call LoadData()
                Call AutoDataToJobCostOverHead(pPIDate)
            End If
        ...
        With wAddPaid
            ...
            If .AddMat Then
                ...
                For Each R As DataRow In dtpoList.Rows
                    ...
                    cmd &= vbCrLf & ",FTPaidDate='" & HI.UL.ULDate.ConvertEnDB(pContactDate) & "' "
                    cmd &= vbCrLf & ",FTPaidNote='" & HI.UL.ULF.rpQuoted(pNote) & "' "
                    cmd &= vbCrLf & "  WHERE FTPurchaseNo ='" & HI.UL.ULF.rpQuoted(R!FTPurchaseNo.ToString) & "'  AND ISNULL(FTStatePaid,'') <>'1' "
    ...
    'Private Sub RepCheckEdit_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles RepCheckEdit.EditValueChanging
    '    Try
    ...
    '        Dim _FTPIDate As String = ""

    '        With Me.ogvtime
    '            _FTPIDate = "" & .GetFocusedRowCellValue("FTPIDate").ToString
    ...
    Private Sub ocmeditpi_Click(sender As Object, e As EventArgs) Handles ocmeditpi.Click
        ...
        With wAddPI
            ...
            Try
                For Each R As DataRow In dtpoList.Rows
                    .AddPo(R!FTPurchaseNo.ToString, R!FNPOQuantity, R!FNPOGrandAmt, R!FNPOBalQuantity, R!FNPOBalGrandAmt, R!FNPIPOQuantity, R!FNPIPONetAmt, R!FTUnitCode.ToString)
                    '' .AddPo(R!FTPurchaseNo.ToString, R!FNPOQuantity, R!FNPOGrandAmt, R!FNPOBalQuantity, R!FNPOBalGrandAmt, R!FNPOBalQuantity, R!FNPOBalGrandAmt, R!FTUnitCode.ToString)
                Next
            Catch ex As Exception
    ...
    Sub AutoDataToJobCostOverHead(pidate As String)

        ''Check OverHead Has been Calculated
        Dim cmdstring As String = ""
        cmdstring = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.SP_PI_To_OverHead '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULDate.ConvertEnDB(pidate) & "'"
        HI.Conn.SQLConn.ExecuteNonQuery(cmdstring, Conn.DB.DataBaseName.DB_PUR)


        '' Call CheckApproveJobCostOverHead(pidate)
    End Sub


    Sub CheckApproveJobCostOverHead(pidate As String)

        ''Check OverHead Has been Calculated
        Dim cmdstring As String = ""
        Dim CheckApproved As Integer = 0

        cmdstring = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.SP_Check_Approve_OverHead '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULDate.ConvertEnDB(pidate) & "'"
        ''CheckApproved = HI.Conn.SQLConn.GetField(cmdstring, Conn.DB.DataBaseName.DB_PUR, 0)

        If HI.Conn.SQLConn.GetField(cmdstring, Conn.DB.DataBaseName.DB_PUR, 0) > 0 Then
            MG.ShowMsg.mProcessError(202210261712, "พบการคำนวนต้นทุนอื่นๆแล้ว  กรุณาคำนวนต้นทุนอื่นๆใหม่", Me.Text, MessageBoxIcon.Information)
    ...
    Private Function CheckApproveJobCostOverHead_Chk(pidate As String) As String
    ...
    Private Sub RepCheckEdit_EditValueChanged(sender As Object, e As EventArgs) Handles RepCheckEdit.EditValueChanged
        Select Case ogvtime.FocusedColumn.FieldName.ToString

            Case "FTSelect"

                Dim _PIDate As String = ogvtime.GetRowCellValue(ogvtime.FocusedRowHandle, "FTPIDate").ToString()
                Dim _chk As String
                If _PIDate <> "" Then

                    _chk = CheckApproveJobCostOverHead_Chk(_PIDate)

                    ''ogvtime.SetRowCellValue(ogvtime.FocusedRowHandle, ogvtime.FocusedColumn, "FTSelect", _chk)
                    ogvtime.SetRowCellValue(ogvtime.FocusedRowHandle, ogvtime.FocusedColumn.FieldName.ToString, _chk)
