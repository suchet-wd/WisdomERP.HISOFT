...
Public Class wReceiveService
    ...
    Private _AddItemPopup As wReceiveServiceItem
    ...
    Sub New()
        ...
        _AddItemPopup = New wReceiveServiceItem
...
#Region "Procedure"
    ...
    Private Sub LoadRcvDetail(PoKey As String)
        ...
        _Str &= vbCrLf & " FROM             [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail AS D WITH (NOLOCK)  "
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService WHERE FTReceiveNo='" & HI.UL.ULF.rpQuoted(Me.FTReceiveNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail WHERE FTReceiveNo='" & HI.UL.ULF.rpQuoted(Me.FTReceiveNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService WHERE FTReceiveNo='" & HI.UL.ULF.rpQuoted(Me.FTReceiveNo.Text) & "'")
            Return True
        Catch ex As Exception
            HI.Conn.SQLConn.Tran.Rollback()
            HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTReceiveNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .Formular = "{TPURTReceiveService.FTReceiveNo}='" & HI.UL.ULF.rpQuoted(FTReceiveNo.Text) & "' "
                .Preview()
            End With
...
#Region " Proc "
    ...
    Private Sub LoadPOInfo(PoKey As String, Optional LoadRcv As Boolean = False)
        ...
        _Str &= vbCrLf & "  FROM             [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService AS H WITH (NOLOCK) INNER JOIN"
...
#End Region
    ...
    Private Sub FNHSysCurId_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles FNHSysCurId.EditValueChanged
        ...
        Else
            ...
            If Not (_ProcLoad) Then
                ...
                _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService AS PH WITH(NOLOCK) "
            ...
            If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_MASTER, "") = "1" Then
                ...
                _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TPURTReceiveService_Detail WITH(NOLOCK)"
    ...
    Private Sub ocmadd_Click(sender As System.Object, e As System.EventArgs) Handles ocmadd.Click
        ...
        _Str &= vbCrLf & ",P.FNSeq,P.FTDescription ,P.FTNote"
        _Str &= vbCrLf & ",P.FTOrderNo "
        ...
        _Str &= vbCrLf & "  FROM"
        _Str &= vbCrLf & "(SELECT        FTPurchaseNo, FNSeq, FTOrderNo, FNPrice, FTDescription,FTNote"
        _Str &= vbCrLf & ", SUM(FNQuantity) AS FNQuantity "
        _Str &= vbCrLf & " From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchaseService_Detail As P With (NOLOCK) "
        _Str &= vbCrLf & " Where (FTPurchaseNo = N'" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Properties.Tag.ToString) & "') "
        _Str &= vbCrLf & "Group By FTPurchaseNo, FNSeq, FTOrderNo, FNPrice, FTDescription,FTNote) As P  "
        _Str &= vbCrLf & "Left OUTER JOIN (Select        RH.FTPurchaseNo, RD.FNSeqRef, RD.FTOrderNo "
        ...
        _Str &= vbCrLf & "From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService As RH With (NOLOCK) INNER Join "
        _Str &= vbCrLf & "     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail AS RD WITH (NOLOCK) ON RH.FTReceiveNo = RD.FTReceiveNo "
        _Str &= vbCrLf & "Where (RH.FTPurchaseNo = N'" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Properties.Tag.ToString) & "') "
        _Str &= vbCrLf & "Group By RH.FTPurchaseNo, RD.FNSeqRef, RD.FTOrderNo ) As RCV "
            ...
            If .ProcessProc Then
                ...
                If _dt.Select(" FTStateSelect='1' AND FNRcvQty > 0 ").Length > 0 Then
                    ...
                    Try
                        ...
                        _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail "
                        ...
                        For Each R As DataRow In _dt.Select(" FTStateSelect='1'  AND FNRcvQty > 0")
                            ...
                            _Str = " INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail ( FTInsUser, FDInsDate, FTInsTime, FTReceiveNo, FNSeqRef, FTOrderNo, FTDescription, FNQuantity, FNPrice, FNAmount, FTNote) "
    ...
    Private Sub ocmremove_Click(sender As System.Object, e As System.EventArgs) Handles ocmremove.Click
        With ogvdetail
            ...
            Try
                ...
                _Str = " Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail   WHERE FTReceiveNo='" & HI.UL.ULF.rpQuoted(Me.FTReceiveNo.Text) & "' AND  FNSeqRef=" & Val(_MatID) & " "
                If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    HI.Conn.SQLConn.Tran.Rollback()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
                ...
                HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " Delete From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTReceiveService_Detail   WHERE FTReceiveNo='" & HI.UL.ULF.rpQuoted(Me.FTReceiveNo.Text) & "' AND  FNSeqRef=" & Val(_MatID) & " ")
