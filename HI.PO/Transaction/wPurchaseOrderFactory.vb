...
Public Class wPurchaseOrderFactory
    ...
    Private _AddItemPopup As wAddItemPOFactory
    ...
    Sub New()
        ...
        _AddItemPopup = New wAddItemPOFactory
...
#Region "Procedure"
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase WHERE FTFacPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTFacPurchaseNo.Text) & "'")
            Return True
...
#Region "MAIN PROC"
    ...
    Private Function CheckDocumentAuto() As Boolean
        ...
        cmdstring = "select top 1 FTPurchaseNoRef from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase AS X WITH(NOLOCK) where FTFacPurchaseNo='" & HI.UL.ULF.rpQuoted(FTFacPurchaseNo.Text) & "'"

        If HI.Conn.SQLConn.GetField(cmdstring, Conn.DB.DataBaseName.DB_PUR, "") <> "" Then
            HI.MG.ShowMsg.mInfo("เอกสารมาจาการ Auto ไม่สารมารถทำการลบหรือแก้ไขได้ !!!", 1902100457, Me.Text,, MessageBoxIcon.Warning)
            Return True
        Else
            Return False
        End If
...
#End Region
    ...
    Private Sub ocmadd_Click(sender As System.Object, e As System.EventArgs) Handles ocmadd.Click
        ...
        If (CheckReservePurchase(Me.FTFacPurchaseNo.Text, "", 0, False) = False) Then Exit Sub

        If FTFacPurchaseNo.Text = HI.TL.Document.GetDocumentNo(Me.SysDBName, Me.SysTableName, Me.SysDocType, True, _CmpH) Then
            If Me.VerrifyData() Then
                If Me.SaveData Then
                Else
                    Exit Sub
            ...
            End If
        Else
            If Me.FTFacPurchaseNo.Text = "" Then Exit Sub
            If (CheckReceive(Me.FTFacPurchaseNo.Text) = False) Then Exit Sub
        ...
        With _AddItemPopup
            ...
            If (.AddMat) Then
                ...
                _Str = "SELECT TOP 1 FTFacPurchaseNo  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo WITH(NOLOCK) "
                _Str &= vbCrLf & " WHERE FTFacPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTFacPurchaseNo.Text) & "' "
                ...
                If HI.Conn.SQLConn.GetField(_Str, _DBEnum, "") = "" Then
                    ...
                    _Str = "Insert into  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo(FTInsUser, FDInsDate, FTInsTime"
                    _Str &= vbCrLf & " , FTFacPurchaseNo,FTOrderNo, FNHSysRawMatId, FNHSysUnitId, FNPrice, FNDisPer, "
                    _Str &= vbCrLf & "    FNDisAmt, FNQuantity, FNNetAmt, FTRemark ,FTFabricFrontSize,FTRawMatColorNameTH,FTRawMatColorNameEN,FNSurchangeAmt,FTPurchaseNo)"
                    _Str &= vbCrLf & "  SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                    _Str &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB & ""
                    _Str &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB & ""
                    _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTFacPurchaseNo.Text) & "' "
                ...
                Else
                    ...
                    _Str = "Update [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo SET  FTUpdUser='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                    ...
                    _Str &= vbCrLf & " ,FTPurchaseNo='" & HI.UL.ULF.rpQuoted(.FTPurchaseNo.Text) & "' "
                    _Str &= vbCrLf & " WHERE FTFacPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTFacPurchaseNo.Text) & "' "
                    _Str &= vbCrLf & " AND FTOrderNo='" & HI.UL.ULF.rpQuoted(.FTOrderNo.Text) & "' "
                    _Str &= vbCrLf & " AND FNHSysRawMatId=" & Val(.FNHSysRawMatId.Properties.Tag.ToString) & " "
                ...
                Try
                    ...
                    Me.LoadPoDetail(Me.FTFacPurchaseNo.Text)
    ...
    Private Sub ocmremove_Click(sender As System.Object, e As System.EventArgs) Handles ocmremove.Click
        ...
        With ogvdetail
            ...
            If (CheckReceive(Me.FTFacPurchaseNo.Text, _MatID) = False) Then Exit Sub
            If (CheckReceive(Me.FTFacPurchaseNo.Text, , False) = False) Then
                _CheckRcv = True
            End If

            If (CheckReservePurchase(Me.FTFacPurchaseNo.Text, _JobNo, _MatID) = False) Then Exit Sub
            ...
            Dim _Str As String = "Delete From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo  WHERE FTFacPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTFacPurchaseNo.Text) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_JobNo) & "' AND FNHSysRawMatId=" & Val(_MatID) & " "
    ...
    Private Function CheckReceive(POKey As String, Optional SysMatId As Integer = 0, Optional ShowMsg As Boolean = True) As Boolean
        ...
        If SysMatId = 0 Then
            _Str = "Select TOP 1 FTFacPurchaseNo FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive As R WITH(NOLOCK) WHERE FTFacPurchaseNo='" & HI.UL.ULF.rpQuoted(POKey) & "'  "
    ...
    Private Sub ogvdetail_DoubleClick(sender As Object, e As System.EventArgs) Handles ogvdetail.DoubleClick
        With ogvdetail
            ...
            If (CheckReceive(Me.FTFacPurchaseNo.Text, SysMatId) = False) Then Exit Sub
            If (CheckReceive(Me.FTFacPurchaseNo.Text, , False) = False) Then
                _CheckRcv = True
            End If

            If (CheckReservePurchase(Me.FTFacPurchaseNo.Text, "" & ogvdetail.GetFocusedRowCellValue("FTOrderNo").ToString, SysMatId) = False) Then Exit Sub
            If CheckOwner() = False Then Exit Sub
            If CheckDocumentAuto() Then Exit Sub
            Dim _Qry As String = ""
            ...
            With _AddItemPopup
                ...
                .FTPurchaseNo.Text = "" & ogvdetail.GetFocusedRowCellValue("FTPurchaseNo").ToString
                ...
                If (.AddMat) Then
                    ...
                    If HI.Conn.SQLConn.GetField(_Str, _DBEnum, "") = "" Then
                        ...
                        _Str = "Insert into  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase_OrderNo(FTInsUser, FDInsDate, FTInsTime"
                        _Str &= vbCrLf & " , FTFacPurchaseNo,FTOrderNo, FNHSysRawMatId, FNHSysUnitId, FNPrice, FNDisPer, "
                        _Str &= vbCrLf & "    FNDisAmt, FNQuantity, FNNetAmt, FTRemark ,FTFabricFrontSize,FTRawMatColorNameTH,FTRawMatColorNameEN,FNSurchangeAmt)"
                        _Str &= vbCrLf & "  SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
                        _Str &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB & ""
                        _Str &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB & ""
                        _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTFacPurchaseNo.Text) & "' "
    ...
    Private Sub ocmsendpoapprove_Click(sender As Object, e As EventArgs) Handles ocmsendpoapprove.Click
        ...
        If Me.FTFacPurchaseNo.Text <> "" And Me.FTFacPurchaseNo.Properties.Tag.ToString <> "" Then
            ...
            If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_PUR, "") <> "1" Then
                ...
                _Qry = " UPDATE  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTFacPurchase "
