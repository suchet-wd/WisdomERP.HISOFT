...
#Region "Procedure"
    Private Sub LoadDataInfo()
        ...
        If Me.FNHSysEmpTypeId.Text <> "" Then
            _Qry = "   Select  TOp 1  FDCalDateEnd  From THRMCfgPayDT WITH(NOLOCK) where FNMonth = " & (FNMonth.SelectedIndex + 1) & "  and FTPayYear = '" & HI.UL.ULF.rpQuoted(Me.FTPayYear.Text) & "' and FNHSysEmpTypeId = " & Integer.Parse(Me.FNHSysEmpTypeId.Properties.Tag.ToString) & " )  Order by FDCalDateEnd desc "
            _EndDate = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "")
        Else
            _Qry = "   Select  TOp 1  FDCalDateEnd  From THRMCfgPayDT WITH(NOLOCK) where FNMonth = " & (FNMonth.SelectedIndex + 1) & "  and FTPayYear = '" & HI.UL.ULF.rpQuoted(Me.FTPayYear.Text) & "'   )  Order by FDCalDateEnd desc "
            _EndDate = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "")
        End If


        _Qry = " SELECT   ROW_NUMBER ( ) over(  ORDER  BY M.FTEmpCodeRefer  asc ) AS FNRowNo , M.FTEmpCode , "
        ...
        _Qry &= vbCrLf & "    , max(M.FDBirthDate )as FDBirthDate   , max(M.FDDateStart  ) as FDDateStart    ,(( sum(R.FNNetpay) - ( sum(VP.[044]) +  sum(VP.[046]))) * Isnull( R.FNTaxExchangeRate,1) ) AS FNSalaryReal "

        _Qry &= vbCrLf & "  , sum(R.FNNetpay) - ( sum(VP.[044]) +  sum(VP.[046])) as FNNetpay  , sum(R.FNTotalRecalSSO)  as FNTotalRecalSSO  ,max( S.FTUnitSectNameTH ) AS FTUnitSectName   ,max(X.FTNameTH)  As  FNEmpSex "
        ...
        _Qry &= vbCrLf & "  , sum(R.FNNetIncome )    as FNTotalIncome " '+ sum( VP.[046])
        _Qry &= vbCrLf & "  , convert(numeric(18,0) , sum(R.FNTotalIncome ) * R.FNTaxExchangeRate )  as FNTotalIncomeReil  "

        _Qry &= vbCrLf & "   , sum(R.FNTotalRecalTAX) as FNTotalRecalTAXUS "
        _Qry &= vbCrLf & "   , (sum(R.FNTotalRecalTAX) *  R.FNTaxExchangeRate  )  as  FNTotalRecalTAX "
        ...
        _Qry &= vbCrLf & "   ,(Select top 1  FNAllowance   From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMConfigTaxRate With(NOLOCK) WHERE   FCAmtBegin   <= convert(numeric(18,0) , sum(R.FNTotalIncome ) * R.FNTaxExchangeRate )     And   FCAmtEnd >=   convert(numeric(18,0) , sum(R.FNTotalIncome ) * R.FNTaxExchangeRate )  ) As FNAllowance  "
        _Qry &= vbCrLf & "  , sum(R.FNSocial) As FNSocial  ,sum(R.FNTransportAmt) As FNTransportAmt  , sum(R.FNChildCareAmt) As FNChildCareAmt ,sum(R.FNHealtCareAmt) As FNHealtCareAmt ,sum(R.FNOTMealAmtUS) As FNOTMealAmtUS ,R.FNTaxExchangeRate "
        ...
        If HI.ST.Lang.Language = HI.ST.SysInfo.LanguageLocal Then
            _Qry &= vbCrLf & ", max(S.FTUnitSectNameTH)  AS FTUnitSectName "
            _Qry &= vbCrLf & ", max(X.FTNameTH) As  FNEmpSex"
            _Qry &= vbCrLf & ", max(N.FTNationalityNameTH)  as FTNationalityName"
            _Qry &= vbCrLf & ", max(P.FTPositNameTH) AS FTPositName"
            _Qry &= vbCrLf & ", max(L.FTNameTH)  As FNEmpStatus  "

            _Qry &= vbCrLf & ", MAX(LGroup.FTNameTH) AS FTEmpTypeGroup "
            _Qry &= vbCrLf & ", MAX(ET.FTEmpTypeCode) AS FTEmpTypeCode "
        Else
            _Qry &= vbCrLf & ", max( S.FTUnitSectNameEN )  AS FTUnitSectName "
            _Qry &= vbCrLf & ", max(X.FTNameEN) As  FNEmpSex"
            _Qry &= vbCrLf & ", max(N.FTNationalityNameEN)  as FTNationalityName"
            _Qry &= vbCrLf & ", max(P.FTPositNameEN) AS FTPositName"
            _Qry &= vbCrLf & ", max(L.FTNameEN)  As FNEmpStatus  "
            _Qry &= vbCrLf & ", MAX(LGroup.FTNameEN) AS FTEmpTypeGroup "
            _Qry &= vbCrLf & ", MAX(ET.FTEmpTypeCode) AS FTEmpTypeCode "
        ...
        _Qry &= vbCrLf & "  OUTER APPLY (SELECT TOP 1 * FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SYSTEM) & "].[dbo].[HSysListData] WHERE FTListName='FNEmpTypeGroup' AND ET.FNEmpTypeGroup=FNListIndex ) LGroup  "

        _Qry &= vbCrLf & "  OUTER APPLY ( select FNHSysEmpID , count(FNChildAge) as FNChildAge    from ( select FNHSysEmpID  ,  dbo.FN_Get_Emp_WorkAge(  FDChildBirthDate  ,  '" & _EndDate & "')   as FNChildAge     "
        _Qry &= vbCrLf & "  from  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployeeChild with(NOlock)     "
        _Qry &= vbCrLf & "   where isnull( FTStateNotDisTax ,'0')  = '0'  and isdate(FDChildBirthDate) = 1 ) AS T  "
        _Qry &= vbCrLf & "  where  FNChildAge  >=  " & HI.HRCAL.HCfg._DiscountTax.Cfg_ModChildAllowanceAgeStart * 12
        _Qry &= vbCrLf & "  and FNChildAge <=  " & HI.HRCAL.HCfg._DiscountTax.Cfg_ModChildAllowanceAgeEnd * 12
        _Qry &= vbCrLf & " And   T.FNHSysEmpID = M.FNHSysEmpID  "
        _Qry &= vbCrLf & " group by FNHSysEmpID "
        _Qry &= vbCrLf & "   )   ZX "


        '_Qry &= vbCrLf & "  LEFT OUTER JOIN  ( select FNHSysEmpID , count(FNChildAge) as FNChildAge    from ( select FNHSysEmpID  ,  dbo.FN_Get_Emp_WorkAge(  FDChildBirthDate  ,  '" & _EndDate & "')   as FNChildAge     "
        '_Qry &= vbCrLf & "  from  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployeeChild with(NOlock)     "
        '_Qry &= vbCrLf & "   where isnull( FTStateNotDisTax ,'0')  = '0'  and isdate(FDChildBirthDate) = 1 ) AS T  "
        '_Qry &= vbCrLf & "  where  FNChildAge  >=  " & HI.HRCAL.HCfg._DiscountTax.Cfg_ModChildAllowanceAgeStart * 12
        '_Qry &= vbCrLf & "  and FNChildAge <=  " & HI.HRCAL.HCfg._DiscountTax.Cfg_ModChildAllowanceAgeEnd * 12
        '_Qry &= vbCrLf & " group by FNHSysEmpID "
        '_Qry &= vbCrLf & "   )  AS ZX ON  ZX.FNHSysEmpID = R.FNHSysEmpID"


        _Qry &= vbCrLf & "  LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_HR) & "].dbo.V_PayrollFin AS VP ON M.FNHSysEmpID=VP.FNHSysEmpID and R.FTPayYear = VP.FTPayYear and R.FTPayTerm  = VP.FTPayTerm "
        ...
        _Qry &= vbCrLf & "   group by M.FTEmpCodeRefer , M.FTSocialNo  , M.FTEmpIdNo   ,  M.FTEmpCode , R.FNTaxExchangeRate     "
        _Qry &= vbCrLf & "ORDER BY  ROW_NUMBER ( ) over(  ORDER  BY M.FTEmpCodeRefer  asc )   asc "
