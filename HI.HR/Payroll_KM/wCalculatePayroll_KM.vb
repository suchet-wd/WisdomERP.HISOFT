Public Class wCalculatePayroll_KM
...
#Region "General"
    ...
    Private Function GenQuery(SDate As String, EDate As String, _FNHSysEmpTypeId As Integer, Optional StateDel As Boolean = False) As String
        ...
        'CASE WHEN ISNULL(M.FDDateTransfer,'') <> '' AND ISNULL(M.FDDateTransfer,'') > M.FDDateStart THEN ISNULL(M.FDDateTransfer,'') ELSE M.FDDateStart  END
                ...
                If .Select("FTSelect='1'").Length > 0 Then
                    For Each RT As DataRow In .Select("FTSelect='1'")
                        ...
                        For Each dr In dtGroup_EmpType.Rows
                            ...
                            Dim _FNWorkDayInWeek As Integer = 13
                            Dim _FNWorkDayInMonth As Integer = 26
                            ...
                            For Each R As DataRow In _Dt.Rows
                                ...
                                If HI.HRCAL.Calculate.CalculateWeekEnd_KM(HI.ST.UserInfo.UserName, R!FNHSysEmpID.ToString _
                                            ...
                                            , False, _StateVacationRet, _FTStaDeductAbsent, _FTStaCalPayRoll, _FNStateSalaryType, FNExchangeRate.Value,
                                            FNExchangeRateTHB.Value, _FNWorkDayInWeek, _FNWorkDayInMonth, _FNWorkDayInWeekBF, FNSocialExchangeRate.Value, FNTaxExchangeRate.Value, R!FNCalType.ToString) = False Then
                            ...
                            _Qry &= vbCrLf & " ,FNSocialExchangeRate=" & FNSocialExchangeRate.Value & ""
                            _Qry &= vbCrLf & " ,FNTaxExchangeRate=" & FNTaxExchangeRate.Value & ""
                            _Qry &= vbCrLf & " WHERE FNHSysEmpTypeId =" & Val(_FNHSysEmpTypeId) & " "
                            _Qry &= vbCrLf & " AND FTPayYear = '" & _FTPayYear & "'"
                            _Qry &= vbCrLf & " AND FTPayTerm = '" & _FTPayTerm & "'"
    ...
    Private Sub ocmcalculate_Click(sender As System.Object, e As System.EventArgs) Handles ocmcalculate.Click
        ...
        If CDbl(FNExchangeRateTHB.Value) <= 0 Then
            ...
            FNExchangeRateTHB.Focus()
            Exit Sub
        End If


        If CDbl(FNSocialExchangeRate.Value) <= 0 Then
            HI.MG.ShowMsg.mInfo("กรุณาทำการระบุอัตราแลกเปลี่ยน  (Social) !!!", 1408167751, Me.Text, , System.Windows.Forms.MessageBoxIcon.Warning)
            FNSocialExchangeRate.Focus()
            Exit Sub
        End If


        If CDbl(FNTaxExchangeRate.Value) <= 0 Then
            HI.MG.ShowMsg.mInfo("กรุณาทำการระบุอัตราแลกเปลี่ยน (Tax) !!!", 1408167752, Me.Text, , System.Windows.Forms.MessageBoxIcon.Warning)
            FNTaxExchangeRate.Focus()
            Exit Sub
        End If
    ...
    Private Sub FNHSysEmpTypeId_EditValueChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            Else
                If FNHSysEmpTypeId.Text <> "" Then
                    ...
                    _Qry = "SELECT TOP 1  FNHSysEmpTypeId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.THRMEmpType WITH(NOLOCK) WHERE FTEmpTypeCode=N'" & HI.UL.ULF.rpQuoted(FNHSysEmpTypeId.Text) & "'"
                    ...
                    _Qry &= vbCrLf & "  D.FTStateTermEndOfYear,D.FDPayDate,D.FNExchangeRate,D.FNExchangeRateTHB,D.FNSocialExchangeRate,D.FNTaxExchangeRate "
                    ...
                    For Each R As DataRow In _Dt.Rows
                        ...
                        Me.FNExchangeRateTHB.Value = Val(R!FNExchangeRateTHB.ToString)
                        Me.FNSocialExchangeRate.Value = Val(R!FNSocialExchangeRate.ToString)
                        Me.FNTaxExchangeRate.Value = Val(R!FNTaxExchangeRate.ToString)
                        ...
                        If Val(R!FNExchangeRate.ToString) = 0 Then
                            ...
                            _Qry = " SELECT TOP 1  Max(ISNULL(FNSocialExchangeRate,0) )  AS FNSocialExchangeRate FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMCfgPayDT AS D WITH (NOLOCK) "
                            _Qry &= vbCrLf & " WHERE D.FDCalDateEnd ='" & HI.UL.ULF.rpQuoted(R!FDCalDateEnd.ToString) & "' "
                            _Qry &= vbCrLf & " AND ISNULL(FNSocialExchangeRate,0) > 0 "

                            Me.FNSocialExchangeRate.Value = Val(HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "0"))

                            _Qry = " SELECT TOP 1  Max(ISNULL(FNTaxExchangeRate,0) )  AS FNTaxExchangeRate FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMCfgPayDT AS D WITH (NOLOCK) "
                            _Qry &= vbCrLf & " WHERE D.FDCalDateEnd ='" & HI.UL.ULF.rpQuoted(R!FDCalDateEnd.ToString) & "' "
                            _Qry &= vbCrLf & " AND ISNULL(FNTaxExchangeRate,0) > 0 "

                            Me.FNTaxExchangeRate.Value = Val(HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "0"))
...
#End Region
    ...
    Private Sub bindEmptypeGroup()
        ...
        If Not (HI.ST.SysInfo.Admin) Then
            ...
            _Qry &= vbCrLf & "  AND E.FNHSysEmpTypeId IN ("
            _Qry &= vbCrLf & " Select DISTINCT UPT.FNHSysEmpTypeId"
            _Qry &= vbCrLf & " FROM             [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEUserLoginPermission AS UP WITH (NOLOCK) INNER JOIN"
            _Qry &= vbCrLf & "     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEPermissionMenu AS UPM WITH (NOLOCK) ON UP.FNHSysPermissionID = UPM.FNHSysPermissionID INNER JOIN"
            _Qry &= vbCrLf & "     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SECURITY) & "].dbo.TSEPermissionEmployeeType AS UPT WITH (NOLOCK) ON UP.FNHSysPermissionID = UPT.FNHSysPermissionID AND UPM.FNHSysPermissionID = UPT.FNHSysPermissionID"
    ...
    Private Sub BindFNHSysEmpTypeId_Detail(_FNHSysEmpTypeId As String)

        Try

            If _FNHSysEmpTypeId <> "" Then
                Dim _Qry As String = ""
                Dim _Dt As DataTable
                ...
                _Qry &= vbCrLf & "  ,ISNULL(D.FNExchangeRate,0) AS 'FNExchangeRate', ISNULL(D.FNExchangeRateTHB,0) AS 'FNExchangeRateTHB' "
                _Qry &= vbCrLf & "  ,ISNULL(D.FNSocialExchangeRate,0) AS 'FNSocialExchangeRate', ISNULL(D.FNTaxExchangeRate,0) AS 'FNTaxExchangeRate' "
                ...
                _Qry &= vbCrLf & "    H.FNHSysEmpTypeId = D.FNHSysEmpTypeId INNER JOIN"
                _Qry &= vbCrLf & "    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.V_ListMonth ON D.FNMonth = V_ListMonth.FNListIndex"
                _Qry &= vbCrLf & " WHERE H.FNHSysEmpTypeId =" & Val(_FNHSysEmpTypeId) & " "
        ...
        Try
            ...
            With ogvemptype
                If Not (.DataSource Is Nothing) And ogvemptype.RowCount > 0 Then
                    ...
                    With ogvemptype
                        For I As Integer = 0 To .RowCount - 1
                            ...
                            If (.GetRowCellValue(I, .Columns.ColumnByFieldName("FTEmpTypeGroup")) = _FTEmpTypeGroupName) Then
                                ...
                                If _State = "1" Then
                                    _FNHSysEmptypeId = .GetRowCellValue(I, .Columns.ColumnByFieldName("FNHSysEmpTypeId"))
                                End If
                    ...
                    'CType(ogvemptype.DataSource, DataTable).AcceptChanges()
            ...
            If _FNHSysEmptypeId <> "" Then
                BindFNHSysEmpTypeId_Detail(_FNHSysEmptypeId)
            End If
    ...
    Private Sub FNHSysEmpTypeId_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs)

    End Sub

    Private Sub FNHSysEmpTypeId_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs)

    End Sub

    Private Sub FNHSysEmpTypeId_KeyUp(sender As Object, e As System.Windows.Forms.KeyEventArgs)

    End Sub

    Private Sub FNHSysEmpTypeId_LostFocus(sender As Object, e As EventArgs)
