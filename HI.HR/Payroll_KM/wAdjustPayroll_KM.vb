...
Public Class wAdjustPayroll_KM
...
#Region "Procedure"
                ...
                Select Case Val(R!FNCalType.ToString)
                    Case 0, 1
                        ...
                        If Val(R!FNCalType.ToString) = 0 Then
                            Me.SalaryPerDay = FNSalary.Value / 26
                        Else
    ...
    Private Sub SetShowFinance(ByVal EmpCode As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & ",CASE WHEN ISNULL(THRMEmployeeFin.FTFinCode,N'')=N'' THEN   Convert(numeric(18,3),0.000) ELSE   FTFinAmt  END AS FTFinAmt"
    ...
    Private Sub LoadPayroll()
        ...
        _Qry &= vbCrLf & " FNSocialInsuranceEmployee as   FNSocial, convert(numeric(18,2),FNTax) as FNTax, FHolidayBaht, FNNetpay, FNAccumulateIncomeYear, FNAccumulateSocialYear, FNAccumulateTax, FTStateInDustin, FNTotalCalContributedAmt,"
        _Qry &= vbCrLf & "  FNContributedAmt, FNCmpContributedAmt, FNTotalCalWorkmen, FNWorkmenAmt,  FNAmtRetire,"
        _Qry &= vbCrLf & "  FNPayLeaveSSo, FNWorkingDay, FNAdjBeforeCal, FNAttandanceAmt, FNHealtCareAmt, FNTransportAmt, FNChildCareAmt, FNOTMealAmt, FNSocialBase, FNWorkAgeSalary, FNOTMealAmtUS, FNExchangeRate,FNSickLeaveBaht"
        ...
        For Each R As DataRow In _dt.Rows
            ...
            Me.FNOTMealAmtUS.Value = Val(R!FNOTMealAmtUS.ToString)
            Me.FNExchangeRate.Value = Val(R!FNExchangeRate.ToString)
            ...
            Me.FNSickLeaveBaht.Value = Val(R!FNSickLeaveBaht.ToString)
        Next
...
#Region "General"
    ...
    Private Sub FNHSysEmpID_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNHSysEmpID.EditValueChanged
        ...
        Else
            ...
            If FNHSysEmpID.Text <> "" Then
                'Dim _Qryx As String = "SELECT TOP 1 FNHSysEmpTypeId  FROM THRMEmployee WITH(NOLOCK) WHERE FTEmpCode =N'" & HI.UL.ULF.rpQuoted(FNHSysEmpID.Text) & "' "
                'Dim EmptypeId As Integer = HI.Conn.SQLConn.GetField(_Qryx, Conn.DB.DataBaseName.DB_HR, "0")
                'If Not CheckPermissionSalary(EmptypeId) Then
                '    Exit Sub
                'End If
    ...
    Private Function CheckPermissionSalary(EmptypeId As Integer) As Boolean
    ...
    Private Function SaveData(ByVal oSpls As HI.TL.SplashScreen) As Boolean
        Try
            ...
            _Qry = " SELECT top 1   FNParturitionLeaveBaht "
            ...
            _Qry &= vbCrLf & "and FTPayYear = '" & _PayYear & "'  and  FTPayTerm='" & _PayTerm & "'"
            Dim _FNParturitionLeaveBaht As Double = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "0")
            ...
            For Each R As DataRow In _Dtemp.Rows
                ...
                If R!FTCalTaxSta.ToString <> "1" Then
                    ...
                    With _EmpDisTax
                        ...
                        .OtherSlary = _FTOtherAdd_CalTAX + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FNAttandanceAmt.Value + FNHealtCareAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value + FNPayLeaveOtherBaht.Value + FNPayLeaveVacationBaht.Value
                    ...
                    _TaxOtherAmt = 0
                    Dim _TotalTax As Double = HI.HRCAL.Calculate.GETnTax_KM(_Total, _TaxOther, _TaxOtherAmt)
            ...
            _tmpTotalincome = Double.Parse(Format(((FCBaht.Value + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FHolidayBaht.Value) + _AmtAdd + FNAttandanceAmt.Value + FNHealtCareAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value + (FNDiligentBaht.Value + FNPayLeaveVacationBaht.Value + FNPayLeaveOtherBaht.Value + FNSickLeaveBaht.Value)) - (_AmtDeduct), "0.000"))
            _tmpTotalNetPay = Double.Parse(Format(((FCBaht.Value + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FHolidayBaht.Value) + _AmtAdd + FNAttandanceAmt.Value + FNHealtCareAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value + (FNDiligentBaht.Value + FNPayLeaveVacationBaht.Value + FNPayLeaveOtherBaht.Value + FNSickLeaveBaht.Value)) - (_AmtDeduct + CDbl(Format(FNTax.Value / FNExchangeRate.Value, "0.00")) + FNContributedAmt.Value), "0.000"))

            _Net = Double.Parse(Format(((FCBaht.Value + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FHolidayBaht.Value) + _AmtAdd + FNAttandanceAmt.Value + FNHealtCareAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value + (FNDiligentBaht.Value + FNPayLeaveVacationBaht.Value + FNPayLeaveOtherBaht.Value + FNSickLeaveBaht.Value)) - (_AmtDeduct), "0.00"))
            _FNNetpay = Double.Parse(Format(((FCBaht.Value + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FHolidayBaht.Value) + _AmtAdd + FNAttandanceAmt.Value + FNHealtCareAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value + (FNDiligentBaht.Value + FNPayLeaveVacationBaht.Value + FNPayLeaveOtherBaht.Value + FNSickLeaveBaht.Value)) - (_AmtDeduct + CDbl(Format(FNTax.Value / FNExchangeRate.Value, "0.00")) + FNContributedAmt.Value), "0.00"))

            '+ลาคลอดหน้าแก้ไข 20170526

            _Net += +_FNParturitionLeaveBaht
            _FNNetpay += +_FNParturitionLeaveBaht
            '+ลาคลอดหน้าแก้ไข 
            ...
            _Qry &= vbCrLf & " , FNSocial= ( " & FNSocial.Value & "  /  FNSocialExchangeRate ) "

            _Qry &= vbCrLf & " , FNTax= ( " & FNTax.Value & " / FNTaxExchangeRate )  "

            _Qry &= vbCrLf & " , FNNetpay=" & _FNNetpay & " -  ( " & FNSocial.Value & "  /  FNSocialExchangeRate ) "
            ...
            _Qry &= vbCrLf & ", FNTotalIncomeDiff=" & _DiffTotalincome & ""
            _Qry &= vbCrLf & " ,FNNetpayDiff=" & _DiffTotalNetPay & " "
            _Qry &= vbCrLf & " WHERE FTPayYear=N'" & Me.FTPayYear.Text & "' "
            _Qry &= vbCrLf & " AND FTPayTerm=N'" & Me.FTPayTerm.Text & "' "
            _Qry &= vbCrLf & " AND FNHSysEmpID=" & Val(Me.FNHSysEmpID.Properties.Tag.ToString) & " "
    ...
    Private Sub FNWorkingDay_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNWorkingDay.EditValueChanged
        ...
        If Not (_InitLoad) Then
            FCBaht.Value = Format(Me.SalaryPerDay * FNWorkingDay.Value, "0.000")
        End If
    End Sub
    ...
    Private Sub FNOt1_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNOt1.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FCOt1_Baht.Value = Format(((Me.SalaryPerDay / 8) * _RateOT1) * FNOt1.Value, "0.000")
        End If
    End Sub
    ...
    Private Sub FNOt15_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNOt15.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FCOt15_Baht.Value = Format(((Me.SalaryPerDay / 8) * _RateOT15) * FNOt15.Value, "0.000")
        End If
    End Sub
    ...
    Private Sub FNOt2_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNOt2.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FCOt2_Baht.Value = Format(((Me.SalaryPerDay / 8) * _RateOT2) * FNOt2.Value, "0.000")
        End If
    End Sub
    ...
    Private Sub FNOt3_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNOt3.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FCOt3_Baht.Value = Format(((Me.SalaryPerDay / 8) * _RateOT3) * FNAttandanceAmt.Value, "0.000")
        End If
    End Sub
    ...
    Private Sub FNOt4_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNOt4.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FCOt4_Baht.Value = Format(((Me.SalaryPerDay / 8) * _RateOT4) * FNOt4.Value, "0.000")
        End If
    End Sub
    ...
    Private Sub FNHoliday_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNHoliday.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FHolidayBaht.Value = Format(Me.SalaryPerDay * FNHoliday.Value, "0.000")
        End If
    End Sub
