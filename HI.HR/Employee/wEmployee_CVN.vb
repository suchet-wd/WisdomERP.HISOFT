...
#Region "Procedure"
    ...
    Private Sub InitForm()
        ...
        _Qry &= vbCrLf & " WHERE FTDynamicFormName=N'" & HI.UL.ULF.rpQuoted("wEmployee") & "' "
            ...
            Try
                ...
                Else
                    If FNHSysEmpID.Text <> "" Then
                        _Qry = ""
                        _Qry = "SELECT TOP 1 FNHSysEmpID  FROM " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & ".dbo.THRMEmployee WITH(NOLOCK) WHERE FTEmpCode =N'" & HI.UL.ULF.rpQuoted(FNHSysEmpID.Text) & "' "
    ...
    Private Function VerrifyData() As Boolean
        ...
        If FTEmpIdNo_lbl.ForeColor = Color.Blue Then
            Dim _Text As String = ""
            _Qry = "SELECt TOP 1  FTEmpCode  FROM " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & ".dbo.THRMEmployee WITH(NOLOCK) WHERE FTEmpIdNo=N'" & HI.UL.ULF.rpQuoted(FTEmpIdNo.Text) & "' AND FNHSysEmpID<>" & Val(FNHSysEmpID.Properties.Tag.ToString) & "  AND ISNULL(FNEmpStatus,0) <>2   AND ISNULL(FTEmpIdNo,N'')<>''  "
        ...
        If FTAccNo.Text <> "" Then
            ...
            Dim _Text As String = ""
            _Qry = "SELECt TOP 1  FTEmpCode  FROM " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & ".dbo.THRMEmployee WITH(NOLOCK) WHERE FTAccNo=N'" & HI.UL.ULF.rpQuoted(FTAccNo.Text) & "' AND FNHSysEmpID<>" & Val(FNHSysEmpID.Properties.Tag.ToString) & "  AND ISNULL(FNEmpStatus,0) <>2   AND ISNULL(FTAccNo,N'')<>''  "
            _Text = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "")
...
#Region "MAIN PROC"
    ...
    Private Sub UpdateDataEmpToOtherServer(FNHSysEmpID As Integer)
        ...
        Try
            ...
            _StrFiled &= "FTHealthInsurIDFatherMate, FTHealthInsurIDMotherMate, FTFundIDNo, FDFundBegin, FDFundEnd, FCExceptAgeOver, FCExceptAgeOverMate, FDRetire, FTStaCalMonthEnd, FDDateTransfer, FTDeligentCode"
            ...
            If HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR_PAYROLL) = False Then
                ...
                _Qry &= vbCrLf & " FTHealthInsurIDFatherMate, FTHealthInsurIDMotherMate, FTFundIDNo, FDFundBegin, FDFundEnd, FCExceptAgeOver, FCExceptAgeOverMate, FDRetire, FTStaCalMonthEnd, FDDateTransfer, FTDeligentCode"
                _Qry &= vbCrLf & ")"
...
#Region " Proc "
    ...
    Protected Sub ShowLeaveInfo(ByVal EmpCode As String)
        ...
        Try
            ...
            '_Qry &= vbCrLf & ", (Right('000' + Convert(varchar(30),Convert(numeric(18,0),Floor((ISNULL(FNLeaveRight,0))/ " & _baseWorkMin & "))),3)"
            ...
            '_Qry &= vbCrLf & " , (Right('000' + Convert(varchar(30),Convert(numeric(18,0),Floor((ISNULL(FNLeaveUsed,0))/ " & _baseWorkMin & "))),3)"
            ...
            '_Qry &= vbCrLf & " , (Right('000' + Convert(varchar(30),Convert(numeric(18,0),Floor((ISNULL(FNLeaveBal,0))/ " & _baseWorkMin & "))),3)"
    ...
    Private Sub ShowPunishment(ByVal EmpCode As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & ",FTCause,FTPunishmentBy"
    ...
    Private Sub SetShowFinance(ByVal EmpCode As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " AND ISNULL(FTStaActive,N'')=N'1' "
    ...
    Private Sub ShowExperience(ByVal EmpCode As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " FROM THRTEmployeeExperience WITH(NOLOCK)"
            _Qry &= vbCrLf & " WHERE  FNHSysEmpID=" & Val(EmpCode) & ""
...
#End Region
        ...
        Try
            ...
            Else
                If FNHSysPositId.Text <> "" Then
                    ...
                    Dim _Qry As String = "SELECT TOP 1 FNHSysPositId  FROM " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & ".dbo.TCNMPosition WITH(NOLOCK) WHERE FTPositCode =N'" & HI.UL.ULF.rpQuoted(FNHSysPositId.Text) & "' "
    ...
    Private Sub CheckFund(ByVal EmpCode As String)

        Dim _Qry As String
        Dim _FNStateCmpFund As String


        Try

            _Qry = " SELECT FNStateCmpFund "
            _Qry &= vbCrLf & " From [HITECH_HR].[dbo].[THRMCmpSocialDiscountTaxConfig] "
            _Qry &= vbCrLf & " WHERE FNHSysCmpId=" & Val("" & Me.FNHSysCmpId.Properties.Tag.ToString)
            _FNStateCmpFund = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR)

            Select Case _FNStateCmpFund
                Case Is = "0" 'None
                    ocmFundRateSelect.Visible = False


                Case Is = "1" 'type Company
                    ocmFundRateSelect.Visible = False
                Case Is = "2" 'type Employee
                    ocmFundRateSelect.Visible = True
        ...
        If Me.VerrifyData Then
            If Me.SaveData() Then
                ...
                If HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR) = False Then
                    ...
                    _Qry = "INSERT INTO THRTEmployeePunishment(FNHSysEmpID,FNSeqNo, FTCmpCodeRef, FDDatePunishment, FNHSysPunishmentLvId, FTCause, FTPunishmentBy, FTStaOutstanding "
                    _Qry &= vbCrLf & ", FTInsUser, FDInsDate, FTInsTime)  "
                    _Qry &= vbCrLf & " SELECT " & Val(FNHSysEmpID.Properties.Tag.ToString) & "," & tSeqNo & ""
                    _Qry &= vbCrLf & ",N'',N'" & HI.UL.ULDate.ConvertEnDB(FDDatePunishment.Text) & "'," & Val(FNHSysPunishmentLvId.Properties.Tag.ToString) & ""
        ...
        If Me.VerrifyData Then
            If Me.SaveData() Then
                ...
                If HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR) = False Then
                    ...
                    _Qry = "INSERT INTO " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & ".dbo.THRTEmployeeExperience(FNHSysEmpID,FNSeqNo, FTCmpName, FTBusinessType, FTContractName, FTPosition, FTResponsibility, FDStartDate, FDEndDate, FTEndSalary, FTResignCause "
                    _Qry &= vbCrLf & ", FTInsUser, FDInsDate, FTInsTime)  "
                    _Qry &= vbCrLf & " SELECT " & Val(FNHSysEmpID.Properties.Tag.ToString) & "," & tSeqNo & ""
...
#Region "PassPort"
    ...
    Private Sub ogvdetail_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ogvdetail.Click
        With ogvdetail
            ...
            Try
                FDDateofIssue2.DateTime = HI.UL.ULDate.ConvertEnDB("" & .GetRowCellValue(.FocusedRowHandle, "FDDateofIssue").ToString)
            Catch ex As Exception
...
#Region "Other"
    ...
    Private Sub OcmFundRateSelect_Click_1(sender As Object, e As EventArgs) Handles ocmFundRateSelect.Click
        Try
            ...
            If FDDateStart.Text <> "" Then
                ...
                _Qry &= vbCrLf & "  WHERE DATEDIFF(MONTH, '" & HI.UL.ULDate.ConvertEnDB(FDDateStart.Text) & "',GETDATE()) > FNAgeBegin "
                _Qry &= vbCrLf & "  AND DATEDIFF(MONTH, '" & HI.UL.ULDate.ConvertEnDB(FDDateStart.Text) & "',GETDATE()) <= FNAgeEnd "
            ...
            Else
                HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, FDDateStart_lbl.Text)
                FDDateStart.Focus()
            End If
    ...
    Private Sub FNEmpSex_SelectedIndexChanged(sender As Object, e As EventArgs) Handles FNEmpSex.SelectedIndexChanged
    ...
    Private Sub objToolTipController_GetActiveObjectInfo(sender As Object, e As ToolTipControllerGetActiveObjectInfoEventArgs) 
        Try
            If e.Info Is Nothing AndAlso e.SelectedControl Is ogcdetail Then
                ...
                'If info.InRowCell AndAlso (info.RowHandle = lastRowHandle OrElse info.Column Is lastColumn) Then
                ...
                Dim index As Integer = view.GetDataRowHandleByGroupRowHandle(info.RowHandle)
                Dim cellValue = view.GetRowCellValue(index, "FBFileImage")

                'Dim text As String = view.GetRowCellDisplayText(info.RowHandle, info.Column)
                'Dim cellKey As String = info.RowHandle.ToString() & " - " & info.Column.ToString()


                Dim ToolTipControlInfo As ToolTipControlInfo = New ToolTipControlInfo

                Dim sTooltip1 As SuperToolTip = New SuperToolTip
                Dim titleItem1 As ToolTipTitleItem = New ToolTipTitleItem

                'Dim img As Image
                'img.Size = Size.Width(50)


                titleItem1.Text = "Edit Popup Menu"
                Dim item1 As ToolTipItem = New ToolTipItem
                item1.Image = FTEmpPicName.Image
                ''item1.Image.Width = 50
                ...
                ToolTipControlInfo.SuperTip = sTooltip1
    ...
    Private Sub XtraScrollableControl3_Click(sender As Object, e As EventArgs) Handles XtraScrollableControl3.Click
