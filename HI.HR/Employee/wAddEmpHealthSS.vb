...
#Region "Procedure"
    ...
    Private Sub LoadDataEdit()
        ...
        _Qry &= vbCrLf & "  WHERE  (W.FNHSysEmpID =" & Val(Me.EmpSysID) & ")"
        _Qry &= vbCrLf & "  AND (W.FNSeqNo =" & Val(Me.HealthSeq) & ")"
    ...
    Private Function VerifyData() As Boolean
        ...
        If Me.FDWorkDate.Text <> "" Then
            _Pass = True
        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FDWorkDate_lbl.Text)
            FDWorkDate.Focus()
        End If
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            ...
            Try
                ...
                _Qry = "UPDATE   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee_WorkSickness SET "
                _Qry &= vbCrLf & " FDWorkDate='" & HI.UL.ULDate.ConvertEnDB(FDWorkDate.Text) & "'"
                _Qry &= vbCrLf & ",FTInjuredBody=N'" & HI.UL.ULF.rpQuoted(FTInjuredBody.Text) & "' "
                _Qry &= vbCrLf & ",FTCauseOfInjury=N'" & HI.UL.ULF.rpQuoted(FTCauseOfInjury.Text) & "' "
                _Qry &= vbCrLf & ",FTDisability=N'" & FTDisability.EditValue.ToString & "' "
                _Qry &= vbCrLf & ",FTLossOfSomeOrgans=N'" & FTLossOfSomeOrgans.EditValue.ToString & "' "
                _Qry &= vbCrLf & ",FTLeaveWorkMore=N'" & FTLeaveWorkMore.EditValue.ToString & "' "
                _Qry &= vbCrLf & ",FTLeaveWorkNiMore=N'" & FTLeaveWorkNiMore.EditValue.ToString & "' "
                ...
                If HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR) = False Then
                    _Qry = "SELECT MAX(FNSeqNo) AS FNSeqNo FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee_WorkSickness WHERE  FNHSysEmpID=" & Val(Me.EmpSysID) & " "

                    Dim tSeqNo As String = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "0")
                    tSeqNo = Val(tSeqNo) + 1

                    _Qry = "INSERT INTO   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee_WorkSickness(FNHSysEmpID, FNSeqNo, FDWorkDate, FTInjuredBody,  FTCauseOfInjury, FTDisability, FTLossOfSomeOrgans, FTLeaveWorkMore, FTLeaveWorkNiMore"
                    _Qry &= vbCrLf & ", FTInsUser, FDInsDate, FTInsTime)  "
                    _Qry &= vbCrLf & " SELECT " & Val(Me.EmpSysID) & "," & tSeqNo & ""
                    _Qry &= vbCrLf & " ,'" & HI.UL.ULDate.ConvertEnDB(FDWorkDate.Text) & "'"
                    _Qry &= vbCrLf & ",N'" & HI.UL.ULF.rpQuoted(FTInjuredBody.Text) & "' "
                    _Qry &= vbCrLf & ",N'" & HI.UL.ULF.rpQuoted(FTCauseOfInjury.Text) & "' "
                    _Qry &= vbCrLf & ",N'" & FTDisability.EditValue.ToString & "' "
                    _Qry &= vbCrLf & ",N'" & FTLossOfSomeOrgans.EditValue.ToString & "' "
                    _Qry &= vbCrLf & ",N'" & FTLeaveWorkMore.EditValue.ToString & "' "
                    _Qry &= vbCrLf & ",N'" & FTLeaveWorkNiMore.EditValue.ToString & "' "
                ...
                _Qry = "UPDATE  THRMEmployee_WorkSickness SET FNSeqNo=FNNo"
                _Qry &= vbCrLf & " FROM  THRMEmployee_WorkSickness INNER JOIN "
                _Qry &= vbCrLf & "(SELECT ROW_NUMBER() OVER(ORDER BY FDInsDate, FTInsTime) AS FNNo, FNSeqNo,FNHSysEmpID"
                _Qry &= vbCrLf & " FROM  THRMEmployee_WorkSickness"
                _Qry &= vbCrLf & " WHERE FNHSysEmpID=" & Val(Me.EmpSysID) & ""
...
#Region "General"
    ...
    Private Sub wAddEmpHealthSS_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Try
