...
Public Class wEmployeeLeave_20240203
...
#Region "MAIN PROC"
    ...
    Private Sub ProcessSave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmsave.Click
        If Me.VerrifyData("Save") Then

            If Not ChkProbation() Then
                HI.MG.ShowMsg.mInfo("ไม่สามารถบันทึกการได้ เนื่องจากพนักงานยังไม่ผ่านทดลองงาน !!!", 202207015, Me.Text, , System.Windows.Forms.MessageBoxIcon.Warning)
                Exit Sub
            End If

            'If FNLeaveType.SelectedIndex = 2 And (FTStateLeavepay.EditValue.ToString <> "1" Or FNLeaveDay.SelectedIndex = 3) Then
            '    HI.MG.ShowMsg.mInfo("ลากิจธุระอันจำเป็น ต้องเป็นลาเต็มวัน,ครึ่งวันเเช้า,ครึ่งวันบ่าย แบบจ่ายเงินเท่านั้น กรุณาตรวจสอบ !!!", 202001170001, Me.Text, , System.Windows.Forms.MessageBoxIcon.Warning)
            '    Exit Sub
            'End If
            ...
            If TotalLeave < (FTNetDay.Value * ocetotaltime.Value) Then
                ...
                If HI.TL.CboList.GetListValue("" & FNLeaveType.Properties.Tag.ToString, FNLeaveType.SelectedIndex) = "98" Then
                    Exit Sub
                End If
                If HI.TL.CboList.GetListValue("" & FNLeaveType.Properties.Tag.ToString, FNLeaveType.SelectedIndex) = "998" Then
                    Exit Sub
                End If
                If HI.TL.CboList.GetListValue("" & FNLeaveType.Properties.Tag.ToString, FNLeaveType.SelectedIndex) = "999" Then
                    Exit Sub
                End If
                If HI.TL.CboList.GetListValue("" & FNLeaveType.Properties.Tag.ToString, FNLeaveType.SelectedIndex) = "2" Then
                    Exit Sub
                End If
            End If
        ...
        If Me.FNHSysEmpId.Text <> "" And Me.FNHSysEmpId.Properties.Tag.ToString <> "" Then
            For Each _ReportName As String In _AllReportName.Split(",")
                With New HI.RP.Report
                    ...
                    Dim sumBusinessLeave2 As String = "0"
                    ...
                    Call SumLeaveReport(sumBusinessLeave, sumBusinessLeave2, sumSickLeave, sumVacationLeave, sumAbsent, sumLate, LeaveVacation, sumSpecialLeave, sumLeaveNotpaid, sumAccident)

                    .AddParameter("sumBusinessLeave", sumBusinessLeave)
                    .AddParameter("sumBusinessLeave2", sumBusinessLeave2)
    ...
    Private Function VerrifyData(Action As String) As Boolean
        ...
        If Me.FNHSysEmpId.Text <> "" And FNHSysEmpId.Properties.Tag.ToString <> "" Then
            If FNLeaveType.SelectedIndex > -1 Then
                If HI.UL.ULDate.CheckDate(FTStartDate.Text) <> "" And HI.UL.ULDate.CheckDate(FTEndDate.Text) <> "" Then
                    ...
                    If (HI.UL.ULDate.UDayDiff(DateInterval.Year, FTStartDate.Text, FTEndDate.Text) = 0 And _LeaveCode = "98") Or _LeaveCode <> "98" Then
                        ...
                        If Me.FTSTime.Text <> "" And Me.FTETime.Text <> "" Then
                            If Me.FTSTime.Text <> Me.FTETime.Text Then
                                ...
                                If Me.FTNetDay.Value > 0 Then
                                    ...
                                    If Me.FNNetTime.Value > 0 Then

                                        If Action = "Save" Then

                                            Dim _flag As Boolean = False
                                            _flag = ChkOverLapDateTime()
                                            '' Check Overlap Date Time
                                            If _flag Then
                                                _Pass = True

                                            Else
                                                HI.MG.ShowMsg.mInvalidData("พบการลาซ้ำวันหรือเวลา  กรุณาทำการระบุข้อมูลวันให้ถูกต้อง !!!", 202006010006, Me.Text)
                                                FTEndDate.Focus()
                                            End If
                                        Else
                                            _Pass = True
                                    ...
                                    Else
                                        HI.MG.ShowMsg.mInvalidData("กรุณาใส่เวลาเริ่มต้น สิ้นสุดให้ถูกต้อง !!!", 202006010007, Me.Text)
                                        FTEndDate.Focus()
                                    End If
                                Else
                        ...
                        End If
                    Else
                        HI.MG.ShowMsg.mInvalidData("ไม่สามารถลาข้ามปีได้ กรุณาแบ่งการลา !!!", 2004050002, Me.Text)
                        FTEndDate.Focus()
                    End If
            ...
            Else
                ...
                HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FNLeaveType_lbl.Text)
                FNLeaveType.Focus()
...
#End Region
    ...
    Private Sub Checkpay(ByVal leavekey As String, Optional ByVal SetCheckPay As Boolean = True)
        ...
        If leavekey = "2" Then
            ...
            '* ปรับเรื่องลากิจอันจำเป็น ในกรณีที่ลาแล้วยังไม่ได้ อนุมัติ.... 20190523

            _Qry = "Select   SUM(FNLeaveTotalDay * FNLeaveTotalTimeMin) AS FNTotalMinute "
            _Qry &= vbCrLf & "   FROM THRTLeaveAdvanceDaily WITH (NOLOCK)"
            _Qry &= vbCrLf & " WHERE (FNHSysEmpID = " & Val(Me.FNHSysEmpId.Properties.Tag.ToString) & ") "
            _Qry &= vbCrLf & " AND (FTStartDate >= N'" & _DateReset & "')"
            _Qry &= vbCrLf & " AND FTLeaveType='2'"
            _Qry &= vbCrLf & " AND (FTStartDate NOT BETWEEN '" & HI.UL.ULDate.ConvertEnDB(Me.FTStartDate.Text) & "' AND  '" & HI.UL.ULDate.ConvertEnDB(Me.FTEndDate.Text) & "') "
        ...
        If (SetCheckPay) Then
            ...
            If _Leave < (FTNetDay.Value * ocetotaltime.Value) And leavekey = "2" Then
        ...
        If leavekey = "98" Or leavekey = "2" Then
            TotalLeave = _Leave
        Else
        ...
        If (SetCheckPay) Then
            ...
            ElseIf ChkPayLeave(leavekey) Then
                'If ((FNHSysEmpTypeId.Text.ToUpper <> "M" Or FNHSysEmpTypeId.Text.ToUpper <> "N" Or FNHSysEmpTypeId.Text.ToUpper <> "O" Or FNHSysEmpTypeId.Text.ToUpper <> "M1" Or FNHSysEmpTypeId.Text.ToUpper <> "M2") And leavekey = "2") Then  '' leavekey = "2" = ลากิจธุระจำเป็น
                If leavekey = "97" Then  '' ลาคลอด   ถ้าลาหลายวัน  เช็คแล้วถ้ายังเหลือวัน ลาจ่าย
                    ...
                    End If
                Else
                    If ((FNHSysEmpTypeId.Text.ToUpper <> "M" Or FNHSysEmpTypeId.Text.ToUpper <> "N" Or FNHSysEmpTypeId.Text.ToUpper <> "O" Or FNHSysEmpTypeId.Text.ToUpper <> "M1" Or FNHSysEmpTypeId.Text.ToUpper <> "M2" Or FNHSysEmpTypeId.Text.ToUpper <> "M3")) Then
                        ''If (FNLeaveDay.SelectedIndex <> 3 And (_LeavePay >= _GLeavePay + (FTNetDay.Value * ocetotaltime.Value))) Then  ''FNLeaveDay.SelectedIndex <> 3 ลาระบุเวลา
                        If (_LeavePay >= _GLeavePay + (FTNetDay.Value * ocetotaltime.Value)) Then
    ...
    Private Sub SumLeaveReport(ByRef sumBusinessLeave As String, ByRef sumBusinessLeave2 As String, ByRef sumSickLeave As String, ByRef sumVacationLeave As String, ByRef sumAbsent As String, ByRef sumLate As String, ByRef LeaveVacation As String, ByRef sumSpecailLeave As String, ByRef sumLeaveNotPaid As String, ByRef sumAccident As String)
        Try
            Dim _Qry As String
            Dim _dt As DataTable
            Dim _DateReset As String
            ...
            For Each R As DataRow In _dt.Rows
                ...
                Select Case R!FTLeaveType.ToString
                    ...
                    Case "2"
                        sumBusinessLeave2 = Format((_Sum \ 480), "00") & ":" & Format(((_Sum Mod 480) \ 60), "00") & ":" & Format(((_Sum Mod 480) Mod 60), "00") & ""
    ...
    Private Sub FNHSysEmpId_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNHSysEmpId.EditValueChanged
        ...
        Else
            ...
            Call CheckPermissionEdit()
            FTStateMedicalCertificate.Checked = False
            _HRProcess = ""

            FNLeaveType.SelectedIndex = -1
        End If

    End Sub

    Public Sub CheckPermissionEdit()

        If HI.ST.SysInfo.Admin Or ChkActiveLeaveEdit() Then
            Exit Sub
        End If
        FTStateNotMergeHoliday.Enabled = False
        FTStateLeavepay.Enabled = False
        FTStateCalSSo.Enabled = False
    ...
    Public Sub LoadEmpCodeByEmpIDInfoCustom(ByVal Key As Object)
        ...
        FNLeaveDay.SelectedIndex = HI.TL.CboList.GetIndexByValue(FNLeaveDay.Properties.Tag.ToString, "" & arr(2).ToString)

        FTStartDate.DateTime = arr(3).ToString
        FTEndDate.DateTime = arr(4).ToString

        FTSTime.Text = arr(5).ToString
        FTETime.Text = arr(6).ToString

        FTStateCalSSo.Checked = arr(7).ToString
        FTStateLeavepay.Checked = arr(8).ToString

        FTStateDeductVacation.Checked = arr(9).ToString
        FTStateNotMergeHoliday.Checked = arr(10).ToString

        Call Load_PDF()

        FTStateMedicalCertificate.Checked = arr(11).ToString
    ...
    Private Sub FTNetDay_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FTNetDay.EditValueChanged
        ...
        Else
            Try
                If FNHSysEmpId.Text <> "" And FNLeaveType.SelectedIndex >= 0 Then
    ...
    End Sub
    Private Function ChkActiveLeaveEdit() As Boolean
        Try
            Dim _Qry As String = ""
    ...
    Private Function ChkProbation() As Boolean
        Try
            ...
            'Else
            Dim _Qry As String = ""
            _Qry = "   SELECT  TOP 1  dbo.FN_Get_Emp_leave_Chk_Probation(FNHSysEmpID,FNHSysEmpTypeId,ISNULL(FDDateStart,''),ISNULL(FDDateEnd,''),ISNULL(FDDateProbation,'')" & ",'" & HI.UL.ULDate.ConvertEnDB(Me.FTStartDate.Text) & "'" & ",'" & HI.UL.ULDate.ConvertEnDB(Me.FTEndDate.Text) & "'," & HI.TL.CboList.GetListValue("" & FNLeaveType.Properties.Tag.ToString, FNLeaveType.SelectedIndex) & ") AS ChkPro"
            _Qry &= vbCrLf & "   FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee  AS M WITH(NOLOCK)"
            _Qry &= vbCrLf & "  WHERE  M.FNHSysEmpID=" & Val(Me.FNHSysEmpId.Properties.Tag.ToString) & " "
    ...
    Private Function ChkActiveLeaveHRDelete() As Boolean
        Try
            ...
            Else
                ...
                '        Return True
                '    Case _SendApp And Not (_MngApp) And _Approve And _Premis
                '        Return True
