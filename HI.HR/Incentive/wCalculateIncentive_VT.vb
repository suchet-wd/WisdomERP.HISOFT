...
Public Class wCalculateIncentive_VT
...
#Region " Procedure "
    ...
    Private Function GetEmpTime(_UnitSectID As Integer, _CalDate As String, Optional _TFNHSysEmpID As Integer = 0) As DataTable
        ...
        _Qry = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.SP_GET_EMPTIME_CAL_INCENTIVE_PROD_EFF_VT " & _UnitSectID & ",'" & HI.UL.ULDate.ConvertEnDB(_CalDate) & "'," & _TFNHSysEmpID & ""
        ...
        For Each R As DataRow In _dt.Rows
            ...
            If _FNHSysEmpID = 2111930952 Then
                Beep()
            End If
                ...
                If _dtmove.Select("FNHSysEmpID=" & _FNHSysEmpID & " And FTStartTime<='" & _StartTime & "' AND FTEndTime>='" & _EndTime & "'").Length <= 0 Then
                    ...
                    Select Case _FNHour
                        Case 1, 2, 3, 4
                            ...
                            If _FTState = "1" Then
                                If _FNHSysEmpID = 2111930613 Then
                                    Dim X As String
                                    X = "99999"
                                End If
                                Select Case True
                            ...
                            If _FTState = "1" Then
                                ...
                                Select Case True
                                    ...
                                    Case (_StartTime >= _FTIn2 And _FTOut2 < _EndTime And _FTIn2 <> "") And (_FTIn2 <> "" And _FTOut2 <> "" And _FTEndTime >= _EndTime)
                                        ...
                                        Catch ex As Exception
                                        End Try
                                        '' Best  addd case move in     เวลาไม่เต็ม ชม
                                    Case (_FTIn2 <= _StartTime And _FTOut2 >= _EndTime) And (_FTStartTime >= _StartTime And _FTEndTime >= _EndTime) And (_FTIn2 <> "" And _FTOut2 <> "")
                                        Try
                                            If _dttimeLeave.Select("FTLeaveStartTime<='" & _StartTime & "' AND FTLeaveEndTime>='" & _EndTime & "' ").Length <= 0 Then
                                                '_FNWorkMinute = 60
                        ...
                        Case 9, 10, 11, 12, 13
                            ...
                            If _FNHSysEmpID = 2111930283 Then
                                Dim X As String
                                X = "99999"
                            End If
                            ...
                            If _FNOTRequestMin > 0 Then
                                ...
                                Else
                                    If _FTState = "1" Then
                                        ...
                                        Select Case True
                                            ...
                                            Case (_StartTime >= _FTIn3 And _FTOut3 < _EndTime And _FTIn3 <> "" And _FTOut3 <> "" And _FTIn3 > _FTOut2 And _StartTime < _FTOut3 And _EndTime <= _FTEndTime)
                                                ...
                                                ''best202207
                                            Case (_StartTime >= _FTIn3 And _FTOut3 < _EndTime And _FTIn3 <> "" And _FTOut3 <> "" And _FTIn3 > _FTOut2 And _StartTime < _FTOut3 And (_FTEndTime >= _EndTime Or (_FTEndTime <= _EndTime And _FTEndTime >= _StartTime)))
                                    ...
                                    Else
                                        If _FNHSysEmpID = 2111931086 Then
                                            Dim X As String
                                            X = "99999"
                                        End If
                                        Select Case True
            ...
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FDScanDate,FTUnitSectCode")
                ...
                If _LineIncentiveAmt > 0 Then
                    ...
                    If _TotalCountStyle > 1 Then
                        _PriceCost = Double.Parse(Format(_SumPrice / _TotalCountStyle, "0.00"))
                        _FNSam = Double.Parse(Format(_SumSam / _TotalCountStyle, "0.00"))
                    Else
...
#Region "Process Calculate"
    ...
    Private Function CalculateSewing(Spls As HI.TL.SplashScreen) As Boolean
        ...
        Dim _IncentiveOTQty As Integer = 0
        Dim _LineIncentiveAmt As Double = 0
        Dim _FNMultiple As Double = 0
        Dim _LineIncentiveAmtOT As Double = 0
        Dim _EmpIncentiveAmt As Double = 0
        Dim _FNEFFOverPer As Double = 0
        ...
        Dim FNAmtEffTopUp_Max As Double = 0

        Dim FNTimeMinutes_Max As Double = 0
        Dim FNOT1Minutes_Max As Double = 0
        Dim FNTimeTT_Max As Double = 0

        Dim FNInsuranceNet As Double = 0
        Dim FNPerformance As Double = 0
        Dim FNIncentiveOverMax As Double = 0



        Dim _FNLateNormalCut As Integer = 0
        Dim _FNLeaveTotalPayMinute As Integer = 0
        Dim _FNLeaveTotalNotPayMinute As Integer = 0
        ...
        Try
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FTUnitSectCode")
                ...
                FNAmtEffTopUp_Max = Val(R!FNTopUp.ToString)

                FNTimeMinutes_Max = Val(R!FNTimeMinutes_Max.ToString)
                FNOT1Minutes_Max = Val(R!FNOT1Minutes_Max.ToString)
                FNTimeTT_Max = FNTimeMinutes_Max + FNOT1Minutes_Max
                FNInsuranceNet = Val(R!FNInsuranceNet.ToString)

                FNPerformance = Val(R!FNPerformance.ToString)
                FNIncentiveOverMax = Val(R!FNIncentiveOverMax.ToString)
                ...
                If Not HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR) Then
                ...
                For Each Rt As DataRow In _dtemptime.Rows
                    ...
                    _Latenomalcp = Integer.Parse(Val(Rt!FNLateNormalmin.ToString))
                    _FNLateNormalCut = Integer.Parse(Val(Rt!FNLateNormalCut.ToString))
                    _FNLeaveTotalPayMinute = Integer.Parse(Val(Rt!FNLeaveTotalPayMinute.ToString))
                    ...
                    _FNNetAmt = _FNAmtNormal + _FNAmtOT
                    Dim worktimeOTMin As Integer = 0
                    worktimeOTMin = Integer.Parse(Val(Rt!FNWorkingOTMin.ToString))
                    ...
                    '_Qry &= vbCrLf & ", FNEffPer = " & Double.Parse(Val(R!FNEFFPer.ToString))
                    ...
                    _Qry &= vbCrLf & ", FNSalary, FNAmtNormal, FNAmtOT, FNNetAmt, FNAmtOldIncentive, FNAmtOTOldIncentive,"
                    _Qry &= vbCrLf & "  FNNetAmtOldIncentive, FNAmtNewIncentive, FNAmtOTNewIncentive, FNNetAmtNewIncentive,FNQAValue,FTInsuranceHour,FTInsuranceAmt ,FNEffPer"
                    _Qry &= vbCrLf & ")"
                    _Qry &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                    _Qry &= vbCrLf & " ," & HI.UL.ULDate.FormatDateDB & ""
                    _Qry &= vbCrLf & " ," & HI.UL.ULDate.FormatTimeDB & ""
                    ...
                    _Qry &= vbCrLf & " ," & _FixHour & ""
                    _Qry &= vbCrLf & " ,0"
                    _Qry &= vbCrLf & "," & Double.Parse(Val(R!FNEFFPer.ToString))
                ...
                _Qry &= vbCrLf & "  , FNCostIncentiveTotal, FNPerformance "
                ...
                _Qry &= vbCrLf & " ," & Double.Parse((R!FNCostIncentiveTotal.ToString)) & ""
                _Qry &= vbCrLf & " ," & Double.Parse((R!FNPerformance.ToString)) & ""
                ...
                _LineIncentiveAmt = Double.Parse(Val(R!FNTotalIncentive.ToString))
                _LineIncentiveAmtOT = Double.Parse(Val(R!FNTotalIncentiveOT.ToString))
                _TimeMaxperline = Double.Parse(Val(_dtTimeMaxPerline.Rows(0).Item("FNTotalTime").ToString))
                _TimeMaxperlineOT = Double.Parse(Val(_dtTimeMaxPerline.Rows(0).Item("FNTotalTimeOT").ToString))
                _FNMultiple = Double.Parse(Val(R!FNMultiple.ToString))
                _FNEFFOverPer = Double.Parse(Val(R!FNEFFOverPer.ToString))


                _Qry = "Select Top 1 FNRateLateCust"
                _Qry &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRIncentiveCostEFF EF WITH(NOLOCK) "
                _Qry &= vbCrLf & "where   right(EF.FTDate,4)+'/'+right(left(EF.FTDate,5),2) + '/' + left(EF.FTDate,2)  <='" & _CalDate & "'"
                _Qry &= vbCrLf & "and     right(EF.FTDateTo,4)+'/'+right(left(EF.FTDateTo,5),2) + '/' + left(EF.FTDateTo,2)   >='" & _CalDate & "'"
                Dim _dtReteIncentiveCfg As Double = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR)
                ...
                Dim FNAmtEffTopUp As Double = 0.0
                ...
                For Each Rt As DataRow In _dtemptime.Select("FTStateCal<>'1'  AND FNTotalWorkingMin>0 ", "FNTotalWorkingMin")
                    ...
                    If Rt!FNHSysEmpID.ToString = "1959680028" Or Rt!FNHSysEmpID.ToString = "1959680026" Then
                    ...
                    '' _EmpIncentiveAmt = ((_FNEFFOverPer * _FNMultiple) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) + Val(Rt!FNLateNormalMin.ToString))) - (Val(Rt!FNLateNormalMin.ToString) * _dtReteIncentiveCfg)



                    ''  _EmpIncentiveAmt = ((FNIncentiveOverMax / FNTimeTT_Max) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) + Val(Rt!FNLateNormalMin.ToString) - Val(Rt!FNLateNormalCut.ToString))) ''- (Val(Rt!FNLateNormalMin.ToString))
                    ''  FNAmtEffTopUp = ((FNAmtEffTopUp_Max / FNTimeTT_Max) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) + Val(Rt!FNLateNormalMin.ToString) - Val(Rt!FNLateNormalCut.ToString))) ''- (Val(Rt!FNLateNormalMin.ToString))

                    '' _EmpIncentiveAmt = ((FNIncentiveOverMax / FNTimeTT_Max) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) + Val(Rt!FNLateNormalMin.ToString))) ''- (Val(Rt!FNLateNormalMin.ToString))
                    '' FNAmtEffTopUp = ((FNAmtEffTopUp_Max / FNTimeTT_Max) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) + Val(Rt!FNLateNormalMin.ToString))) ''- (Val(Rt!FNLateNormalMin.ToString))

                    _EmpIncentiveAmt = ((FNIncentiveOverMax / FNTimeTT_Max) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)))) ''- (Val(Rt!FNLateNormalMin.ToString))
                    FNAmtEffTopUp = ((FNAmtEffTopUp_Max / FNTimeTT_Max) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)))) ''- (Val(Rt!FNLateNormalMin.ToString))
                    ...
                    _Qry &= vbCrLf & "   , FNAmtEffTopUp =  " & (FNAmtEffTopUp) & ""

                    _Qry &= vbCrLf & "   , FNAmtEffTopUpMax =  " & (FNAmtEffTopUp_Max) & ""
                    _Qry &= vbCrLf & "   , FNIncentiveOverMax =  " & (FNIncentiveOverMax) & ""
                ...
                For Each Rt As DataRow In _dtemptime.Rows
                    ...
                    '_EmpIncentiveAmt = ((_FNEFFOverPer * _FNMultiple) * (Integer.Parse(Val(Rt!FNWorkingMin.ToString)) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)))) - (Val(Rt!FNLateNormalMin.ToString) * 0.05)
                    _EmpIncentiveAmt = ((_FNEFFOverPer * _FNMultiple) * ((Integer.Parse(Val(Rt!FNWorkingMin.ToString))) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) + Val(Rt!FNLateNormalMin.ToString))) - (Val(Rt!FNLateNormalMin.ToString) * _dtReteIncentiveCfg)
                    ...
                    _Qry &= vbCrLf & ",  Sum(FNAmtOldIncentive)  FNProNormal"
                    _Qry &= vbCrLf & ",0,0 AS FNProOther" '  Sum(FNAmtOTOldIncentive) AS FNAmtOTNewIncentive
                    _Qry &= vbCrLf & ", Sum(FNAmtOldIncentive)  + max (fnnetamt) as FNNetProAmt " 'Sum(FNNetAmtOldIncentive) AS FNNetAmtNewIncentive
                    ...
                    _Qry &= vbCrLf & " ," & Integer.Parse(Val(Rt!FNHSysEmpID.ToString)) & ""
                    _Qry &= vbCrLf & " ,'" & _CalDate & "'   "
                    _Qry &= vbCrLf & ", Sum(FNAmtEffTopUp) AS FNAmtEffTopUp "
                    _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Emp  AS X WITH(NOLOCK) "
                    _Qry &= vbCrLf & "  WHERE  FNHSysEmpID=" & Integer.Parse(Val(Rt!FNHSysEmpID.ToString)) & ""
                    _Qry &= vbCrLf & "       AND FTCalDate='" & _CalDate & "'"
                    HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                    ...
                    HI.HRCAL.Calculate.CalculateWageDaily_VT(HI.ST.UserInfo.UserName, Rt!FNHSysEmpID.ToString, _EmpTypeId.ToString(), _CalDate, _CalDate, _FNWorkDayInWeekBF)
            ...
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FDScanDate,FTUnitSectCode")
                ...
                For Each Rxt As DataRow In _dtsewstyle.Rows
                    ...
                    If Double.Parse(Val(Rxt!FNStateSewPack.ToString)) = 0 Then
                        _NetPrice = Double.Parse(Format(Double.Parse(Val(Rxt!FNPrice.ToString)) * Double.Parse(Val(Rxt!FNMultiple.ToString)), "0.00"))
                    Else
                        _NetPrice = Double.Parse(Format((Double.Parse(Val(Rxt!FNPrice.ToString)) - Double.Parse(Val(Rxt!FNDisPrice.ToString))) * Double.Parse(Val(Rxt!FNMultiple.ToString)), "0.00"))
                    End If
    ...
    Private Function CalculateChiefBU(Spls As HI.TL.SplashScreen) As Boolean
        Try
            ...
            For Each R As DataRow In _oDt.Select("FTSelect ='1' ", "FDScanDateOrg , FTEmpCode , FDScanDateOrg ")
                ...
                HI.HRCAL.Calculate.CalculateWageDaily_CD(HI.ST.UserInfo.UserName, R!FNHSysEmpID.ToString, _EmpTypeId.ToString(), _CalDate, _CalDate, _FNWorkDayInWeekBF)
        ...
        For Each _UnitSect As Integer In grpfodata
            ...
            For Each _Date As String In grpfodataDate
                ...
                For Each Rt As DataRow In _dtemptime.Select("FTStateCal<>'1'  AND FNTotalWorkingMin>0 ", "FNTotalWorkingMin")
                    ...
                    HI.HRCAL.Calculate.CalculateWageDaily_CD(HI.ST.UserInfo.UserName, Rt!FNHSysEmpID.ToString, _EmpTypeId.ToString(), _Date, _Date, _FNWorkDayInWeekBF)
        ...
        Try
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FTUnitSectCode")
                ...
                For Each Rt As DataRow In _dtemptime.Rows
                    ...
                    HI.HRCAL.Calculate.CalculateWageDaily_CD(HI.ST.UserInfo.UserName, Rt!FNHSysEmpID.ToString, _EmpTypeId.ToString(), _CalDate, _CalDate, _FNWorkDayInWeekBF)
    ...
    Private Function DeleteData(Spls As HI.TL.SplashScreen) As Boolean
        ...
        Try
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FDScanDateOrg,FTUnitSectCode")
                ...
                _Qry &= vbCrLf & " DELETE  A  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTWageDaily_Bonus AS A  "
                _Qry &= vbCrLf & " INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Emp  AS X WITH(NOLOCK) ON A.FTDateTrans=X.FTCalDate AND A.FNHSysEmpID =X.FNHSysEmpID "
                _Qry &= vbCrLf & " WHERE  (X.FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (X.FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                ...
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Emp "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Level "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Style "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Style_Hour "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Style_Hour_Level "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Emp_Deduct "
                _Qry &= vbCrLf & " WHERE  (FTCalDate = '" & HI.UL.ULDate.ConvertEnDB(R!FDScanDateOrg.ToString) & "') "
                _Qry &= vbCrLf & "   AND (FNHSysUnitSectId = " & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & ")"
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                _Qry &= vbCrLf & "--"
                _Qry &= vbCrLf & "DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Emp_Wring_Detail "
                _Qry &= vbCrLf & " WHERE FNHSysUnitSectId  =" & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & " "
                _Qry &= vbCrLf & " AND FTCalDate='" & _CalDate & "'  "
                'HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                '_Qry &= vbCrLf & "--"
                '_Qry &= vbCrLf & "DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_EmdPrintLaserOther "
                '_Qry &= vbCrLf & " WHERE FNHSysUnitSectId  =" & Integer.Parse(Val(R!FNHSysUnitSectId.ToString)) & " "
                '_Qry &= vbCrLf & " AND FTCalDate='" & _CalDate & "'  "
                HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
    ...
    Private Sub LoadDataInfo()
        Try
            ...
            Do While _SDate <= _EDate
                ...
                _Qry = "Exec [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.[SP_CALCULATE_STATIC_INCENTIVE_EFF_BYCMP_VT]  '" & _SDate & "','" & _SDate & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysUnitSectId.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysUnitSectIdTo.Text) & "' , " & Val(Me.FNHSysCmpId.Properties.Tag.ToString)
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        If Not (Me.ogc.DataSource Is Nothing) Then
            With CType(Me.ogc.DataSource, DataTable)
                ...
                Dim results = (From row In .Select("FTSelect='1'").AsEnumerable()
                               Select FNHSysUnitSectId = row.Field(Of Integer)("FNHSysUnitSectId"), FDScanDateOrg = row.Field(Of String)("FDScanDateOrg")
               ).Distinct().ToList()
                ...
                If .Select("FTSelect='1'").Length > 0 Then
                    ...
                    For i As Integer = 0 To results.Count - 1
                        Dim _FM As String = ""
                        ' _FM = " {TCNMCmp.FNHSysCmpId}=" & Val(Me.FNHSysCmpId.Properties.Tag)
                        _FM = "  {THRTIncentive.FNHSysUnitSectId}=" & Val(results(i).FNHSysUnitSectId) & " AND {THRTIncentive.FTCalDate}='" & HI.UL.ULDate.ConvertEnDB(results(i).FDScanDateOrg.ToString) & "'"
                        ...
                        With New HI.RP.Report
                            ...
                            .ReportName = "RptIncentiveEff_VT.rpt"
