...
Public Class wCalculateIncentive_CD
        ...
        Try
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FTUnitSectCode")
                ...
                For Each Rt As DataRow In _dtemptime.Select("FTStateCal<>'1'  AND FNTotalWorkingMin>0 ", "FNTotalWorkingMin")
                    ...
                    'End If
                    'HI.MG.ShowMsg.mInfo("_TimeMaxperline" & _TimeMaxperline, 33375, "test")
                    'MsgBox("time", Rt!FNWorkingMin.ToString & " " & Rt!FNWorkingOTMin.ToString)
                    'MsgBox("_LineIncentiveAmt", _LineIncentiveAmt)

                    _EmpIncentiveAmt = CDbl(Format(((((Integer.Parse(Val(Rt!FNWorkingMin.ToString)) + Integer.Parse(Val(Rt!FNWorkingOTMin.ToString))) * 100) / (_TimeMaxperline + _TimeMaxperlineOT)) * _LineIncentiveAmt) / 100, "0.0000"))
    ...
    Private Sub LoadDataInfo()
        Try
            ...
            Do While _SDate <= _EDate
                ...
                _Qry = "Exec [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.[SP_CALCULATE_STATIC_INCENTIVE_EFF]  '" & _SDate & "','" & _SDate & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysUnitSectId.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysUnitSectIdTo.Text) & "'"
