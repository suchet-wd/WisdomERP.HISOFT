                                ...
                                If _FTState = "1" Then
                                    ...
                                    Select Case True
                                        ...
                                        Case (_StartTime > _FTIn3 And _FTOut3 < _EndTime And _FTIn3 <> "" And _FTOut3 <> "" And _FTIn3 > _FTOut2 And _StartTime < _FTOut3)
                                            ...
                                            End Try
                                                  'best Add
                                        Case (_FTOut2 = _FTIn3 And _FTOut2 <> "" And _FTIn3 <> "" And _FNHour = 9)
            ...
            For Each R As DataRow In _dt.Select("FTSelect='1'", "FTUnitSectCode")
                ...
                For Each Rt As DataRow In _dtemptime.Rows
                    ...
                    If _TotalTimeHR >= 480 Then
                        _FNAmtNormal = _Salary
                    Else
                        _FNAmtNormal = Double.Parse(Format((_Salary / 480) * _TotalTimeHR, "0.0000"))
                    End If

                    _FNAmtOT = Double.Parse(Format(((_Salary / 480) * 1.5) * _TotalTimeOTHR, "0.0000"))
                ...
                For Each Rt As DataRow In _dtemptime.Select("FTStateCal<>'1'  AND FNTotalWorkingMin>0 ", "FNTotalWorkingMin")
                    ...
                    'End If
                    _EmpIncentiveAmt = CDbl(Format((((Integer.Parse(Val(Rt!FNWorkingMin.ToString)) * 100) / _TimeMaxperline) * _LineIncentiveAmt) / 100, "0.0000"))
                    If _TimeMaxperlineOT > 0 Then
                        _FNAmtOT = CDbl(Format((((Integer.Parse(Val(Rt!FNWorkingOTMin.ToString)) * 100) / _TimeMaxperlineOT) * _LineIncentiveAmtOT) / 100, "0.0000"))
    ...
    Private Sub LoadDataInfo()
        ...
        Try
            ...
            For Each R As DataRow In _dtcmpSelect.Rows
                ...
                _Qry &= vbCrLf & "  AND (FNHSysCmpId  =" & Val(R!FNHSysCmpId) & " OR ISNULL(FNHSysCmpId,0) = 0) "


                _Qry &= vbCrLf & "  ORDER BY FTUnitSectCode"
                dtline = GetSewFGData(_Qry, _ConnectString) 'HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MASTER)
                ...
                For Each Rx As DataRow In dtline.Rows
                    ...
                    _Qry = "Exec [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.[sp_getEffByline]  '" & _SDate & "','" & _EDate & "', " & Val(Rx!FNHSysUnitSectId.ToString)
                    _Qry &= vbCrLf & "," & Val(R!FNHSysCmpId)
                    _Qry &= vbCrLf & ", " & Val(Me.FNHSysStyleId.Properties.Tag)
            ...
            Dim views As New DataView(_Dt)
            Dim colums() As String = {"FNTotalMinPack", "FTUnitSectCode", "FNTotalMin", "EmpTotal", "EmpTotalSw", "EmpTotalSwActual", "EmpTotalActual", "FNMoveInTime", "FNMoveOutTime"}

            Try
                Dim _distinctdt As DataTable = views.ToTable(True, colums)

                Me.ogcEmpTime.DataSource = _distinctdt.Copy
            Catch ex As Exception
...
#End Region
    ...
    Private TotalEffper As Double = 0
    Private sumTotalTime As Integer = 0
    Private subQty As Integer = 0
    Private samtotal As Double = 0
    Private totalcale As Double = 0
    Private _SectCode As String = ""

    Private Sub ogv_CustomSummaryCalculate(sender As Object, e As CustomSummaryEventArgs) Handles ogv.CustomSummaryCalculate
        Try
            Dim view As GridView = DirectCast(sender, GridView)
            'Dim sumId As Double = Double.Parse(DirectCast(e.Item, GridSummaryItem).Tag)




            Dim item As GridSummaryItem = TryCast(e.Item, GridSummaryItem)
            If item.FieldName = "EffPerLine" Then
                ...
                If (e.SummaryProcess = CustomSummaryProcess.Calculate) Then
                    ...
                    TotalEffper += +e.FieldValue
                    sumTotalTime += +Val(view.GetRowCellValue(e.RowHandle, "TimeByStyle"))
                    subQty = Val(view.GetRowCellValue(e.RowHandle, "FNQuantity"))
                    subQty += +Val(view.GetRowCellValue(e.RowHandle, "FNQuantityOT"))
                    samtotal += +Val(view.GetRowCellValue(e.RowHandle, "FNSam")) * (subQty)


                End If
                If (e.SummaryProcess = CustomSummaryProcess.Finalize) Then

                    TotalEffper = (samtotal / sumTotalTime) * 100
                    Me.effper_lbl.Text = TotalEffper.ToString("0.00")
                    e.TotalValue = TotalEffper
            ...
            Else
                ...
                If (e.SummaryProcess = CustomSummaryProcess.Calculate) Then
                    ...
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle, "FTUnitSectCode")
                    If _SectCode <> val1 Then
                        totalcale += +e.FieldValue
                    End If
                    _SectCode = val1
    ...
    Private Sub ogv_CellMerge(sender As Object, e As CellMergeEventArgs) Handles ogv.CellMerge
        Try

            If FNReportGridMergeFormat.SelectedIndex = 0 Then
                If (e.Column.FieldName = "FNTotalMin") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
                    e.Handled = True
                End If

                If (e.Column.FieldName = "EmpWorkingHour") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
                ...
                If (e.Column.FieldName = "FNTotalMinPack") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
                ...
                If (e.Column.FieldName = "EmpTotal") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
                ...
                If (e.Column.FieldName = "EmpTotalActual") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
                ...
                If (e.Column.FieldName = "EmpTotalSw") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
                    e.Handled = True
                End If
                If (e.Column.FieldName = "EmpTotalSwActual") Then
                    Dim view As GridView = CType(sender, GridView)
                    Dim val1 As String = view.GetRowCellValue(e.RowHandle1, "FTUnitSectCode")
                    Dim val2 As String = view.GetRowCellValue(e.RowHandle2, "FTUnitSectCode")
                    e.Merge = (val1 = val2)
    ...
    Private Sub ogvEmpTime_RowStyle(sender As Object, e As RowStyleEventArgs) Handles ogvEmpTime.RowStyle
