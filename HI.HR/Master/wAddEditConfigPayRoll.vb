...
Public Class wAddEditConfigPayRoll
...
#Region "General"
    Private Sub Form_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            If Me.TableName <> "" Then
                ...
                If _KeyFiled(0).FiledValue.ToString <> "" And _KeyFiled(1).FiledValue.ToString <> "" Then
    ...
    Private Sub ocmaddnew_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmaddnew.Click
        If VerrifyData() Then
            ...
            If HI.Conn.SQLConn.ExecuteNonQuery(_Str, Conn.DB.DataBaseName.DB_SYSTEM) Then
                ...
                Dim _FTEmpType As String = ""
                Dim _FTPayYear As String = ""
                Dim _FTPayTerm As String = ""

                For Each Obj As Object In Me.Controls.Find("FNHSysEmpTypeId", True)
                    Try
                        With CType(Obj, DevExpress.XtraEditors.ButtonEdit)
                            _FTEmpType = "" & .Properties.Tag.ToString
                        End With
                    Catch ex As Exception
                    End Try
                    Exit For
                Next
                For Each Obj As Object In Me.Controls.Find("FTPayTerm", True)
                    _FTPayTerm = Obj.text
                    Exit For
                Next

                For Each Obj As Object In Me.Controls.Find("FTPayYear", True)
                    _FTPayYear = Obj.text
                    Exit For
                Next

                _Str = " SELECT  TOP 1  FTPayTerm "
                _Str &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMCfgPayHD AS A WITH(NOLOCK)"
                _Str &= vbCrLf & "  WHERE FNHSysEmpTypeId=" & Integer.Parse(Val(_FTEmpType)) & " "

                If HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_HR, "") = "" Then
                    _Str = " INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMCfgPayHD"
                    _Str &= vbCrLf & " (FTInsUser, FDInsDate, FTInsTime, FTPayTerm, FTPayYear, FNHSysEmpTypeId) "
                    _Str &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                    _Str &= vbCrLf & " ," & HI.UL.ULDate.FormatDateDB & " "
                    _Str &= vbCrLf & " ," & HI.UL.ULDate.FormatTimeDB & " "
                    _Str &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(_FTPayTerm) & "'"
                    _Str &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(_FTPayYear) & "'"
                    _Str &= vbCrLf & " ," & Integer.Parse(Val(_FTEmpType)) & " "
    ...
    Private Function VerrifyData() As Boolean
        ...
        If _StrCheckDupl <> "" Then
            ...
            _Str &= vbCrLf & "  WHERE    " & _StrCheckDupl
    ...
    Private Sub ocmedit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmedit.Click
        If VerrifyData() Then
            ...
            For I As Integer = 0 To _KeyFiled.ToArray.Count - 1
                ...
                If _KeyFiled(I).FiledName.ToString.ToUpper = "FNHSysEmpTypeId".ToString.ToUpper Then
                    _value = HI.Conn.SQLConn.GetField("SELECt TOP 1 FNHSysEmpTypeId FROM THRMEmpType WITH (NOLOCK) WHERE FTEmpTypeCode='" & HI.UL.ULF.rpQuoted(_value) & "' AND FNHSysCmpId= " & Val(HI.ST.SysInfo.CmpID), Conn.DB.DataBaseName.DB_MASTER, "0")
                End If
    ...
    Private Sub ocmdelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmdelete.Click
        ...
        If _StrWhere = "" Then Exit Sub
        Dim _Str As String = " Delete From " & Me.TableName & " " & _StrWhere
    ...
    Private Sub LoadDataEdit(ByVal HSysId As String)
        ...
        For I As Integer = 0 To _KeyFiled.ToArray.Count - 1
            _value = _KeyFiled(I).FiledValue.ToString
            If _KeyFiled(I).FiledName.ToString.ToUpper = "FNHSysEmpTypeId".ToString.ToUpper Then
                _value = HI.Conn.SQLConn.GetField("SELECt TOP 1 FNHSysEmpTypeId FROM THRMEmpType WITH (NOLOCK) WHERE FTEmpTypeCode='" & HI.UL.ULF.rpQuoted(_value) & "' AND FNHSysCmpId=" & HI.ST.SysInfo.CmpID, Conn.DB.DataBaseName.DB_MASTER, "0")
            End If
            If _StrWhere = "" Then
                _StrWhere = "  WHERE  " & _KeyFiled(I).FiledName.ToString & "='" & HI.UL.ULF.rpQuoted(_value) & "' "
            Else
