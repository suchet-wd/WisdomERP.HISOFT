Public Class wEmployeeLeaveVacationPayBack_VN
...
#Region "Procedure"
    ...
    Private Shared ReadOnly Property LoadConfigAllowanceProbation() As System.Data.DataTable
        Get
            ...
            Return HI.Conn.SQLConn.GetDataTable(sSQL, HI.Conn.DB.DataBaseName.DB_HR)
    ...
    Private Function LoadDataInfo(Spls As HI.TL.SplashScreen) As Boolean
        ...
        _Qry &= vbCrLf & ", (CASE WHEN ISNULL(M.FDDateProbation,'') <='" & _FTYear & "/12/31" & "' THEN (CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),0) "
        _Qry &= vbCrLf & " ELSE (CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1'  THEN  Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 )) ELSE Convert(numeric(18,2),0)  END) END ) END ) AS  FNHarmful "
       
        _Qry &= vbCrLf & ", (CASE WHEN ISNULL(M.FDDateProbation,'') <='" & _FTYear & "/12/31" & "' THEN (CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),0) "
        _Qry &= vbCrLf & " ELSE"
        _Qry &= vbCrLf & " ( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1' AND  ISNULL(ET.FTStatePaySkill,'') ='1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(Convert(numeric(18,2),(((M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 ))) * " & FNSkillRate & ") /100.0 ))  )) "
        _Qry &= vbCrLf & "  ELSE "
        _Qry &= vbCrLf & " ( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),0) "
        _Qry &= vbCrLf & " ELSE"
        _Qry &= vbCrLf & "  (( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') ='1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),( Convert(numeric(18,2),((M.FNSalary * " & FNSkillRate & ") /100.0 )) )) "
        _Qry &= vbCrLf & "  ELSE  Convert(numeric(18,2),0)  END))  END ) END )"
        _Qry &= vbCrLf & "END) ELSE Convert(numeric(18,2),0) END) AS FNSkill"

        _Qry &= vbCrLf & ", (CASE WHEN ISNULL(M.FDDateProbation,'') <='" & _FTYear & "/12/31" & "' THEN (CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),M.FNSalary) "
        _Qry &= vbCrLf & " ELSE"
        _Qry &= vbCrLf & " ( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1' AND  ISNULL(ET.FTStatePaySkill,'') ='1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 )) + Convert(numeric(18,2),(((M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 ))) * " & FNSkillRate & ") /100.0 ))  )) "
        _Qry &= vbCrLf & "  ELSE "
        _Qry &= vbCrLf & " ( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 )) )) "
        _Qry &= vbCrLf & " ELSE"
        _Qry &= vbCrLf & "  (( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') ='1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNSkillRate & ") /100.0 )) )) "
        _Qry &= vbCrLf & "  ELSE  Convert(numeric(18,2),M.FNSalary)  END))  END ) END )"
        _Qry &= vbCrLf & "END) ELSE Convert(numeric(18,2),M.FNSalary) END) AS FNMinimumSalary"
        ...
        '_Qry &= vbCrLf & " ,MST.FNEmpVacation "
        '_Qry &= vbCrLf & " ,CONVERT(numeric(18, 2), MST.FNLeaveVacation / 480.00) AS FNEmpVacationUsed"
        '_Qry &= vbCrLf & ", (MST.FNEmpVacation - (CONVERT(numeric(18, 2), MST.FNLeaveVacation / 480.00) )) AS FNEmpVacationBal"
        ...
        _Qry &= vbCrLf & ", Convert(numeric(18,2),(MST.FNEmpVacation - (CONVERT(numeric(18, 2), MST.FNLeaveVacation / 480.00) )) * "
        _Qry &= vbCrLf & " (CASE WHEN ISNULL(M.FDDateProbation,'') <='" & _FTYear & "/12/31" & "' THEN (CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,5),M.FNSalary/26) "
        _Qry &= vbCrLf & " ELSE"
        _Qry &= vbCrLf & " ( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1' AND  ISNULL(ET.FTStatePaySkill,'') ='1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 )) + Convert(numeric(18,2),(((M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 ))) * " & FNSkillRate & ") /100.0 ))  )/26) "
        _Qry &= vbCrLf & "  ELSE "
        _Qry &= vbCrLf & " ( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') ='1' AND  ISNULL(ET.FTStatePaySkill,'') <>'1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNHarmfulRate & ") /100.0 )) )/26) "
        _Qry &= vbCrLf & " ELSE"
        _Qry &= vbCrLf & "  (( CASE WHEN ISNULL(ET.FTStatePayHarmful,'') <>'1' AND  ISNULL(ET.FTStatePaySkill,'') ='1' THEN "
        _Qry &= vbCrLf & " Convert(numeric(18,2),(M.FNSalary + Convert(numeric(18,2),((M.FNSalary * " & FNSkillRate & ") /100.0 )) )/26) "
        _Qry &= vbCrLf & "  ELSE  Convert(numeric(18,2),M.FNSalary/26)  END))  END ) END )"
        _Qry &= vbCrLf & "END) ELSE Convert(numeric(18,2),M.FNSalary/26) END)"
        _Qry &= vbCrLf & ") AS FNEmpVacationRetAmt"
