...
#Region "MAIN PROC"
    ...
    '            _Qry &= vbCrLf & "Where (FTFormName = N'wEmployeeOutSite') AND (FTObjName = N'ocmapprove'))) AND (P.FTUserName = '" & HI.ST.UserInfo.UserName & "')"
    ...
    '            _Qry = "SELECT TOP 1  Isnull( D.FTSendApproveState,'0') as FTSendApproveState , Isnull(D.FTApproveState,'0') AS FTApproveState  , Isnull(D.FTUpdUser,D.FTInsUser)  as FTInsUser"
    '            _Qry &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTEmpployeeWorkOutSite As D  With(NOLOCK)"
    ...
    '            Select Case True
    '                Case _SendApp And Not (_Approve) And _User.ToUpper = HI.ST.UserInfo.UserName.ToUpper
    '                    Return True
    '                Case _SendApp And Not (_Approve) And _User.ToUpper = HI.ST.UserInfo.UserName.ToUpper
    '                    Return True
    '                Case _SendApp And _Approve And _Premis
    '                    Return True
    '                Case _SendApp And _User.ToUpper = HI.ST.UserInfo.UserName.ToUpper And Not (_Approve)
    '                    Return True
...
#Region " Procedure "
    ...
    '            _Qry &= vbCrLf & ", FTStartTime, FTEndTime, FNTotalHour, FNTotalMin, FTNote,FNTotalDay"
    '            Select Case True
    '                Case Not (ocmSendApprove.Visible) And Not (ocmapprove.Visible) And Not (_StateMngApp)
    ...
    '            _Qry &= vbCrLf & ",FNTotalDay=" & FTNetDay.Value & " "
    '            _Qry &= vbCrLf & " WHERE FNHSysEmpID = " & Val(FNHSysEmpId.Properties.Tag.ToString) & ""
    '            _Qry &= vbCrLf & " AND FTStartDate = '" & HI.UL.ULDate.ConvertEnDB(Me.FTStartDate.Text) & "'"
    '            _Qry &= vbCrLf & " AND FTEndDate = '" & HI.UL.ULDate.ConvertEnDB(Me.FTEndDate.Text) & "'"
    '            _Qry &= vbCrLf & " AND FTStartTime = '" & Me.FTStartTime.Text & "'"
    ...
    '                            _Pass = True
    '                        Else
    '                            HI.MG.ShowMsg.mInvalidData("", 1104100001, Me.Text)
    '                            FTEndDate.Focus()
    '                        End If
    '                    Else
...
#Region "General"
    ...
    Private Sub LoadHistory()
        ...
        _Qry &= vbCrLf & ",FTSendApproveTime"
        _Qry &= vbCrLf & ",FTSendApproveBy"
        _Qry &= vbCrLf & ",isnull(FTMngApproveState,0) AS FTMngApproveState"
        _Qry &= vbCrLf & " , Convert(nvarchar(10) ,convert(datetime,FDMngApproveDate) , 103) AS FDMngApproveDate "
        _Qry &= vbCrLf & ",FTMngApproveTime"
        _Qry &= vbCrLf & ",FTMngApproveBy"
        _Qry &= vbCrLf & ",isnull(FTDirApproveState,0) AS FTDirApproveState"
        _Qry &= vbCrLf & " , Convert(nvarchar(10) ,convert(datetime,FDDirApproveDate) , 103) AS FDDirApproveDate "
        _Qry &= vbCrLf & ",FTDirApproveTime"
        _Qry &= vbCrLf & ",FTDirApproveBy"
    ...
    Private Sub FNHSysEmpId_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNHSysEmpId.EditValueChanged
        ...
        Call LoadHistory()
        Call FNOperating_SelectedIndexChanged(sender, e)
    ...
    Private Sub FTETime_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FTEndTime.EditValueChanged, FTEndDate.EditValueChanged
        If (_Prepare) Then Exit Sub
    ...
    Private Sub ogv_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles ogv.DoubleClick
        ...
        With ogv
            ...
            If Me.FTStartTime.Text = Me.FTIn1.Text And Me.FTEndTime.Text = Me.FTOut2.Text Then
                Me.FNOperating.SelectedIndex = 0
            ElseIf Me.FTStartTime.Text = Me.FTIn1.Text And Me.FTEndTime.Text = Me.FTOut1.Text Then
                Me.FNOperating.SelectedIndex = 1
            ElseIf Me.FTStartTime.Text = Me.FTIn2.Text And Me.FTEndTime.Text = Me.FTOut2.Text Then
                Me.FNOperating.SelectedIndex = 2
            ElseIf (Me.FTStartTime.Text <> Me.FTIn1.Text And FTStartTime.Text <> Me.FTIn2.Text) Or (Me.FTEndTime.Text <> Me.FTIn2.Text And Me.FTEndTime.Text <> Me.FTOut2.Text) Then
                Me.FNOperating.SelectedIndex = 3
            End If
            Me.FTRemark.Focus()
            ...
            If Me.FNHSysEmpId.Text <> "" Then
                ...
                Select Case Me.FNOperating.SelectedIndex
                    Case 0
                        ...
                        'Me.FTStartTime.Text = "08:00" : Me.FTEndTime.Text = "17:00"
                        Me.FTStartTime.Text = Me.FTIn1.Text : Me.FTEndTime.Text = Me.FTOut2.Text

                    Case 1
                        Me.FTStartTime.ReadOnly = True : Me.FTEndTime.ReadOnly = True
                        Me.FTStartTime.BackColor = Drawing.Color.LightCyan : Me.FTEndTime.BackColor = Drawing.Color.LightCyan
                        ' Me.FTStartTime.Text = "08:00" : Me.FTEndTime.Text = "12:00"
                        Me.FTStartTime.Text = Me.FTIn1.Text : Me.FTEndTime.Text = Me.FTOut1.Text
                    Case 2
                        Me.FTStartTime.ReadOnly = True : Me.FTEndTime.ReadOnly = True
                        Me.FTStartTime.BackColor = Drawing.Color.LightCyan : Me.FTEndTime.BackColor = Drawing.Color.LightCyan
                        ' Me.FTStartTime.Text = "13:00" : Me.FTEndTime.Text = "17:00"
                        Me.FTStartTime.Text = Me.FTIn2.Text : Me.FTEndTime.Text = Me.FTOut2.Text
                    Case 3
    ...
    Private Function Approve() As Boolean
        Try
            ...
            _Cmd &= vbCrLf & ", FTApproveBy='" & HI.ST.UserInfo.UserName & "'"
