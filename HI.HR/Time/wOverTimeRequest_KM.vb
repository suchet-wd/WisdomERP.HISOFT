Public Class wOverTimeRequest_KM
...
#Region " Procedure "
                    ...
                    If HI.HRCAL.Time.CheckClosePeriod(FTDateRequest.Text, 0, Integer.Parse(Val(R!FNHSysEmpTypeId.ToString))) = False Then
                        ...
                        If Not (_FoundOver) Then
                            ...
                            If HI.Conn.SQLConn.GetFieldOnBeginTrans(_Qry, Conn.DB.DataBaseName.DB_HR, "") <> "" Then
                                ...
                                _Qry &= vbCrLf & " ,FTOtMIn='" & Me.FTOtMIn.Text & "' "
                                _Qry &= vbCrLf & " ,FTOtMOut='" & Me.FTOtMOut.Text & "' "
                                _Qry &= vbCrLf & " WHERE FNHSysEmpID = " & Val(R!FNHSysEmpID) & ""
                                _Qry &= vbCrLf & " AND FTDateRequest = '" & HI.UL.ULDate.ConvertEnDB(Me.FTDateRequest.Text) & "'"
                            ...
                            Else
                                ...
                                _Qry &= vbCrLf & ",FTOtIn3,FTOtOut3,FNOtNetTime,FNOtNetTimeMinute,FTStateDaily,FTOtMIn,FTOtMOut )  "
                                ...
                                _Qry &= vbCrLf & " ,'" & Me.FTOtMIn.Text & "','" & Me.FTOtMOut.Text & "' "
            ...
            If Not (ogc.DataSource Is Nothing) Then
                ...
                If CType(ogc.DataSource, DataTable).Rows.Count > 0 Then
                    If CType(ogc.DataSource, DataTable).Select("FTSelect='1'").Length > 0 Then
                        ...
                        If Me.FTOtMIn.Text = Me.FTOtMOut.Text Then
                        ...
                        If (Me.otba1starttime.Text <> "" And Me.otba1endtime.Text <> "") Or
                                (Me.otba2starttime.Text <> "" And Me.otba2endtime.Text <> "") Or (Me.FTOtMIn.Text <> "" And Me.FTOtMOut.Text <> "") Or (Me.FTStateDaily.Checked) Then
    ...
    Private Sub LoadDataInfo()
        ...
        _Qry &= vbCrLf & ",ISNULL(OTR.FTOtMIn,'') AS FTOtMIn "
        _Qry &= vbCrLf & ",ISNULL(OTR.FTOtMOut,'') AS FTOtMOut "
        ...
        _Qry &= vbCrLf & ",ISNULL(OT.FTOtMIn,'') AS FTOtMIn "
        _Qry &= vbCrLf & ",ISNULL(OT.FTOtMOut,'') AS FTOtMOut "
...
#Region "General"
    ...
    Private Sub time_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles otba1starttime.EditValueChanged,
        ...
        otba2endtime.EditValueChanged, FTOtMIn.EditValueChanged, FTOtMOut.EditValueChanged
        ...
        Select Case sender.name.ToString.ToUpper
            ...
            Case "otba1starttime".ToUpper, "otba1endtime".ToUpper,
                 ...
                 "FTOtMIn".ToUpper, "FTOtMOut".ToUpper

                Dim _T0 As Integer = 0
                Dim _T1 As Integer = 0
                Dim _T2 As Integer = 0
                Dim _Res As Integer = 0

                '------------------------OT เช้าก่อนเข้างาน -------------------------------
                If FTOtMIn.Text <> "" And FTOtMOut.Text <> "" Then
                    If FTOtMIn.Text > FTOtMOut.Text Then
                        _T0 = DateDiff(DateInterval.Minute, CDate(Me.ActualDate & "  " & FTOtMIn.Text), CDate(Me.ActualNextDate & "  " & FTOtMOut.Text))
                    Else
                        _T0 = DateDiff(DateInterval.Minute, CDate(Me.ActualDate & "  " & FTOtMIn.Text), CDate(Me.ActualDate & "  " & FTOtMOut.Text))
                    End If
                End If
                '------------------------OT เช้าก่อนเข้างาน -------------------------------
                ...
                ocetotaltimea.Value = _T1 + _T2 + _T0
                rest2.Value = _Res
        End Select
