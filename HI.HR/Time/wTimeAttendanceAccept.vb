...
Public Class wTimeAttendanceAccept
...
#Region "Procedure"
    ...
    Private Sub LoadData(Optional StateTimeError As Boolean = False)
        ...
        _Qry = " SELECT '0' AS FTSelect, T.FNHSysEmpID, M.FTEmpCode"
        ...
        _Qry &= vbCrLf & "  AND  ISNULL(FTStateAccept,'0') <>'1' "
...
#End Region
    ...
    Private Sub ocmsave_Click(sender As Object, e As EventArgs) Handles ogctime.Click
    ...
    Private Function SaveData() As Boolean
        ...
        Dim _Qry As String
        CType(ogctime.DataSource, DataTable).AcceptChanges()
        Dim _Dt As DataTable = CType(ogctime.DataSource, DataTable)
        Try
            ...
            For Each R As DataRow In _Dt.Select("FTSelect='1'")
                ...
                If R!FTSelect.ToString = "1" Then
                    ...
                    _Qry &= vbCrLf & "  SET  "
                    _Qry &= vbCrLf & " FTStateAccept='1'"
                    _Qry &= vbCrLf & " ,FTStateAcceptDate=" & HI.UL.ULDate.FormatDateDB & ""
                    _Qry &= vbCrLf & " ,FTStateAcceptTime=" & HI.UL.ULDate.FormatTimeDB & ""
                    _Qry &= vbCrLf & " ,FTStateAcceptBy='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                
                    _Qry &= vbCrLf & " WHERE FNHSysEmpID =" & Val(R!FNHSysEmpID) & " "
                    _Qry &= vbCrLf & " AND FTDateTrans ='" & HI.UL.ULDate.ConvertEnDB(R!FTDateTrans.ToString) & "' "
    ...
    Private Sub ocmsave_Click_1(sender As Object, e As EventArgs) Handles ocmsave.Click
        Try
            If CType(ogctime.DataSource, DataTable).Rows.Count > 0 Then

                If CType(ogctime.DataSource, DataTable).Select("FTSelect='1'").Length > 0 Then
