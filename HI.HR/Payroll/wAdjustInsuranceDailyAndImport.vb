...
Public Class wAdjustInsuranceDailyAndImport
...
#Region "Procedure"
                        ...
                        With Me.ogvtime
                            ...
                            Select Case .FocusedColumn.FieldName.ToString.ToUpper
                                Case "FNNetProAmt".ToUpper
                                    ...
                                    _Amt3 = CDbl("" & .GetRowCellValue(.FocusedRowHandle, "FNProOT").ToString)
                                Case "FNProOther".ToUpper
                                    _Amt2 = CDbl("" & .GetRowCellValue(.FocusedRowHandle, "FNNetProAmt").ToString)
                                    _Amt3 = CDbl("" & .GetRowCellValue(.FocusedRowHandle, "FNProOT").ToString)
                                Case "FNProOT".ToUpper
                                    _Amt2 = CDbl("" & .GetRowCellValue(.FocusedRowHandle, "FNProOther").ToString)
                                    _Amt3 = CDbl("" & .GetRowCellValue(.FocusedRowHandle, "FNNetProAmt").ToString)
                            End Select

                            .SetRowCellValue(.FocusedRowHandle, "FNGGAmt", CDbl(Format(_Amt1 + _Amt2 + _Amt3, "0.00")))
    ...
    Private Sub LoadData()
        ...
        Do While _FTStartCalculateDate <= _FTEndCalculateDate
            ...
            _Qry &= vbCrLf & " AND M.FDDateStart <='" & _FTStartCalculateDate & "' "
            _Qry &= vbCrLf & " AND (M.FDDateEnd ='' OR M.FDDateEnd >'" & _FTStartCalculateDate & "' )   "
        ...
        Try
            If Not (StateCal) Then
                ...
                With ogvtime
                    ...
                    HI.HRCAL.Calculate.CalculateWageDaily(HI.ST.UserInfo.UserName, "" & .GetRowCellValue(.FocusedRowHandle, "FNHSysEmpID").ToString, "" & .GetRowCellValue(.FocusedRowHandle, "FNHSysEmpTypeId").ToString, HI.UL.ULDate.ConvertEnDB("" & .GetRowCellValue(.FocusedRowHandle, "FTDateTrans").ToString), HI.UL.ULDate.ConvertEnDB("" & .GetRowCellValue(.FocusedRowHandle, "FTDateTrans").ToString))

                    Dim _Qry As String
                    Dim _Amt As Double = 0
                    Dim _AmtOther As Double = 0
                    Dim _AmtOt As Double = 0
                    ...
                    If IsNumeric("" & .GetRowCellValue(.FocusedRowHandle, "FNProOT").ToString) Then
                        _AmtOt = CDbl("" & .GetRowCellValue(.FocusedRowHandle, "FNProOT").ToString)
                    End If
                    ...
                    If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "") <> "" Then
                        ...
                        _Qry &= vbCrLf & ", FNProOT=" & _AmtOt & ""
                        _Qry &= vbCrLf & ", FNProOther=" & _AmtOther & ""
                        _Qry &= vbCrLf & ", FNNetProAmt=" & _Amt + _AmtOther + _AmtOt & " "
                    ...
                    Else
                        ...
                        _Qry &= vbCrLf & " ,0"
                        _Qry &= vbCrLf & " ,0"
                        _Qry &= vbCrLf & " ," & _Amt & "," & _AmtOt & "," & _AmtOther & "," & _Amt + _AmtOther + _AmtOt & ""
                    End If
                                    ...
                                    For Each R As DataRow In tmpRow
                                        ...
                                        If HI.UL.ULDate.CheckDate(R.Item(0).ToString) <> "" AndAlso IsNumeric(R.Item(8).ToString) Then
                                            ...
                                            For Each Rx As DataRow In _dtemp.Rows
                                                ...
                                                If _FNEmpID > 0 Then
                                                    HI.HRCAL.Calculate.CalculateWageDaily(HI.ST.UserInfo.UserName, _FNEmpID.ToString, _FNEmpTypeID.ToString, _Date, _Date)
