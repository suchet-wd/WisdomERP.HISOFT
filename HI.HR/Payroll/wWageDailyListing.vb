...
Public Class wWageDailyListing
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        '_Qry &= vbCrLf & " ,ISNULL(Prod.FNNetAmt,0)  AS FNNetPay "
        '_Qry &= vbCrLf & " ,CASE WHEN   ISNULL(Prod.FNNetProAmt,0)  > ISNULL(Prod.FNNetAmt,0) THEN  (ISNULL(Prod.FNNetProAmt,0) -ISNULL(Prod.FNNetAmt,0))   ELSE 0   END  AS FNNetIncen "
        '_Qry &= vbCrLf & ",isnull(Bonus.FNAmt,0)  as FNAmt  ,  CASE WHEN   ISNULL(Prod.FNNetProAmt,0)  > ISNULL(Prod.FNNetAmt,0) THEN  ISNULL(Prod.FNNetProAmt,0)   ELSE ISNULL(Prod.FNNetAmt,0)   END  +  isnull(Bonus.FNAmt,0)  as FNNetPerDay "
        ...
        '_Qry &= vbCrLf & " Else (Case When   ISNULL(Prod.FNNetProAmt,0)  >case when ISNULL(Prod.FNAmtNormal,0) > M.FNSalary then M.FNSalary else ISNULL(Prod.FNAmtNormal,0) end  +  ISNULL(Prod.FNAmtOT,0) Then  (ISNULL(Prod.FNNetProAmt,0) -(case when ISNULL(Prod.FNAmtNormal,0) > M.FNSalary then M.FNSalary else ISNULL(Prod.FNAmtNormal,0) end  +  ISNULL(Prod.FNAmtOT,0)))   Else 0   End) End  As FNNetIncen "
