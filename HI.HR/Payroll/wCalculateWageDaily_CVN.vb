Public Class wCalculateWageDaily_CVN
    Sub New()
...
#Region "General"
    ...
    Private Sub ocmcalculate_Click(sender As System.Object, e As System.EventArgs) Handles ocmcalculate.Click
        If FNHSysEmpTypeId.Text <> "" And FNHSysEmpTypeId.Properties.Tag.ToString <> "" Then
            If HI.UL.ULDate.CheckDate(Me.FTStartDate.Text) <> "" And HI.UL.ULDate.CheckDate(Me.FTEndDate.Text) <> "" Then


                'If HI.HRCAL.Time.CheckClosePeriod(FTEndDate.Text) = True Then
                ...
                _Qry &= vbCrLf & " , P.FTPreNameNameTH + ' ' +  M.FTEmpNameTH + '  ' +  M.FTEmpSurnameTH AS FTEmpName,M.FNHSysEmpTypeId, ET.FNCalType "
                ...
                For Each R As DataRow In _Dt.Rows
                    ...
                    _Rec = _Rec + 1
                    _Spls.UpdateInformation("Calculate Employee Code " & R!FTEmpCode.ToString & " " & R!FTEmpName.ToString & " " & vbCrLf & "Record  " & _Rec.ToString & " Of " & _TotalRec.ToString & "  (" & Format((_Rec * 100.0) / _TotalRec, "0.00") & " % ) ")

                    '' HI.HRCAL.Calculate.CalculateWageDaily_CVN(HI.ST.UserInfo.UserName, R!FNHSysEmpID.ToString, Me.FNHSysEmpTypeId.Properties.Tag.ToString, HI.UL.ULDate.ConvertEnDB(FTStartDate.Text), HI.UL.ULDate.ConvertEnDB(FTEndDate.Text))


                    If R!FNCalType.ToString = "2" Then
                        _Qry = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.SP_CALCULATE_EMP_Monthly '" & HI.ST.UserInfo.UserName & "'," & HI.ST.SysInfo.CmpID & "," & R!FNHSysEmpID.ToString & ",'" & HI.UL.ULDate.ConvertEnDB(FTStartDate.Text) & "','" & HI.UL.ULDate.ConvertEnDB(FTEndDate.Text) & "'," & 0 & ""
                        HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)

                    Else
                        _Qry = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.SP_CALCULATE_EMP_DAILY_WITHOUT_INCENTIVE '" & HI.ST.UserInfo.UserName & "'," & HI.ST.SysInfo.CmpID & "," & R!FNHSysEmpID.ToString & ",'" & HI.UL.ULDate.ConvertEnDB(FTStartDate.Text) & "','" & HI.UL.ULDate.ConvertEnDB(FTEndDate.Text) & "'," & 0 & ""
                        HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)
                ...
                HI.MG.ShowMsg.mInfo("", 1404020001, Me.Text)
            ...
            End If
        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.SelectData, Me.Text, Me.FNHSysEmpTypeId_lbl.Text)
