...
#Region "Proc And Func"
        ...
        Try
            If Me.ogvDivertSrc.RowCount > 0 And Me.ogvDivertDT.RowCount > 0 Then
                ...
                If SubOrderDivertSeq <= 0 Then
                    ...
                    _Qry &= vbCrLf & "," & Val(HI.TL.CboList.GetIndexByText(Me.FNPackCartonSubType.Properties.Tag.ToString, Me.FNPackCartonSubType.SelectedItem.ToString)) & "," & Val(Me.FNPackPerCaton.Value.ToString)
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) > 0 Then
                ...
                Call SaveSubOrder_Component(_FTSubOrderSrc, Maxleng)
                Call SaveSubOrder_Sew(_FTSubOrderSrc, Maxleng)
                Call SaveSubOrder_Pack(_FTSubOrderSrc, Maxleng)
                Call SaveSubOrder_Bundle(_FTSubOrderSrc, Maxleng)
                Call SaveSubOrder_SizeSpec(_FTSubOrderSrc, Maxleng)
    ...
    Private Function SaveSubOrder_Sew(ByVal paraFTSubOrderNoSrc As String, ByVal paraMaxleng As Integer) As Boolean
        Try
            ...
            _Dt = ogdOrderSubSewing.DataSource
            ...
            If (_Dt.Rows.Count <> 0) Then
                ...
                _Qry &= vbCrLf & "WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNoSrc) & "' AND FTSubOrderNo = '" & HI.UL.ULF.rpQuoted(paraFTSubOrderNoSrc) & "' "
                _Qry &= vbCrLf & " AND FNDivertSeq = '" & Val(Maxleng) & "' AND FNSewSeq >" & _Seq
                HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            End If
        Catch ex As Exception
        End Try
    ...
    Private Function SaveSubOrder_Pack(ByVal paraFTSubOrderNoSrc As String, ByVal paraMaxleng As Integer) As Boolean
        Try
            ...
            _Dt = ogdOrderSubPack.DataSource
            ...
            If (_Dt.Rows.Count <> 0) Then
                ...
                _Qry &= vbCrLf & "WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNoSrc) & "' AND FTSubOrderNo = '" & HI.UL.ULF.rpQuoted(paraFTSubOrderNoSrc) & "' "
                _Qry &= vbCrLf & " AND FNDivertSeq = '" & Val(Maxleng) & "' AND FNPackSeq >" & _Seq
                HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            End If
        Catch ex As Exception
        End Try
            ...
            If Not (ogdOrderSubPackCarton.DataSource Is Nothing) Then
                ...
                For Each R As DataRow In dt.Rows
                    ...
                    For Each oRow As DataRow In oDBdtSizeBreakdown.Rows
                        ...
                        _Qry = "UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_Bundle"
                        _Qry &= vbCrLf & "SET FTUpdUser='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                        _Qry &= vbCrLf & ",FDUpdDate=" & HI.UL.ULDate.FormatDateDB
                        _Qry &= vbCrLf & ",FTUpdTime=" & HI.UL.ULDate.FormatTimeDB
                        _Qry &= vbCrLf & ",FNQuantity=" & (R.Item(oRow.Item("FTMatSizeCode").ToString()).ToString())
                        _Qry &= vbCrLf & "WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNoSrc) & "' AND FTSubOrderNo = '" & HI.UL.ULF.rpQuoted(paraFTSubOrderNoSrc) & "'"
                        _Qry &= vbCrLf & " AND FNDivertSeq = " & Val(Maxleng) & " AND FTColorway ='" & R!FTColorway.ToString & "' AND FTSizeBreakDown ='" & oRow!FTMatSizeCode.ToString & "'"

                        If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                            _Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_Divert_Bundle"
                            _Qry &= vbCrLf & " (FTInsUser, FDInsDate, FTInsTime, FTOrderNo, FTSubOrderNo, FNDivertSeq, FTColorway, FTSizeBreakDown, FNQuantity"
                            _Qry &= vbCrLf & ")"
                            _Qry &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                            _Qry &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB
                            _Qry &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB
                            ...
                            _Qry &= vbCrLf & ",'" & oRow!FTMatSizeCode.ToString & "'"
                            _Qry &= vbCrLf & "," & CInt("0" & (R.Item(oRow.Item("FTMatSizeCode").ToString()).ToString()))
                            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                                HI.Conn.SQLConn.Tran.Rollback()
                                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
    ...
    Private Function SaveSubOrder_Bundle2(ByVal paraFTSubOrderNoSrc As String) As Boolean
        Try
            Dim _Qry As String = ""
            Dim _QryCheckSeq As String = ""
            Dim bRet As Boolean = False
                ...
                For Each oColPackCarton As System.Data.DataColumn In CType(Me.ogdOrderSubPackCarton.DataSource, System.Data.DataTable).Columns
                    Select Case oColPackCarton.ColumnName.ToUpper
                        Case "FTOrderNo".ToUpper, "FTSubOrderNo".ToUpper, "FTColorway".ToUpper
                            '...Nothing
                        Case Else
                            sColumnSizeBreakdown = oColPackCarton.ColumnName.ToString             '...FTSizeBreakDown
                            ...
                            sSQL &= Environment.NewLine & "FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[TMERTOrderSub_Divert_Bundle] AS A"
                            ...
                            If HI.Conn.SQLConn.ExecuteNonQuery(sSQL, HI.Conn.DB.DataBaseName.DB_MERCHAN) = False Then
                                '...add new record
                                sSQL = ""
                                sSQL = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[TMERTOrderSub_Divert_Bundle] ([FTInsUser], [FDInsDate], [FTInsTime], [FTOrderNo], [FTSubOrderNo], [FTColorway], [FTSizeBreakDown], [FNQuantity])"
...
#End Region
    ...
    Private Sub ocmok_Click(sender As Object, e As EventArgs) Handles ocmok.Click
        ...
        If PROC_GETbValidateSubOrderNoInfo() = True Then
            ...
            If ValidateData() Then
                ...
                If System.Windows.Forms.MessageBox.Show("Are you sure, do you want to divert sub breakdown" & Environment.NewLine & "from Factory Order No. : " & _FTOrderNoSrc & Environment.NewLine & "Factory Sub Order No. : " & Me.FTSubOrderNoSrc.Properties.Items(Me.FTSubOrderNoSrc.SelectedIndex).ToString, "Confirm Divert Sub Breakdown", Windows.Forms.MessageBoxButtons.YesNo, Windows.Forms.MessageBoxIcon.Question, Windows.Forms.MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub LoadDataDivertSeq(paramFTOrderNo As String, paramFTSubOrderNo As String)
        ...
        sSQL &= Environment.NewLine & "	,FTCustRef"
