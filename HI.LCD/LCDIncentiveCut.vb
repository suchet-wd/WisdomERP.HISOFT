...
Public Class LCDIncentiveCut
    ...
    Sub New()
        ...
        If Me.LineId > 0 Then
            ...
            _Qry = "   Select Top 1 FDSaveDate as FDDate "
            _Qry &= vbCrLf & "   From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTCutOrderRecord_Detail AS X WITH(NOLOCK) "
            _Qry &= vbCrLf & "  Where (FNHSysUnitSectId = " & Me.LineId & ") "
            _Qry &= vbCrLf & "  And (FDSaveDate < N'" & TransactionDate & "') "
            _Qry &= vbCrLf & "  Order By FDSaveDate DESC "
...
#Region "Procedure "
    ...
    Private Sub GetEmployeeActualFromHR()
        ...
        _Qry &= vbCrLf & "	  AND Emp.FNHSysEmpTypeId IN(SELECT FNHSysEmpTypeId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.THRMEmpType AS ET WITH (NOLOCK) WHERE FTEmpTypeCode ='C'  )"
        ...
        Else
            Try
                ...
                If _TotalTarget > 0 Then
                    ...
                    If StateActualDate = False And DateBefore <> "" Then
                        _Cmd = "EXEC  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..SP_DATA_LCD_CUT_DETAIL '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'," & Integer.Parse(Val(LineId)) & ",'" & DateBefore & "' "
                    Else
                        _Cmd = "EXEC  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..SP_DATA_LCD_CUT_DETAIL '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'," & Integer.Parse(Val(LineId)) & ",'" & _DateNow & "' "
                    End If
                    ...
                    If _dttime.Rows.Count > 0 Then
                        ...
                        '_Qry &= vbCrLf & " Select  O.FDSaveDate AS FDScanDate, isnull( FTUpdTime,FTInsTime) as FDScanTime , O.FNQuantity as FNScanQuantity  "
                        '_Qry &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline AS O WITH(NOLOCK)  "
                        '_Qry &= vbCrLf & ""


                        _Qry &= vbCrLf & " Select  O.FDSaveDate AS FDScanDate, isnull( FTUpdTime,FTInsTime) as FDScanTime , O.FNQuantity as FNScanQuantity  "
                        _Qry &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTCutOrderRecord_Detail AS O WITH(NOLOCK)  "
                        _Qry &= vbCrLf & ""
                        ...
                        If StateActualDate = False And DateBefore <> "" Then
                            _Qry &= vbCrLf & "   AND O.FDSaveDate ='" & Me.DateBefore & "'"
                        Else
                            _Qry &= vbCrLf & "   AND O.FDSaveDate ='" & Me.TransactionDate & "'"
                        End If
        ...
        Else
            Try
                ...
                If _TotalTarget > 0 Then
                    ...
                    If StateActualDateL2 = False And DateBeforeL2 <> "" Then
                        _Cmd = "EXEC  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..SP_DATA_LCD_CUT_DETAIL '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'," & Integer.Parse(Val(L2LineId)) & ",'" & DateBeforeL2 & "' "
                    Else
                        _Cmd = "EXEC  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..SP_DATA_LCD_CUT_DETAIL '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'," & Integer.Parse(Val(L2LineId)) & ",'" & _DateNow & "' "
                    End If
                    ...
                    If _dttime.Rows.Count > 0 Then
                        ...
                        _Qry &= vbCrLf & "Select  FDSaveDate AS FDScanDate , isnull(FTUpdTime, FTInsTime )  AS   FDScanTime   ,FNQuantity AS FNScanQuantity "
                        _Qry &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTCutOrderRecord_Detail AS O WITH(NOLOCK)  LEFT OUTER JOIN"

                        _Qry &= vbCrLf & "   WHERE O.FNHSysUnitSectId =" & Integer.Parse(Val(L2LineId)) & ""

                        If StateActualDateL2 = False And DateBeforeL2 <> "" Then
                            _Qry &= vbCrLf & "   And O.FDSaveDate ='" & Me.DateBeforeL2 & "'"
                        Else
                            _Qry &= vbCrLf & "   And O.FDSaveDate ='" & Me.TransactionDate & "'"
                        End If
