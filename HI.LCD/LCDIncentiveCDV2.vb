...
Public Class LCDIncentiveCDV2
    ...
    Sub New()
        ...
        _Qry = "   Select  TOP 1  FNOTIncentive "
        _Qry &= vbCrLf & "  From THRIncentiveCostEFF AS X WITH(NOLOCK) "
        _Qry &= vbCrLf & "  WHERE  FTDate<='" & HI.UL.ULDate.ConvertEnDB(TransactionDate) & "' AND  FTDateTo>='" & HI.UL.ULDate.ConvertEnDB(TransactionDate) & "'"

        LinOTIncentive = Val(HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_HR, "0"))
    ...
    Private Sub SetDefualtControl()
        ...
        olbsheader0.Visible = False
        olbslvper11.Visible = False
        olbslvper21.Visible = False
        olbslvtper1.Visible = False

        olbsheader1.Visible = True
        olbslv11.Visible = True
        olbslv21.Visible = True
        olbslvt1.Visible = True

        olbsheader02.Visible = False
        olbslvtarget11.Visible = False
        olbslvtarget21.Visible = False
        olbslvttarget1.Visible = False

        olbsheader2.Visible = True
        olbslv14.Visible = True
        olbslv24.Visible = True
        olbslvt4.Visible = True
...
#Region "Property"
    ...
    Private _TimeInOTM As String = ""
    Property TimeInOTM As String
        Get
            Return _TimeInOTM
        End Get
        Set(value As String)
    ...
    Private _TimeOutOTM As String = ""
    Property TimeOutOTM As String
        Get
            Return _TimeOutOTM
        End Get
        Set(value As String)
...
#Region "Procedure "
    ...
    Private Sub GetEmployeeActualFromHR()
        ...
        _Qry &= vbCrLf & "  , MAX(FTOtMIn) AS FTOtMIn "
        _Qry &= vbCrLf & "  , MAX(FTOtMOut) AS FTOtMOut "
        ...
        _Qry &= vbCrLf & " ,ISNULL(XXOT2.FTOtMIn,'') As FTOtMIn  "
        _Qry &= vbCrLf & " ,ISNULL(XXOT2.FTOtMOut,'') As FTOtMOut  "
        ...
        If StateActualDate = False And DateBefore <> "" Then
            ...
            _Qry &= vbCrLf & "                  Select TOP 1 FTOtIn, FTOtOut,FTOtMIn,FTOtMOut  "
        ...
        For Each R As DataRow In dtleave.Rows
            ...
            TimeInOTM = R!FTOtMIn.ToString
            TimeOutOTM = R!FTOtMOut.ToString
            ...
            Try
                ...
                If _TotalTarget > 0 Then
                    ...
                    If IncenFormulaIdLine1 <= 0 Then
                        ...
                        _Qry &= vbCrLf & "  , 0.000 AS FNPerRateEff"
                    ...
                    If _dtPrice.Rows.Count > 0 And _FNSam > 0 Then
                        ...
                        Dim dtdataincentive As DataTable
                        _Qry = "   Select   FNLabourCost, FNOTIncentive, FNEFFMinIncentivePer, FNMultiplierAmt "
                        _Qry &= vbCrLf & "  From THRIncentiveCostEFF "

                        If StateActualDate = False And DateBefore <> "" Then
                            _Qry &= vbCrLf & "  WHERE  FTDate<='" & HI.UL.ULDate.ConvertEnDB(DateBefore) & "' AND  FTDateTo>='" & HI.UL.ULDate.ConvertEnDB(DateBefore) & "'"
                        Else
                            _Qry &= vbCrLf & "  WHERE  FTDate<='" & HI.UL.ULDate.ConvertEnDB(_DateNow) & "' AND  FTDateTo>='" & HI.UL.ULDate.ConvertEnDB(_DateNow) & "'"
                        End If
                        dtdataincentive = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_HR)
                        ...
                        For Each Rxp As DataRow In _dtPrice.Select("FNLVSeq>=0", "FNLVSeq")
                            ...
                            For Each RxpI As DataRow In dtdataincentive.Select(" FNEFFMinIncentivePer<=" & Val(Rxp!FNStartEff.ToString()) & "", "FNEFFMinIncentivePer DESC")
                                _AmtMax = _AmtMax + Val(RxpI!FNMultiplierAmt.ToString)
                                Exit For
                            Next


                            Rxp!FNAmountMax = _AmtMax

                        Next


                        dtdataincentive.Dispose()
                                ...
                                Select Case _RowIdx
                                    Case 1
                                        ...
                                        If Val(Rxp!FNLVSeq.ToString) <= 1 Then
                                            olbslvper11.Text = Format(Val(Rxp!FNEndEff.ToString), "0.00")
                                        Else
                                            olbslvper11.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")
                                        End If

                                        If Val(Rxp!FNLVSeq.ToString) = 3 And Val(Rxp!FNAmount.ToString) = 0 Then
                                            For Each RxpLV2 As DataRow In _dtPrice.Select("FNActBalQty >0 AND FNLVSeq=2", "FNLVSeq")
                                                Rxp!FNAmount = RxpLV2!FNAmount
                                            Next
                                        End If

                                        olbslvtarget11.Text = Rxp!FNTargetQty.ToString

                                        olbslv11.Text = Format(Val(Rxp!FNPriceMul.ToString), "0.00")

                                        If Val(Rxp!FNLVSeq.ToString) <= 1 Then
                                            olbslv11.Text = Format(Val(Rxp!FNEndEff.ToString), "0.00")
                                        Else
                                            olbslv11.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")
                                        End If


                                        olbslv14.Text = Rxp!FNActBalQty.ToString
                                        olbslv13.Tag = "0"

                                        If Val(Rxp!FNAmount.ToString) = 0 Then

                                            olbslv13.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") '"-"

                                        Else

                                            ' olbslv13.ForeColor = Drawing.Color.Blue
                                            olbslv13.Tag = "1"
                                            olbslv13.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") ' Rxp!FNAmount.ToString
                                    ...
                                    Case 2
                                        ...
                                        olbslvper21.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")
                                        olbslvtarget21.Text = Rxp!FNTargetQty.ToString


                                        olbslv21.Text = Format(Val(Rxp!FNPriceMul.ToString), "0.00")
                                        olbslv21.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")


                                        olbslv24.Text = Rxp!FNActBalQty.ToString
                                        olbslv23.Tag = "0"

                                        If Val(Rxp!FNAmount.ToString) = 0 Then
                                            olbslv23.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") '"-"
                                        Else
                                            olbslv23.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") ' Rxp!FNAmount.ToString
                                        End If
                                    ...
                                    Case 3
                                        ...
                                        olbslvtper1.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")
                                        olbslvttarget1.Text = Rxp!FNTargetQty.ToString


                                        olbslvt1.Text = Format(Val(Rxp!FNPriceMul.ToString), "0.00")
                                        olbslvt1.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")

                                        olbslvt4.Text = Rxp!FNActBalQty.ToString
                                        olbslvt3.Tag = "0"

                                        If Val(Rxp!FNAmount.ToString) = 0 Then
                                            olbslvt3.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") '"-"
                                        Else
                                            olbslvt3.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") ' Rxp!FNAmount.ToString
                                        End If
                                End Select
    ...
    Private Function GetTimeMinuteData() As Integer
        ...
        If _Time >= TimeInM Then
            ...
            If _Time > TimeInOTM And _Time > TimeOutOTM And TimeInOTM <> "" And TimeOutOTM <> "" Then

                If _Time >= TimeOutOTM Then
                    SumMinute = SumMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTM), CDate(Me.Actualdate & "  " & TimeOutOTM))
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTM), CDate(Me.Actualdate & "  " & TimeOutOTM))
                Else
                    SumMinute = SumMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTM), CDate(Me.Actualdate & "  " & _Time))
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTM), CDate(Me.Actualdate & "  " & _Time))
                End If
            End If
            ...
            If _Time > TimeInOT And TimeInOT <> "" And TimeOutOT <> "" Then
                If _Time >= TimeOutOT Then
                    ...
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOT), CDate(Me.Actualdate & "  " & TimeOutOT))
                Else
                    SumMinute = SumMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOT), CDate(Me.Actualdate & "  " & _Time))
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOT), CDate(Me.Actualdate & "  " & _Time))
                End If
            End If


            If SumOtMinute > 0 Then
                lblgrade.Text = Format((LinOTIncentive / 60.0) * SumOtMinute, "0.00")
            End If
    ...
    Private Sub SwipShowData()
        ...
        olbsheader0.Visible = Not olbsheader0.Visible
        olbslvper11.Visible = Not olbslvper11.Visible
        olbslvper21.Visible = Not olbslvper21.Visible
        olbslvtper1.Visible = Not olbslvtper1.Visible

        olbsheader1.Visible = Not olbsheader1.Visible
        olbslv11.Visible = Not olbslv11.Visible
        olbslv21.Visible = Not olbslv21.Visible
        olbslvt1.Visible = Not olbslvt1.Visible

        olbsheader02.Visible = Not olbsheader02.Visible
        olbslvtarget11.Visible = Not olbslvtarget11.Visible
        olbslvtarget21.Visible = Not olbslvtarget21.Visible
        olbslvttarget1.Visible = Not olbslvttarget1.Visible

        olbsheader2.Visible = Not olbsheader2.Visible
        olbslv13.Visible = Not olbslv13.Visible
        olbslv23.Visible = Not olbslv23.Visible
        olbslvt3.Visible = Not olbslvt3.Visible
        ...
        L2olbslv23.Visible = Not L2olbslv23.Visible
        L2olbslvt3.Visible = Not L2olbslvt3.Visible
    ...
    Public Sub SetSize()
        ...
        Dim _PlineTimeW As Integer = 80 'opntime1
        ...
        opntime1.Width = _MeWidth * 0.12618296529968451

        olbsheader0.Width = _PH0W
        olbsheader02.Width = _PH2W
        olbsheader1.Width = _PH1W
        olbsheader2.Width = _PH4W
        olbsheader3.Width = _PH3W

        olbslvper11.Width = _PH0W
        olbslvtarget11.Width = _PH2W

        olbslv11.Width = _PH1W
        olbslvper11.Width = _PH1W
        olbslvtarget11.Width = _PH1W

        olbslv13.Width = _PH4W
        olbslv14.Width = _PH3W

        olbslvper21.Width = _PH0W
        olbslvtarget21.Width = _PH2W
        olbslvper21.Width = _PH1W
        olbslvtarget21.Width = _PH1W

        olbslv21.Width = _PH1W
        olbslv23.Width = _PH4W
        olbslv24.Width = _PH3W

        olbslvtper1.Width = _PH0W
        olbslvttarget1.Width = _PH2W
        olbslvt1.Width = _PH1W
        olbslvt3.Width = _PH4W
        olbslvt4.Width = _PH3W
        ...
        Dim _FontLVTile As Integer = _PLVTITLE * 0.25 ' 30 '90
        Dim FFontLVTile As New Font("Khmer OS Muol Light", _FontLVTile, FontStyle.Bold)


        Dim _FontLVTile2 As Integer = _PLVTITLE * 0.3 ' 30 '90
        Dim FFontLVTile2 As New Font("Khmer OS Muol Light", _FontLVTile, FontStyle.Bold)
        ...
        Dim FontHeaderLineSize3 As Integer = (_P1 * 0.14)
        Dim FHeader3 As New Font("Khmer OS Muol Light", FontHeaderLineSize3, FontStyle.Bold)
    ...
    Public Sub SetData(Optional StateSwipDate As Boolean = False)
        ...
        Try
            ...
            If LineNo <> "" Then
                ...
                'Where (ISNULL(e.FDDateEnd, '') = '') AND (ET.FTEmpTypeCode = N'D')
...
#Region "Property"
    ...
    Private _LinOTIncentive As Double = 0
    Property LinOTIncentive As Double
        Get
            Return _LinOTIncentive
        End Get
        Set(value As Double)
            _LinOTIncentive = value
        End Set
    End Property
    ...
    Private _TimeInOTL2M As String = ""
    Property TimeInOTL2M As String
        Get
            Return _TimeInOTL2M
        End Get
        Set(value As String)
    ...
    Private _TimeOutOTL2M As String = ""
    Property TimeOutOTL2M As String
        Get
            Return _TimeOutOTL2M
        End Get
        Set(value As String)
            _TimeOutOTL2M = value
        End Set
...
#Region "Procedure "
            ...
            Try
                ...
                If _TotalTarget > 0 Then
                    ...
                    If _dtPrice.Rows.Count > 0 And _FNSam > 0 Then
                        ...
                        If _StateActualDateL2 = False And DateBeforeL2 <> "" Then
                            _Qry &= vbCrLf & "  WHERE  FTDate<='" & HI.UL.ULDate.ConvertEnDB(DateBeforeL2) & "' AND  FTDateTo>='" & HI.UL.ULDate.ConvertEnDB(DateBeforeL2) & "'"
                        Else
                            _Qry &= vbCrLf & "  WHERE  FTDate<='" & HI.UL.ULDate.ConvertEnDB(_DateNow) & "' AND  FTDateTo>='" & HI.UL.ULDate.ConvertEnDB(_DateNow) & "'"
                        End If
                        dtdataincentive = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_HR)
                            ...
                            For Each Rxp As DataRow In _dtPrice.Select("FNActBalQty >0 AND FNLVSeq<>2", "FNLVSeq")
                                ...
                                Select Case _RowIdx
                                    Case 1
                                        ...
                                        If Val(Rxp!FNLVSeq.ToString) <= 1 Then
                                            L2olbslv11.Text = Format(Val(Rxp!FNEndEff.ToString), "0.00")
                                        Else
                                            L2olbslv11.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")
                                        End If


                                        L2olbslv14.Text = Rxp!FNActBalQty.ToString
                                        L2olbslv13.Tag = "0"

                                        If Val(Rxp!FNAmount.ToString) = 0 Then

                                            L2olbslv13.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") '"-"

                                        Else

                                            ' L2olbslv13.ForeColor = Drawing.Color.Blue
                                            L2olbslv13.Tag = "1"
                                            L2olbslv13.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") ' Rxp!FNAmount.ToString
                                    ...
                                    Case 2
                                        ...
                                        L2olbslv21.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")

                                        L2olbslv24.Text = Rxp!FNActBalQty.ToString
                                        L2olbslv23.Tag = "0"

                                        If Val(Rxp!FNAmount.ToString) = 0 Then
                                            L2olbslv23.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") '"-"
                                        Else
                                            L2olbslv23.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") ' Rxp!FNAmount.ToString
                                        End If
                                    ...
                                    Case 3
                                        ...
                                        L2olbslvt1.Text = Format(Val(Rxp!FNStartEff.ToString), "0.00")

                                        L2olbslvt4.Text = Rxp!FNActBalQty.ToString
                                        L2olbslvt3.Tag = "0"

                                        If Val(Rxp!FNAmount.ToString) = 0 Then
                                            L2olbslvt3.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") '"-"
                                        Else
                                            L2olbslvt3.Text = Format(Val(Rxp!FNAmountMax.ToString), "0.00") ' Rxp!FNAmount.ToString
                                        End If
                                End Select
    ...
    Private Function L2GetTimeMinuteData() As Integer
        ...
        If _Time >= TimeInML2 Then
            ...
            If _Time > TimeInOTL2M And _Time > TimeOutOTL2M And TimeInOTL2M <> "" And TimeOutOTL2M <> "" Then

                If _Time >= TimeOutOTL2M Then
                    SumMinute = SumMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2M), CDate(Me.Actualdate & "  " & TimeOutOTL2M))
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2M), CDate(Me.Actualdate & "  " & TimeOutOTL2M))
                Else
                    SumMinute = SumMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2M), CDate(Me.Actualdate & "  " & _Time))
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2M), CDate(Me.Actualdate & "  " & _Time))
                End If
            End If
            ...
            If _Time > TimeInOTL2 And TimeInOTL2 <> "" And TimeOutOTL2 <> "" Then
                If _Time >= TimeOutOTL2 Then
                    ...
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2), CDate(Me.Actualdate & "  " & TimeOutOTL2))

                Else
                    SumMinute = SumMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2), CDate(Me.Actualdate & "  " & _Time))
                    SumOtMinute = SumOtMinute + DateDiff(DateInterval.Minute, CDate(Me.Actualdate & "  " & TimeInOTL2), CDate(Me.Actualdate & "  " & _Time))
                End If
            End If

            If SumOtMinute > 0 Then
                L2lblgrade.Text = Format((LinOTIncentive / 60.0) * SumOtMinute, "0.00")
            End If
        End If
    ...
    Public Sub L2SetSize()
        ...
        L2opntime1.Width = _MeWidth * 0.12618296529968451

        L2olbsheader0.Width = _PH0W
        L2olbsheader02.Width = _PH2W
        L2olbsheader1.Width = _PH1W
        L2olbsheader2.Width = _PH4W
        L2olbsheader3.Width = _PH3W
        ...
        L2olbslv13.Width = _PH4W
        L2olbslv14.Width = _PH3W
        ...
        L2olbslv23.Width = _PH4W
        L2olbslv24.Width = _PH3W

        L2olbslvtper1.Width = _PH0W
        L2olbslvttarget1.Width = _PH2W
        L2olbslvt1.Width = _PH1W
        L2olbslvt3.Width = _PH4W
        L2olbslvt4.Width = _PH3W
        ...
        L2olbsheader3.Font = FFontLVTile2
    ...
    Private Sub L2olbslv13_Click(sender As Object, e As EventArgs) Handles L2olbslv13.Click
    ...
    Private Sub L2olbslv13_TextChanged(sender As Object, e As EventArgs) Handles L2olbslv13.TextChanged
        Try
            L2olbslv13.ForeColor = Drawing.Color.Blue
            If IsNumeric(L2olbslv13.Text) Then

                If CDbl(L2olbslv13.Text) > 0 Then
                    L2olbslv13.ForeColor = Drawing.Color.Green
                Else
                    L2olbslv13.ForeColor = Drawing.Color.Red
                End If
    ...
    Private Sub olbslv13_TextChanged(sender As Object, e As EventArgs) Handles olbslv13.TextChanged
        Try

            olbslv13.ForeColor = Drawing.Color.Blue

            If IsNumeric(olbslv13.Text) Then

                If CDbl(olbslv13.Text) > 0 Then
                    olbslv13.ForeColor = Drawing.Color.Green
                Else
                    olbslv13.ForeColor = Drawing.Color.Red
                End If
