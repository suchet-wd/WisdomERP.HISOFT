...
Public Class wAssetSafety
    ...
    Private objForm As wAddItemAssetSafety
    ...
    Sub New()
        ...
        objForm = New wAddItemAssetSafety(FormName, Me.Text, Me.FormObjID, HI.ST.SysInfo.PathFileDLL, "", Me)
        ...
        With ogvdetail
            '.Columns("FNStartQty").Summary.Add(DevExpress.Data.SummaryItemType.Sum, "FNStartQty")
            '.Columns("FNStartQty").SummaryItem.DisplayFormat = "{0:n" & HI.ST.Config.AmtDigit & "}"
            '.Columns("FNEndQty").Summary.Add(DevExpress.Data.SummaryItemType.Sum, "FNEndQty")
            '.Columns("FNEndQty").SummaryItem.DisplayFormat = "{0:n" & HI.ST.Config.AmtDigit & "}"
...
#Region "Proc"
    ...
    Public Sub LoadData()
        ...
        Try
            _Str = "Select  M.FNHSysAssetSafetyId,L.FTNameTH,M.FTUserName,C.FTCmpCode as FNHSysCmpId,M.FTStateActive"
            If ST.Lang.Language = ST.Lang.eLang.TH Then
                _Str &= vbCrLf & ",L.FTNameTH as FNFixedAssetType,ISNULL(TA.FTAssetTypeNameTH,TP.FTAssetPartTypeNameTH) AS FNHSysAssetTyped"
            Else
                _Str &= vbCrLf & ",L.FTNameEN as FNFixedAssetType,ISNULL(TA.FTAssetTypeNameEN,TP.FTAssetPartTypeNameEN) AS FNHSysAssetTyped"
            End If
            _Str &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetSafety AS M WITH(NOLOCK) "
            _Str &= vbCrLf & "LEFT OUTER JOIN (select L.FNListIndex,L.FTNameEN,L.FTNameTH from"
            _Str &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L with(nolock) where L.FTListName='FNFixedAssetType') AS L ON M.FNFixedAssetType=L.FNListIndex "
            _Str &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp AS C  with(nolock) ON M.FNHSysCmpId=C.FNHSysCmpId"
            _Str &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetType AS TA with(nolock) ON M.FNFixedAssetType=TA.FNFixedAssetType AND M.FNHSysAssetTyped=TA.FNHSysAssetTyped"
            _Str &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetPartType AS TP with(nolock) ON M.FNHSysAssetTyped=TP.FNHSysAssetPartTyped"
            _Str &= vbCrLf & " order by L.FTNameTH "
...
#End Region
    ...
    Private Sub ocmedit_Click(sender As Object, e As EventArgs) Handles ocmedit.Click
        ...
        With ogvdetail
            ...
            Dim _Key As Integer = .GetRowCellValue(.FocusedRowHandle, "FNHSysAssetSafetyId").ToString.Trim
            'Dim _Key As String = "" & .GetRowCellValue(.FocusedRowHandle, Me.MainKeyID).ToString.Trim
            ...
            With objForm
                ...
                .ocmaddnew.Visible = False
                .FNHSysConvAssetId.Properties.Tag = Val(_Key)
                Try
                    Qry = "Select  Top 1 (D.FTNameTH )as FNFixedAssetType,UC.FTUserName,UC.FNHSysAssetSafetyId,C.FTCmpCode as FNHSysCmpId,D.FNListIndex,UC.FTStateActive,ISNULL(TA.FTAssetTypeCode,TP.FTAssetPartTypeCode) AS FTAssetTypeCode "
                    Qry &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetSafety AS UC WITH(NOLOCK) "
                    Qry &= vbCrLf & "LEFT OUTER JOIN  (select D.FTNameTH,D.FNListIndex"
                    Qry &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS D  WITH(NOLOCK) "
                    Qry &= vbCrLf & "where D.FTListName='FNFixedAssetType' "
                    Qry &= vbCrLf & ")AS D ON UC.FNFixedAssetType=D.FNListIndex"
                    Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp AS C WITH(NOLOCK) ON UC.FNHSysCmpId=C.FNHSysCmpId"
                    Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetType AS TA with(nolock) ON UC.FNFixedAssetType=TA.FNFixedAssetType AND UC.FNHSysAssetTyped=TA.FNHSysAssetTyped"
                    Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetPartType AS TP with(nolock) ON UC.FNHSysAssetTyped=TP.FNHSysAssetPartTyped"
                    Qry &= vbCrLf & "Where UC.FNHSysAssetSafetyId ='" & _Key & "'"
                    ...
                    For Each R As DataRow In dt.Rows
                        ...
                        .FNHSysAssetTyped.Text = R!FTAssetTypeCode

                        '.FNHSysAssetTyped.Properties.Tag = R!FTAssetTypeCode
                        .FTUserName.Text = R!FTUserName
                        .FNHSysCmpId.Text = R!FNHSysCmpId
                        .MainKeyID = Val(R!FNHSysAssetSafetyId)
    ...
    Private Sub wAddItemAssetSafety_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Call LoadData()
    End Sub
