...
Imports HI.TL
Public Class wAddItemAssetAppPO
...
#End Region
    ...
    Private Sub ocmdelete_Click(sender As Object, e As EventArgs) Handles ocmdelete.Click
        ...
        If HI.MG.ShowMsg.mConfirmProcess(HI.MG.ShowMsg.ProcessType.mDelete, Me.FNFixedAssetType.Text, Me.Text) = True Then
            ...
            Qry = "DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetApprovedPO WITH(NOLOCK) WHERE FNHSysAssetPOId = " & Me._MainKeyID & ""
    ...
    Private Sub ocmedit_Click(sender As Object, e As EventArgs) Handles ocmedit.Click
        ...
        Try
            If VerityData() Then
                _DG = HI.Conn.SQLConn.GetField("SELECT FNHSysDirectorGrpId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMDirectorGrp WITH(NOLOCK) where FTDirectorGrpCode='" & Me.FNHSysDirectorGrpId.Text & "' ", Conn.DB.DataBaseName.DB_MASTER, "")
                Qry = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetApprovedPO "
                ...
                Qry &= vbCrLf & ",FNHSysDirectorGrpId='" & Val(_DG) & "'" '" & Me.FNHSysDirectorGrpId.Text & "'"
                Qry &= vbCrLf & ",FTManagerName='" & Me.FTManagerUserName.Text & "'"
                Qry &= vbCrLf & "WHERE FNHSysAssetPOId=" & Me.MainKeyID & ""
    ...
    Private Sub ocmaddnew_Click(sender As Object, e As EventArgs) Handles ocmaddnew.Click
        ...
        Try
            If VerityData() Then
                ...
                _SystemKey = HI.TL.RunID.GetRunNoID("" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "..TASMAssetApprovedPO", "FNHSysAssetPOId", Conn.DB.DataBaseName.DB_MASTER)
                _DG = HI.Conn.SQLConn.GetField("SELECT FNHSysDirectorGrpId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMDirectorGrp WITH(NOLOCK) where FTDirectorGrpCode='" & Me.FNHSysDirectorGrpId.Text & "' ", Conn.DB.DataBaseName.DB_MASTER, "")

                Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetApprovedPO"
                Qry &= vbCrLf & "(FTInsUser,FDInsDate,FTinsTime,FNHSysAssetPOId,FNFixedAssetType,FNHSysDirectorGrpId,FTManagerName)"
                Qry &= vbCrLf & "Select '" & ST.UserInfo.UserName & "'," & UL.ULDate.FormatDateDB & "," & UL.ULDate.FormatTimeDB & ""
                Qry &= vbCrLf & "," & Val(_SystemKey) & "," & FNFixedAssetType.SelectedIndex & ",'" & Val(_DG) & "','" & FTManagerUserName.Text & "'"
                If HI.Conn.SQLConn.Execute_Tran(Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) > 0 Then
                    HI.Conn.SQLConn.Tran.Commit()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
    ...
    Private Sub LoadUserInfo()
        ...
        _Str = "SELECT TOP 1  A.* ,B.FTMerTeamCode ,C.FTTeamGrpCode  "
        _Str &= vbCrLf & " ,ISNULL((SELECT TOP 1 H.FTEmpCode"
        _Str &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee AS H WITH(NOLOCK) "
        _Str &= vbCrLf & "  WHERE H.FNHSysEmpID=A.FNHSysEmpID"
        _Str &= vbCrLf & " ),'') AS FTEmpCode"
        ...
        _Str &= vbCrLf & "  WHERE A.FTUserName='" & FNHSysDirectorGrpId.Text & "'  "
        Dim _dt As DataTable = HI.Conn.SQLConn.GetDataTable(_Str, Conn.DB.DataBaseName.DB_SECURITY)

        For Each R As DataRow In _dt.Rows

            FNHSysDirectorGrpId.Text = R!FTUserName.ToString

        Next

    End Sub

    Private Sub wAddItemAssetAppPO_Load(sender As Object, e As EventArgs) Handles MyBase.Load
