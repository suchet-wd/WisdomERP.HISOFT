...
Public Class wAddEditAssetModel
...
#End Region
    ...
    Private Sub ocmaddnew_Click(sender As Object, e As EventArgs) Handles ocmaddnew.Click
        ...
        Dim _dataBinary As Byte() = Nothing
        Dim dt As DataTable
        ...
        Try
            If Me.otbModel.SelectedTabPage.Name = TabModel.Name Then
                Try
                    If VerifyDataTabModel() Then
                        HI.Conn.SQLConn._ConnString = HI.Conn.DB.ConnectionString(Conn.DB.DataBaseName.DB_MASTER)
                        HI.Conn.SQLConn.SqlConnectionOpen()
                        HI.Conn.SQLConn.Cmd = HI.Conn.SQLConn.Cnn.CreateCommand
                        HI.Conn.SQLConn.Tran = HI.Conn.SQLConn.Cnn.BeginTransaction
                        _SystemKey = HI.TL.RunID.GetRunNoID("" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "..TASMAssetModel", "FNHSysAssetModelId", Conn.DB.DataBaseName.DB_MASTER)
                        _ModelID = _SystemKey


                        Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModel"
                        Qry &= vbCrLf & "(FTInsUser,FDInsDate,FTInsTime,FNHSysAssetModelId,FTAssetModelCode,FTAssetModelNameEN,FTAssetModelNameTH,FTRemark,FTStateActive)"
                        Qry &= vbCrLf & "SELECT '" & HI.ST.UserInfo.UserName & "'," & HI.UL.ULDate.FormatDateDB & "," & HI.UL.ULDate.FormatTimeDB & ""
                        Qry &= vbCrLf & "," & Val(_SystemKey) & ",'" & Me.FTAssetModelCode.Text & "','" & Me.FTAssetModelNameEN.Text & "','" & Me.FTAssetModelNameTH.Text & "','" & Me.FTRemark.Text & "','" & _State & "'"
                        If HI.Conn.SQLConn.Execute_Tran(Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) > 0 Then
                            If opnDialog.Text <> "" Then
                                Dim br As New BinaryReader(New FileStream(Me.opnDialog.Text, FileMode.Open, FileAccess.Read))
                                _dataBinary = br.ReadBytes(CInt(New FileInfo(Me.opnDialog.Text).Length))
                                Dim _cmd As String = ""
                                _cmd = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModel"
                                _cmd &= " Set  FPImage=@FPImage"
                                _cmd &= " WHERE FNHSysAssetModelId=@FNHSysAssetModelId"
                                Dim cmd As New SqlCommand(_cmd, HI.Conn.SQLConn.Cnn, HI.Conn.SQLConn.Tran)
                                cmd.Parameters.AddWithValue("@FNHSysAssetModelId", _SystemKey)
                                Dim _pdf As New SqlParameter("@FPImage", SqlDbType.VarBinary)
                                _pdf.Value = _dataBinary
                        ...
                        ' หลังจาก ADD TAB แรกแล้ว GET MAX FNSeq FROM TB..TASMAssetModelPart
                        Try
                            Me.FNHSysAssetModelId.Properties.Tag = Integer.Parse(Val(_SystemKey.ToString))
                            QryAfterAdd = "SELECt MAX(FNSeq) AS FNSeq  FroM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart AS MO WITH(NOLOCK) WHERE FNHSysAssetModelId=" & _SystemKey & ""
                        ...
                        End Try
                        'END ' หลังจาก ADD TAB แรกแล้ว GET MAX FNSeq FROM TB..TASMAssetModelPart
                        otbModel.SelectedTabPage.TabControl.TabPages(1).PageEnabled = True
                        otbModel.SelectedTabPageIndex = 1
                        HI.TL.HandlerControl.ClearControl(Me)
                ...
                End Try
            Else
                If VerifyDataTabModelPart() Then
                    Try
                        Qry = "SELECt MAX(FNSeq) AS FNSeq  FroM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart AS MO WITH(NOLOCK) WHERE FNHSysAssetModelId=" & Me.FNHSysAssetModelId.Properties.Tag & ""
                        ...
                        Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart"
                        Qry &= vbCrLf & "(FTInsUser,FDInsDate,FTInsTime,FNHSysAssetModelId,FNHSysAssetPartId,FNSeq,FNQuantity)"
                        Qry &= vbCrLf & "SELECT '" & HI.ST.UserInfo.UserName & "'," & HI.UL.ULDate.FormatDateDB & "," & HI.UL.ULDate.FormatTimeDB & ""
                        Qry &= vbCrLf & "," & Me.FNHSysAssetModelId.Properties.Tag & "," & Me.FNHSysAssetPartId.Properties.Tag & "," & Me.FNSeq.Value & "," & Me.FNQuantity.Value & ""
                        If HI.Conn.SQLConn.Execute_Tran(Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) > 0 Then
                            HI.Conn.SQLConn.Tran.Commit()
                            HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                            HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
                            HI.MG.ShowMsg.mProcessComplete(MG.ShowMsg.ProcessType.mSave, Me.Text)
                            Me.FNHSysAssetPartId.Text = ""
    ...
    Private Sub ocmedit_Click(sender As Object, e As EventArgs) Handles ocmedit.Click
        ...
        Try
            If Me.otbModel.SelectedTabPage.Name = TabModel.Name Then
                Qry = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModel"
                Qry &= vbCrLf & "SET FTUpdUser='" & HI.ST.UserInfo.UserName & "',FDUpdDate=" & HI.UL.ULDate.FormatDateDB & ",FTUpdTime=" & HI.UL.ULDate.FormatTimeDB & ""
                Qry &= vbCrLf & ",FTAssetModelCode='" & Me.FTAssetModelCode.Text & "',FTAssetModelNameEN='" & Me.FTAssetModelNameEN.Text & "',FTAssetModelNameTH='" & Me.FTAssetModelNameTH.Text & "'"
                Qry &= vbCrLf & ",FTRemark='" & Me.FTRemark.Text & "',FTStateActive='" & _State & "' WHERE FNHSysAssetModelId=" & Me.MainKeyID & ""
                If HI.Conn.SQLConn.Execute_Tran(Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    HI.Conn.SQLConn.Tran.Rollback()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
                ...
                _cmd = "UPDATE [" & Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModel"
                _cmd &= " Set  FPImage=@FPImage"
                _cmd &= " WHERE FNHSysAssetModelId=@FNHSysAssetModelId"
                Dim cmd As New SqlCommand(_cmd, HI.Conn.SQLConn.Cnn, HI.Conn.SQLConn.Tran)
                cmd.Parameters.AddWithValue("@FNHSysAssetModelId", MainKeyID)
                Dim _pdf As New SqlParameter("@FPImage", SqlDbType.VarBinary)
                If Me.opnDialog.Text <> "" Then
                    ...
                    cmd.ExecuteNonQuery()
                Else
                    If gbFPImage.Controls.Count > 0 Then
                        Dim _PP As New DevExpress.XtraPdfViewer.PdfViewer
                        _PP = Me.Parent_Form.GetType.GetProperty("_PDFs").GetValue(Me.Parent_Form, Nothing)
                        Dim _Stream As New MemoryStream()
                        _PP.SaveDocument(_Stream)
                        _pdf.Value = _Stream.ToArray
                    Else
                        _pdf.Value = DBNull.Value
                    End If
                    cmd.Parameters.Add(_pdf)
                    cmd.ExecuteNonQuery()
                End If
                ...
                Me.Close()
            Else
                Qry = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart"
                Qry &= vbCrLf & "SET FTUpdUser='" & HI.ST.UserInfo.UserName & "',FDUpdDate=" & HI.UL.ULDate.FormatDateDB & ",FTUpdTime=" & HI.UL.ULDate.FormatTimeDB & ""
                Qry &= vbCrLf & ",FNQuantity=" & Me.FNQuantity.Value & ""
                Qry &= vbCrLf & "WHERE FNHSysAssetModelId=" & Me.FNHSysAssetModelId.Properties.Tag & " AND FNSeq=" & Me.FNSeq.Value & " AND FNHSysAssetPartId=" & Me.FNHSysAssetPartId.Properties.Tag & ""
                If HI.Conn.SQLConn.Execute_Tran(Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    HI.Conn.SQLConn.Tran.Rollback()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
    ...
    Private Sub ocmclear_Click(sender As Object, e As EventArgs) Handles ocmclear.Click
        If otbModel.SelectedTabPage.Name = TabModel.Name Then
            Me.FTAssetModelCode.Text = "" : Me.FTAssetModelNameEN.Text = "" : Me.FTAssetModelNameTH.Text = "" : Me.FTRemark.Text = "" : Me.opnDialog.Text = "" : Me.FTStateActive.Checked = False
            Me.gbFPImage.Controls.Clear()
        Else
            Me.FNHSysAssetPartId.Text = "" : Me.FNHSysAssetPartId_None.Text = "" : Me.FNQuantity.Value = 0
            Me.ocmedit.Visible = False
            Me.ocmaddnew.Visible = True
    ...
    Private Sub ocmdelete_Click(sender As Object, e As EventArgs) Handles ocmdelete.Click
        ...
        If HI.MG.ShowMsg.mConfirmProcess(HI.MG.ShowMsg.ProcessType.mDelete, Me.FTAssetModelCode.Text, Me.Text) = True Then

                _Str = " Delete From " & Me.TableName & " " & "  WHERE  " & Me.MainKey & "='" & Me.FTAssetModelCode.Text & "' "
                _Str &= vbCrLf & "Delete from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart WHERE FNHSysAssetModelId=" & Me.MainKeyID & ""
            If Not (HI.Conn.SQLConn.ExecuteNonQuery(_Str, _DBEnum)) Then
                    HI.MG.ShowMsg.mProcessNotComplete(MG.ShowMsg.ProcessType.mDelete, Me.Text)
        ...
        Else
            If VerifyDataTabModelPart() Then
                If HI.MG.ShowMsg.mConfirmProcess(HI.MG.ShowMsg.ProcessType.mDelete, Me.FTAssetModelCode.Text, Me.Text) = True Then
                    ...
                    _Str = " Delete From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart "
                    _Str &= vbCrLf & "WHERE FNHSysAssetModelId=" & Me.FNHSysAssetModelId.Properties.Tag & " AND FNSeq=" & Me.FNSeq.Value & " AND FNHSysAssetPartId=" & Me.FNHSysAssetPartId.Properties.Tag & ""
                    ...
                    Else
                        ...
                        Me.ocmaddnew.Visible = True
                        Me.ocmdelete.Enabled = False
                        _Str = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart "
                        _Str &= vbCrLf & "SET FNSeq=FNSeq-1 WHERE FNHSysAssetModelId=" & Me.FNHSysAssetModelId.Properties.Tag & "AND FNSeq>" & Me.FNSeq.Value & ""
    ...
    Private Sub ogvModelPartDetail_DoubleClick(sender As Object, e As EventArgs) Handles ogvModelPartDetail.DoubleClick

        Try
            With ogvModelPartDetail
                If Not (CType(ogcModelPartDetail.DataSource, DataTable) Is Nothing) Then
                    For Each RR As DataRow In CType(ogcModelPartDetail.DataSource, DataTable).Select("FNSeq=" & .GetRowCellValue(.FocusedRowHandle, "FNSeq" & ""))
                        Me.FNSeq.Value = Val(RR!FNSeq.ToString)
                        Me.FNHSysAssetModelId.Text = RR!FTAssetModelCode.ToString
    ...
    Private Sub ocmAddEdit_Click(sender As Object, e As EventArgs) Handles ocmAddEdit.Click
        Dim Qry As String = ""
        Dim _MaxFNSeq As String = ""
        Try
            Qry = "SELECt MAX(FNSeq) AS FNSeq  FroM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModelPart AS MO WITH(NOLOCK) WHERE FNHSysAssetModelId=" & Me.FNHSysAssetModelId.Properties.Tag & ""
    ...
    Private Sub LodaGridPartDetail()
        ...
        Qry &= vbCrLf & "WHERE MP.FNHSysAssetModelId=" & Me.FNHSysAssetModelId.Properties.Tag & " order by FNSeq asc"
        dt = HI.Conn.SQLConn.GetDataTable(Qry, Conn.DB.DataBaseName.DB_MASTER)
        Me.ogcModelPartDetail.DataSource = dt
        dt.Dispose()

    End Sub

    Private Function VerifyDataTabModel() As Boolean
        Dim _Pass As Boolean = False
        If Me.FTAssetModelCode.Text <> "" Then
            If Me.FTAssetModelNameEN.Text <> "" Then
                If Me.FTAssetModelNameTH.Text <> "" Then
                    'If Me.opnDialog.Text <> "" Then
                    'If Me.gbFPImage.Controls.Item(0).Created Then
                    _Pass = True
                    'Else
                    '    HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FPImage_lbl.Text)
                    'End If
                    'Else
                    '    HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FPImage_lbl.Text)
                    'End If
                Else
                    HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FTAssetModelNameTH_lbl.Text)
                    Me.FTAssetModelNameTH.Focus()
                End If
            Else
                HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FTAssetModelNameEN_lbl.Text)
                Me.FTAssetModelNameEN.Focus()
            End If
        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FTAssetModelCode_lbl.Text)
            Me.FTAssetModelCode.Focus()
        End If
        Return _Pass
    End Function

    Private Function VerifyDataTabModelPart() As Boolean
        Dim _Pass As Boolean = False
        If Me.FNHSysAssetPartId.Text <> "" Then
            ...
            End If
        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, Me.FNHSysAssetPartId_lbl.Text)
            Me.FNHSysAssetPartId.Focus()
        End If
        Return _Pass
    End Function

    Private Sub otbModel_SelectedPageChanged(sender As Object, e As DevExpress.XtraTab.TabPageChangedEventArgs) Handles otbModel.SelectedPageChanged
        Try
            If Me.CheckTab Then
                If otbModel.SelectedTabPage.Name = TabModel.Name Then
                ...
                End If
            Else
                If otbModel.SelectedTabPage.Name = TabModelPart.Name And Me.FNHSysAssetPartId.Text = "" Then
    ...
    Private Sub opnDialog_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles opnDialog.ButtonClick
    ...
    Private Sub ocmclearpdf_Click(sender As Object, e As EventArgs) Handles ocmclearpdf.Click
        Try
            Me.gbFPImage.Controls.Clear()
            Me.opnDialog.Text = ""
        Catch ex As Exception
