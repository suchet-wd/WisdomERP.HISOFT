...
Imports HI.TL
Public Class wAddItemAssetSafety
...
#End Region
    ...
    Private Sub ocmdelete_Click(sender As Object, e As EventArgs) Handles ocmdelete.Click
        ...
        If HI.MG.ShowMsg.mConfirmProcess(HI.MG.ShowMsg.ProcessType.mDelete, Me.FNFixedAssetType.Text, Me.Text) = True Then
            ...
            Qry = "DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetSafety WHERE FNHSysAssetSafetyId = " & Me._MainKeyID & ""
    ...
    Private Function VerityData() As Boolean
        ...
        If FNFixedAssetType.Text = "" Then
            ...
            Return False
        End If
        If FNHSysAssetTyped.Text = "" Then
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.Text, FNHSysAssetTyped_lbl.Text)
            FNHSysAssetTyped.Focus()
            Return False
        End If
        Return True
    ...
    Private Sub ocmedit_Click(sender As Object, e As EventArgs) Handles ocmedit.Click
        ...
        Try
            ...
            End If
            If VerityData() Then
                Qry = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetSafety "
                ...
                Qry &= vbCrLf & ", FNHSysAssetTyped = '" & Me.FNHSysAssetTyped.Properties.Tag & "'"
                Qry &= vbCrLf & ",FTUserName='" & Me.FTUserName.Text & "'"
                Qry &= vbCrLf & ",FNHSysCmpId='" & Me.FNHSysCmpId.Properties.Tag & "'"
                Qry &= vbCrLf & ",FTStateActive='" & _StateFa & "'"
                Qry &= vbCrLf & "WHERE FNHSysAssetSafetyId='" & Me.MainKeyID & "'"
    ...
    Private Sub ocmaddnew_Click(sender As Object, e As EventArgs) Handles ocmaddnew.Click
        ...
        'FNHSysAssetTyped_None.Text = ""
        'FNHSysCmpId.Text = ""
        _DG = HI.Conn.SQLConn.GetField("SELECT FNHSysAssetTyped,FTAssetTypeCode FROM(SELECT TY.FNHSysAssetTyped,TY.FTAssetTypeCode  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetType AS TY  WITH(NOLOCK) UNION SELECT TY.FNHSysAssetPartTyped AS FNHSysAssetTyped ,TY.FTAssetPartTypeCode AS FTAssetTypeCode FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetPartType AS TY )AS TY where FTAssetTypeCode='" & Me.FNHSysAssetTyped.Text & "' ", Conn.DB.DataBaseName.DB_MASTER, "")
        ...
        Try
            If VerityData() Then
                ...
                Else
                    _StateFa = "0"
                End If
                _SystemKey = HI.TL.RunID.GetRunNoID("" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "..TASMAssetSafety", "FNHSysAssetSafetyId", Conn.DB.DataBaseName.DB_MASTER)

                Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetSafety"
                Qry &= vbCrLf & "(FTInsUser,FDInsDate,FTinsTime,FNHSysAssetSafetyId,FNFixedAssetType,FTUserName,FNHSysAssetTyped,FNHSysCmpId,FTStateActive)"
                Qry &= vbCrLf & "Select '" & ST.UserInfo.UserName & "'," & UL.ULDate.FormatDateDB & "," & UL.ULDate.FormatTimeDB & ""
                Qry &= vbCrLf & "," & Val(_SystemKey) & "," & FNFixedAssetType.SelectedIndex & ",'" & FTUserName.Text & "'," & _DG & "," & Me.FNHSysCmpId.Properties.Tag & "," & _StateFa & ""
                If HI.Conn.SQLConn.Execute_Tran(Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) > 0 Then
                    HI.Conn.SQLConn.Tran.Commit()
                    HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                    HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
