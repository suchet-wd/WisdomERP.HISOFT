...
Public Class wReturnToStockAsset
...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        ogcdetail.DataSource = BarcodeAsset.LoadDocumentBarcode(Key, BarcodeAsset.DocType.ReturnToStock)
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReturnToStock WHERE FTReturnStockNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnStockNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_IN WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnStockNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReturnToStock WHERE FTReturnStockNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnStockNo.Text) & "'")
...
#Region "MAIN PROC"
    ...
    Private Sub Proc_Delete(sender As System.Object, e As System.EventArgs) Handles ocmdelete.Click
        ...
        If BarcodeAsset.CheckDocumentRefOut(Me.FTReturnStockNo.Text) Then
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTReturnStockNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .ReportName = "ReturnToStockAsset.rpt"
                .Formular = "{TFIXEDTReturnToStock.FTReturnStockNo}='" & HI.UL.ULF.rpQuoted(FTReturnStockNo.Text) & "' "
                .Preview()
            End With
...
#Region " Proc "
    ...
    Private Function SaveBarcode() As Boolean
        ...
        Try
            _Str = "select top 1 BI.FTDocumentNo from [" & Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_IN AS BI WITH(NOLOCK) where BI.FTDocumentNo='" & Me.FTReturnStockNo.Text & "'"
            _Str &= vbCrLf & " and BI.FTBarcodeNo='" & Me.FTBarcodeNo.Text.Trim() & "' and BI.FTDocumentRefNo='" & Me.DocRefNo & "'"
    ...
    Private Function DeleteBarcode(BarcodeKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_IN  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTReturnStockNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "'"
...
#End Region
    ...
    Private Sub AddBarCode(BarcodeNo As String, StateAdd As Boolean, Optional Qty As Double = 0)
        ...
        Dim dt As DataTable = BarcodeAsset.BarCodeOutForRet(FTBarcodeNo.Text, FNHSysWHAssetId.Properties.Tag, FTReturnStockNo.Text)
        If dt.Select("FNQuantityBal>=" & Qty & "and FNQuantityBal>0").Length > 0 Then
            For Each R As DataRow In dt.Rows
                Me.DocRefNo = R!FTDocumentNo.ToString
                Exit For
            Next
            If BarcodeAsset.CheckTransactionOUT(BarcodeNo, FTReturnStockNo.Text, FNHSysWHAssetId.Properties.Tag.ToString) Then
    ...
    Private Sub DeleteBarCode()
        ...
        With ogvdetail
            ...
            For Each i As Integer In .GetSelectedRows()
                ...
                If BarcodeAsset.CheckTransactionOUT(_Barcode, FTReturnStockNo.Text, FNHSysWHAssetId.Properties.Tag.ToString) Then
