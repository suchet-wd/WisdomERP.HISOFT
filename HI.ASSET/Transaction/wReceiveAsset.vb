...
#Region "Procedure"
    ...
    Private Sub LoadPOInfo(PoKey As String, Optional LoadRcv As Boolean = False)
        ...
        _Str = " SELECT        H.FTPurchaseNo, H.FNExchangeRate, S.FTSuplCode , C.FTCurCode,A.FTNameTH AS FNFixedAssetType "
        ...
        _Str &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail AS P WITH (NOLOCK) ON H.FTPurchaseNo=P.FTPurchaseNo INNER JOIN"
        ...
        _Str &= vbCrLf & "LEFT OUTER JOIN (		 SELECT L.FTNameTH ,L.FNListIndex"
        _Str &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L"
        _Str &= vbCrLf & "WHERE L.FTListName ='FNFixedAssetType'"
        _Str &= vbCrLf & "  )AS A ON P.FNFixedAssetType= A.FNListIndex"
    ...
    Private Sub LoadRcvDetail(PoKey As String)
        ...
        Dim _Type As String = HI.Conn.SQLConn.GetField("SELECT TOP 1 P.FNFixedAssetType FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail AS P  WITH(NOLOCK) WHERE  P.FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' ", Conn.DB.DataBaseName.DB_MASTER, "")
        'If (_State) Then
        ...
        '    Else
        '        _Str &= vbCrLf & " , isnull(AP.FTAssetPartNameEN,'-') AS FTAssetName"
        '    End If

        '    _Str &= vbCrLf & "  , D.FNHSysUnitId, U.FTUnitAssetCode as FTUnitCode, D.FNPrice, D.FNDisPer,AP.FNHSysAssetBrandId,"
        '    _Str &= vbCrLf & "  D.FNDisAmt, D.FNNetPrice, D.FNQuantity, D.FNNetAmt, '' AS FTRemark  ,AP.FTProductCode"
        ...
        '    _Str &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReceive AS H WITH (NOLOCK) ON D.FTReceiveNo=H.FTReceiveNo LEFT OUTER jOIN"
        ...
        '    _Str &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail AS P WITH (NOLOCK) ON H.FTPurchaseNo=P.FTPurchaseNo LEFT OUTER jOIN"
        ...
        '    _Str &= vbCrLf & "LEFT OUTER JOIN (		 SELECT L.FTNameTH ,L.FTNameEN ,L.FNListIndex"
        '    _Str &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L"
        '    _Str &= vbCrLf & "WHERE L.FTListName ='FNFixedAssetType'"
        '    _Str &= vbCrLf & "  )AS A ON P.FNFixedAssetType= A.FNListIndex"
        '    _Str &= vbCrLf & " WHERE        (D.FTReceiveNo = N'" & HI.UL.ULF.rpQuoted(PoKey) & "')"
        '    _Str &= vbCrLf & " ORDER BY D.FNHSysFixedAssetId"

        '    Me.ogcdetail.DataSource = HI.Conn.SQLConn.GetDataTable(_Str, _DBEnum)

        '    _Str = " SELECT        D.FNHSysFixedAssetId, isnull(AP.FTAssetPartCode,'-') as aFTAssetCode"

        '    If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
        '        _Str &= vbCrLf & " , isnull(AP.FTAssetPartNameTH,'-') AS aFTAssetName"
        '    Else
        '        _Str &= vbCrLf & " , isnull(AP.FTAssetPartNameEN,'-') AS aFTAssetName"
        '    End If


        '    _Str &= vbCrLf & ", D.FNHSysUnitId, U.FTUnitAssetCode as FTUnitCode, D.FNPrice, D.FNDisPer,isnull(AP.FTProductCode,'-') AS FTProductCode, "
        ...
        '    _Str &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L"
        '    _Str &= vbCrLf & "WHERE L.FTListName ='FNFixedAssetType'"
        '    _Str &= vbCrLf & "  )AS A ON PD.FNFixedAssetType= A.FNListIndex"
        ...
        If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
            _Str &= vbCrLf & " , isnull(M.FTAssetNameTH,AP.FTAssetPartNameTH) AS FTAssetName,A.FTNameTH AS FNFixedAssetType"
        Else
            _Str &= vbCrLf & " , isnull(M.FTAssetNameEN,AP.FTAssetPartNameEN) AS FTAssetName,A.FTNameEN AS FNFixedAssetType"
        End If

        _Str &= vbCrLf & "  , D.FNHSysUnitId, U.FTUnitAssetCode as FTUnitCode, D.FNPrice, D.FNDisPer, MO.FTAssetModelCode,MO.FNHSysAssetModelId,ISNULL(M.FNHSysAssetBrandId,AP.FNHSysAssetBrandId)AS FNHSysAssetBrandId,"
        _Str &= vbCrLf & "  D.FNDisAmt, D.FNNetPrice, D.FNQuantity, D.FNNetAmt, '' AS FTRemark  ,ISNULL(M.FTProductCode,AP.FTProductCode)AS FTProductCode"
        _Str &= vbCrLf & " ,Convert(numeric(18,2),(D.FNNetAmt * " & _VatPer & ")/100.00) AS FNVatAmt"
        _Str &= vbCrLf & " ,D.FNNetAmt + Convert(numeric(18,2),(D.FNNetAmt * " & _VatPer & ")/100.00) AS FNNetVatAmt"

        _Str &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReceive_Detail AS D WITH (NOLOCK) LEFT OUTER jOIN"
        _Str &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReceive AS H WITH (NOLOCK) ON D.FTReceiveNo=H.FTReceiveNo LEFT OUTER jOIN"
        _Str &= vbCrLf & "           [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail AS P WITH (NOLOCK) ON H.FTPurchaseNo=P.FTPurchaseNo AND D.FNHSysFixedAssetId=P.FNHSysFixedAssetId LEFT OUTER jOIN"
        _Str &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAsset AS M WITH (NOLOCK) ON D.FNHSysFixedAssetId = M.FNHSysFixedAssetId LEFT OUTER jOIN"
        _Str &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetPart as AP WITH(NOLOCK ) ON  D.FNHSysFixedAssetId = AP.FNHSysAssetPartId  Left OUTER JOIN "
        _Str &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitAsset AS U WITH (NOLOCK) ON D.FNHSysUnitId = U.FNHSysUnitAssetId LEFT OUTER jOIN"
        _Str &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetModel AS MO ON M.FNHSysAssetModelId=MO.FNHSysAssetModelId"
        ...
        If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
            _Str &= vbCrLf & " , isnull(M.FTAssetNameTH,AP.FTAssetPartNameTH) AS aFTAssetName,A.FTNameTH AS cFNFixedAssetType"
        Else
            _Str &= vbCrLf & " , isnull(M.FTAssetNameEN,AP.FTAssetPartNameEN) AS aFTAssetName,A.FTNameEN AS cFNFixedAssetType"
        End If
...
#End Region
    ...
    Private Sub ocmadd_Click(sender As System.Object, e As System.EventArgs) Handles ocmadd.Click
        ...
        _Str = "SELECT FNFixedAssetType FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail"
        ...
        If _AssetType <> 1 Then
            ...
            _Str &= vbCrLf & "   ISNULL(IM.FTAssetCode,AP.FTAssetPartCode) AS FTAssetCode ,MO.FTAssetModelCode AS Model,"

            If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                _Str &= vbCrLf & "  ISNULL(IM.FTAssetNameTH,AP.FTAssetPartNameTH) AS FTAssetName,"
            Else
                _Str &= vbCrLf & "  ISNULL(IM.FTAssetNameEN,AP.FTAssetPartNameEN)  AS FTAssetName,"
            End If
            ...
            _Str &= vbCrLf & "   Left OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAssetPart as AP WITH(NOLOCK ) ON  P.FNHSysFixedAssetId = AP.FNHSysAssetPartId"
            ...
            _Str &= vbCrLf & " FROM            [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReceive AS RH WITH (NOLOCK) LEFT OUTER JOIN"
            _Str &= vbCrLf & "     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReceive_Detail AS RD WITH (NOLOCK) ON RH.FTReceiveNo = RD.FTReceiveNo"
        ...
        Else
            ...
            _Str &= vbCrLf & " ORDER BY   AP.FTAssetPartCode "
        End If
        _Odt = HI.Conn.SQLConn.GetDataTable(_Str, Conn.DB.DataBaseName.DB_FIXED)
    ...
    Private Sub PrepareListBarcode()
        ...
        Dim dt As New DataTable
        Dim _Odt As DataTable
        'Qry = "SELECT FNFixedAssetType FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail"
        'Qry &= vbCrLf & "  WHERE      (FTPurchaseNo = N'" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Properties.Tag.ToString) & "') AND FNHSysFixedAssetId=" & Val(_IDasset) & ""
        'Dim _AssetType As Integer = HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_FIXED)

        Dim _AssetType As String = HI.Conn.SQLConn.GetField("SELECT TOP 1 P.FNFixedAssetType FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTPurchase_Detail AS P  WITH(NOLOCK) WHERE  P.FTPurchaseNo='" & HI.UL.ULF.rpQuoted(Me.FTPurchaseNo.Text) & "' ", Conn.DB.DataBaseName.DB_MASTER, "")
        ...
        '    Dim _Qry As String = "select B.FNFixedAssetType FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAsset AS B WITH(NOLOCK) where B.FNHSysFixedAssetId=" & Val(_IDasset) & ""
        '    Dim _AssetType As String = Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_FIXED)
        'Else
        ...
        Try
            With dt
                ...
                For Each R As DataRow In CType(ogcdetail.DataSource, DataTable).Rows
                    ...
                    If HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "6" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "7" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "1" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "2" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "4" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "9" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "10" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "11" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "12" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "13" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "14" Then
                    ...
                    If .Rows.Count > 0 Then
                        ...
                        If HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "" Then
                            For i As Integer = 0 To .Rows.Count - 1 Step 1
                                ...
                                Qry &= vbCrLf & "where A.FNFixedAssetType<>6 and  A.FNFixedAssetType<>7 and  A.FNFixedAssetType<>1  and  A.FNFixedAssetType<>2  and  A.FNFixedAssetType<>4  and  A.FNFixedAssetType<>9  and  A.FNFixedAssetType<>10  and  A.FNFixedAssetType<>11  and  A.FNFixedAssetType<>12  and  A.FNFixedAssetType<>13  and  A.FNFixedAssetType<>14 and A.FNHSysFixedAssetId=" & Val(_IDAsset) & ""
    ...
    Private Sub ocmgenbarcode_Click(sender As Object, e As EventArgs) Handles ocmgenbarcode.Click
        ...
        If _PO = "เครื่องจักร" Or _PO = "อุปกรณ์คอมพิวเตอร์" Or _PO = "เครื่องใช้ไฟฟ้า" Or _PO = "ยานพาหนะ" Then
            Call addmaster(FTReceiveNo.ToString)
        Else
