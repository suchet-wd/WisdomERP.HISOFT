...
#End Region
    ...
    Private Sub ocmadd_Click(sender As System.Object, e As System.EventArgs) Handles ocmadd.Click
        ...
        _Str &= vbCrLf & "    UNION ALL "
        _Str &= vbCrLf & "  Select M.FTAssetPartCode As FTAssetCode"
        _Str &= vbCrLf & "  ,MAX(M.FTAssetName) AS FTAssetName"
        _Str &= vbCrLf & "  ,MAX(M.FTUnitAssetCode) As FTUnitAssetCode "
        _Str &= vbCrLf & "  ,MAX(ISNULL(U.FTUnitAssetCode,'')) AS FNHSysUnitAssetId"
        _Str &= vbCrLf & "  ,MAX(ISNULL(U.FNHSysUnitAssetId,0)) As  FNHSysUnitAssetId_Hide"
        ...
        _Str &= vbCrLf & "UNION ALL"
        _Str &= vbCrLf & "Select P.FNHSysFixedAssetId,A.FTAssetPartCode AS FTAssetCode"
        If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
        ...
        Try
            With dt
                ...
                For Each R As DataRow In CType(ogcdetail.DataSource, DataTable).Rows
                    ...
                    If HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "6" Or HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) = "7" Then
                    ...
                    If .Rows.Count > 0 Then
                        ...
                        Else
                            ...
                            For k As Integer = 0 To .Rows.Count - 1 Step 1
                                ...
                                Qry = "select B.FNHSysFixedAssetId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAsset AS B WITH(NOLOCK) where B.FNFixedAssetType<>6 and B.FNFixedAssetType <>7 and B.FNHSysFixedAssetId=" & _IDAsset & ""
                                ...
                                If HI.Conn.SQLConn.GetField(Qry, Conn.DB.DataBaseName.DB_MASTER) <> "" Then
                                    ...
                                    Qry &= vbCrLf & "and A.FTReceiveNo='" & Me.FTReceiveNo.Text & "' and A.FTPurchaseNo='" & Me.FTPurchaseNo.Text & "' and A.FNFixedAssetType<>6 and A.FNfixedAssetType<>7"
        ...
        Try
            With ogvbarcode
                ...
                Else
                    ...
                    Qry = "select B.FNHSysFixedAssetId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TASMAsset AS B WITH(NOLOCK) where B.FNFixedAssetType<>6 and B.FNFixedAssetType<>7 and B.FNHSysFixedAssetId=" & _val & ""
                    ...
                    If Conn.SQLConn.GetDataTable(Qry, Conn.DB.DataBaseName.DB_FIXED).Rows.Count < CType(ogcdetail.DataSource, DataTable).Rows.Count Then
                        Try
                            For Each R As DataRow In CType(ogcdetail.DataSource, DataTable).Select("FNHSysFixedAssetId='" & _ID & "'")
                                ...
                                HI.Conn.SQLConn.ExecuteNonQuery(Qry, Conn.DB.DataBaseName.DB_FIXED)
                                Qry = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "]dbo.TASMTasst"
                            Next
                        Catch ex As Exception
                            MsgBox(ex.Message)
                        End Try
