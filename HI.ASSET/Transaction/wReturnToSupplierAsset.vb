...
Public Class wReturnToSupplierAsset
...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ogcdetail.DataSource = BarcodeAsset.LoadDocumentBarcode(Key, BarcodeAsset.DocType.ReturnToSupplier)
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReturnToSupplier WHERE FTReturnSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_OUT WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTReturnToSupplier WHERE FTReturnSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'")
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTReturnSuplNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .ReportName = "ReturnToSupplier.rpt"
                .Formular = "{TFIXEDTReturnToSupplier.FTReturnSuplNo}='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' "
                .Preview()
            End With
...
#Region "Proc Barcode"
    Private Function SaveBarcode() As Boolean
        ...
        Try
            _Str = "Select  Top 1 BO.FTDocumentNo from [" & Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_OUT As BO With(NOLOCK)"
            _Str &= vbCrLf & "where BO.FTDocumentNo ='" & Me.FTReturnSuplNo.Text & "' and BO.FTBarcodeNo='" & Me.FTBarcodeNo.Text.Trim & "' and BO.FTDocumentRefNo='" & Me.DocRefNo & "'"
    ...
    Private Function DeleteBarcode(BarcodeKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_FIXED) & "].dbo.TFIXEDTBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' ")
...
#End Region
    ...
    Private Sub AddBarCode(BarcodeNo As String, StateAdd As Boolean, Optional Qty As Double = 0)
        If CheckOwner() = False Then Exit Sub
        Dim dt As DataTable = BarcodeAsset.BarCodeBalance(FTBarcodeNo.Text, 0.ToString, FTReturnSuplNo.Text)
        If dt.Rows.Count > 0 Then
            If dt.Select("FNQuantityBal >=" & Qty & " AND FNQuantityBal >0").Length > 0 Then
                If dt.Select("FNQuantityBal >=" & Qty & " AND FNQuantityBal >0 AND FNHSysCmpId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & "").Length <= 0 Then
                    HI.MG.ShowMsg.mInvalidData("Barcode ไม่ใช่ของ Company นี้  !!!", 1312788101, Me.Text)
                    Exit Sub
                End If
                If dt.Select("FTPurchaseNo='" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "' AND FNQuantityBal >0 AND FNHSysCmpId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & "").Length > 0 Then
                    For Each R As DataRow In dt.Rows
                        Me.WH = Val(R!FNHSysWHAssetId.ToString)
                        Me.DocRefNo = R!FTDocumentNo.ToString
                        Exit For
                    Next
                    If (StateAdd) And Me.WH <> 0 Then
