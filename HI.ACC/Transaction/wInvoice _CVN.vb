...
#Region "Procedure"
    ...
    Private Sub PrepareForm()
        ...
        _Str &= vbCrLf & " WHERE FTDynamicFormName='wInvoice' "
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        Call LoadDocumentNo(Key.ToString)
    ...
    Private Sub LoadDocumentNo(PoKey As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " WHERE FTInvoiceNo='" & HI.UL.ULF.rpQuoted(Me.FTInvoiceNo.Text) & "' AND ISNULL(FTDocumentNo,'') <>'' "
        ...
        Try
            ...
            If Not (ogcdetail.DataSource Is Nothing) Then
                ...
                For Each R As DataRow In dt.Select("FTDescription <> '' and FNQuantity <> 0 and  FNHSysUnitId <> ''", "FNGrpSeq")
                    ...
                    _Qry &= vbCrLf & ",FTDocumentNo='" & HI.UL.ULF.rpQuoted(R!FTDocumentNo.ToString) & "'"
                    ...
                    If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                        ...
                        _Qry &= vbCrLf & ",FNPriceSale, FNChargeService, FNChargeClear, FNNetPrice,FNHSysUnitId,FTDescription, FTDocumentNo)"
...
#End Region
    ...
    Private Sub LoadDetail()
        Try
            ...
            _Qry &= vbCrLf & "    FTDescription  ,FNNetPrice *  FNQuantity AS FNAmount , FTDocumentNo "
            _Qry &= vbCrLf & "FROM    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoice_Detail AS D WITH (NOLOCK)"
            _Qry &= vbCrLf & "LEFT OUTER JOIN   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS U WITH(NOLOCK) ON D.FNHSysUnitId = U.FNHSysUnitId "
    ...
    Private Sub FTDocRefNo_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles FTDocRefNo.ButtonClick
        Try
            ...
            Dim _dtUsed As DataTable

            _Where &= vbCrLf & "  FTSaleAndTerminateNo not in(SELECT     FTDocumentNo  FROM  [HITECH_ACCOUNT]..TACCTInvoice_Detail WITH(NOLOCK) WHERE     (FTInvoiceNo = N'" & HI.UL.ULF.rpQuoted(Me.FTInvoiceNo.Text) & "'))"
            ...
            With New wDynamicBrowseSelectInfo(e.Button.Tag.ToString, _Where)
                ...
                Dim _msgUsed As String

                Dim n As Integer = 0

                Dim _flag_duplicate As String = ""

                For Each R As DataRow In _oDt.Select("FTSelect = '1'")

                    n = 0
                    _flag_duplicate = ""
                    ''validation   docref used
                    _Cmd = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.[SP_GET_SaleAndTerminate_Used] '" & R!DocumentNo.ToString & "', '" & FTInvoiceNo.Text & "'"
                    _dtUsed = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT)

                    For Each RR As DataRow In _dtUsed.Rows

                        n = Val(RR!n.ToString)
                        If n > 0 Then
                            If _msgUsed = "" Then
                                _msgUsed &= R!DocumentNo.ToString
                            Else
                                _msgUsed &= " ," & R!DocumentNo.ToString
                            End If
                    ...
                    For Each RR As DataRow In _dtdoc.Select("FTDocumentNo = '" & R!DocumentNo.ToString & "'")

                        _flag_duplicate = "Y"

                    Next
                    If _flag_duplicate = "" Then
                        _dtdoc.Rows.Add(R!DocumentNo.ToString)
                    End If
                ...
                If _msgUsed <> "" Then
                    ...
                    HI.MG.ShowMsg.mInfo("พบเอกสารอ้างอิงถูกนำไปใช้งานแล้ว......", 2506181749, Me.Text, _msgUsed, MessageBoxIcon.Stop)
                    FTDocRefNo.Focus()
    ...
    Private Sub LoadDocumentData()
        Try
            ...
            Dim FTDocumentNo As String = ""

            ''Remove
            For Each R As DataRow In _dtdoc.Rows

                FTDocumentNo = R!FTDocumentNo.ToString

                For Each RR As DataRow In _dtdetail.Select("FTDocumentNo = '" & FTDocumentNo & "'")

                    _dtdetail.Rows.Remove(RR)
            ...
            '' x = _dtdetail.Rows.Count

            For Each RR As DataRow In _dtdetail.Select(" FNGrpSeq > 0   ")

                If Val(RR!FNGrpSeq) > 0 And RR!FTDocumentNo.ToString = "" And RR!FNQuantity.ToString = "" Then
                    _dtdetail.Rows.Remove(RR)
                End If

            Next

            For Each RR As DataRow In _dtdetail.Select("FNGrpSeq > 0  AND FNQuantity <> '' ")

                x = x + 1
            Next

            'If x = 0 Then
            '    For Each RR As DataRow In _dtdetail.Rows
            ...
            For Each R As DataRow In _dtdoc.Rows
                ...
                _Qry = " "
                _Qry = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.[SP_GET_SaleAndTerminate] '" & FTDocumentNo & "'"
                _oDt = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_INVEN)

                x = x + 1

                For Each rr As DataRow In _oDt.Rows
                    _dtdetail.Rows.Add("", 0, x, rr!FNQuantity, rr!FNPriceSale, rr!FNChargeService, rr!FNChargeClear, rr!FNNetPrice, rr!FNHSysUnitId, rr!FNHSysUnitId_Hide, rr!FTDescription, rr!FNAmount, rr!FTDocumentNo)
                Next
    ...
    ''best for select Dockey  SaleAndTerminate
    Public Shared Function LoadDocumentBarcode(DocKey As String, Optional BarcodeKey As String = "") As DataTable
        Dim _Str As String = ""
        Dim dt As New DataTable
    ...
    Private Sub ogvdocref_CustomFilterDialog(sender As Object, e As CustomFilterDialogEventArgs) Handles ogvdocref.CustomFilterDialog
