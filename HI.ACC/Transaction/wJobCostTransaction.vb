Public Class wJobCostTransaction 
...
#End Region
    ...
    Public Sub LoadOrderCostDataInfo(ByVal Key As Object)
        ...
        Try
            ...
            Call LoadImport(Key)
            Call CalculatePer()
            Call LoadTotalAllocate()
            Call LoadAllocateCmp()
            Call LoadAllocatePICust()
            Call LoadAllocatePICustPO()
            Call LoadAllocatePIPO()
            'Call LoadAllocateProd()
...
#End Region
    ...
    Private Sub ocmsave_Click(sender As Object, e As EventArgs) Handles ocmsave.Click
        If Me.FTOrderNo.Text <> "" Then
            ...
            _Qry = " SELECT TOP 1 ISNULL(X.FTOrderNoRef,'') AS FTOrderNoRef "
            _Qry &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS X WITH(NOLOCK)"
            _Qry &= vbCrLf & " INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH(NOLOCK) ON ISNULL(X.FTOrderNoRef,'') = O.FTOrderNo "
            _Qry &= vbCrLf & " WHERE X.FTOrderNo='" & HI.UL.ULF.rpQuoted(FTOrderNo.Text) & "'  AND O.FNOrderType = 22"
    ...
    Private Sub LoadAllocate()
        Try
            ...
            _Cmd = " select O.FTInvoiceNo,case when isdate(R.FDReceiveDate) = 1 then CONVERT(varchar(10),convert(datetime,R.FDReceiveDate),103) else '' end as FDReceiveDate,R.FNQuantity,R.FNNetAmt as FNAmountJob ,O.FNAmount"
            If ST.Lang.Language = ST.Lang.eLang.TH Then
                _Cmd &= vbCrLf & ",( SELECT TOP 1 STUFF ((SELECT  ', ' + L.FTNameTH FROM      ( select  DISTINCT L.FTNameTH,L.FNListIndex ,R.FTInvoiceNo,RO.FTOrderNo"
                _Cmd &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L WITH(NOLOCK) left outer join"
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive AS R WITH(NOLOCK)  ON L.FNListIndex=R.FNRceceiveType left outer join "
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive_Detail_Order AS RO WITH(NOLOCK)  ON R.FTReceiveNo=RO.FTReceiveNo"
                _Cmd &= vbCrLf & " where L.FTListName='FNRceceiveType' ) L "
                _Cmd &= vbCrLf & "where L.FTInvoiceNo=O.FTInvoiceNo and L.FTOrderNo=R.FTOrderNo FOR XML PATH('')), 1, 2, '')  ) as FNRceceiveType"

            Else
                _Cmd &= vbCrLf & ",( SELECT TOP 1 STUFF ((SELECT  ', ' + L.FTNameEN FROM      ( select  DISTINCT L.FTNameEN,L.FNListIndex ,R.FTInvoiceNo,RO.FTOrderNo"
                _Cmd &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData AS L WITH(NOLOCK) left outer join"
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive AS R WITH(NOLOCK)  ON L.FNListIndex=R.FNRceceiveType left outer join "
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive_Detail_Order AS RO WITH(NOLOCK)  ON R.FTReceiveNo=RO.FTReceiveNo"
                _Cmd &= vbCrLf & " where L.FTListName='FNRceceiveType' ) L "
            ...
            _Cmd &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceCharge_Order as O  WITH(NOLOCK)  left outer join"
            _Cmd &= vbCrLf & "(select R.FTInvoiceNo,RO.FTOrderNo,sum(RO.FNQuantity)as FNQuantity,sum(RO.FNNetAmt)AS FNNetAmt,R.FDReceiveDate"
            ...
            _Cmd &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive_Detail_Order as RO ON R.FTReceiveNo =RO.FTReceiveNo"
            _Cmd &= vbCrLf & "group by R.FTInvoiceNo,RO.FTOrderNo,R.FDReceiveDate)AS R ON O.FTInvoiceNo=R.FTInvoiceNo and O.FTOrderNo=R.FTOrderNo"
            _Cmd &= vbCrLf & "where R.FTOrderNo='" & Me.FTOrderNo.Text & "'"
            _Cmd &= vbCrLf & " group by  O.FTInvoiceNo,R.FDReceiveDate,O.FNAmount,R.FTOrderNo,R.FNQuantity,R.FNNetAmt"


            Me.ogcchange.DataSource = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT)
            Me.ogvchange.ExpandAllGroups()
        Catch ex As Exception
        End Try
    End Sub

    Private Sub LoadTotalAllocate()
        Dim _Qry As String = ""
        Dim Str As String = ""


        _Qry = " SELECT isnull(sum(O.FNAmount),'0')as FNTotalAllocate"
        _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceCharge_Order AS O WITH(NOLOCK)  "
        _Qry &= vbCrLf & "   WHERE  O.FTOrderNo='" & Me.FTOrderNo.Text & "'"

        FNTotalAllocate.Value = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)

    End Sub


    Private Sub LoadAllocatePIPO()
        Dim _Qry As String = ""
        Dim Str As String = ""


        _Qry = " SELECT isnull(sum(BO.FNAmount),'0') as FNAmount"
        _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge AS B  WITH(NOLOCK)  LEFT OUTER JOIN "
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge_Order AS BO ON B.FTInvoiceBankNo=BO.FTInvoiceBankNo"
        _Qry &= vbCrLf & "   WHERE  BO.FTOrderNo='" & Me.FTOrderNo.Text & "' AND B.FNPIChargeType='0' "

        FNPIAllocatePO.Value = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)


    End Sub
    Private Sub LoadAllocatePICustPO()
        Dim _Qry As String = ""
        Dim Str As String = ""
        ...
        _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge AS B  WITH(NOLOCK)  LEFT OUTER JOIN "
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge_Order AS BO ON B.FTInvoiceBankNo=BO.FTInvoiceBankNo"
        _Qry &= vbCrLf & "   WHERE  BO.FTOrderNo='" & Me.FTOrderNo.Text & "' AND B.FNPIChargeType='1' "

        FNPIAllocateCustPO.Value = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)
    End Sub
    Private Sub LoadAllocatePICust()
        Dim _Qry As String = ""
        Dim Str As String = ""
        ...
        _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge AS B  WITH(NOLOCK)  LEFT OUTER JOIN "
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge_Order AS BO ON B.FTInvoiceBankNo=BO.FTInvoiceBankNo"
        _Qry &= vbCrLf & "   WHERE  BO.FTOrderNo='" & Me.FTOrderNo.Text & "' AND B.FNPIChargeType='3' "

        FNPIAllocateCust.Value = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)
    End Sub
    Private Sub LoadAllocateCmp()
        Dim _Qry As String = ""
        Dim Str As String = ""
        ...
        _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge AS B  WITH(NOLOCK)  LEFT OUTER JOIN "
        _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTInvoiceBankCharge_Order AS BO ON B.FTInvoiceBankNo=BO.FTInvoiceBankNo"
        _Qry &= vbCrLf & "   WHERE  BO.FTOrderNo='" & Me.FTOrderNo.Text & "' AND B.FNPIChargeType='2' "

        FNPIAllocateCmp.Value = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)
    End Sub
    'Private Sub LoadAllocateProd()
    '    Dim _Qry As String = ""
    '    Dim Str As String = ""


    '    _Qry = " SELECT isnull( sum(P.FNAmount),'0') as FNAmount"
    '    _Qry &= vbCrLf & "   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTProdCost_Order AS P  WITH(NOLOCK)   "
    '    _Qry &= vbCrLf & "   WHERE  P.FTOrderNo='" & Me.FTOrderNo.Text & "' "

    '    FNProdCost.Value = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)
    'End Sub
End Class