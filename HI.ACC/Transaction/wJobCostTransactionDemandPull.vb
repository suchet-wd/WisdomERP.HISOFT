Public Class wJobCostTransactionDemandPull
...
#Region "Initial Grid"
    ...
    Private Sub InitGridOrdercharge()
        '------Start Add Summary Grid-------------
        Dim sFieldCount As String = ""
        Dim sFieldSum As String = ""
        Dim sFieldSumAmt As String = "FNEmbSubCost|FNPrintSubCost|FNEmbBranchCost|FNPrintBranchCost|FNEmbFacCost|FNWageCutSew|FNWageCutTrim"

        Dim sFieldGrpCount As String = ""
        Dim sFieldGrpSum As String = ""
        Dim sFieldGrpSumAmt As String = "FNEmbSubCost|FNPrintSubCost|FNEmbBranchCost|FNPrintBranchCost|FNEmbFacCost|FNWageCutSew|FNWageCutTrim"
...
#End Region
    ...
    '                _Str &= vbCrLf & "   FNAccFabStockOtherBalCost, FNExportNetCostAmt, FNNetProfitRcv, FNProfitPerPieceRcv, FNWageCutSew, FNWageCutTrim,FNWagePull,FNWagePullAct"
    ...
    '                _Str &= vbCrLf & "  FNTransportAirCost, FNOtherCost, FNNetCost, FNNetProfit, FNProfitPerPiece, FTRemark, FNExportQty, FNConductedCostPer, FNCommissionCostPer, FNAccFabStockOtherCost, FNFabricRcvCost,"
    '                _Str &= vbCrLf & "   FNAccessroryRcvCost, FNFabricDummyCost, FNAccessroryDummyCost, FNFabricBalCost, FNAccessroryBalCost, FNAccFabStockRcvCost, FNAccFabStockOtherRcvCost, FNAccFabStockBalCost,"
    '                _Str &= vbCrLf & "   FNAccFabStockOtherBalCost, FNExportNetCostAmt, FNNetProfitRcv, FNProfitPerPieceRcv, FNWageCutSew, FNWageCutTrim,FNWagePull,0 AS FNWagePullAct"
    ...
    '                _Str &= vbCrLf & "   FNExportNetCostAmt, FNNetProfitRcv, FNProfitPerPieceRcv, FNWageCutSew, FNWageCutTrim,FNExportQty AS FNTotalExportQty,FNExportQty AS FNOrderQty,0 AS FNMonthSeq,0 AS FNTotalExportBFQty"
    ...
    '                _Str &= vbCrLf & "     A.FNCalAccFabStockOtherRcvCost, A.FNCalAccFabStockBalCost, A.FNCalAccFabStockOtherBalCost, A.FNCalExportNetCostAmt, A.FNCalNetProfitRcv, A.FNCalProfitPerPieceRcv, A.FNCalWageCutSew, A.FNCalWageCutTrim"
    ...
    '                                                                                           _FNExportNetCostAmt, _FNNetProfitRcv, _FNProfitPerPieceRcv, _FNWageCutSew, _FNWageCutTrim, FNTotalExportQty, _FNOrderQty, FNMonthSeq, FNTotalExportBFQty)
    ...
    '                    _FNCalExportNetAmt = FNMonthlyExportAmt - (TotalFNFabricCost + TotalFNAccessroryCost + TotalFNAccFabStockCost + TotalFNServiceCost + TotalFNAccFabStockOtherCost)
    '                    _FNCalExportNetCostAmt = FNMonthlyExportAmt - ((TotalFNFabricRcvCost + TotalFNFabricDummyCost) + (TotalFNAccessroryRcvCost + TotalFNAccessroryDummyCost) + TotalFNAccFabStockRcvCost + TotalFNServiceCost + TotalFNAccFabStockOtherRcvCost)
    ...
    Private Function SaveData() As Boolean
        ...
        With CType(Me.ogcordercost.DataSource, DataTable)
            .AcceptChanges()
            dtorder = .Copy
        End With
        ...
        Try
            ...
            _Str &= vbCrLf & "  WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "' "
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                ...
                _Str &= vbCrLf & " , " & HI.UL.ULDate.FormatDateDB & ""
                _Str &= vbCrLf & " , " & HI.UL.ULDate.FormatTimeDB & ""
                _Str &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "'"
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_Invoice WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "'"
            ...
            For Each R As DataRow In dt.Select("FTInvoiceNo<>'' AND FDInvoiceDate<>'' ")
                ...
                If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    _Str &= vbCrLf & ", FNStockQuantity, FNPrice, FNExchangeRate, FNExportAmt, FNExportAmtTHB,FTOrderNoRef"
                    _Str &= vbCrLf & " )"
                    _Str &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                    _Str &= vbCrLf & " ," & HI.UL.ULDate.FormatDateDB & ""
                    _Str &= vbCrLf & " ," & HI.UL.ULDate.FormatTimeDB & ""
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_Invoice_Cost WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "'"
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_OrderRef_Cost WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "'"
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

            For Each R As DataRow In dtorder.Select("FTOrderNoRef<>'' ")

                _Str = " UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_OrderRef_Cost"
                _Str &= vbCrLf & " SET FTUpdUser='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                _Str &= vbCrLf & " , FDUpdDate=" & HI.UL.ULDate.FormatDateDB & ""
                _Str &= vbCrLf & " , FTUpdTime=" & HI.UL.ULDate.FormatTimeDB & ""
                _Str &= vbCrLf & " , FNEmbSubCost=" & Val(R!FNEmbSubCost.ToString) & ""
                _Str &= vbCrLf & " , FNPrintSubCost=" & Val(R!FNPrintSubCost.ToString) & ""
                _Str &= vbCrLf & " , FNEmbBranchCost=" & Val(R!FNEmbBranchCost.ToString) & ""
                _Str &= vbCrLf & " , FNPrintBranchCost=" & Val(R!FNPrintBranchCost.ToString) & ""
                _Str &= vbCrLf & " , FNEmbFacCost=" & Val(R!FNEmbFacCost.ToString) & ""
                _Str &= vbCrLf & " , FNWageCutSew=" & Val(R!FNWageCutSew.ToString) & ""
                _Str &= vbCrLf & " , FNWageCutTrim=" & Val(R!FNWageCutTrim.ToString) & ""
                _Str &= vbCrLf & "   WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "'"
                _Str &= vbCrLf & "       AND FTOrderNoRef='" & HI.UL.ULF.rpQuoted(R!FTOrderNoRef.ToString) & "'"

                If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then

                    _Str = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_OrderRef_Cost"
                    _Str &= vbCrLf & " ("
                    _Str &= vbCrLf & " FTInsUser, FDInsDate, FTInsTime, FTOrderNo"
                    _Str &= vbCrLf & ", FTOrderNoRef,FNEmbSubCost,FNPrintSubCost,FNEmbBranchCost,FNPrintBranchCost,FNEmbFacCost,FNWageCutSew,FNWageCutTrim"
                    _Str &= vbCrLf & " )"
                    _Str &= vbCrLf & " SELECT '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                    _Str &= vbCrLf & " ," & HI.UL.ULDate.FormatDateDB & ""
                    _Str &= vbCrLf & " ," & HI.UL.ULDate.FormatTimeDB & ""
                    _Str &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Properties.Tag.ToString) & "'"
                    _Str &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(R!FTOrderNoRef.ToString) & "'"
                    _Str &= vbCrLf & " ," & Val(R!FNEmbSubCost.ToString) & ""
                    _Str &= vbCrLf & " ," & Val(R!FNPrintSubCost.ToString) & ""
                    _Str &= vbCrLf & " ," & Val(R!FNEmbBranchCost.ToString) & ""
                    _Str &= vbCrLf & " ," & Val(R!FNPrintBranchCost.ToString) & ""
                    _Str &= vbCrLf & " ," & Val(R!FNEmbFacCost.ToString) & ""
                    _Str &= vbCrLf & " ," & Val(R!FNWageCutSew.ToString) & ""
                    _Str &= vbCrLf & " ," & Val(R!FNWageCutTrim.ToString) & ""
    ...
    Private Function DeleteData() As Boolean
        ...
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(Me.FNHSysStyleId.Properties.Tag.ToString) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_Invoice WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(Me.FNHSysStyleId.Properties.Tag.ToString) & "'"

            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_Monthly WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(Me.FNHSysStyleId.Properties.Tag.ToString) & "'"
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)

            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_Monthly WHERE FTOrderNoMain='" & HI.UL.ULF.rpQuoted(Me.FNHSysStyleId.Properties.Tag.ToString) & "'"
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(Me.FNHSysStyleId.Text) & "'")
    ...
    Public Sub LoadOrderCostDataInfo(ByVal _FNHSysStyleId As Object)
        ...
        Try
            ...
            _Qry &= vbCrLf & " 	WHERE Sub.FTOrderNo IN (SELECT XX.FTOrderNo "
            _Qry &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS XX WITH(NOLOCK)  "
            _Qry &= vbCrLf & "   WHERE XX.FNHSysStyleId=" & Val(_FNHSysStyleId) & " AND XX.FNOrderType =22 "
            _Qry &= vbCrLf & " )"
            ...
            _Qry &= vbCrLf & "   WHERE O.FNHSysStyleId=" & Val(_FNHSysStyleId) & " AND O.FNOrderType =22 "
            _Qry &= vbCrLf & " 	 ) AS A"
            ...
            _Qry &= vbCrLf & "   WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(_FNHSysStyleId.ToString) & "'"
            ...
            Call LoadOrderNoRef(_FNHSysStyleId.ToString)
            Call LoadDetailOrderInvoicecharge(_FNHSysStyleId.ToString)
            Call LoadDetailOrderCostcharge(_FNHSysStyleId.ToString)
            Call LoadDetailOrderCost(_FNHSysStyleId.ToString)
            ...
            Dim _dtdemandpull As DataTable
            _Qry = "SELECT XX.FTOrderNo "
            _Qry &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS XX WITH(NOLOCK)  "
            _Qry &= vbCrLf & "   WHERE XX.FNHSysStyleId=" & Val(_FNHSysStyleId) & " AND XX.FNOrderType =22"
            _Qry &= vbCrLf & " ORDER BY XX.FTOrderNo "

            _dtdemandpull = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN)

            For Each R As DataRow In _dtdemandpull.Rows

                Call LoadDetailOrderCostRawmat(R!FTOrderNo.ToString)
                Call LoadWagePull(R!FTOrderNo.ToString)
                Call LoadImport(R!FTOrderNo.ToString)

            Next


            Call LoadDetailOrderCostRawmatDetail(_FNHSysStyleId.ToString)
    ...
    Private Sub LoadWagePull(Key As String)
        ...
        FNWagePull.Value = FNWagePull.Value + Val(HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT, "0"))
    ...
    Private Sub LoadImport(Key As String)
        Try
            ...
            FNImportCost.Value = FNImportCost.Value + Double.Parse(HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT, "0"))
        Catch ex As Exception
    ...
    Public Sub LoadOrderNoRef(ByVal Key As Object)
        ...
        _Qry &= vbCrLf & "   WHERE  (A.FNHSysStyleIdPull = " & Val(Key) & " AND ISNULL(A.FNHSysStyleIdPull,0) > 0 )"
        _Qry &= vbCrLf & ") AS A"
    ...
    Private Sub LoadDetailOrderCostcharge(Key As String)
        Dim _Qry As String
        Dim _dt As DataTable

        _Qry = "SELECT A.FTOrderNoRef"
        _Qry &= vbCrLf & " , A.FNEmbSubCost, A.FNPrintSubCost, A.FNEmbBranchCost"
        _Qry &= vbCrLf & " , A.FNPrintBranchCost, A.FNEmbFacCost"
        _Qry &= vbCrLf & " ,  A.FNWageCutSew, A.FNWageCutTrim"
        _Qry &= vbCrLf & "   FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTJobCost_OrderRef_Cost AS A WITH(NOLOCK) "
        _Qry &= vbCrLf & "   WHERE  (A.FTOrderNo = N'" & HI.UL.ULF.rpQuoted(Key) & "')"
        _Qry &= vbCrLf & "   ORDER BY A.FTOrderNoRef"

        _dt = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT)

        Me.ogcordercost.DataSource = _dt.Copy
    ...
    Private Sub LoadDetailOrderCost(Key As String)
        ...
        _Qry &= vbCrLf & "  , A.FNExchangeRate, A.FNExportAmt, A.FNExportAmtTHB"
        ...
        _Qry &= vbCrLf & " 	),'') AS FDShipDate"
        _Qry &= vbCrLf & "   FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS A WITH(NOLOCK)"
        _Qry &= vbCrLf & "   WHERE  (A.FNHSysStyleIdPull = " & HI.UL.ULF.rpQuoted(Key) & " AND ISNULL(A.FNHSysStyleIdPull,0) > 0 )"
        _Qry &= vbCrLf & ") AS A"
        _Qry &= vbCrLf & " "
    ...
    Private Sub LoadDetailOrderCostRawmat(Key As String)
        ...
        _Qry = "SELECt TOP 1 FNHSysCmpId  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS A WITH(NOLOCK) WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(Key) & "' "
        ...
        '_Qry &= vbCrLf & "    FROM      [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT AS XB WITH(NOLOCK) INNER JOIN"
        '_Qry &= vbCrLf & "              [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReserve AS XRSV WITH(NOLOCK) ON XB.FTDocumentRefNo = XRSV.FTReserveNo "
        '_Qry &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS XWH WITH(NOLOCK) ON XB.FNHSysWHId = XWH.FNHSysWHId "
        '_Qry &= vbCrLf & "     WHERE   XB.FTBarcodeNo=B.FTBarcodeNo "
        '_Qry &= vbCrLf & "     AND    XB.FTDocumentNo=B.FTDocumentRefNo "
        ...
        '_Qry &= vbCrLf & "          [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReserve AS RSV WITH(NOLOCK) ON B.FTDocumentRefNo = RSV.FTReserveNo "
        ...
        FNServiceCost.Value = FNServiceCost.Value + Val(HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_PUR, "0"))

        FNFabricCost.Value = FNFabricCost.Value + _FNFabricCost
        FNAccessroryCost.Value = FNAccessroryCost.Value + _FNAccessroryCost
        FNAccFabStockCost.Value = FNAccFabStockCost.Value + _FNAccFabStockCost
        FNAccFabStockOtherCost.Value = FNAccFabStockOtherCost.Value + _FNAccFabStockCostOther
    ...
    Private Sub LoadDetailOrderCostRawmatDataRcv(Key As String)
        ...
        For Each Rx As DataRow In _dtdummy.Rows
            ...
            '_Qry &= vbCrLf & "   FROM    "
            '_Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive_Detail_Order AS B WITH(NOLOCK)  "
            '_Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReceive AS C WITH(NOLOCK)  ON B.FTReceiveNo = C.FTReceiveNo INNER JOIN"
            '_Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS W WITH(NOLOCK)  ON C.FNHSysWHId = W.FNHSysWHId"
            '_Qry &= vbCrLf & "  	   INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMaterial AS IM WITH(NOLOCK) ON B.FNHSysRawMatId = IM.FNHSysRawMatId "
            '_Qry &= vbCrLf & "      LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMainMat AS MM WITH(NOLOCK) ON IM.FTRawMatCode = MM.FTMainMatCode "
        ...
        FNFabricRcvCost.Value = FNFabricRcvCost.Value + _FNFabricRcvCost
        FNAccessroryRcvCost.Value = FNAccessroryRcvCost.Value + _FNAccessroryRcvCost
        FNFabricDummyCost.Value = FNFabricDummyCost.Value + _FNFabricDummyCost
        FNAccessroryDummyCost.Value = FNAccessroryDummyCost.Value + _FNAccessroryDummyCost

        _FNFabricBalCost = (FNFabricRcvCost.Value + FNFabricDummyCost.Value) - FNFabricCost.Value
        _FNAccessroryBalCost = (FNAccessroryRcvCost.Value + FNAccessroryDummyCost.Value) - FNAccessroryCost.Value

        FNFabricBalCost.Value = _FNFabricBalCost
        FNAccessroryBalCost.Value = _FNAccessroryBalCost

        FNFabricSum.Value = FNFabricSum.Value + _FNFabricRcvCost + _FNFabricDummyCost
        FNAccessrorySum.Value = FNAccessrorySum.Value + _FNAccessroryRcvCost + _FNAccessroryDummyCost
        ...
        FNAccFabStockRcvCost.Value = +FNAccFabStockRcvCost.Value + _FNAccFabStockRcvCost
        FNAccFabStockOtherRcvCost.Value = FNAccFabStockOtherRcvCost.Value + _FNAccFabStockOtherRcvCost

        _FNAccFabStockBalCost = FNAccFabStockRcvCost.Value - FNAccFabStockCost.Value
        _FNAccFabStockOtherBalCost = FNAccFabStockOtherRcvCost.Value - FNAccFabStockOtherCost.Value
    ...
    Private Sub LoadDetailOrderCostRawmatDetail(Key As String)
        ...
        _Qry &= vbCrLf & "   WHERE   (FTOrderNo IN ("

        _Qry &= vbCrLf & "SELECT XX.FTOrderNo "
        _Qry &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS XX WITH(NOLOCK)  "
        _Qry &= vbCrLf & "   WHERE XX.FNHSysStyleId=" & Val(Key) & " AND XX.FNOrderType =22"
        ...
        _Qry &= vbCrLf & "   WHERE   (H.FTOrderNo IN ("
...
#End Region
    ...
    Private Sub ocmdelete_Click(sender As Object, e As EventArgs) Handles ocmdelete.Click
        If Me.FNHSysStyleId.Text <> "" Then
            If HI.MG.ShowMsg.mConfirmProcess(MG.ShowMsg.ProcessType.mDelete, Me.FNHSysStyleId.Text) = True Then
                    ...
                    Try
                        If Not (Me.ogc.DataSource Is Nothing) Then
                            ...
                            If Me.ogcinvoicecharge.DataSource Is Nothing Then
                                Call LoadDetailOrderInvoicecharge(Me.FNHSysStyleId.Properties.Tag.ToString())
                            End If

                            If Me.ogcordercost.DataSource Is Nothing Then
                                Call LoadDetailOrderCostcharge(Me.FNHSysStyleId.Properties.Tag.ToString)
                            End If
                            ...
                            dtorder.BeginInit()
                            For Each R As DataRow In dtorder.Select("FTOrderNoRef<>''")
                                If dtm.Select("FTOrderNoRef='" & HI.UL.ULF.rpQuoted(R!FTOrderNoRef.ToString) & "'").Length <= 0 Then
                                    R.Delete()
                                End If
                            Next
                            dtorder.EndInit()

                            For Each R As DataRow In dtm.Select("FTOrderNoRef<>''")
                                If dtorder.Select("FTOrderNoRef='" & HI.UL.ULF.rpQuoted(R!FTOrderNoRef.ToString) & "'").Length <= 0 Then
                                    dtorder.Rows.Add(R!FTOrderNoRef.ToString, 0, 0, 0, 0, 0, 0, 0)
                                End If
                            Next

                            Me.ogcinvoicecharge.DataSource = dtinv.Copy
                            Me.ogcordercost.DataSource = dtorder.Copy
    ...
    Private Sub ocmsave_Click(sender As Object, e As EventArgs) Handles ocmsave.Click
        If Me.FNHSysStyleId.Text <> "" And Val(FNHSysStyleId.Properties.Tag.ToString) > 0 Then
            _StateSaveJobType = 0
            Dim _Qry As String = ""
            ...
            If HI.MG.ShowMsg.mConfirmProcess(MG.ShowMsg.ProcessType.mSave, Me.FNHSysStyleId.Text) = True Then
                Dim _Spls As New HI.TL.SplashScreen("Saving....Data,Please Wait.....")

                If Me.SaveData() Then
                    _Spls.UpdateInformation("Calculeting...,Please wait...")
                    Call CalculateCostData(Me.FNHSysStyleId.Properties.Tag.ToString)
                    _Spls.Close()
                    Me.otbmain.SelectedTabPageIndex = 0
                    Me.otbexport.SelectedTabPageIndex = 0
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        ...
        If Me.FNHSysStyleId.Text <> "" Then
            ...
            With New HI.RP.Report
                ...
                .ReportName = "JobCostDemandPullSlip.rpt"
                .Formular = "{TACCTJobCost.FTOrderNo}='" & HI.UL.ULF.rpQuoted(Me.FNHSysStyleId.Properties.Tag.ToString()) & "'"
                .Preview()
            End With
    ...
    Private Sub wJobCostTransaction_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ...
            ogvordercost.OptionsView.ShowAutoFilterRow = False
    ...
    Private Sub ogv_RowCountChanged(sender As Object, e As EventArgs) Handles ogv.RowCountChanged
        Try
            If Not (Me.ogc.DataSource Is Nothing) Then
                ...
                If Me.ogcordercost.DataSource Is Nothing Then
                    LoadDetailOrderCostcharge(Me.FNHSysStyleId.Properties.Tag.ToString)
                End If
    ...
    Private Sub SumOrderCharge()
        ...
        With CType(ogcordercost.DataSource, DataTable)
            ...
            Dim _FNEmbFacCost As Double = 0
            Dim _FNWageCutSew As Double = 0
            Dim _FNWageCutTrim As Double = 0

            For Each R As DataRow In .Rows
                _FNEmbSubCost = _FNEmbSubCost + Val(R!FNEmbSubCost)
                _FNPrintSubCost = _FNPrintSubCost + Val(R!FNPrintSubCost)
                _FNEmbBranchCost = _FNEmbBranchCost + Val(R!FNEmbBranchCost)
                _FNPrintBranchCost = _FNPrintBranchCost + Val(R!FNPrintBranchCost)
                _FNEmbFacCost = _FNEmbFacCost + Val(R!FNEmbFacCost)
                _FNWageCutSew = _FNWageCutSew + Val(R!FNWageCutSew)
                _FNWageCutTrim = _FNWageCutTrim + Val(R!FNWageCutTrim)
            Next
            ...
            FNEmbFacCost.Value = _FNEmbFacCost
            FNWageCutSew.Value = _FNWageCutSew
            FNWageCutTrim.Value = _FNWageCutTrim
    ...
    Private Sub FNHSysStyleId_EditValueChanged(sender As Object, e As EventArgs) Handles FNHSysStyleId.EditValueChanged
        ...
        Else
            If FNHSysStyleId.Text <> "" Then
                ...
                Dim _FNHSysStyleId As Integer = Val(HI.Conn.SQLConn.GetField("SELECT TOP 1 FNHSysStyleId FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMStyle AS A WITH(NOLOCK) WHERE FTStyleCode='" & HI.UL.ULF.rpQuoted(FNHSysStyleId.Text) & "'", Conn.DB.DataBaseName.DB_MASTER, "0"))
                FNHSysStyleId.Properties.Tag = _FNHSysStyleId.ToString

                If _FNHSysStyleId > 0 Then

                    Call LoadOrderCostDataInfo(_FNHSysStyleId.ToString)
    ...
    Private Sub RepOrderCost_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles RepOrderCost.EditValueChanging
        Try
            If e.NewValue < 0 Then
                e.Cancel = True
            Else
                ...
                ogvordercost.SetFocusedRowCellValue(ogvordercost.FocusedColumn.FieldName.ToString, e.NewValue)
