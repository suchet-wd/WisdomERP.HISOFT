...
Public Class wAccTranferWHTracking
...
#Region "Initial Grid"
    Private Sub InitGrid()
        ...
        Dim sFieldCount As String = "FTTransferWHNo"
        Dim sFieldSum As String = "FNQuantityAmt|FNQuantityInAmt|FNQuantityInTransitAmt"

        Dim sFieldGrpCount As String = "FTTransferWHNo"
        Dim sFieldGrpSum As String = "FNQuantityAmt|FNQuantityInAmt|FNQuantityInTransitAmt"
        ...
        With ogvtime
            ...
            For Each Str As String In sFieldGrpSum.Split("|")
                If Str <> "" Then
                    .GroupSummary.Add(DevExpress.Data.SummaryItemType.Sum, Str, Nothing, "(Sum by " & .Columns.ColumnByFieldName(Str).Caption & "={0:n" & HI.ST.Config.AmtDigit.ToString & "})")
                End If
            Next
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        _Qry &= vbCrLf & "  ,FTPOTypeName"

        _Qry &= vbCrLf & "   ,FTFacPurchaseNo,FTInvoiceNo,FNPriceFac  "

        _Qry &= vbCrLf & "  ,FNPrice"
        _Qry &= vbCrLf & "  , SUM(FNQuantity) AS FNQuantity"
        _Qry &= vbCrLf & "  , Convert(numeric(18,2),SUM(FNQuantity) * FNPrice) AS FNQuantityAmt"
        _Qry &= vbCrLf & "  , SUM(FNQuantityIn) AS FNQuantityIn"
        _Qry &= vbCrLf & "  ,  Convert(numeric(18,2),SUM(FNQuantityIn)* FNPrice)  AS FNQuantityInAmt"
        _Qry &= vbCrLf & "  , SUM(FNQuantity - FNQuantityIn) AS FNQuantityInTransit"
        _Qry &= vbCrLf & "  ,  Convert(numeric(18,2),SUM(FNQuantity - FNQuantityIn)* FNPrice) AS FNQuantityInTransitAmt"
        _Qry &= vbCrLf & "  FROM     ("
        '-----------------------Start Transafer Warehouse To Ware House------------------------------------
        If FNIntransitDocType.SelectedIndex = 0 Or FNIntransitDocType.SelectedIndex = 1 Then


            _Qry &= vbCrLf & " SELECT A.FTTransferWHNo"
            ...
            _Qry &= vbCrLf & "  , B.FTPurchaseNo"
            _Qry &= vbCrLf & "  , B.FNPrice "
            ...
            _Qry &= vbCrLf & "  , CB.FTCmpCode AS FTCmpCodeTo,A.FTFacPurchaseNo,A.FTInvoiceNo,ISNULL(X2.FNPriceFac,0) AS FNPriceFac"
            ...
            If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                ...
                _Qry &= vbCrLf & "   , PMN.FTNameTH AS FTPOTypeName"
            Else
                ...
                _Qry &= vbCrLf & "   , PMN.FTNameEN AS FTPOTypeName"
            End If
            ...
            _Qry &= vbCrLf & "  LEFT OUTER JOIN (  SELECT P.FTPurchaseNo,B.FTNameTH,B.FTNameEN "
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchase AS P WITH (NOLOCK) INNER JOIN (SELECT FNListIndex, FTNameTH, FTNameEN"
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_SYSTEM) & "].dbo.HSysListData WITH(NOLOCK)"
            _Qry &= vbCrLf & "   WHERE  (FTListName = N'FNPoState')) AS B ON P.FNPoState=B.FNListIndex"
            _Qry &= vbCrLf & " ) AS PMN ON  B.FTPurchaseNo = PMN.FTPurchaseNo"


            _Qry &= vbCrLf & " OUTER APPLY (SELECT  TOP 1 X.FNPrice As FNPriceFac  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PUR) & "]..TPURTFacPurchase_OrderNo AS X WITH (NOLOCK)  WHERE X.FTFacPurchaseNo =A.FTFacPurchaseNo AND X.FNHSysRawMatId =B.FNHSysRawMatId  ) AS X2"
            ...
            End If
            '-----------------------End Transafer Warehouse To Ware House------------------------------------
        End If

        If FNIntransitDocType.SelectedIndex = 0 Then
            _Qry &= vbCrLf & " UNION ALL "
        End If

        If FNIntransitDocType.SelectedIndex = 0 Or FNIntransitDocType.SelectedIndex = 2 Then

            '-----------------------Start Transafer To Center------------------------------------
            _Qry &= vbCrLf & " SELECT A.FTTransferCenterNo"
            _Qry &= vbCrLf & "  , A.FDTransferCenterDate"
            _Qry &= vbCrLf & "  , A.FTTransferCenterBy"
            _Qry &= vbCrLf & "  , WHA.FTWHCode"
            _Qry &= vbCrLf & "  , WHB.FTWHCode AS FTWHCodeTo"
            _Qry &= vbCrLf & "  , A.FTRemark"
            _Qry &= vbCrLf & "  , '1' AS FTStateApprove"
            _Qry &= vbCrLf & "  , A.FTTransferCenterBy AS  FTApproveBy"
            _Qry &= vbCrLf & "  ,  A.FDTransferCenterDate AS  FDApproveDate"
            _Qry &= vbCrLf & "  , BO.FTOrderNo "
            _Qry &= vbCrLf & "  , B.FNHSysRawMatId"
            _Qry &= vbCrLf & "  , IM.FTRawMatCode"
            _Qry &= vbCrLf & "  , ISNULL(IMC.FTRawMatColorCode, '') AS FTRawMatColorCode"
            ...
            _Qry &= vbCrLf & "  , ISNULL(BI.FNQuantity, 0) AS FNQuantityIn"
            _Qry &= vbCrLf & "  , CA.FTCmpCode"
            _Qry &= vbCrLf & "  , CB.FTCmpCode AS FTCmpCodeTo,'' AS FTFacPurchaseNo,'' FTInvoiceNo,0.00 AS FNPriceFac"
            ...
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferCenter AS A WITH(NOLOCK)   INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS WHA WITH(NOLOCK)   ON A.FNHSysWHId = WHA.FNHSysWHId INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS WHB WITH(NOLOCK)   ON A.FNHSysWHIdTo = WHB.FNHSysWHId INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT AS BO  WITH(NOLOCK)  ON A.FTTransferCenterNo = BO.FTDocumentNo INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode AS B WITH(NOLOCK)   ON BO.FTBarcodeNo = B.FTBarcodeNo INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMaterial AS IM  WITH(NOLOCK)    ON B.FNHSysRawMatId = IM.FNHSysRawMatId INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS U WITH(NOLOCK)   ON IM.FNHSysUnitId = U.FNHSysUnitId INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp AS CA WITH(NOLOCK)   ON WHA.FNHSysCmpId = CA.FNHSysCmpId INNER JOIN"
            ...
            _Qry &= vbCrLf & "  WHERE A.FTTransferCenterNo<>'' "
            ...
            If FTStartDate.Text <> "" Then
                _Qry &= vbCrLf & " AND A.FDTransferCenterDate >='" & HI.UL.ULDate.ConvertEnDB(FTStartDate.Text) & "'  "
            End If

            If FTEndDate.Text <> "" Then
                _Qry &= vbCrLf & " AND A.FDTransferCenterDate <='" & HI.UL.ULDate.ConvertEnDB(FTEndDate.Text) & "'  "
            End If
            '-----------------------End Transafer To Center------------------------------------
        ...
        Select Case FNIntransitType.SelectedIndex
            Case 0
            Case 1
                _Qry &= vbCrLf & "  HAVING SUM(FNQuantity - FNQuantityIn)  > 0 "
            Case 2
                _Qry &= vbCrLf & "  HAVING SUM(FNQuantityIn)  > 0 "
            Case Else
