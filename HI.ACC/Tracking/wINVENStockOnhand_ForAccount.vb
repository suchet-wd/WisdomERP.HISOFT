...
Public Class wINVENStockOnhand_ForAccount
...
#Region "Initial Grid"
    Private Sub InitGrid()
        ...
        Dim sFieldSum As String = "FNQuantity|FNAmount"

        Dim sFieldGrpCount As String = ""
        Dim sFieldGrpSum As String = "FNQuantity|FNAmount"
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        _Qry = " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTempStockTransaction_Account_By_Purchase WHERE FTUserLogIn='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
        HI.Conn.SQLConn.ExecuteOnly(_Qry, Conn.DB.DataBaseName.DB_INVEN)


        _Qry = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.SP_Stock_Onhand_Acc_By_Purchase '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysWHId.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdTo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FTOrderNoTo.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysMerMatId.Text) & "','" & HI.UL.ULF.rpQuoted(Me.FNHSysMerMatIdTo.Text) & "','','9999/99/99' "
        HI.Conn.SQLConn.ExecuteOnly(_Qry, Conn.DB.DataBaseName.DB_INVEN)
        ...
        _Qry &= vbCrLf & ", A.FTPurchaseNo,PO.FTPurchaseDate"
        _Qry &= vbCrLf & ", A.FNAmount"
        ...
        If Me.FNOnhandState.SelectedIndex = 0 Then
            _Qry &= vbCrLf & " SELECT FNHSysWHId,'' AS  FTOrderNo,FNHSysRawMatId, Sum(FNQuantity) AS FNQuantity ,FTPurchaseNo , sum(FNAmount) AS FNAmount"
            _Qry &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTempStockTransaction_Account_By_Purchase WITH(NOLOCK)"
            _Qry &= vbCrLf & "  WHERE FTUserLogIn='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
            _Qry &= vbCrLf & " GROUP BY FNHSysWHId,FNHSysRawMatId,FTPurchaseNo"
        Else
            _Qry &= vbCrLf & " SELECT FNHSysWHId, FTOrderNo,FNHSysRawMatId, SUM(FNQuantity) AS FNQuantity ,FTPurchaseNo, sum(FNAmount) AS FNAmount"
            _Qry &= vbCrLf & " FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTempStockTransaction_Account_By_Purchase WITH(NOLOCK)"
            _Qry &= vbCrLf & "  WHERE FTUserLogIn='" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "' "
            _Qry &= vbCrLf & " GROUP BY FNHSysWHId, FTOrderNo,FNHSysRawMatId , FTPurchaseNo "
        End If
        ...
        _Qry &= vbCrLf & "    outer apply(select top 1  CASE WHEN ISDATE(FDPurchaseDate) = 1 Then Convert(Date, FDPurchaseDate) ELSE NULL END AS FTPurchaseDate  from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TPURTPurchase AS X WITH(NOLOCK) WHERE X.FTPurchaseNo =A.FTPurchaseNo    ) AS PO "
