...
Public Class wDailySpecial_VN


    Private _ListEmpOver As wListEmpWorkTimeOver_VN
    Sub New()
        ...
        _ListEmpOver = New wListEmpWorkTimeOver_VN
...
#Region "MAIN PROC"
    ...
    Private Sub ProcessSave(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmsave.Click
        ...
        If Me.VerrifyData Then
            If Me.FTDateRequestTo.Text = "" Then
                HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.FTDateRequestTo_lbl.Text)
                Me.FTDateRequestTo.Focus()
                Exit Sub
            End If
    ...
    Private Sub ProcessDelete(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmdelete.Click
        ...
        If CType(ogc.DataSource, System.Data.DataTable).Rows.Count > 0 Then
    ...
    Private Sub ProcessPreview(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ocmpreview.Click
        ...
        If Me.FTDateRequest.Text <> "" And Me.FTDateRequestTo.Text <> "" Then
            With New HI.RP.Report
                ...
                _Fm &= vbCrLf & " AND {THRTDailySpecial.FTDate}<='" & HI.UL.ULDate.ConvertEnDB(Me.FTDateRequest.Text) & "'"
...
#Region " Procedure "
    ...
    Private Function SaveDataOver(dtdata As System.Data.DataTable, Spls As HI.TL.SplashScreen) As Boolean

        Dim _Qry As String
        Dim _Dt As System.Data.DataTable = dtdata
    ...
    Private Shared _SysHoliday As System.Data.DataTable = Nothing
    Private Shared ReadOnly Property LoadSysHoliday() As System.Data.DataTable
        Get
    ...
    Private Function SaveData(Spls As HI.TL.SplashScreen) As Boolean
        ...
        Dim _Qry As String
        CType(ogc.DataSource, System.Data.DataTable).AcceptChanges()
        Dim _Dt As System.Data.DataTable = CType(ogc.DataSource, System.Data.DataTable)
        Dim _oDtHoliday As System.Data.DataTable = LoadSysHoliday
        ...
        Try
            ...
            Dim _TmpSDate As String = HI.UL.ULDate.ConvertEnDB(Me.FTDateRequest.Text)
            Dim _TmpEDate As String = HI.UL.ULDate.ConvertEnDB(Me.FTDateRequestTo.Text)
            Dim _TmpSunDay As String = ""
            Dim _TmpHoliDay As String = ""

            Do While _TmpSDate <= _TmpEDate
                If Convert.ToDateTime(_TmpSDate).DayOfWeek <> DayOfWeek.Sunday Then
                    For Each IR As DataRow In _oDtHoliday.Select("FDHolidayDate  <> '" & HI.UL.ULDate.ConvertEnDB(_TmpSDate) & "'")
                        ...
                        For Each R As DataRow In _Dt.Select("FTSelect='1'")
                            ...
                            If R!FTSelect.ToString = "1" Then
                                ...
                                _Qry = " SELECT TOP 1 FTDate  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTDailySpecial WITH (NOLOCK)"
                                _Qry &= vbCrLf & " WHERE FNHSysEmpID = " & Val(R!FNHSysEmpID) & " "
                                _Qry &= vbCrLf & " AND FTDate = '" & _TmpSDate & "'"
                                ...
                                If HI.Conn.SQLConn.GetFieldOnBeginTrans(_Qry, Conn.DB.DataBaseName.DB_HR, "") <> "" Then
                                    ...
                                    _Qry &= vbCrLf & " , FTStateChildcare='" & FTStateChildcare.EditValue.ToString & "' "
                                    _Qry &= vbCrLf & ",FTTimeMOut='" & Me.otba1Mendtime.Text & "'"
                                    _Qry &= vbCrLf & " WHERE FNHSysEmpID = " & Val(R!FNHSysEmpID) & ""
                                    _Qry &= vbCrLf & " AND FTDate = '" & _TmpSDate & "'"
                                ...
                                Else
                                    ...
                                    _Qry = " INSERT INTO   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTDailySpecial (  FTInsUser, FTInsDate, FTInsTime "
                                    _Qry &= vbCrLf & "  , FNHSysEmpID, FTDate, FTTimeOut, FTStateStop,FTStatePlangnent,FTTimeMOut ,FTStateChildcare )  "
                                    _Qry &= vbCrLf & " SELECT '" & HI.ST.UserInfo.UserName & "'," & HI.UL.ULDate.FormatDateDB & "," & HI.UL.ULDate.FormatTimeDB & "  "
                                    _Qry &= vbCrLf & " ,'" & Val(R!FNHSysEmpID) & "'"
                                    _Qry &= vbCrLf & " ,'" & HI.UL.ULDate.ConvertEnDB(_TmpSDate) & "','" & Me.otba1endtime.Text & "','" & FTStateStop.EditValue.ToString & "','" & FTStatePlangnent.EditValue.ToString & "' "
                                    _Qry &= vbCrLf & ",'" & Me.otba1Mendtime.Text & "'"
                                    _Qry &= vbCrLf & " , '" & FTStateChildcare.EditValue.ToString & "' "
            ...
            _TmpSDate = HI.UL.ULDate.ConvertEnDB(Me.FTDateRequest.Text)
            Do While _TmpSDate <= _TmpEDate
                If Convert.ToDateTime(_TmpSDate).DayOfWeek <> DayOfWeek.Sunday Then
                    For Each IR As DataRow In _oDtHoliday.Select("FDHolidayDate  <> '" & HI.UL.ULDate.ConvertEnDB(_TmpSDate) & "'")
                        ...
                        For Each R As DataRow In _Dt.Select("FTSelect='1'")
                            If R!FTSelect.ToString = "1" Then
                                ...
                                HI.HRCAL.Calculate.CalculateWorkTime(HI.ST.UserInfo.UserName, Val(R!FNHSysEmpID), _TmpSDate, _TmpSDate)
                            End If
                        Next
            ...
            Do While _TmpSDate <= _TmpEDate
                If Convert.ToDateTime(_TmpSDate).DayOfWeek <> DayOfWeek.Sunday Then
                    For Each IR As DataRow In _oDtHoliday.Select("FDHolidayDate  <> '" & HI.UL.ULDate.ConvertEnDB(_TmpSDate) & "'")
                        ...
                        For Each R As DataRow In _Dt.Select("FDDate='" & _TmpSDate & "'")
                            ...
                            If R!FTSelect.ToString = "1" Then
                                ...
                                _Qry &= vbCrLf & " WHERE FNHSysEmpID = " & Val(R!FNHSysEmpID) & " "
                                _Qry &= vbCrLf & " AND FTDate = '" & HI.UL.ULDate.ConvertEnDB(_TmpSDate) & "'"
                    ...
                    For Each IR As DataRow In _oDtHoliday.Select("FDHolidayDate  <> '" & HI.UL.ULDate.ConvertEnDB(_TmpSDate) & "'")
                        ...
                        For Each R As DataRow In _Dt.Rows
                            If R!FTSelect.ToString = "1" Then
                                ...
                                HI.HRCAL.Calculate.CalculateWorkTime(HI.ST.UserInfo.UserName, Val(R!FNHSysEmpID), HI.UL.ULDate.ConvertEnDB(_TmpSDate), HI.UL.ULDate.ConvertEnDB(_TmpSDate))
    ...
    Private Sub LoadDataInfo()
        ...
        _Qry = " SELECT      '0' AS FTSelect,  M.FNHSysEmpID, M.FTEmpCode , CASE WHEN Isdate(OTR.FTDate)=1 Then convert(nvarchar(10),convert(datetime,OTR.FTDate),103) Else '' END AS FTDate"
        _Qry &= vbCrLf & ",OTR.FTDate AS FDDate"
        ...
        _Qry &= vbCrLf & "	SELECT FTTimeMOut,FTTimeOut,FTStateStop, FNHSysEmpID, FTDate,ISNULL(FTStatePlangnent,'0') AS FTStatePlangnent,ISNULL(FTStateChildcare,'0') AS FTStateChildcare"
        ...
        _Qry &= vbCrLf & "  AND  FTDate <='" & HI.UL.ULDate.ConvertEnDB(FTDateRequestTo.Text) & "'"
    ...
    Private Sub LoadTimeInWeekOverDataInfo()
        ...
        Try
            ...
            _Qry &= vbCrLf & "   LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitSect AS US WITH (NOLOCK) ON M.FNHSysUnitSectId = US.FNHSysUnitSectId "
            _Qry &= vbCrLf & " LEFT OUTER JOIN("
            _Qry &= vbCrLf & "	SELECT FTTimeMOut,FTTimeOut,FTStateStop, FNHSysEmpID, FTDate"
            _Qry &= vbCrLf & "	FROM    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTDailySpecial AS OT WITH (NOLOCK)"
            _Qry &= vbCrLf & "	WHERE   (FTDate ='" & HI.UL.ULDate.ConvertEnDB(FTDateRequest.Text) & "')"
            _Qry &= vbCrLf & " ) AS OTR ON M.FNHSysEmpID = OTR.FNHSysEmpID"
            ...
            With _ListEmpOver
                ...
                If .ProcessOK Then
                    ...
                    Try
                        If Me.SaveDataOver(CType(.ogc.DataSource, System.Data.DataTable), _Spls) Then
                            _Spls.Close()
                            HI.MG.ShowMsg.mProcessComplete(MG.ShowMsg.ProcessType.mSave, Me.Text)
                            Me.ocmload_Click(ocmload, New System.EventArgs)
                        Else
...
#Region "General"
    ...
    Private Sub FTDateRequest_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles FTDateRequest.EditValueChanged, FTDateRequestTo.EditValueChanged
        Me.ogc.DataSource = Nothing
        ochkselectall.Checked = False
    End Sub
