...
Public Class wAdjustPayroll_VN
...
#Region "Property"
    ...
    Property SalaryPerDay As Double
        Get
...
#Region "Procedure"
    ...
    Private Sub LoadEmpInfo(ByVal FNHSysEmpID As String)
        ...
        _Qry &= vbCrLf & "  M.FNHSysEmpID, M.FTEmpPicName, M.FNHSysPositId, P.FTPositCode,M.FNSalary,ET.FNCalType,ET.FNSalaryDivide,ET.FNStateSalaryType "
        ...
        If _dt.Rows.Count > 0 Then
            For Each R As DataRow In _dt.Rows
                ...
                'End If
                Dim _NSalary As Double = HI.HRVN.Calculate._GetNewSalary(FNHSysEmpID)
                If _NSalary > 0 Then
                    FNSalary.Value = Format(_NSalary / Me.FNSalaryDivide, "0.00")
                Else
                    FNSalary.Value = Format(Val(R!FNSalary.ToString) / Me.FNSalaryDivide, "0.00")
                End If

                _FNCalType = Val(R!FNCalType.ToString)
                _FNHSysEmpTypeId = Val(R!FNHSysEmpTypeId.ToString)

                'Select Case Val(R!FNCalType.ToString)
                '    Case 0, 1

                '        If Val(R!FNCalType.ToString) = 0 Then
                '            Me.SalaryPerDay = FNSalary.Value / HI.HRVN.Calculate.GetDayPerMonth(_PayTerm, _PayYear, Val(R!FNHSysEmpTypeId.ToString))

                '        Else
                '            Me.SalaryPerDay = FNSalary.Value
                '        End If
                ...
                For Each R2 As DataRow In _Dt2.Rows
                    ...
                    _PayYear = R2!FTPayYear.ToString
                    _PayTerm = R2!FTPayTerm.ToString
        ...
        Dim _dtot As System.Data.DataTable
        ...
        Select Case _FNCalType
            Case 0, 1

                If _FNCalType = 0 Then
                    Dim FNWageScale As Double = 0

                    With DirectCast(ogdIncome.DataSource, System.Data.DataTable)
                        .AcceptChanges()
                        For Each R As DataRow In .Select("FTFinCode = '027'")
                            FNWageScale = Double.Parse("0" & R!FCFinAmt.ToString)
                        Next

                    End With

                    Me.SalaryPerDay = Calculate.HelperRoundUpBasicSalary((FNSalary.Value + FNSkillBaht.Value + FNHarmfulBaht.Value + FNWageScale)) / HI.HRVN.Calculate.GetDayPerMonth(_PayTerm, _PayYear, _FNHSysEmpTypeId)
                Else
                    Me.SalaryPerDay = FNSalary.Value
                End If
    ...
    Private Sub SetShowFinance(ByVal EmpCode As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & ",CASE WHEN ISNULL(THRMEmployeeFin.FTFinCode,N'')=N'' THEN   Convert(numeric(18,2),0.00) ELSE   FTFinAmt  END AS FTFinAmt"
            ...
            _Qry &= vbCrLf & " WHERE   ISNULL(FTStaActive,N'')=N'1' "
            ...
            _Qry &= vbCrLf & " WHERE FTPayYear=N'" & FTPayYear.Text & "' AND FTPayTerm=N'" & FTPayTerm.Text & "' AND FNHSysEmpID=" & Val(Me.FNHSysEmpID.Properties.Tag.ToString) & " "
    ...
    Private Sub LoadPayroll()
        ...
        _Qry = " SELECT        FTPayYear, FTPayTerm, FNHSysEmpID, FNHoliday, FNSalary, FNWorkingHour, FNOt15, FNOt2, FNOt3, FNOt4, FNTotalLeavePay, "
        ...
        _Qry &= vbCrLf & "  FNContributedAmt, FNCmpContributedAmt, FNTotalCalWorkmen, FNWorkmenAmt,  FNAmtRetire,FNIncentiveAmt,"
        _Qry &= vbCrLf & "  FNPayLeaveSSo, FNWorkingDay, FNAdjBeforeCal, FNAttandanceAmt, FNHealtCareAmt, FNTransportAmt, FNChildCareAmt, FNOTMealAmt, FNSocialBase, FNWorkAgeSalary, FNOTMealAmtUS, FNExchangeRate"
        _Qry &= vbCrLf & " , FNUnemploymentInsuranceEmployee, FNUnionInsuranceEmployee ,  FNSkillBaht, FNHarmfulBaht, FNPayRestOTBaht  , FNPayRestOTMin   "
        _Qry &= vbCrLf & " , FNOt1 + FLOOR(CONVERT(NUMERIC(18,2), FNPayRestOTMin / 60) )    + CONVERT(NUMERIC(18,2),(( FNPayRestOTMin  % 60) / 100.00)) as FNOt1   "
        ...
        For Each R As DataRow In _dt.Rows
            ...
            Me.FNAttandanceAmt.Value = Val(R!FNOt3.ToString)
            Me.FNOt4.Value = Val(R!FNOt4.ToString)

            Me.FCOt1_Baht.Value = Val(R!FCOt1_Baht.ToString) + Val(R!FNPayRestOTBaht.ToString)

            Me.FCOt15_Baht.Value = Val(R!FCOt15_Baht.ToString)
            Me.FCOt2_Baht.Value = Val(R!FCOt2_Baht.ToString)
            Me.FNHealtCareAmt.Value = Val(R!FCOt3_Baht.ToString)
            ...
            Me.FNAttandanceAmt.Value = Val(R!FNAttandanceAmt.ToString)
            Me.FNHealtCareAmt.Value = Val(R!FNHealtCareAmt.ToString)
            Me.FNTransportAmt.Value = Val(R!FNTransportAmt.ToString)
            Me.FNChildCareAmt.Value = Val(R!FNChildCareAmt.ToString)
            Me.FNOTMealAmtUS.Value = Val(R!FNOTMealAmtUS.ToString)
            Me.FNSocialBase.Value = Val(R!FNSocialBase.ToString)
            Me.FNWorkAgeSalary.Value = Val(R!FNWorkAgeSalary.ToString)
            Me.FNOTMealAmtUS.Value = Val(R!FNOTMealAmtUS.ToString)
            Me.FNExchangeRate.Value = Val("0" & R!FNExchangeRate.ToString)

            'new
            Me.FNUnemploymentInsurance.Value = Val(R!FNUnemploymentInsuranceEmployee.ToString)
            Me.FNUnionInsurance.Value = Val(R!FNUnionInsuranceEmployee.ToString)

            Me.FNPayRestOTMin.Value = Val(R!FNPayRestOTMin.ToString)
            Me.FNPayRestOTBaht.Value = Val(R!FNPayRestOTBaht.ToString)

            Me.FNHarmfulBaht.Value = Val(R!FNHarmfulBaht.ToString)
            Me.FNSkillBaht.Value = Val(R!FNSkillBaht.ToString)

            Me.FNIncentiveAmt.Value = Val(R!FNIncentiveAmt.ToString)
            Me.FNTotalLeavePay.Value = Format(IIf(Val(R!FNTotalLeavePay.ToString) > 0, Val(R!FNTotalLeavePay.ToString) / 8, 0), "0.00")
            Me.FNTotalLeavePayBaht.Value = Double.Parse("0" & R!FNPayLeaveOtherBaht.ToString) + Double.Parse("0" & R!FNPayLeaveVacationBaht.ToString)
        Next
...
#Region "General"
    ...
    Private Sub FNHSysEmpID_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNHSysEmpID.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        ...
        Else
            If FNHSysEmpID.Text <> "" Then
                ...
                Dim _Qry As String = "SELECT TOP 1 FNHSysEmpID  FROM THRMEmployee WITH(NOLOCK) WHERE FTEmpCode =N'" & HI.UL.ULF.rpQuoted(FNHSysEmpID.Text) & "' "
    ...
    Private Function SaveData(ByVal oSpls As HI.TL.SplashScreen) As Boolean
        Try
            ...
            _Qry &= vbCrLf & " ,N'' AS FTLineCode, '' As FTGrpCode, M.FNHSysDeptId AS FTDeptCode, "
            ...
            _Qry &= vbCrLf & " , M.FNHSysPositId AS FTPositCode,N'' as FTJobGrade,N'' AS FTCostCNCode, M.FNLateCutSta AS FTLateCutSta,"
            ...
            '------------------------- อายุงาน -----------------------------------
            _Qry &= vbCrLf & ",ISNULL(ET.FTStatePayHoliday,N'') AS FTStatePayHoliday "
            ...
            Dim _Dtemp As System.Data.DataTable = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_HR)
            ...
            For Each R As DataRow In _Dtemp.Rows
                ...
                Dim _dt As System.Data.DataTable = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_HR)
                ...
                '----------- Get Summary ------------------
                Calculate.LoadIncomeTax(_FTEmpIdNo, _PayYear, _PayTerm, _FCAccumulateIncome, _FCAccumulateTax, _FCAccumulateSocial, CountTerm, FTTotalCalContributedAcc, FTTotalCalWorkmenAcc, Integer.Parse(Val(Me.FNHSysEmpID.Properties.Tag.ToString)))
                '----------- Get Summary ------------------
                ...
                Dim _Totalcaltax As Double = (FCBaht.Value)
                Dim _FTOtherAdd_CalTAX As Double = 0

                For Each R2 As DataRow In CType(ogdIncome.DataSource, System.Data.DataTable).Rows
                ...
                If R!FTCalTaxSta.ToString <> "1" Then
                    ...
                    With _EmpDisTax
                        ...
                        .FTSosial = 0 '_FCAccumulateSocial + FNSocial.Value + (FNSocial.Value * (_TotalInstalment - _Instalment))
                        '.BaseSlary = (_Totalcaltax * (_TotalInstalment - _Instalment)) + _Totalcaltax
                        '.OtherSlary = _FTOtherAdd_CalTAX + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FNHealtCareAmt.Value + FCOt4_Baht.Value
                        ...
                        .BaseSlary = CDbl(Format((_Totalcaltax * FNExchangeRate.Value), "0.00")) '(_TotalCalTax * (_TotalInstalment - _Instalment)) + _TotalCalTax
                        .OtherSlary = _FTOtherAdd_CalTAX + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FNAttandanceAmt.Value + FNHealtCareAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value
                        .OtherSlary = Format(.OtherSlary * FNExchangeRate.Value, "0.00")
                    ...
                    Dim _Total As Double = Calculate.GETnRecalDiscTax(_EmpDisTax, _EmpTaxYear)

                    _EmpTaxYear.FTSocial = _FCAccumulateSocial + FNSocial.Value   'ประกันสังคม

                    _EmpTaxYear.FTTotalCalTax = _Total
                    _TaxOtherAmt = 0
                    Dim _TotalTax As Double = Calculate.GETnTax(_Total, _TaxOther, _TaxOtherAmt)
                    ...
                    If _FTEmpIdNo <> "" Then
                        ...
                        _Qry &= vbCrLf & "  WHERE FTYear=N'" & _PayYear & "' "
                        _Qry &= vbCrLf & "  AND  FTEmpIdNo=N'" & HI.UL.ULF.rpQuoted(_FTEmpIdNo) & "' AND FNHSysCmpId=" & HI.ST.SysInfo.CmpID & " "
                        ...
                        _Qry &= vbCrLf & " SELECT " & HI.ST.SysInfo.CmpID & ",N'" & _PayYear & "',N'" & HI.UL.ULF.rpQuoted(_FTEmpIdNo) & "' "
            ...
            For Each R As DataRow In CType(ogdIncome.DataSource, System.Data.DataTable).Rows
                ...
                Select Case True
                    ...
                    Case (Val(R!FTFinAmt.ToString) = (Val(R!FCFinAmt.ToString)))
                        FNTotalAdd = FNTotalAdd + Val(R!FCFinAmt.ToString)
                        'FNTotalAddOther = FNTotalAddOther + (Val(R!FTFinAmt.ToString))
                End Select
                ...
                _Qry &= vbCrLf & " WHERE FTPayYear=N'" & Me.FTPayYear.Text & "' "
                _Qry &= vbCrLf & " AND FTPayTerm=N'" & Me.FTPayTerm.Text & "' "
                _Qry &= vbCrLf & " AND FNHSysEmpID=" & Val(Me.FNHSysEmpID.Properties.Tag.ToString) & " "
                _Qry &= vbCrLf & " AND FTFinCode=N'" & HI.UL.ULF.rpQuoted(R!FTFinCode.ToString) & "' "
                ...
                If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    _Qry &= vbCrLf & " ,N'" & Me.FTPayYear.Text & "' "
                    _Qry &= vbCrLf & " ,N'" & Me.FTPayTerm.Text & "' "
                    _Qry &= vbCrLf & " ," & Val(Me.FNHSysEmpID.Properties.Tag.ToString) & " "
                    _Qry &= vbCrLf & " ,N'" & HI.UL.ULF.rpQuoted(R!FTFinCode.ToString) & "'"
                    _Qry &= vbCrLf & " ,0,0"
            ...
            _Qry &= vbCrLf & " , FNNetpay=" & Math.Round((FCBaht.Value + Me.FNTotalLeavePayBaht.Value + FNIncentiveAmt.Value + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FHolidayBaht.Value + FNPayRestOTBaht.Value) + _AmtAdd + FNAttandanceAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value) - (_AmtDeduct + CDbl(Format(FNTax.Value / FNExchangeRate.Value, "0.00")) + FNContributedAmt.Value + FNHealtCareAmt.Value + FNSocial.Value + FNUnemploymentInsurance.Value + FNUnionInsurance.Value) & ""
            ...
            _Qry &= vbCrLf & " , FNTotalIncome=" & Math.Round((FCBaht.Value + Me.FNTotalLeavePayBaht.Value + FNIncentiveAmt.Value + FCOt1_Baht.Value + FCOt15_Baht.Value + FCOt2_Baht.Value + FCOt3_Baht.Value + FCOt4_Baht.Value + FHolidayBaht.Value + FNPayRestOTBaht.Value) + _AmtAdd + FNAttandanceAmt.Value + FNTransportAmt.Value + FNChildCareAmt.Value + FNWorkAgeSalary.Value + FNOTMealAmtUS.Value) & ""
            _Qry &= vbCrLf & " , FNAttandanceAmt=" & FNAttandanceAmt.Value & ""
            _Qry &= vbCrLf & " , FNHealtCareAmt=" & FNHealtCareAmt.Value & ""
            _Qry &= vbCrLf & " , FNTransportAmt=" & FNTransportAmt.Value & ""
            _Qry &= vbCrLf & ", FNChildCareAmt=" & FNChildCareAmt.Value & ""
            _Qry &= vbCrLf & ", FNOTMealAmt=" & FNOTMealAmtUS.Value * FNExchangeRate.Value & ""
            _Qry &= vbCrLf & ", FNSocialBase=" & FNSocialBase.Value & ""
            _Qry &= vbCrLf & ", FNWorkAgeSalary=" & FNWorkAgeSalary.Value & ""
            _Qry &= vbCrLf & ", FNOTMealAmtUS=" & FNOTMealAmtUS.Value & ""
            'new 
            _Qry &= vbCrLf & ",FNUnemploymentInsuranceEmployee=" & Me.FNUnemploymentInsurance.Value
            _Qry &= vbCrLf & ",FNUnionInsuranceEmployee=" & Me.FNUnionInsurance.Value
            _Qry &= vbCrLf & ",FNHealthInsuranceEmployee=" & Me.FNHealtCareAmt.Value
            _Qry &= vbCrLf & ",FNSocialInsuranceEmployee=" & Me.FNSocial.Value
            '***
            _Qry &= vbCrLf & ",FNPayRestOTBaht=" & Me.FNPayRestOTBaht.Value
            _Qry &= vbCrLf & ",FNPayRestOTMin=" & Me.FNPayRestOTMin.Value
            '***
            _Qry &= vbCrLf & " WHERE FTPayYear=N'" & Me.FTPayYear.Text & "' "
            _Qry &= vbCrLf & " AND FTPayTerm=N'" & Me.FTPayTerm.Text & "' "
            _Qry &= vbCrLf & " AND FNHSysEmpID=" & Val(Me.FNHSysEmpID.Properties.Tag.ToString) & " "
    ...
    Private Sub FNOt3_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNOt3.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FCOt3_Baht.Value = Format(((Me.SalaryPerDay / 8) * _RateOT3) * FNAttandanceAmt.Value, "0.00")
        End If
    End Sub
    ...
    Private Sub FNHoliday_EditValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FNHoliday.EditValueChanged, FNTotalLeavePay.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
    ...
    Private Sub FNPayRestOTMin_EditValueChanged(sender As Object, e As EventArgs) Handles FNPayRestOTMin.EditValueChanged
        If Not (_StateLoad) Then Exit Sub
        If Not (_InitLoad) Then
            FNPayRestOTBaht.Value = Format((((Me.SalaryPerDay / 8) * _RateOT1) / 60) * FNPayRestOTMin.Value, "0.00")
        End If
    End Sub

#End Region

    Private Sub FNOt3_EditValueChanged_1(sender As Object, e As EventArgs) Handles FNOt3.EditValueChanged
    ...
    Private Sub ocmrefresh_Click(sender As Object, e As EventArgs) Handles ocmrefresh.Click
        Try
            Call FNHSysEmpID_EditValueChanged(sender, e)
        Catch ex As Exception
