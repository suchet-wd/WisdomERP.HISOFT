...
Public Class wPayrollListingCover_VN
    Sub New()
...
#Region "Initial Grid"
    Private Sub InitGrid()
        ...
        sFieldSum = "FNEmpQty|FNSalary|FCOtBaht|024|015|018|023|011|FNTotalAddOther|FNTotalExpenseOther|FNTotalIncome"
        sFieldSum &= "|FNSocialInsuranceEmployee|FNHealthInsuranceEmployee|FNUnemploymentInsuranceEmployee|FNUnionInsuranceEmployee|FNTax|FNNetpay|FNCmpPay"
        sFieldSum &= "|FNSocialInsuranceEmployer|FNHealthInsuranceEmployer|FNUnemploymentInsuranceEmployer|FNUnionInsuranceEmployer"
        ...
        With ogv
            ...
            .Columns("FTEmpTypeName").Group()
            '.Columns("FTUnitSectName").Group()
...
#Region "Procedure"
    Private Sub LoadDataInfo()
        ...
        Try
            ...
            _Qry = "    SELECT       "
            If HI.ST.Lang.Language = HI.ST.SysInfo.LanguageLocal Then
                _Qry &= vbCrLf & "  ISNULL(ET.FTEmpTypeCode,'') +' '+ET.FTEmpTypeNameTH  AS FTEmpTypeName "
                _Qry &= vbCrLf & "  ,ISNULL(OrgUnitSect.FTUnitSectCode,'') +' '+OrgUnitSect.FTUnitSectNameTH  AS FTUnitSectName "
            Else
                _Qry &= vbCrLf & "  ISNULL(ET.FTEmpTypeCode,'') +' '+ET.FTEmpTypeNameEN  AS FTEmpTypeName "
                _Qry &= vbCrLf & "  ,ISNULL(OrgUnitSect.FTUnitSectCode,'')  +' '+OrgUnitSect.FTUnitSectNameEN  AS FTUnitSectName "
            End If
            _Qry &= vbCrLf & " ,COUNT(*) as FNEmpQty"
            _Qry &= vbCrLf & ",sum( P.FNChildCareAmt) AS   [012] "
            _Qry &= vbCrLf & " ,SUM(P.FCBaht) as FNSalary "
            _Qry &= vbCrLf & ",SUM(P.FCOt1_Baht) + SUM(P.FCOt15_Baht) + SUM(P.FCOt2_Baht) + SUM(P.FCOt3_Baht) + SUM(P.FCOt4_Baht)  as FCOtBaht"
            _Qry &= vbCrLf & ",SUM(T.[024]) AS  [024],SUM(T.[015]) AS [015],SUM(T.[018]) AS [018]"
            _Qry &= vbCrLf & " ,SUM(T.[023]) AS  [023] ,SUM(T.[011])+sum(ISNULL(P.FNIncentiveAmt,0))  AS [011]"
            _Qry &= vbCrLf & ",SUM(T.[025]) AS  [025] ,sum(P.FNPayLeaveVacationBaht)+ sum(P.FNPayLeaveOtherBaht) AS FNPayLeaveOtherBaht "
            _Qry &= vbCrLf & " ,SUM((P.FNTotalAddOther+P.FNTotalAdd)-(T.[018]+T.[015]+T.[011]+T.[023]+T.[024]  + T.[025] )) + sum(P.FNPayRestOTBaht)  AS  FNTotalAddOther ,sum(P.FNTotalExpenseOther ) AS FNTotalExpenseOther"
            _Qry &= vbCrLf & " ,SUM( P.FNTotalIncome) AS FNTotalIncome "
            _Qry &= vbCrLf & ",sum( P.FNSocialInsuranceEmployee) AS FNSocialInsuranceEmployee"
            _Qry &= vbCrLf & ",SUM(P.FNHealthInsuranceEmployee) AS  FNHealthInsuranceEmployee"
            _Qry &= vbCrLf & ",SUM(P.FNUnemploymentInsuranceEmployee) AS FNUnemploymentInsuranceEmployee"
            _Qry &= vbCrLf & " ,SUM(P.FNUnionInsuranceEmployee) as FNUnionInsuranceEmployee"
            _Qry &= vbCrLf & " ,SUM(P.FNTax) as FNTax"
            _Qry &= vbCrLf & " ,SUM(P.FNNetpay) AS FNNetpay"
            _Qry &= vbCrLf & " ,SUM(P.FNSocialInsuranceEmployer) as FNSocialInsuranceEmployer , SUM(P.FNHealthInsuranceEmployer) as FNHealthInsuranceEmployer"
            _Qry &= vbCrLf & " ,SUM(P.FNUnemploymentInsuranceEmployer) as FNUnemploymentInsuranceEmployer, SUM(P.FNUnionInsuranceEmployer) as FNUnionInsuranceEmployer "
            _Qry &= vbCrLf & " ,SUM(P.FNSocialInsuranceEmployer) + SUM(P.FNHealthInsuranceEmployer) + SUM(P.FNUnemploymentInsuranceEmployer) + SUM(P.FNUnionInsuranceEmployer) AS FNCmpPay"
            ...
            _Qry &= vbCrLf & "Group by  ISNULL(ET.FTEmpTypeCode,'')  ,  ET.FTEmpTypeNameTH ,  ET.FTEmpTypeNameEN , ISNULL(OrgUnitSect.FTUnitSectCode,'')  ,OrgUnitSect.FTUnitSectNameTH ,OrgUnitSect.FTUnitSectNameEN  "
            ...
            gcolHeader2.Caption = GetCaption.ToString
    ...
    Private Function GetCaption() As String
        Try
            Dim _Cmd As String = ""
            _Cmd = "SELECT     distinct      RIGHT(FDCalDateBegin,2)+'-'+ RIGHT(FDCalDateEnd,2)+' / '+L.FTNameEN  +' / '+FTPayYear AS FTTerm"
            _Cmd &= vbCrLf & "FROM      [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMCfgPayDT as d with(nolock) LEFT OUTER JOIN ("
            ...
            _Cmd &= vbCrLf & "WHERE     (FTListName = N'FNMonth')) AS L ON d.FNMonth = L.FNListIndex"
            _Cmd &= vbCrLf & "WHERE     (FTPayYear = '" & Me.FTPayYear.Text & "') and FTPayTerm ='" & Me.FTPayTerm.Text & "'"
            Return HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_HR, "")
        Catch ex As Exception
