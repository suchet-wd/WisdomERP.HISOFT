...
Public Class wAdjustIncentive
    ...
    Private Function AvgWageIncentive(ByVal _DateTrans As String, ByVal _UnitSectId As Integer) As Boolean
        Try
            ...
            _Cmd &= vbCrLf & "FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTMoveTeamMoveType AS M WITH(NOLOCK) INNER JOIN"
            _Cmd &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTTrans AS T WITH(NOLOCK)  ON M.FNHSysEmpID = T.FNHSysEmpID AND M.FDDate = T.FTDateTrans"
            _Cmd &= vbCrLf & "WHERE    (M.FNHSysUnitSectIdTo <> " & Integer.Parse(_UnitSectId) & ") AND  (M.FNHSysUnitSectId = " & Integer.Parse(_UnitSectId) & ") AND (M.FDDate = '" & HI.UL.ULDate.ConvertEnDB(_DateTrans) & "') ) AS T"
            ...
            For Each R As DataRow In _oDt.Rows
                ...
                Dim _incentiveMoveTeam As Double = GetIncentiveMoveTeam(Integer.Parse(R!FNHSysEmpID.ToString), _DateTrans, _UnitSectId)
                '***********************
                ...
                _Cmd &= vbCrLf & ", FNNetProAmt=" & _IncentiveOT + _Incentive + _FCBaht + _FCBahtOT15 + _incentiveMoveTeam & " + FNProOther"
...
#Region "Cal Incentive"
    Private Function GetIncentiveMoveTeam(ByVal _EmpId As Integer, ByVal _DateTrans As String, ByVal _UnitSectId As Integer) As Double
        Try
            ...
            _oDt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_HR)
            For Each R As DataRow In _oDt.Select("FNHSysUnitSectIdTo <> " & _UnitSectId)
                _TotalTime = GetSumWorkingTime(_DateTrans, R!FNHSysUnitSectIdTo.ToString) + GetSumWorkingTimeOT(_DateTrans, R!FNHSysUnitSectIdTo.ToString) ' + Double.Parse(R!FNTimeMin.ToString) + Double.Parse(R!FNTimeOTMin.ToString)
    ...
    Private Sub RepositoryFTSelect_EditValueChanging(sender As Object, e As DevExpress.XtraEditors.Controls.ChangingEventArgs) Handles RepositoryFTSelect.EditValueChanging
        Try
            If e.NewValue = True Then
                If Me.FNIncentive.Value = 0 Then
                    Me.FNIncentive.Value = Me.ogvDetail.GetRowCellValue(Me.ogvDetail.FocusedRowHandle, "FNNomalIncentive").ToString
                End If
                If Me.FNIncentiveOT.Value = 0 Then
                    Me.FNIncentiveOT.Value = Me.ogvDetail.GetRowCellValue(Me.ogvDetail.FocusedRowHandle, "FNOTIncentive").ToString
                End If
            End If
        Catch ex As Exception
        End Try
