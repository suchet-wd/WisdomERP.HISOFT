...
Public Class wExpCMInvoicePost
    ...
    Public Sub LoadInvoiceInfo(ByVal Key As Object)
        _ProcLink = True
        Dim _Qry As String = "SELECT TOP 1 FTInvoiceNo   FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post WITH(NOLOCK) WHERE FTInvoiceNo =" & Val(Key) & " "
        FTInvoiceNo.Text = HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_ACCOUNT, "")

        FTInvoiceNo.Properties.Tag = Key
    ...
    Private Sub FTInvoiceNo_EditValueChanged(sender As Object, e As EventArgs) Handles FTInvoiceNo.EditValueChanged
        ...
        Else
            ...
            If HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT, "") = "" Then Exit Sub
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post WHERE FTInvoiceNo='" & HI.UL.ULF.rpQuoted(Me.FTInvoiceNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post_D WHERE FTInvoiceNo='" & HI.UL.ULF.rpQuoted(Me.FTInvoiceNo.Text) & "'"
    ...
    Private Sub ocmpreview_Click(sender As Object, e As EventArgs) Handles ocmpreview.Click
        ...
        With New HI.RP.Report
            ...
            Select Case FNHSysCustId.Text
                Case "NI"
                    .ReportName = "RptExportInvoicePost_Nike.rpt"
                Case "AE"
    ...
    Private Sub LoadData()
        Try
            ...
            _Cmd = " Select D.FTInvoiceNo, D.FTPORef, D.FNHSysStyleId, D.FNCTNS, D.FNTNW, D.FNTGW, D.FNQuantity,   D.FNUnitPrice  as FNUnitPrice "
            _Cmd &= vbCrLf & " ,  D.FNUnitPrice  * D.FNQuantity as FNTotalAmount, T.FTStyleCode , D.FTColorway, D.FTSizeBreakDown ,D.FTNikePOLineItem ,D.FNHSysCartonId , C.FTCartonCode ,D.FTRangeNo , D.FTLineNo  "
            _Cmd &= vbCrLf & " , '' as FTPORefNo"
            _Cmd &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post_D As D with(nolock) INNER JOIN"
            _Cmd &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post AS H WITH(NOLOCK) ON  D.FTInvoiceNo = H.FTInvoiceNo INNER JOIN  "
            ...
            _Cmd &= vbCrLf & "  FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.V_TMERTOrder_Info"
            _Cmd &= vbCrLf & " SELECT top 1  "
            _Cmd &= vbCrLf & "    dbo.TEXPTCMInvoice_Post.FTInvoiceNo ,V_ExportInvoice_H.FNHSysProvinceId, V_ExportInvoice_H.FNHSysCmpId , M.FTAddressInvoiceEN as FTAddressInvoice"
            _Cmd &= vbCrLf & "   , V.FTRemark AS FTAddrShip ,HITECH_MASTER.dbo.TMERMPlant.FTPlantCode,  HITECH_MASTER.dbo.TMERMPlant.FTAddr1EN AS FTAddrSold,C.FTHTSData , R.FTSuplCode  "
            '_Cmd &= vbCrLf & "  INTO #V_ReportExportInv_ShipDesc "
            _Cmd &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post WITH (nolock) LEFT OUTER JOIN "
            _Cmd &= vbCrLf & "    #TmpOrderInfo as  V_ExportInvoice_H ON dbo.TEXPTCMInvoice_Post.FNHSysProvinceId = V_ExportInvoice_H.FNHSysProvinceId LEFT OUTER JOIN  "
            _Cmd &= vbCrLf & "     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TEXPTCMInvoice_Post_D  WITH (nolock)  ON V_ExportInvoice_H.FTPORef = dbo.TEXPTCMInvoice_Post_D.FTPORef AND dbo.TEXPTCMInvoice_Post.FTInvoiceNo = dbo.TEXPTCMInvoice_Post_D.FTInvoiceNo LEFT OUTER JOIN "
            _Cmd &= vbCrLf & "     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMPlant WITH (nolock) ON V_ExportInvoice_H.FNHSysPlantId = HITECH_MASTER.dbo.TMERMPlant.FNHSysPlantId "
            _Cmd &= vbCrLf & "   LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMHTSData AS C  WITH (NOLOCK) ON V_ExportInvoice_H.FNHSysProdTypeId = C.FNHSysProdTypeId AND V_ExportInvoice_H.FNHSysGenderId = C.FNHSysGenderId  "
            _Cmd &= vbCrLf & "  INNER JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp AS M WITH (NOLOCK) ON V_ExportInvoice_H.FNHSysCmpId = M.FNHSysCmpId  "
            _Cmd &= vbCrLf & "  LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCMMProvince AS V WITH (NOLOCK)  ON  V_ExportInvoice_H.FNHSysProvinceId = V.FNHSysProvinceId"
            _Cmd &= vbCrLf & "  LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMContinent AS PT WITH(NOLOCK) ON V_ExportInvoice_H.FNHSysContinentId = PT.FNHSysContinentId  "
            ...
            '  _Cmd &= vbCrLf & "WHERE  (dbo.TEXPTCMInvoice.FTInvoiceNo IS NOT NULL)  "
            _Cmd &= vbCrLf & "WHERE  dbo.TEXPTCMInvoice_Post.FTInvoiceNo ='" & HI.UL.ULF.rpQuoted(Me.FTInvoiceNo.Text) & "'"
            _oDt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT)
                ...
                Select Case e.SummaryProcess
                    ...
                    Case CustomSummaryProcess.Calculate
                        If _RangNo = "" And _LineNo = "" Then
                            ...
                            End If
                        Else
                            If _RangNo = _RangNoOld And _LineNo <> _LineNoOld Then
                                _rowCount += +1
                            Else
                                _rowCount = 1
                            End If
                        ...
                        Select Case DirectCast(e.Item, DevExpress.XtraGrid.GridSummaryItem).FieldName.ToString.ToUpper
                            Case "FNQuantity".ToUpper
                                _pQty += +_Qty
                            Case "FNCTNS".ToUpper
                                If Not (_RangNo = _RangNoOld) Then
                                    _pCTN += +_CTNS
                                End If
                            Case "FNTotalAmount".ToUpper
                                _pAmt += +_Amt
                        End Select
                        _RangNo = _RangNoOld : _LineNo = _LineNoOld
