...
#End Region
    ...
    Private Sub ocmload_Click(sender As Object, e As EventArgs) Handles ocmload.Click
        Try
            ...
            '    HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.InputData, Me.FTEndShipment_lbl.Text)
            ...
            _Cmd = "Exec  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.SP_GET_DATAFORBOOKING_V2 '" & HI.UL.ULDate.ConvertEnDB(Me.FTStartShipment.Text) & "','" & HI.UL.ULDate.ConvertEnDB(Me.FTEndShipment.Text) & "'"
            _Cmd &= "," & Val(Me.FNHSysVenderPramId.Properties.Tag) & ", " & Val(Me.FNHSysShipModeId.Properties.Tag) & "," & Val(Me.FNHSysPlantId.Properties.Tag) & ",'" & HI.UL.ULF.rpQuoted(Me.FTCustomerPO.Text) & "'"
            Dim _dt As New System.Data.DataTable
    ...
    Private Function ExportPackingPlan(oDt As System.Data.DataTable, _Path As String) As Boolean
        ...
        Try
            ...
            Dim _oDT As System.Data.DataTable
            Dim _oDtSubFooter As System.Data.DataTable
            ...
            For Each R As DataRow In oDt.Rows
                ...
                _Cmd = " SELECT  distinct P.FTPckPlanNo, P.FTPORef, FNQuantity, P.FNPackcount, FNNet, FNTotalNet, FNGrossWeight,P.FNHSysUnitId, FNVol, FTVolUnit, isnull( FTApproveState ,'0') as FTApproveState , FTApproveBy,  FDApproveDate "
                _Cmd &= vbCrLf & "    ,  isnull(FNHSysMainMatSpecId,0) as  FNHSysMainMatSpecId   , P.FNHSysProvinceId as  FNHSysProvinceId_Hide  ,    isnull( FTStyleNameShort   , s.FTStyleNameEN) as FTStyleNameShort  "
                '_Cmd &= vbCrLf & " ,  (Select Top 1 isnull(FTMainMatSpecEN,'') as FTMainMatSpecEN  From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMMainMatSpec  WITH (NOLOCK)  Where FNHSysMainMatSpecId = P.FNHSysMainMatSpecId )  as FTMainMatSpec  "
                '_Cmd &= vbCrLf & " ,  (Select Top 1 isnull(FTProvinceCode,'') as FTProvinceCode  From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCMMProvince  WITH (NOLOCK)  Where FNHSysProvinceId = P.FNHSysProvinceId )  as FNHSysProvinceId  "
                ...
                _Cmd &= vbCrLf & " From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "]..TEXPTPackPlan AS P WITH(NOLOCK)  "
                _Cmd &= vbCrLf & " LEFT OUTER JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "]..TEXPTPackPlan_D AS D WITH(NOLOCK)  ON p.FTPORefNo = D.FTPORefNo "
                _Cmd &= vbCrLf & " LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].. TMERMStyle s with(nolock) on d.FTStyleCode = s.FTStyleCode    "
                ...
                _Cmd &= vbCrLf & " outer apply (select top 1  FTStyleNameShort   , FTMainMatSpecCode , FTMainMatSpecEN    from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "]..TMERMMainMatSpec M  where   M.FNHSysStyleId  = s.FNHSysStyleId  ) as M "
                ...
                _Cmd = "Select   D.FTPckPlanNo , D.FTPORef , D.FTPOLineNo   , sum (D.FNItemQty) as  FNItemQty ,  B.FNPackCount  FNPackCount "
                '_Cmd &= vbCrLf & " , sum (FNTotalNetWeight *  d.FNPackCount  )    FNTotalNetWeight"
                _Cmd &= vbCrLf & "  ,    dbo.fn_get_netweight( D.FTPORef ,  D.FTPORefNo , D.FTPOLineNo)   FNTotalNetWeight"
                '_Cmd &= vbCrLf & " , sum (FNGrossNetWeight  *  d.FNPackCount  )   FNGrossNetWeight "
                _Cmd &= vbCrLf & "  ,  dbo.fn_get_grossweight( D.FTPORef ,  D.FTPORefNo , D.FTPOLineNo)   FNGrossNetWeight"
                _Cmd &= vbCrLf & ", FTItemUnitCode  "
                _Cmd &= vbCrLf & " ,  dbo.fn_get_cbm( D.FTPORef ,  D.FTPORefNo , D.FTPOLineNo)  as  FNVol  , FTUnitCode   "
                _Cmd &= vbCrLf & " From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "]..TEXPTPackPlan_D AS D  WITH(NOLOCK)  "
                _Cmd &= vbCrLf & " LEFT OUTER JOIN dbo.getPackCountBreakLintem('" & HI.UL.ULF.rpQuoted(R!FTPORef.ToString) & "' ,'" & HI.UL.ULF.rpQuoted(R!FTPORefNo.ToString) & "') as b on D.FTPORef = b.FTPORef and  D.FTPOLineNo = b.FTPOLineNo "
                _Cmd &= vbCrLf & " where  D.FTPORef='" & HI.UL.ULF.rpQuoted(R!FTPORef.ToString) & "'"
                _Cmd &= vbCrLf & " and    D.FTPORefNo='" & HI.UL.ULF.rpQuoted(R!FTPORefNo.ToString) & "'"
                _Cmd &= vbCrLf & "group by  D.FTPckPlanNo , D.FTPORef  ,  D.FTPORefNo  , D.FTPOLineNo    , FTItemUnitCode , FTUnitCode ,  B.FNPackCount  "

                '   _Cmd = "Select *   From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.getsumbreakdownpackingplan('" & HI.UL.ULF.rpQuoted(R!FTPORef.ToString) & "','" & HI.UL.ULF.rpQuoted(R!FTPORefNo.ToString) & "')  "
                _oDtSubFooter = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_ACCOUNT)
                ...
                For Each H As DataRow In _oDM.Rows
                    ...
                    With xlWorkSheet
                        ...
                        zRow = zRow + 1
                        .Range(.Cells(zRow, 3), .Cells(zRow, 6)).Merge()
                        .Cells(zRow, 3).Value = "Material.1 :"
                        .Cells(zRow, 7).Value = "" & H!FTStyleNameShort.ToString
                        '  .Range("B:B", misValue).EntireColumn.Hidden = True
                        .Range(.Cells(zRow, 7), .Cells(zRow, 29)).Merge()
                        ...
                        .Rows(_Row + 6).Font.size = 7
                        .Range(.Cells(_Row + 6, _Col + 5), .Cells(_Row + 6, _Col + 6)).Merge() : .Range(.Cells(_Row + 6, _Col + 7), .Cells(_Row + 6, _Col + 8)).Merge() : .Range(.Cells(_Row + 6, _Col + 9), .Cells(_Row + 6, _Col + 10)).Merge()
                        .Cells(_Row + 6, _Col + 2).Value = "PO Number" : .Cells(_Row + 6, _Col + 3).Value = "Ref Number" : .Cells(_Row + 6, _Col + 4).Value = "MAIN LINE #"
                        .Cells(_Row + 6, _Col + 5).Value = "Item Qty" : .Cells(_Row + 6, _Col + 7).Value = "Pkg Count" : .Cells(_Row + 6, _Col + 9).Value = "Net"
                        .Cells(_Row + 6, _Col + 11).Value = "Gross" : .Cells(_Row + 6, _Col + 12).Value = "Unit" : .Cells(_Row + 6, _Col + 13).Value = "Vol" : .Cells(_Row + 6, _Col + 14).Value = "Unit"

                        _Row = _Row + 7
                        Dim _rowfirst As Integer = _Row
                        For Each ZR As DataRow In _oDtSubFooter.Rows


                            'values
                            .Rows(_Row).Font.Name = "Arial"
                            .Rows(_Row).Font.size = 7
                            .Rows(_Row).RowHeight = 20
                            .Range(.Cells(_Row, _Col + 5), .Cells(_Row, _Col + 6)).Merge() : .Range(.Cells(_Row, _Col + 7), .Cells(_Row, _Col + 8)).Merge() : .Range(.Cells(_Row, _Col + 9), .Cells(_Row, _Col + 10)).Merge()
                            .Cells(_Row, _Col + 2).Value = HI.UL.ULF.rpQuoted(ZR!FTPckPlanNo.ToString) ' packing No
                            .Cells(_Row, _Col + 3).Value = HI.UL.ULF.rpQuoted(ZR!FTPORef.ToString) ' packing No
                            .Cells(_Row, _Col + 4).Value = HI.UL.ULF.rpQuoted(ZR!FTPOLineNo.ToString) ' packing No

                            .Cells(_Row, _Col + 5).Value = ZR!FNItemQty.ToString 'Item Qty
                            .Cells(_Row, _Col + 7).Value = ZR!FNPackcount.ToString
                            .Cells(_Row, _Col + 9).Value = ZR!FNTotalNetWeight.ToString
                            .Cells(_Row, _Col + 11).Value = ZR!FNGrossNetWeight.ToString
                            .Cells(_Row, _Col + 12).Value = "KG"
                            .Cells(_Row, _Col + 13).Value = ZR!FNVol.ToString
                            .Cells(_Row, _Col + 14).Value = "CMB"
                            _Row = _Row + 1
                        ...
                        With .Range("C" & _rowfirst - 1 & ":O" & _Row & "")
                            .Borders.Weight = 2
                            '.Borders(8).Weight = 2
                            '.Borders(9).Weight = 2
                            '.Borders(10).Weight = 2
                        End With
                        'With .Range("X" & _Row - 2 & ":AB" & _Row - 2 & "")
                        '    .Borders(7).Weight = 2
                        '    .Borders(8).Weight = 2
                        '    .Borders(9).Weight = 2
                        '    .Borders(10).Weight = 2
                        'End With
                        'With .Range("U" & _Row - 1 & ":AG" & _Row - 1 & "")
                        '    .Borders(7).Weight = 2
                        '    .Borders(8).Weight = 2
                        '    .Borders(9).Weight = 2
                        '    .Borders(10).Weight = 2
