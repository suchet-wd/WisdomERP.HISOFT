...
Public Class wPckPlandExportExcel
...
#End Region
    ...
    Private Function writeExcel(_oDM As System.Data.DataTable, _oDT As System.Data.DataTable) As Boolean
        Try

            Dim _SpeardSheet As New DevExpress.XtraSpreadsheet.SpreadsheetControl
            Dim _workbook As New Workbook()
            _workbook.Worksheets.Add()

            Dim _worksheet As Worksheet = _workbook.Worksheets(0)
            ...
            For Each H As DataRow In _oDM.Rows
                ...
                With _worksheet
                    ...
                    .Range(.Cells("A1"), .Cells("J1")).Merge()
                    .Rows(1).Font.Name = "Arial"
                    .Rows(1).Font.size = 14
                    .Rows(1).Font.Bold = True
                    .Cells("A1").Value = "Packing Plan"
                    .Range(.Cells("C3"), .Cells("H3")).Merge()
                    .Rows(3).Font.Name = "Arial"
                    .Rows(3).Font.size = 9
                    .Cells("C3").Value = "Plan Ref Number"
                    .Range(.Cells("I3"), .Cells("N3")).Merge()
                    .Cells("I3").Value = HI.UL.ULF.rpQuoted(H!FTPORef.ToString) ' PORef
                    .Cells("I3").Font.Bold = True
                    .Range(.Cells("W3"), .Cells("AF3")).Merge()
                    .Cells("W3").Font.Bold = True
                    .Cells("W3").Value = H!FDShipDate.ToString
                    .Range(.Cells("A4"), .Cells("AG4")).Borders(9).Weight = 2
                    .Range(.Cells("A6"), .Cells("O6")).Merge()
                    .Rows(1).Font.Name = "Arial"
                    .Rows(1).Font.size = 14
                    .Rows(1).Font.Bold = True
                    .Cells("A6").Value = "PO SUMMARY"
                    ' table summary
                    .Range(.Cells("F7"), .Cells("G7")).Merge() : .Range(.Cells("H7"), .Cells("I7")).Merge() : .Range(.Cells("J7"), .Cells("K7")).Merge()
                    .Cells("C7").Value = "PO Number" : .Cells("D7").Value = "Item Qty" : .Cells("E7").Value = "Pkg Count" : .Cells("F7").Value = "Net Net" : .Cells("H7").Value = "Net"
                    .Cells("J7").Value = "Gross" : .Cells("L7").Value = "Unit" : .Cells("M7").Value = "Vol" : .Cells("N7").Value = "Unit"



                    'values
                    .Range(.Cells("F8"), .Cells("G8")).Merge() : .Range(.Cells("H8"), .Cells("I8")).Merge() : .Range(.Cells("J8"), .Cells("K8")).Merge()
                    .Cells("C8").Value = HI.UL.ULF.rpQuoted(H!FTPckPlanNo.ToString) ' packing No
                    .Cells("D8").Value = H!FNQuantity.ToString 'Item Qty
                    .Cells("E8").Value = H!FNPackcount.ToString
                    .Cells("F8").Value = H!FNNet.ToString
                    .Cells("H8").Value = H!FNTotalNet.ToString
                    .Cells("J8").Value = H!FNGrossWeight.ToString
                    .Cells("L8").Value = "KG"
                    .Cells("M8").Value = H!FNVol.ToString
                    .Cells("N8").Value = H!FTVolUnit.ToString
                    'End table summary

                    .Range(.Cells("X7"), .Cells("AB7")).Merge()
                    .Cells("X7").Value = H!FTDiamondMarkCode.ToString
                    .Range(.Cells("U8"), .Cells("AH8")).Merge()
                    .Cells("U8").Value = H!FTProvinceName.ToString


                    .Range(.Cells("A11"), .Cells("AG11")).Borders(9).Weight = 2
                    .Range(.Cells("A13"), .Cells("O13")).Merge()
                    .Cells("A13").Value = "PACKAGE DETAIL"
                    'table Detail
                    .Cells("A13").Value = "PACKAGE DETAIL"

                    .Cells("D15").Value = "Range"
                    .Cells("E15").Value = "From"
                    .Cells("F15").Value = "To"
                    .Range(.Cells("H15"), .Cells("I15")).Merge()
                    .Cells("H15").Value = "Serial From"
                    .Range(.Cells("J15"), .Cells("K15")).Merge()
                    .Cells("J15").Value = "Serial To"
                    .Cells("L15").Value = "Pack Instruction Code"
                    .Cells("M15").Value = "Line #"
                    .Cells("N15").Value = "Buyer Item #"
                    .Cells("O15").Value = "MAIN LINE #"
                    .Range(.Cells("P15"), .Cells("Q15")).Merge()
                    .Cells("P15").Value = "Color Name"
                    .Cells("R15").Value = "Size"
                    .Cells("T15").Value = "Shipment Method"
                    .Cells("U15").Value = "Item Qty"
                    .Cells("V15").Value = "Qty per Pkg/Inner Pack"
                    .Cells("W15").Value = "Pkg Count"
                    .Cells("X15").Value = "R"
                    .Cells("Y15").Value = "Pkg Code"
                    .Cells("Z15").Value = "Net Net"
                    .Cells("AA15").Value = "Net"
                    .Range(.Cells("AB15"), .Cells("AC15")).Merge()
                    .Cells("AB15").Value = "Gross"
                    .Range(.Cells("AD15"), .Cells("AE15")).Merge()
                    .Cells("AD15").Value = "Unit"
                    .Cells("AF15").Value = "L"
                    .Range(.Cells("AG15"), .Cells("AH15")).Merge()
                    .Cells("AG15").Value = "W"
                    .Cells("AI15").Value = "H"
                    .Cells("AJ15").Value = "Unit"


                    'value detail

                    Dim zRow As Integer = 15
                    For Each Z As DataRow In _oDT.Rows
                        zRow += +1

                        .Cells("D" & zRow).Value = "" & Z!FTRangeNo.ToString
                        .Cells("E" & zRow).Value = "" & Z!FNFrom.ToString
                        .Cells("F" & zRow).Value = "" & Z!FNTo.ToString
                        .Cells("H" & zRow).Value = "" & Z!FTSerialFrom.ToString
                        .Cells("J" & zRow).Value = "" & Z!FTSerialTo.ToString
                        .Cells("L" & zRow).Value = "" & Z!FTPackInstructionCode.ToString
                        .Cells("M" & zRow).Value = "" & Z!FTLineNo.ToString
                        .Cells("N" & zRow).Value = "" & Z!FTStyleCode.ToString
                        .Cells("O" & zRow).Value = "" & Z!FTPOLineNo.ToString
                        .Cells("P" & zRow).Value = "" & Z!FTColorWay.ToString
                        .Cells("R" & zRow).Value = "" & Z!FTSizeBreakDown.ToString
                        .Cells("S" & zRow).Value = "" & Z!FTShortDescription.ToString
                        .Cells("T" & zRow).Value = "" & Z!FTShipmentMethod.ToString
                        .Cells("U" & zRow).Value = "" & Z!FNItemQty.ToString
                        .Cells("V" & zRow).Value = "" & Z!FNQtyPerPack.ToString
                        .Cells("W" & zRow).Value = "" & Z!FNPackCount.ToString
                        .Cells("X" & zRow).Value = "" & Z!FTR.ToString
                        .Cells("Y" & zRow).Value = "" & Z!FNInnerPackCount.ToString
                        .Cells("Z" & zRow).Value = "" & Z!FNNetWeight.ToString
                        .Cells("AA" & zRow).Value = "" & Z!FNTotalNetWeight.ToString
                        .Cells("AB" & zRow).Value = "" & Z!FNGrossNetWeight.ToString
                        .Cells("AD" & zRow).Value = "KG" ' & Z!FTUnitCode.ToString
                        .Cells("AF" & zRow).Value = "" & Z!FNL.ToString
                        .Cells("AG" & zRow).Value = "" & Z!FNW.ToString
                        .Cells("AI" & zRow).Value = "" & Z!FNH.ToString
                        .Cells("AJ" & zRow).Value = "" & Z!FTUnitCode.ToString
                    Next
                    'End table Detail
                    zRow = zRow + 3
                    .Range(.Cells("C" & zRow), .Cells("C" & zRow)).Merge()
                    .Cells("C" & zRow).Value = "DESCRIPTION :"
                    .Cells("H" & zRow).Value = "" & H!FTMainMatSpecCode.ToString


                    zRow = zRow + 1
                    .Range(.Cells("C" & zRow), .Cells("C" & zRow)).Merge()
                    .Cells("C" & zRow).Value = "SHELL :"
                    .Cells("H" & zRow).Value = "" & H!FTMainMatSpec.ToString
