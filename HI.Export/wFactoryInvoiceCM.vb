...
Public Class wFactoryInvoiceCM

    Private _InvoiceExport As wFactoryInvoiceCMInvoiceExport
    Private _StateRowChange As Boolean = False
    ...
    Private Sub LoadOrderBreakDown(Key As Object)
        ...
        _FNHSysContinentId = Integer.Parse(Val(FNHSysContinentId.Properties.Tag))
        _FNHSysCountryId = Integer.Parse(Val(FNHSysCountryId.Properties.Tag))
        _FNHSysProvinceId = Integer.Parse(Val(FNHSysProvinceId.Properties.Tag))
        _FNHSysShipModeId = Integer.Parse(Val(FNHSysShipModeId.Properties.Tag))
        _FNHSysShipPortId = Integer.Parse(Val(FNHSysShipPortId.Properties.Tag))
    ...
    Private Function SaveData() As Boolean
        ...
        Try
            ...
            _Qry = " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice"
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                ...
                _Qry = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice"
                _Qry &= vbCrLf & "  ("
                _Qry &= vbCrLf & " FTInsUser, FDInsDate, FTInsTime, FTCustomerPO, FTInvoiceNo, FDInvoiceDate,FNHsysCmpID  "
                _Qry &= vbCrLf & "  , FNHSysContinentId, FNHSysCountryId,FNHSysProvinceId, FNHSysShipModeId, FNHSysShipPortId,FTStateMerApp,FTStateMerReject,FNTotalCarton,FTStateHanger,FTStateSendApp,FTStateWHApp,FTStateWHReject,FTStateApp "
                _Qry &= vbCrLf & " )"
                _Qry &= vbCrLf & " SELECT "
                ...
                _Qry &= vbCrLf & " ," & Integer.Parse(Val(FNHSysContinentId.Properties.Tag.ToString())) & ""
                _Qry &= vbCrLf & " ," & Integer.Parse(Val(FNHSysCountryId.Properties.Tag.ToString())) & ""
                _Qry &= vbCrLf & " ," & Integer.Parse(Val(FNHSysProvinceId.Properties.Tag.ToString())) & ""
                _Qry &= vbCrLf & " ," & Integer.Parse(Val(FNHSysShipModeId.Properties.Tag.ToString())) & ""
                _Qry &= vbCrLf & " ," & Integer.Parse(Val(FNHSysShipPortId.Properties.Tag.ToString())) & ""
                _Qry &= vbCrLf & " ,'0','0'"
                _Qry &= vbCrLf & " ," & FNTotalCarton.Value & ""
                _Qry &= vbCrLf & " ,'" & FTStateHanger.EditValue.ToString & "','0','0','0','0'"
            ...
            With ogvbreakdown
                ...
                For I As Integer = 0 To .RowCount - 1
                    ...
                    _Qry &= vbCrLf & " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice_D"
                    _Qry &= vbCrLf & " WHERE FTCustomerPO='" & HI.UL.ULF.rpQuoted(FTCustomerPO.Text.Trim()) & "'"
                    _Qry &= vbCrLf & " AND FTInvoiceNo='" & HI.UL.ULF.rpQuoted(_SysInvoiceKey) & "'"
                    _Qry &= vbCrLf & " AND FTColorway='" & HI.UL.ULF.rpQuoted(Colorway) & "'"
                    _Qry &= vbCrLf & " AND FTPOLineItem='" & HI.UL.ULF.rpQuoted(POLineItem) & "'"
                    ...
                    For C As Integer = 0 To .Columns.Count - 1
                        Select Case .Columns(C).FieldName.ToString.ToUpper
                            ...
                            Case Else
                                ...
                                If Qty > 0 Then
                                    _Qry &= vbCrLf & "  "
                                    _Qry &= vbCrLf & "  INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice_D"
                                    _Qry &= vbCrLf & "  ("
                                    _Qry &= vbCrLf & "  FTInsUser, FDInsDate, FTInsTime, FTCustomerPO, FTInvoiceNo, FTColorway, FTSizeBreakDown, FNQuantity,FTPOLineItem "
                                    _Qry &= vbCrLf & "  ) "
                                    _Qry &= vbCrLf & " SELECT "
                                    _Qry &= vbCrLf & "  '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                                    _Qry &= vbCrLf & " , " & HI.UL.ULDate.FormatDateDB & ""
                                    ...
                                    _Qry &= vbCrLf & " ,'" & Qty & "'"
                                    _Qry &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(POLineItem) & "'"
    ...
    Private Function DeleteData() As Boolean
        ...
        Try
            ...
            _Qry = " DELETE FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_ACCOUNT) & "].dbo.TACCTFactoryCMInvoice_D"
            _Qry &= vbCrLf & " WHERE FTCustomerPO='" & HI.UL.ULF.rpQuoted(FTCustomerPO.Text.Trim()) & "'"
            _Qry &= vbCrLf & " AND FTInvoiceNo='" & HI.UL.ULF.rpQuoted(FTInvoiceNo.Text.Trim()) & "'"
            HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
    ...
    Private Sub FTInvoiceNo_EditValueChanged(sender As Object, e As EventArgs) Handles FTInvoiceNo.EditValueChanged
        ...
        Else
            ...
            FTStateHanger.Checked = False
            Call LoadCustomerPODefault()
            If (Microsoft.VisualBasic.Right(FTInvoiceNo.Text, 3) <> "###" Or Microsoft.VisualBasic.Right(FTInvoiceNo.Text, 3) <> "") Then
    ...
    Private Sub ogvbreakdown_RowCellStyle(sender As Object, e As DevExpress.XtraGrid.Views.Grid.RowCellStyleEventArgs) Handles ogvbreakdown.RowCellStyle
        Try
            With Me.ogvbreakdown
    ...
    Private Sub FNHSysContinentId_EditValueChanged(sender As Object, e As EventArgs) Handles FNHSysContinentId.EditValueChanged _
           , FNHSysCountryId.EditValueChanged _
          , FNHSysProvinceId.EditValueChanged _
            , FNHSysShipModeId.EditValueChanged _
            , FNHSysShipPortId.EditValueChanged
    ...
    Private Sub ogvbreakdown_RowCountChanged(sender As Object, e As EventArgs) Handles ogvbreakdown.RowCountChanged
        Try
            If _StateRowChange Then
                SetCartonQty()
                _StateRowChange = False
            End If
        Catch ex As Exception
                ...
                For I As Integer = 0 To .RowCount - 1
                    ...
                    For Each GridCol As DevExpress.XtraGrid.Columns.GridColumn In .Columns
                        ...
                        Select Case GridCol.FieldName.ToString.ToUpper
                            Case "FTOrderNo".ToUpper, "FTSubOrderNo".ToUpper, "FTColorway".ToUpper, "Total".ToUpper
                            Case "FTNikePOLineItem".ToUpper
                                If .GetRowCellValue(I, "Total") > 0 Then
    ...
    Private Sub ogvbreakdown_CustomRowFilter(sender As Object, e As RowFilterEventArgs) Handles ogvbreakdown.CustomRowFilter
        Try
            'If e.Handled Then
            _StateRowChange = True
            'End If
        Catch ex As Exception
