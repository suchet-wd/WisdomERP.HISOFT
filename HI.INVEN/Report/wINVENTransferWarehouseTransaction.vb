...
Public Class wINVENReportTransferWarehouseTransaction
    ...
    Private Sub _Preview()
        Try
            ...
            If Me.FTTransferWHNo.Text <> "" Then
                ...
                _FN &= "  {TINVENTransferWH.FTTransferWHNo} >= '" & HI.UL.ULF.rpQuoted(Me.FTTransferWHNo.Text) & "'"
            End If

            If Me.FTTransferWHNoTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferWH.FTTransferWHNo} <= '" & HI.UL.ULF.rpQuoted(Me.FTTransferWHNoTo.Text) & "'"
            End If


            '************Date Receive
            If Me.FDSTransferWHDate.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferWH.FDTransferWHDate} >= '" & HI.UL.ULDate.ConvertEnDB(Me.FDSTransferWHDate.Text) & "'"
            End If

            If Me.FDETransferWHDateTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferWH.FDTransferWHDate} <= '" & HI.UL.ULDate.ConvertEnDB(Me.FDETransferWHDateTo.Text) & "'"
            End If
            ...
            If Me.IcCondition.otpUser.PageVisible Then
                If Me.IcCondition.FTUser.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENTransferWH.FTTransferWHBy} >= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUser.Text) & "'"
                End If

                If Me.IcCondition.FTUserTo.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENTransferWH.FTTransferWHBy} <= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUserTo.Text) & "'"
                End If
            End If
            ...
            If _AllReportName <> "" Then
                For Each _ReportName As String In _AllReportName.Split(",")
                    With New HI.RP.Report
                        ...
                        .Formular = _FN
                        .AddParameter("RcvDate", Me.FDSTransferWHDate.Text)
                        .AddParameter("RcvDateTo", Me.FDETransferWHDateTo.Text)
