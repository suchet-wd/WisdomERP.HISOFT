...
Public Class wINVENReportTransferToStockCenterTransaction
    ...
    Private Sub _Preview()
        Try
            ...
            If Me.FTTransferCenterNo.Text <> "" Then
                ...
                _FN &= "  {TINVENTransferCenter.FTTransferCenterNo} >= '" & HI.UL.ULF.rpQuoted(Me.FTTransferCenterNo.Text) & "'"
            End If

            If Me.FTTransferCenterNoTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferCenter.FTTransferCenterNo} <= '" & HI.UL.ULF.rpQuoted(Me.FTTransferCenterNoTo.Text) & "'"
            End If

            '************Date Receive
            If Me.FDTransferCenterDate.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferCenter.FDTransferCenterDate} >= '" & HI.UL.ULDate.ConvertEnDB(Me.FDTransferCenterDate.Text) & "'"
            End If

            If Me.FDTransferCenterDateTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferCenter.FDTransferCenterDate} <= '" & HI.UL.ULDate.ConvertEnDB(Me.FDTransferCenterDateTo.Text) & "'"
            End If
            ...
            If Me.IcCondition.otpUser.PageVisible Then
                If Me.IcCondition.FTUser.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENTransferCenter.FTTransferCenterBy} >= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUser.Text) & "'"
                End If

                If Me.IcCondition.FTUserTo.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENTransferCenter.FTTransferCenterBy} <= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUserTo.Text) & "'"
                End If
            End If
            ...
            If _AllReportName <> "" Then
                For Each _ReportName As String In _AllReportName.Split(",")
                    With New HI.RP.Report
                        ...
                        .Formular = _FN
                        .AddParameter("RcvDate", Me.FDTransferCenterDate.Text)
                        .AddParameter("RcvDateTo", Me.FDTransferCenterDateTo.Text)
