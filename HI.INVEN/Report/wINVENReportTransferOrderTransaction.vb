...
Public Class wINVENReportTransferOrderTransaction
    ...
    Private Sub _Preview()
        Try
            ...
            If Me.FTTransferOrderNo.Text <> "" Then
                ...
                _FN &= "  {TINVENTransferOrder.FTTransferOrderNo} >= '" & HI.UL.ULF.rpQuoted(Me.FTTransferOrderNo.Text) & "'"
            End If

            If Me.FTTransferOrderNoTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferOrder.FTTransferOrderNo} <= '" & HI.UL.ULF.rpQuoted(Me.FTTransferOrderNoTo.Text) & "'"
            End If
            ...
            '************Date Receive
            If Me.FDSTransferOrderDate.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferOrder.FDTransferOrderDate} >= '" & HI.UL.ULDate.ConvertEnDB(Me.FDSTransferOrderDate.Text) & "'"
            End If

            If Me.FDETransferOrderDateTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENTransferOrder.FDTransferOrderDate} <= '" & HI.UL.ULDate.ConvertEnDB(Me.FDETransferOrderDateTo.Text) & "'"
            End If
            ...
            If Me.IcCondition.otpUser.PageVisible Then
                If Me.IcCondition.FTUser.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENTransferOrder.FTTransferOrderBy} >= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUser.Text) & "'"
                End If

                If Me.IcCondition.FTUserTo.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENTransferOrder.FTTransferOrderBy} <= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUserTo.Text) & "'"
                End If
            End If
            ...
            If _AllReportName <> "" Then
                For Each _ReportName As String In _AllReportName.Split(",")
                    With New HI.RP.Report
                        ...
                        .Formular = _FN
                        .AddParameter("RcvDate", Me.FDSTransferOrderDate.Text)
                        .AddParameter("RcvDateTo", Me.FDETransferOrderDateTo.Text)
