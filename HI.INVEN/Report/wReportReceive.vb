...
Public Class wReportReceive
    ...
    Private Sub _Preview()
        Try
            ...
            If Me.FDReceiveDate.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENReceive.FDReceiveDate} >= '" & HI.UL.ULDate.ConvertEnDB(Me.FDReceiveDate.Text) & "'"
            End If

            If Me.FDReceiveDateTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENReceive.FDReceiveDate} <= '" & HI.UL.ULDate.ConvertEnDB(Me.FDReceiveDateTo.Text) & "'"
            End If
            ...
            If _AllReportName <> "" Then
                For Each _ReportName As String In _AllReportName.Split(",")
                    With New HI.RP.Report
                        ...
                        .Formular = _FN
                        .AddParameter("RcvDate", Me.FDReceiveDate.Text)
                        .AddParameter("RcvDateTo", Me.FDReceiveDateTo.Text)
    ...
    Private Sub wReportReceive_Load(sender As Object, e As EventArgs) Handles Me.Load
        RemoveHandler FDReceiveDate.GotFocus, AddressOf HI.TL.HandlerControl.DateEdit_GotFocus
        RemoveHandler FDReceiveDateTo.GotFocus, AddressOf HI.TL.HandlerControl.DateEdit_GotFocus
    End Sub
End Class