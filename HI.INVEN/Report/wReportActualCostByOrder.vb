...
Public Class wReportActualCostByOrder
    ...
    Private Sub _Preview()
        Try
            ...
            If Me.FTOrderNo.Text <> "" Then
                ...
                _FN &= " {V_RptActualCostByOrder.FTOrderNo} >= '" & HI.UL.ULF.rpQuoted(Me.FTOrderNo.Text) & "'"
            End If

            If Me.FTOrderNoTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {V_RptActualCostByOrder.FTOrderNo} <= '" & HI.UL.ULF.rpQuoted(Me.FTOrderNoTo.Text) & "'"
            End If

            '************Date Receive
            If Me.FDDate.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {V_RptActualCostByOrder.FDOrderDate}>= '" & HI.UL.ULDate.ConvertEnDB(Me.FDDate.Text) & "'"
            End If

            If Me.FDDateTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {V_RptActualCostByOrder.FDOrderDate} <= '" & HI.UL.ULDate.ConvertEnDB(Me.FDDateTo.Text) & "'"
            End If
            ...
            Select Case IcCondition.FNItemCon.SelectedIndex
                Case 1
                    If IcCondition.FNHSysRawMatId.Text <> "" Then
                        _Criteria &= IIf(_Criteria.Trim <> "", " AND ", "")
                        _Criteria &= "  {V_RptActualCostByOrder.FTRawMatCode} >='" & HI.UL.ULF.rpQuoted(IcCondition.FNHSysRawMatId.Text) & "' "
                    End If

                    If IcCondition.FNHSysRawMatIdTo.Text <> "" Then
                        _Criteria &= IIf(_Criteria.Trim <> "", " AND ", "")
                        _Criteria &= "  {V_RptActualCostByOrder.FTRawMatCode} <='" & HI.UL.ULF.rpQuoted(IcCondition.FNHSysRawMatIdTo.Text) & "' "
                    End If
                ...
                Case 2
                    ...
                    If tText.Trim <> "" Then
                        tText = Microsoft.VisualBasic.Left(Trim(tText), Len(Trim(tText)) - 1)
                        _Criteria &= IIf(_Criteria.Trim <> "", " AND ", "")
                        _Criteria &= "  {V_RptActualCostByOrder.FTRawMatCode} IN['" & tText.Replace("|", "','") & "'] "
                    End If
            End Select
        End If
