...
Public Class wINVENReportReturnSuplier
    ...
    Private Sub _Preview()
        Try
            ...
            If Me.FTReturnSuplNo.Text <> "" Then
                ...
                _FN &= "  {TINVENReturnToSupplier.FTReturnSuplNo} >= '" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'"
            End If

            If Me.FTReturnSuplNoTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENReturnToSupplier.FTReturnSuplNo} <= '" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNoTo.Text) & "'"
            End If
            ...
            '************Date Receive
            If Me.FDSReturnSuplDate.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENReturnToSupplier.FDReturnSuplDate} >= '" & HI.UL.ULDate.ConvertEnDB(Me.FDSReturnSuplDate.Text) & "'"
            End If

            If Me.FDEReturnSuplDateTo.Text <> "" Then
                _FN &= IIf(_FN.Trim <> "", " AND ", "")
                _FN &= " {TINVENReturnToSupplier.FDReturnSuplDate} <= '" & HI.UL.ULDate.ConvertEnDB(Me.FDEReturnSuplDateTo.Text) & "'"
            End If
            ...
            If Me.IcCondition.otpUser.PageVisible Then
                If Me.IcCondition.FTUser.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENReturnToSupplier.FTReturnSuplBy} >= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUser.Text) & "'"
                End If

                If Me.IcCondition.FTUserTo.Text <> "" Then
                    _FN &= IIf(_FN.Trim <> "", " AND ", "")
                    _FN &= " {TINVENReturnToSupplier.FTReturnSuplBy} <= '" & HI.UL.ULF.rpQuoted(Me.IcCondition.FTUserTo.Text) & "'"
                End If
            End If
            ...
            If _AllReportName <> "" Then
                For Each _ReportName As String In _AllReportName.Split(",")
                    With New HI.RP.Report
                        ...
                        .Formular = _FN
                        .AddParameter("RcvDate", Me.FDSReturnSuplDate.Text)
                        .AddParameter("RcvDateTo", Me.FDEReturnSuplDateTo.Text)
