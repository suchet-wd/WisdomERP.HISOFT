...
Public Class wTransferWHToWHApprove
...
#Region "Procedure"
    ...
    Private Sub InitFormControl()
        ...
        _Str &= vbCrLf & " WHERE FTDynamicFormName='" & HI.UL.ULF.rpQuoted("wTransferWHToWH") & "' "
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        FTFacPurchaseNo.Text = ""
        FTInvoiceNo.Text = ""

        Dim dt As DataTable

        _Str = "select top 1 FTFacPurchaseNo,FTInvoiceNo from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferWH AS X WITH(NOLOCK) WHERE FTTransferWHNo='" & HI.UL.ULF.rpQuoted(FTTransferWHNo.Text) & "'"
    ...
    Private Sub LoadDucumentDetail(PoKey As String)
        ...
        _Str = "  SELECT     ISNULL(W.FTWHCode,'') AS FNHSysWHId,  BO.FTOrderNo, BO.FTBarcodeNo, IM.FTRawMatCode, ISNULL(C.FTRawMatColorCode,'') AS FTRawMatColorCode,BO.FTDocumentRefNo"
        _Str &= vbCrLf & ", ISNULL(S.FTRawMatSizeCode,'') AS FTRawMatSizeCode, U.FTUnitCode, BO.FNQuantity"
        _Str &= vbCrLf & " ,ISNULL(("
        _Str &= vbCrLf & "  SELECT TOP 1 FNQuantity "
        _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_IN  "
        _Str &= vbCrLf & " WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(Me.FTTransferWHNo.Text) & "' "
        _Str &= vbCrLf & " AND  FTOrderNo=BO.FTOrderNo  "
        _Str &= vbCrLf & " AND  FTBarcodeNo=BO.FTBarcodeNo  "
        _Str &= vbCrLf & "  AND  ( FTDocumentRefNo=BO.FTDocumentRefNo   OR  FTDocumentRefNo ='" & HI.UL.ULF.rpQuoted(Me.FTTransferWHNo.Text) & "' )  "
        ...
        _Str &= vbCrLf & ",ISNULL(BO.FNPriceTrans,-1) AS FNPriceTrans,B.FTPurchaseNo "

        _Str &= vbCrLf & ",ISNULL(BO.FNPriceClose1,-1) AS FNPriceClose1"
        _Str &= vbCrLf & ",ISNULL(BO.FNPriceClose2,-1) AS FNPriceClose2"
        _Str &= vbCrLf & " FROM             [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferWH AS H WITH (NOLOCK) INNER JOIN"
        _Str &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT AS BO WITH (NOLOCK) ON H.FTTransferWHNo = BO.FTDocumentNo INNER JOIN"
        ...
        ogcdetail.DataSource = HI.Conn.SQLConn.GetDataTable(_Str, Conn.DB.DataBaseName.DB_INVEN)
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTTransferWHNo.Text <> "" Then
            ...
            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "Inventrory\"
                .ReportName = "TransferToWarehouseApproveSlip_Stockcard.rpt"
                .Formular = "{TINVENTransferWH.FTTransferWHNo}='" & HI.UL.ULF.rpQuoted(FTTransferWHNo.Text) & "' "
                .Preview()
            End With
...
#Region " Proc "
    ...
    Private Function SaveBarcode(FTBarcodeNo As String, Qty As Double, Optional StateSameCmp As Boolean = True) As Boolean
        Dim _Str As String
        Dim _BarCode As String = FTBarcodeNo
        Dim _StateNew As Boolean
        Try

            _Str = " SELECT TOP 1 FTBarcodeNo  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_IN WITH(NOLOCK) WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTTransferWHNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTDocumentRefNo='" & HI.UL.ULF.rpQuoted(Me.DocRefNo) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(Me.OrderNo) & "'  "
            ...
            Else
                ...
                Else
                    _Str &= vbCrLf & ",FNPriceClose1=CASE WHEN " & Me.PriceClosed2 & "<0 THEN NULL ELSE " & Me.PriceClosed2 & "  END "
                    _Str &= vbCrLf & ",FNPriceClose2=CASE WHEN " & Me.PriceClosed2 & "<0 THEN NULL ELSE " & Me.PriceClosed2 & "  END "
                End If
...
#End Region
    ...
    Private Function CheckPermissionWH() As Boolean
        ...
        Else
            ...
            _Qry = "SELECT TOP 1  FTWHCode "
            _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS W WITH(NOLOCK)"
            _Qry &= vbCrLf & "  WHERE  (FNHSysWHId =" & Integer.Parse(Val(FNHSysWHIdTo.Properties.Tag.ToString())) & ") "
            _Qry &= vbCrLf & " AND (FNHSysCmpId =" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " OR FNHSysCmpManageId =" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & ")"
    ...
    Private Function DeleteBarcode(BarcodeKey As String, FTOrderNoKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_IN  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTTransferWHNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' "
    ...
    Private Sub ocmapprove_Click(sender As System.Object, e As System.EventArgs) Handles ocmapprove.Click
        ...
        _Strsql = "SELECT TOP 1  ISNULL(FTDocumentState,'') AS  FTDocumentState "
        ...
        If HI.Conn.SQLConn.GetField(_Strsql, Conn.DB.DataBaseName.DB_INVEN, "") <> "1" Then
            HI.MG.ShowMsg.mInvalidData("เอกสารยังไม่ได่ทำการอนุมติจากต้นทาง ไม่สามารถทำการบันทึกได้ !!!", 1312227722, Me.Text)
            Exit Sub
        End If
        ...
        _Strsql = "SELECT TOP 1 FNHSysCmpId,FTStateFreeZone "
        _Strsql &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS W WITH(NOLOCK)"
        _Strsql &= vbCrLf & " WHERE FTWHCode='" & HI.UL.ULF.rpQuoted(Me.FNHSysWHIdTo.Text) & "'"
        ...
        _Strsql = "UPDate  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferWH "
        _Strsql &= vbCrLf & " SET FTStateApprove='1',FNHSysWHIdTo=" & Val(FNHSysWHIdTo.Properties.Tag.ToString) & " "
        ...
        HI.Conn.SQLConn.ExecuteOnly(_Strsql, Conn.DB.DataBaseName.DB_INVEN)
    ...
    Private Sub ocmcancel_Click(sender As System.Object, e As System.EventArgs) Handles ocmcancel.Click
        ...
        If Barcode.CheckDocumentRefOut(FTTransferWHNo.Text) Then
        ...
        Dim _Strsql As String
        _Strsql = "UPDate  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferWH "
        _Strsql &= vbCrLf & " SET FTStateApprove='0',FTApproveBy='',FDApproveDate='',FNHSysWHIdTo=" & Val(FNHSysWHIdTo.Properties.Tag.ToString) & " "
        ...
        _Strsql = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_IN  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTTransferWHNo.Text) & "'  "
        HI.Conn.SQLConn.ExecuteOnly(_Strsql, Conn.DB.DataBaseName.DB_INVEN)
    ...
    Private Sub ocmsavebarcode_Click(sender As System.Object, e As System.EventArgs) Handles ocmsavebarcode.Click
        ...
        _Str &= vbCrLf & "  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferWH "
        _Str &= vbCrLf & " WHERE    FTTransferWHNo='" & HI.UL.ULF.rpQuoted(Me.FTTransferWHNo.Text) & "' "

        If HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_INVEN, "") <> "1" Then
            HI.MG.ShowMsg.mInvalidData("ยังไม่ มีการ อนุมัติ  เอกสารมีการ รับเข้า แล้วไม่สามารถทำการลบ หรือแก้ไขได้ !!!", 1312220005, Me.Text)
            Exit Sub
        End If


        Dim StateSameCmp As Boolean = True

        _Str = " SELECT TOP 1  '1' AS FTStateSame  "
        _Str &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS H WITH(NOLOCK) "
        _Str &= vbCrLf & " OUTER APPLY ( SELECT TOP 1  Z.FNHSysCmpId  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMWarehouse AS Z WITH(NOLOCK) WHERE Z.FNHSysWHId=" & Val(FNHSysWHIdTo.Properties.Tag.ToString()) & " ) AS Z "
        _Str &= vbCrLf & "  WHERE H.FNHSysWHId=" & Val(FNHSysWHId.Properties.Tag.ToString()) & " AND H.FNHSysCmpId=Z.FNHSysCmpId "

        StateSameCmp = (HI.Conn.SQLConn.GetField(_Str, Conn.DB.DataBaseName.DB_MERCHAN, "") = "1")
        ...
        For Each R As DataRow In CType(ogcdetail.DataSource, DataTable).Rows
            ...
            If Barcode.CheckTransactionOUT(BarcodeNo, FTTransferWHNo.Text, FNHSysWHId.Properties.Tag.ToString, Me.OrderNo) Then
            Else
                Me.DocRefNo = FTTransferWHNo.Text
                Me.WHTo = Val(FNHSysWHIdTo.Properties.Tag.ToString)

                SaveBarcode(BarcodeNo, Qty, StateSameCmp)
            End If
    ...
    Private Sub DeleteBarcode()
        ...
        With ogvdetail
            ...
            For Each i As Integer In .GetSelectedRows()
                ...
                If Barcode.CheckTransactionOUT(_Barcode, FTTransferWHNo.Text, 0, "") Then
    ...
    Private Sub ogvdetail_RowCountChanged(sender As Object, e As EventArgs) Handles ogvdetail.RowCountChanged
        Try
            ...
            FNHSysWHId.Properties.ReadOnly = (dt.Select("FNStateApproveQty='1'").Length > 0)
            FNHSysWHId.Properties.Buttons.Item(0).Enabled = Not ((dt.Select("FNStateApproveQty='1'").Length > 0))

            FNHSysWHIdTo.Properties.ReadOnly = (dt.Select("FNStateApproveQty='1'").Length > 0)
            FNHSysWHIdTo.Properties.Buttons.Item(0).Enabled = Not ((dt.Select("FNStateApproveQty='1'").Length > 0))
