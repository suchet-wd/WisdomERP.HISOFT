...
Public Class wReturnToSupplier
...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        ogcdetail.DataSource = HI.INVEN.Barcode.LoadDocumentBarcode(Key, Barcode.DocType.ReturnToSupplier)  '   HI.Conn.SQLConn.GetDataTable(_Str, Conn.DB.DataBaseName.DB_INVEN)
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReturnToSupplier WHERE FTReturnSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENReturnToSupplier WHERE FTReturnSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTReturnSuplNo.Text) & "'")
...
#Region "MAIN PROC"
    ...
    Private Function ChkCloseStock() As Boolean
        ...
        _Qry &= vbCrLf & "          [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT AS B WITH(NOLOCK) ON A.FTReturnSuplNo = B.FTDocumentNo"
    ...
    Private Sub Proc_Save(sender As System.Object, e As System.EventArgs) Handles ocmsave.Click
        ...
        If ChkCloseStock() = True Then
            Exit Sub
        End If
        If HI.ST.ValidateData.CloseJobDocOut(Me.FTReturnSuplNo.Text) Then
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTReturnSuplNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .ReportName = "ReturnToSupplierSlip.rpt"
                .Formular = "{TINVENReturnToSupplier.FTReturnSuplNo}='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' "
                .Preview()
            End With
            ...
            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "Inventrory\"
                .ReportName = "ReturnToSupplierSlip_Barcode.rpt"
                .Formular = "{TINVENReturnToSupplier.FTReturnSuplNo}='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' "
                .Preview()
            End With
...
#Region "Proc Barcode"
    Private Function SaveBarcode() As Boolean
        ...
        Try
            ...
            _Str = " SELECT TOP 1 FTBarcodeNo  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT WITH(NOLOCK) WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND  FTDocumentRefNo='" & HI.UL.ULF.rpQuoted(Me.DocRefNo) & "' "
    ...
    Private Function DeleteBarcode(BarcodeKey As String, FTOrderNoKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' "
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTReturnSuplNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' ")
...
#End Region
    ...
    Private Sub AddBarCode(BarcodeNo As String, StateAdd As Boolean, Optional Qty As Double = 0)
        ...
        Dim _Dt As DataTable = Barcode.BarCodeBalance(FTBarcodeNo.Text, 0.ToString, "", Me.FTReturnSuplNo.Text, True)
        If _Dt.Rows.Count > 0 Then
            ...
            If _Dt.Select("FNQuantityBal >=" & Qty & " AND FNQuantityBal >0").Length > 0 Then
                ...
                If _Dt.Select("FTPurchaseNo='" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "' AND FNQuantityBal >0 AND ( FNHSysCmpId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " OR FNHSysCmpManageId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " )").Length > 0 Then

                    Me.OrderNo = ""
                    If Me.FTOrderNo.Text = "" Then
                        For Each R As DataRow In _Dt.Select("FTPurchaseNo='" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "' AND  FNQuantityBal >=" & Qty & " AND FNQuantityBal >0 AND ( FNHSysCmpId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " OR FNHSysCmpManageId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " )")
                    ...
                    If _Dt.Select("FTPurchaseNo='" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "' AND  FNQuantityBal >=" & Qty & " AND FNQuantityBal >0 AND FTOrderNo='" & HI.UL.ULF.rpQuoted(FTOrderNo.Text) & "' AND ( FNHSysCmpId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " OR FNHSysCmpManageId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " )").Length > 0 Then

                        For Each R As DataRow In _Dt.Select("FTPurchaseNo='" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "' AND  FNQuantityBal >=" & Qty & " AND FNQuantityBal >0 AND FTOrderNo='" & HI.UL.ULF.rpQuoted(FTOrderNo.Text) & "' AND ( FNHSysCmpId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " OR FNHSysCmpManageId=" & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " )")
    ...
    Private Sub wReturnToSupplier_Load(sender As Object, e As EventArgs) Handles Me.Load
        FNHSysCmpId.Text = HI.ST.SysInfo.CmpID.ToString
