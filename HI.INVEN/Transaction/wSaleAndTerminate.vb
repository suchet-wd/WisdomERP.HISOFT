...
Public Class wSaleAndTerminate
...
#Region "Procedure"
    ...
    Private Sub LoadIssueDetail(Key As String)
        ...
        ogcdetail.DataSource = HI.INVEN.Barcode.LoadDocumentBarcode(Key, Barcode.DocType.SaleAndTerminate)
    ...
    Private Function SaveData() As Boolean
        ...
        If (_StateNew) Then
            _Key = HI.TL.Document.GetDocumentNo(Me.SysDBName, Me.SysTableName, FNSaleAndTerminateType.SelectedIndex.ToString, False, _CmpH).ToString
        End If
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENSaleAndTerminate WHERE FTSaleAndTerminateNo='" & HI.UL.ULF.rpQuoted(Me.FTSaleAndTerminateNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(Me.FTSaleAndTerminateNo.Text) & "'"
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENSaleAndTerminate WHERE FTSaleAndTerminateNo='" & HI.UL.ULF.rpQuoted(Me.FTSaleAndTerminateNo.Text) & "'")
...
#Region "MAIN PROC"
    ...
    Private Function CheckOwner() As Boolean
        If (HI.ST.UserInfo.UserName.ToUpper = FTSaleAndTerminateBy.Text.ToUpper) Or (HI.ST.SysInfo.Admin) Then
            Return True
        Else
            Dim _Qry As String = ""
            Dim _Qry2 As String = ""
            ...
            _Qry &= vbCrLf & "   WHERE  FTUserName = '" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateBy.Text) & "' "
    ...
    Private Sub Proc_Save(sender As System.Object, e As System.EventArgs) Handles ocmsave.Click
        ...
        If StockValidate.CheckCloseStock(Integer.Parse(Val(FNHSysWHId.Properties.Tag.ToString)), Me.FDSaleAndTerminateDate.Text) = True Then
            Exit Sub
        End If

        If HI.INVEN.StockValidate.CheckDocCreateInvoiceSale(Me.FTSaleAndTerminateNo.Text) Then
    ...
    Private Sub Proc_Delete(sender As System.Object, e As System.EventArgs) Handles ocmdelete.Click
        ...
        If HI.MG.ShowMsg.mConfirmProcessDefaultNo(MG.ShowMsg.ProcessType.mDelete, Me.FTSaleAndTerminateNo.Text, Me.Text) = False Then
            Exit Sub
        End If
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTSaleAndTerminateNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .ReportName = "SaleAndTerminateSlip.rpt"
                .Formular = "{TINVENSaleAndTerminate.FTSaleAndTerminateNo}='" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' "
                .Preview()
            End With
            ...
            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "Inventrory\"
                .ReportName = "SaleAndTerminateSlip_Barcode.rpt"
                .Formular = "{TINVENSaleAndTerminate.FTSaleAndTerminateNo}='" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' "
                .Preview()
            End With

        Else
            HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.SelectData, Me.Text, Me.FTSaleAndTerminateNo_lbl.Text)
            FTSaleAndTerminateNo.Focus()
        End If
    End Sub
...
#Region " Proc "
    ...
    Private Function SaveBarcode() As Boolean
        ...
        Try
            ...
            _Str = " SELECT TOP 1 FTBarcodeNo  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT WITH(NOLOCK) WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTDocumentRefNo='" & HI.UL.ULF.rpQuoted(Me.DocRefNo) & "' "
            ...
            If _StateNew Then
                ...
                _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' "
            ...
            Else
                ...
                _Str &= vbCrLf & " WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' "
    ...
    Private Function DeleteBarcode(BarcodeKey As String, FTOrderNoKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' "
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTSaleAndTerminateNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' ")
...
#End Region
    ...
    Private Sub FNIssueBarType_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles FNIssueBarType.SelectedIndexChanged
        ...
        If FTSaleAndTerminateNo.Text <> "" Then
            FTBarcodeNo.Focus()
        ...
        Select Case e.KeyCode
            Case Keys.Enter
                ...
                If FTBarcodeNo.Text <> "" Then
                    ...
                    If FTSaleAndTerminateNo.Properties.Tag.ToString = "" Then
                        If Me.VerrifyData() Then
                            If Me.SaveData Then
                            Else
                                Exit Sub
                    ...
                    Else
                        ...
                        If Me.FTSaleAndTerminateNo.Text = "" Then Exit Sub
                        LoadDataInfo(Me.FTSaleAndTerminateNo.Text)
    ...
    Private Sub AddBarCode(BarcodeNo As String, StateAdd As Boolean, Optional Qty As Double = 0)
        ...
        Dim _Dt As DataTable = Barcode.BarCodeBalance(FTBarcodeNo.Text, 0.ToString, "", Me.FTSaleAndTerminateNo.Text, True)
        If _Dt.Rows.Count > 0 Then
                    ...
                    If _Dt.Select("FNQuantityBal >=" & Qty & " AND FNHSysWHId=" & Val(FNHSysWHId.Properties.Tag.ToString) & " AND FNQuantityBal >0 AND FTOrderNo='" & HI.UL.ULF.rpQuoted(FTOrderNo.Text) & "'").Length > 0 Then
                        ...
                        If (StateAdd) Then
                            ...
                            If SaveBarcode() Then
                                ...
                                LoadIssueDetail(Me.FTSaleAndTerminateNo.Text)
                                FTBarcodeNo.Focus()
                                FTBarcodeNo.SelectAll()
                                FNQuantityBal.Value = 0
    ...
    Private Sub ogvdetail_RowCountChanged(sender As Object, e As System.EventArgs) Handles ogvdetail.RowCountChanged
        ...
        Try
            ...
            FNSaleAndTerminateType.Properties.ReadOnly = (dt.Rows.Count > 0)
            dt.Dispose()
        Catch ex As Exception
        End Try
    ...
    Private Sub DeleteBarcode()
        ...
        With ogvdetail
            ...
            For Each i As Integer In .GetSelectedRows()
                ...
                If Barcode.CheckTransactionIN(_Barcode, FTSaleAndTerminateNo.Text, 0, Me.FTOrderNo.Text) Then

                    HI.MG.ShowMsg.mInvalidData("Barcode มีการเดิน คืน Stock แล้วไม่สามารถทำการลบ หรือแก้ไขได้ !!!", 1312150001, Me.Text, _Barcode)

                Else

                    If Barcode.CheckTransactionRTSAfterIssue(_Barcode, FTSaleAndTerminateNo.Text, Me.FNHSysWHId.Properties.Tag.ToString, _FTOrderNo) Then
    ...
    Private Sub wSaleAndTerminate_Load(sender As Object, e As EventArgs) Handles Me.Load
        FNHSysCmpId.Text = HI.ST.SysInfo.CmpID.ToString
    End Sub
