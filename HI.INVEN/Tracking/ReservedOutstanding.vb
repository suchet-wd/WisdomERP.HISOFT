...
Public Class ReservedOutstanding
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        _Qry &= vbCrLf & ",FTRawMatSizeCode,FTBarcodeNo,FTUnitCode"
        _Qry &= vbCrLf & ",Reserved AS Reserved"
        _Qry &= vbCrLf & ",Used AS Us"
        _Qry &= vbCrLf & ",Reserved-Used AS SUCCC"
        _Qry &= vbCrLf & ",FTFacPurchaseNo"
        _Qry &= vbCrLf & ",DocIssNo "
        _Qry &= vbCrLf & ",DocIssRemark,FTPurchaseNo "
        _Qry &= vbCrLf & "FROM"
        ...
        _Qry &= vbCrLf & ",BI.FTBarcodeNo,MAX(B.FTPurchaseNo) AS FTPurchaseNo"
        _Qry &= vbCrLf & ",sum( DISTINCT  ISNULL(Bi.FNQuantity,0)) AS Reserved"
        _Qry &= vbCrLf & ", MAX( ISNULL(BOUT.FNQuantity,0)) AS Used"
        ...
        _Qry &= vbCrLf & ",MAX(ISNULL(RPO.DocIssNo,'')) AS DocIssNo "
        _Qry &= vbCrLf & ",MAX(ISNULL(RPO.FTRemark,'')) AS  DocIssRemark "
        _Qry &= vbCrLf & "FROM"
        ...
        _Qry &= vbCrLf & " SELECT SUM(BO.FNQuantity) AS FNQuantity "
        _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENBarcode_OUT As BO With (NOLOCK) "
        _Qry &= vbCrLf & "  WHERE  BO.FTBarcodeNo = BI.FTBarcodeNo And BO.FTDocumentRefNo = BI.FTDocumentNo "
        _Qry &= vbCrLf & "   ) BOUT "

        _Qry &= vbCrLf & " OUTER APPLY ( "
        _Qry &= vbCrLf & " SELECT TOP 1  ISS.FTIssueNo AS DocIssNo,ISS.FTRemark AS FTRemark ,'' AS FTFacPurchaseNo"
        _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENBarcode_OUT As BO With (NOLOCK) INNER JOIN  "
        _Qry &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENIssue AS ISS WITH(NOLOCK) ON  BO.FTDocumentNo = ISS.FTIssueNo"
        _Qry &= vbCrLf & "  WHERE  BO.FTBarcodeNo = BI.FTBarcodeNo And BO.FTDocumentRefNo = BI.FTDocumentNo "

        _Qry &= vbCrLf & " UNION ALL "

        _Qry &= vbCrLf & " SELECT TOP 1  ISS.FTTransferWHNo AS DocIssNo,ISS.FTRemark AS FTRemark ,ISS.FTFacPurchaseNo AS FTFacPurchaseNo"
        _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENBarcode_OUT As BO With (NOLOCK) INNER JOIN  "
        _Qry &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENTransferWH AS ISS WITH(NOLOCK) ON  BO.FTDocumentNo = ISS.FTTransferWHNo"
        _Qry &= vbCrLf & "  WHERE  BO.FTBarcodeNo = BI.FTBarcodeNo And BO.FTDocumentRefNo = BI.FTDocumentNo "
        ...
        If FTReserveNo.Text <> "" Then
            _Qry &= vbCrLf & "AND R.FTReserveNo='" & HI.UL.ULF.rpQuoted(FTReserveNo.Text) & "'"
        End If
        ...
        If FNDropOption.SelectedIndex = 1 Then
            _Qry &= vbCrLf & " AND   ISNULL(BOUT.FNQuantity,0)>=0"
            _Qry &= vbCrLf & " AND    ISNULL(BOUT.FNQuantity,0)< (BI.FNQuantity)"
        End If

        If FNDropOption.SelectedIndex = 2 Then
            _Qry &= vbCrLf & " AND BI.FNQuantity=   ISNULL(BOUT.FNQuantity,0)"
        End If

        _Qry &= vbCrLf & "GROUP BY R.FTOrderNo,R.FTReserveNo,R.FDReserveDate,R.FTReserveBy,C.FTCmpCode,S.FTStyleCode,W.FTWHCode,IM.FTRawMatCode,C2.FTCmpCode"
        _Qry &= vbCrLf & ",BI.FTBarcodeNo"
        '_Qry &= vbCrLf & ",Bi.FNQuantity"
        If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
    ...
    Private Sub LoadListData()
        ...
        _Qry &= vbCrLf & "WHERE FTListName='FNDropOption'"
        dt = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_SYSTEM)
        FNDropOption.Properties.Items.Clear()
        FNDropOption.Properties.Items.Add("")
        For Each R As DataRow In dt.Rows
            FNDropOption.Properties.Items.Add(R!FTName.ToString)
        Next
        'FNDropOption.SelectedIndex = 1
    ...
    Private Function VerifyData() As Boolean
        ...
        If FNDropOption.SelectedIndex <> 0 Then
            _Pass = True
        End If
        ...
        If FTReserveNo.Properties.Tag = "" Then
            _Pass = False
        End If
