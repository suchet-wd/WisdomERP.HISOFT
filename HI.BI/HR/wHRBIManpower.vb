...
#Region "Procedure"
    Private Sub LoadData()
        ...
        Try
            _Qry = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.SP_BI_Manpower '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "','" & HI.UL.ULDate.ConvertEnDB(Me.FTYear.Text) & "','" & HI.UL.ULDate.ConvertEnDB(Me.FTYearTo.Text) & "'," & Me.FNBIManpowerState.SelectedIndex & "," & Integer.Parse(Val(HI.ST.SysInfo.CmpID)) & " "
            HI.Conn.SQLConn.ExecuteNonQuery(_Qry, Conn.DB.DataBaseName.DB_HR)

            _Qry = "  SELECT   A.FTYear, A.FNMonth,  A.FTDate, A.FTEmp, A.FTEmpNew, A.FTEmpResign, A.FTEmpTotal, A.FTEmpF, A.FTEmpM "
            ...
            _Qry &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.TBITManpower AS A WITH(NOLOCK) INNER JOIN"
            ...
            _Qry = "  DELETE FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.TBITManpower"
