...
Public Class wAbsentDashboard_Old
    ...
    Private Sub wAbsentDashboard_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        Call LoadAbbsentDashBoard()
        Call Loaddatadashboard(1, 1, 0, False)

        Me.ogvdetailstock.OptionsView.ShowAutoFilterRow = False

        For Each GridCol As DevExpress.XtraGrid.Columns.GridColumn In ogvdetailstock.Columns
    ...
    Private Sub LoadAbbsentDashBoard()
        Dim _Cmd As String = ""
        Dim _dt As DataTable
        Dim _spls As New HI.TL.SplashScreen("Loading...data Please wait.")

        _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_GET_DASHBOARD_PROD_ABSENTEEISM"
        ' 
        ...
        For Each R As DataRow In _dt.Rows
            ...
            Me.olbh1.Text = Format(Val(R!FNAbsentAllPer.ToString), "0.00") & "%"
            Me.olbhs1.Text = Format(Val(R!FNAbsentCutPer.ToString), "0.00") & "%"
            Me.olbhs2.Text = Format(Val(R!FNAbsentHeatPer.ToString), "0.00") & "%"
            Me.olbhs3.Text = Format(Val(R!FNAbsentLaserPer.ToString), "0.00") & "%"
            Me.olbhs4.Text = Format(Val(R!FNAbsentEmbroidPer.ToString), "0.00") & "%"
            Me.olbhs5.Text = Format(Val(R!FNAbsentSewPer.ToString), "0.00") & "%"
            Me.olbhs6.Text = Format(Val(R!FNAbsentQCPer.ToString), "0.00") & "%"
            Me.olbhs7.Text = Format(Val(R!FNAbsentMechanicPer.ToString), "0.00") & "%"

        Next

        _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_GET_DASHBOARD_WH_FABRIC " & HI.ST.SysInfo.CmpID & ""
        ' 
        _dt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_PROD)

        Dim _TotalWHStock As Double = 0
        Dim _TotalWHActual As Double = 0

        For Each R As DataRow In _dt.Rows
            _TotalWHStock = _TotalWHStock + Val(R!FNFabricMaximum.ToString)
            _TotalWHActual = _TotalWHActual + Val(R!FNQuantity.ToString)
        Next

        If _TotalWHStock > 0 Then
            olbh2.Text = Format(Val((_TotalWHActual / _TotalWHStock) * 100), "0.00") & "%"
        End If


        _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_GET_DASHBOARD_WH_FINISHGOOD " & HI.ST.SysInfo.CmpID & ""
        '
        _dt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_PROD)

        Dim _TotalWHFGStock As Double = 0
        Dim _TotalWHFGActual As Double = 0

        For Each R As DataRow In _dt.Rows
            _TotalWHFGStock = _TotalWHFGStock + Val(R!FNCartonMaximum.ToString)
            _TotalWHFGActual = _TotalWHFGActual + Val(R!FNQty.ToString)
        Next

        If _TotalWHFGStock > 0 Then
            olbh3.Text = Format(Val((_TotalWHFGActual / _TotalWHFGStock) * 100), "0.00") & "%"
        End If

        _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_GET_DASHBOARD_WH_FABRIC_DETAIL " & HI.ST.SysInfo.CmpID & ""
        ' 
        _dt = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_PROD)

        Me.ogcdetailstock.DataSource = _dt.Copy
    ...
    Private Sub Loaddatadashboard(PerformanceCharttype As Integer, DefectAndDowntimetype As Integer, DefectType As Integer, Optional StateFormLoad As Boolean = False, Optional dataselect As DataTable = Nothing)
        ...
        Dim _StateFixLoad As Boolean = True
        dtdashboardName.Columns.Add("Name", GetType(String))

        If ocmnew.Enabled Or btnEditDashboard.Enabled Then
            _StateFixLoad = False
    ...
    Private Sub otmcheck_Tick(sender As Object, e As EventArgs) Handles otmcheck.Tick
        'otmcheck.Enabled = False
        'Call LoadAbbsentDashBoard()
        'otmcheck.Enabled = True
    End Sub

    Private Sub olbh2_Click(sender As Object, e As EventArgs) Handles olbh2.Click
        If Me.opndetailstock.Visible = True Then
            Me.opndetailstock.Visible = False
        Else
            Me.opndetailstock.Visible = True
        End If
    End Sub
