...
#Region "MAIN PROC"
    ...
    Private Sub wOrderListingInfo_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            For Each GridCol As DevExpress.XtraGrid.Columns.GridColumn In ogvdata.Columns


                GridCol.OptionsColumn.AllowShowHide = False
                GridCol.OptionsColumn.ShowInCustomizationForm = False

                Select Case GridCol.FieldName
                    Case "FNPrice", "FNFabricAmount", "FNFabricRcvAmount", "FNAfterCutAmt", "FNAfterSewAmt", "FNAfterPackAmt"
                        GridCol.Visible = (ocmshowamount.Enabled)
                    Case Else
    ...
    Private Sub LoadOrderListingInfo(ByVal _FNHSysCmpId As String, ByVal _FTPORef As String, ByVal _FNHSysStyleId As String, ByVal _FNHSysCustId As String, ByVal _FNHSysBuyId As String, ByVal _FNHSysSeasonId As String)
        ...
        Try
            ...
            Qry = "exec[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "]..[SP_FINANCIAL_DATA_INFO] " & Val(_FNHSysCmpId.ToString) & " "
            ...
            Qry &= vbCrLf & ",'" & HI.UL.ULDate.ConvertEnDB(FTStartShipment.Text) & "','" & HI.UL.ULDate.ConvertEnDB(FTEndShipment.Text) & "'"
    ...
    Private Function PROC_VALIDATEbSHOWBROWSEDATA() As Boolean
        ...
        If Me.FNHSysCmpId.Text = "" And Me.FNHSysPOID.Text = "" And FNHSysStyleId.Text = "" And FNHSysCustId.Text = "" And FNHSysBuyId.Text = "" And FNHSysSeasonId.Text = "" And (FTStartShipment.Text = "" Or FTEndShipment.Text = "") Then
            HI.MG.ShowMsg.mInfo("กรุณาทำการเลือกเงื่อนไข อย่างน้อย 1 รายการ !!!", 1406170001, Me.Text, , System.Windows.Forms.MessageBoxIcon.Warning)
            Return False
        Else
            Return True
