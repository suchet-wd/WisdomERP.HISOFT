...
#Region "Procedure"
    ...
    Public Sub DefaultsData()
        ...
        Call loaddetail(Me.FTRcvMiniFgNo.Text)
    End Sub
                    ...
                    If (_StateNew) Then
                        ...
                        If Not (_FoundControl) Then
                            Select Case UCase(_FieldName)
                                Case UCase("FDUpdDate"), UCase("FTUpdTime"), UCase("FDInsDate"), UCase("FTInsDate"), UCase("FTInsTime"), UCase("FTInsUser"), UCase("FTUpdUser")
                                    _FoundControl = True
                            End Select
                        End If
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKRcvMiniFG WHERE FTRcvMiniFgNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKRcvMiniFG_Detail WHERE FTRcvMiniFgNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "'"
            ...
            'HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENTransferWH WHERE FTTransferWHNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "'")
...
#Region "MAIN PROC"
    ...
    Private Function CheckOwner() As Boolean
        If (HI.ST.UserInfo.UserName.ToUpper = FTRcvMiniFgBy.Text.ToUpper) Or (HI.ST.SysInfo.Admin) Then
            Return True
        Else
            Dim _Qry As String = ""
            Dim _Qry2 As String = ""
            ...
            _Qry &= vbCrLf & "   WHERE  FTUserName = '" & HI.UL.ULF.rpQuoted(FTRcvMiniFgBy.Text) & "' "
    ...
    Private Sub Proc_Save(sender As System.Object, e As System.EventArgs) Handles ocmsave.Click
        ...
        _Strsql &= vbCrLf & "  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKRcvMiniFG "
        _Strsql &= vbCrLf & " WHERE    FTRcvMiniFgNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "' "
        ...
        With DirectCast(Me.ogcdetail.DataSource, DataTable)
            ...
            .AcceptChanges()
            If .Select("FTColorWay IS NULL OR FTColorWay = ''").Length > 0 Then
                If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                    MessageBox.Show("กรุณาเลือกสีผลิตภัณฑ์. ", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Else
                    MessageBox.Show("Please select Colorway.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                End If

                Exit Sub
            End If

            If .Select("FTSizeBreakDown IS NULL OR FTSizeBreakDown = ''").Length > 0 Then
                If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                    MessageBox.Show("กรุณาเลือกไซส์.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Else
                    MessageBox.Show("Please select SizeBreakDown.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                End If
                Exit Sub
            End If


            If .Select("FTGradeCode IS NULL OR FTGradeCode = ''").Length > 0 Then
                If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                    MessageBox.Show("กรุณาเลือกประเภทเกรด.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Else
                    MessageBox.Show("Please select a GradeCode.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                End If
                Exit Sub
            End If
    ...
    Private Sub Proc_Delete(sender As System.Object, e As System.EventArgs) Handles ocmdelete.Click
        ...
        If HI.MG.ShowMsg.mConfirmProcessDefaultNo(MG.ShowMsg.ProcessType.mDelete, Me.FTRcvMiniFgNo.Text, Me.Text) = False Then
            Exit Sub
        End If
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        ...
        Dim _Fm As String = ""
        _Fm = "{TPACKRcvMiniFG.FTRcvMiniFgNo}='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "' "
        If Me.FTRcvMiniFgNo.Text <> "" Then
            With New HI.RP.Report
                .FormTitle = Me.Text
                .ReportFolderName = "Production\"
                .ReportName = "MiniFGReceive.rpt"
                .Formular = _Fm
                .Preview()
        End With
            ...
            '    .Formular = "{TINVENTransferWH.FTTransferWHNo}='" & HI.UL.ULF.rpQuoted(FTRcvMiniFgNo.Text) & "' "
            '    .Preview()
            'End With
...
#End Region
    Private Function DeleteBarcode(BarcodeKey As String, FTOrderNoKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTRcvMiniFgNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' "
            ...
            _Str &= vbCrLf & "WHERE  A.FTTransferWHNo='" & HI.UL.ULF.rpQuoted(FTRcvMiniFgNo.Text) & "' AND FTDocumentState='1'"
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
            ...
            HI.Auditor.CreateLog.CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "].dbo.TINVENBarcode_OUT  WHERE FTDocumentNo='" & HI.UL.ULF.rpQuoted(FTRcvMiniFgNo.Text) & "' AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_FTOrderNo) & "' ")
            Return True
        Catch ex As Exception
...
#End Region
    ...
    Private Sub addrow()
        Try
            If Me.ogcdetail.DataSource Is Nothing Then
                loaddetail(Me.FTRcvMiniFgNo.Text)
            End If
            With DirectCast(Me.ogcdetail.DataSource, DataTable)
                .AcceptChanges()
                If .Select("FNCartonNo = 0").Length <= 0 Then
                    Dim _cmp As String = "" & HI.ST.SysInfo.CmpID.ToString & ""

                    .Rows.Add(.Rows.Count + 1, 0, "", "", "", 0, 0, "", "", 0, "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", _cmp, _cmp)
                End If
    ...
    Private Sub remrow()
        Try
            ...
            If Not SaveDataDetail(Me.FTRcvMiniFgNo.Text) Then
    ...
    Private Sub loaddetail(key As String)
        Try
            Dim _Cmd As String = ""
            '_Cmd = "SELECT  FNSeq, FNCartonNo, FTOrderNo, FTSubOrderNo, FTPORef, D.FNHSysStyleId as FNHSysStyleId_Hide, D.FNHSysSeaSonId as FNHSysSeaSonId_Hide,  FTColorWay, FTSizeBreakDown, FNQuantity , FTGradeCode , FTContinentCode , FTCountryCode , FTProvinceType"
            '_Cmd &= vbCrLf & " , S.FTStyleCode as FNHSysStyleId  , e.FTSeasonCode as FNHSysSeaSonId  , FTOrderNo as FTOrderNo_Hide , FTSubOrderNo as FTSubOrderNo_Hide , FTPORef as FTPORef_Hide  ,FTColorWay as FTColorWay_Hide, FTSizeBreakDown  as FTSizeBreakDown_Hide, FTPOLine , FTPOLine AS FTPOLine_Hide , FTGradeCode AS FTGradeCode_Hide , FTContinentCode AS FTContinentCode_Hide , FTCountryCode AS FTCountryCode_Hide , FTProvinceType AS FTProvinceType_Hide , FNHSysCmpId , FNHSysCmpId AS FNHSysCmpId_Hide "
            ...
            _Cmd = " Select  D.FNSeq, FNCartonNo, D.FTOrderNo, D.FTSubOrderNo, D.FTPORef, D.FNHSysStyleId As FNHSysStyleId_Hide, "
            _Cmd &= vbCrLf & "D.FNHSysSeaSonId as FNHSysSeaSonId_Hide, D.FTColorWay, D.FTSizeBreakDown, D.FNQuantity, FTGradeCode, C.FTContinentCode, N.FTCountryCode"
            _Cmd &= vbCrLf & ", S.FTStyleCode as FNHSysStyleId  , e.FTSeasonCode as FNHSysSeaSonId  , D.FTOrderNo as FTOrderNo_Hide , "
            _Cmd &= vbCrLf & "D.FTSubOrderNo as FTSubOrderNo_Hide , D.FTPORef As FTPORef_Hide  , D.FTColorWay As FTColorWay_Hide,"
            _Cmd &= vbCrLf & "D.FTSizeBreakDown  as FTSizeBreakDown_Hide, FTPOLine, FTPOLine As FTPOLine_Hide , FTGradeCode AS FTGradeCode_Hide "
            _Cmd &= vbCrLf & ", C.FTContinentCode AS FTContinentCode_Hide , N.FTCountryCode AS FTCountryCode_Hide "
            If HI.ST.Lang.Language = ST.Lang.eLang.TH Then
                _Cmd &= vbCrLf & ", P.FTProvinceNameTH as FTProvinceType  "
            Else
                _Cmd &= vbCrLf & ",P.FTProvinceNameEN as FTProvinceType "
            End If
            _Cmd &= vbCrLf & ",P.FTProvinceCode AS FTProvinceType_Hide "
            _Cmd &= vbCrLf & ", FNHSysCmpId, FNHSysCmpId AS FNHSysCmpId_Hide "
            _Cmd &= vbCrLf & " From HITECH_PRODUCTION.. TPACKRcvMiniFG_Detail D with(nolock)  LEFT Join"
            ...
            _Cmd &= vbCrLf & " Left Join HITECH_MASTER .dbo.TMERMSeason e on D.FNHSysSeaSonId = e.FNHSysSeasonId"
            _Cmd &= vbCrLf & " where  D.FTRcvMiniFgNo ='" & HI.UL.ULF.rpQuoted(key) & "'"

            Me.ogcdetail.DataSource = HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_PROD)

            'Dim _Cmd As String = ""
            '_Cmd = "SELECT   FNSeq, FNCartonNo, FTOrderNo, FTSubOrderNo, FTPORef, D.FNHSysStyleId as FNHSysStyleId_Hide, D.FNHSysSeaSonId as FNHSysSeaSonId_Hide,  FTColorWay, FTSizeBreakDown, FNQuantity , FTGradeCode , FTContinentCode , FTCountryCode , FTProvinceType"
            '_Cmd &= vbCrLf & " , S.FTStyleCode as FNHSysStyleId  , e.FTSeasonCode as FNHSysSeaSonId  , FTOrderNo as FTOrderNo_Hide , FTSubOrderNo as FTSubOrderNo_Hide , FTPORef as FTPORef_Hide  ,FTColorWay as FTColorWay_Hide, FTSizeBreakDown  as FTSizeBreakDown_Hide, FTPOLine , FTPOLine AS FTPOLine_Hide , FTGradeCode AS FTGradeCode_Hide , FTContinentCode AS FTContinentCode_Hide , FTCountryCode AS FTCountryCode_Hide , FTProvinceType AS FTProvinceType_Hide ,"
            '_Cmd &= vbCrLf & " from  " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & ".. TPACKRcvMiniFG_Detail D with(nolock) "
            '_Cmd &= vbCrLf & " LEFT JOIN " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & " .dbo.TMERMStyle S on D.FNHSysStyleId = S.FNHSysStyleId "
            '_Cmd &= vbCrLf & " LEFT JOIN " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & " .dbo.TMERMSeason e on d.FNHSysSeaSonId = e.FNHSysSeasonId"
            '_Cmd &= vbCrLf & " LEFT JOIN " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & " .dbo.TCNMCmp C on D.FNHSysCmpId = C.FNHSysCmpId"
            '_Cmd &= vbCrLf & " where  D.FTRcvMiniFgNo ='" & HI.UL.ULF.rpQuoted(key) & "'"
    ...
    Private Function SaveDataDetail(key As String) As Boolean
        Try
            Dim _TableName As String = "  " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & ".dbo.TPACKRcvMiniFG_Detail"
            Dim _Cmd As String = "" : Dim _oDt As System.Data.DataTable
            Dim _CmdAll As String = ""
            Dim _StrFileH As String = "FTRcvMiniFgNo|FNSeq|FNCartonNo|FTOrderNo|FTSubOrderNo|FTPORef|FNHSysStyleId|FNHSysSeaSonId|FTColorWay|FTSizeBreakDown|FNQuantity|FTPOLine|FTGradeCode|FNHSysCmpId"
            Dim _CmdIns As String = "" : Dim _CmdUpd As String = "" : Dim _Value As String = "" : Dim _Where As String = "" : Dim _ValueUpd As String = ""
            Dim _PKey As String = "FTRcvMiniFgNo"
            ...
            _CmdIns &= vbCrLf & "  (FTInsUser, FDInsDate, FTInsTime, FTRcvMiniFgNo, FNSeq, FNCartonNo, FTOrderNo, FTSubOrderNo, FTPORef, FNHSysStyleId, FNHSysSeaSonId, FTColorWay, FTSizeBreakDown, FNQuantity , FTPOLine, FTGradeCode , FNHSysCmpId)"
            ...
            For Each R As DataRow In _oDt.Select("FTOrderNo<>''")
                _Value = ""
                _ValueUpd = "" : _Where = ""
        ...
        Try
            If Me.FTRcvMiniFgNo.Text <> "" Then
                ...
                If Me.FTStateApp.Checked = False Then
                    ...
                    _Cmd &= vbCrLf & "  from " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & ".dbo.TPACKRcvMiniFG a "
                    _Cmd &= vbCrLf & " where  FTRcvMiniFgNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "'"
    ...
    Private Sub ocmreject_Click(sender As Object, e As EventArgs) Handles ocmreject.Click
        Try
            Dim _Strsql As String = ""
            _Strsql = "SELECT TOP 1  FTRcvMiniFgNo "
            _Strsql &= vbCrLf & "  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKIssMiniFG_Detail "
            _Strsql &= vbCrLf & " WHERE    FTRcvMiniFgNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvMiniFgNo.Text) & "' "

            If Not String.IsNullOrEmpty(HI.Conn.SQLConn.GetField(_Strsql, Conn.DB.DataBaseName.DB_PROD, "")) Then
                HI.MG.ShowMsg.mInvalidData("เอกสารมีการ อนุมัติ รับเข้า แล้วไม่สามารถทำการลบ หรือแก้ไขได้ !!!", 1312220002, Me.Text)
                Exit Sub
            End If
