...
Public Class wCMOrderDistination
...
#End Region
    ...
    Private Function ValidateData() As Boolean
        ...
        If Me.FNHSysCmpIdTo.Text = "" Then
            HI.MG.ShowMsg.mInfo("ไม่พบข้อมูลโรงงานผลิต !!!", 2301311149, Me.Text,, MessageBoxIcon.Warning)
            Return False
        End If

        If (Me.FTCutting.Checked = False And Me.FTSew.Checked = False And Me.FTPack.Checked = False) Then
            HI.MG.ShowMsg.mInfo("ไม่พบข้อมูลขั้นตอนผลิต !!!", 2301311150, Me.Text,, MessageBoxIcon.Warning)
            Return False
        End If
    ...
    Private Sub ocmok_Click(sender As Object, e As EventArgs) Handles ocmok.Click
        ...
        If PROC_GETbValidateSubOrderNoInfo() = True Then
            ...
            If ValidateData() Then
                ...
                If HI.MG.ShowMsg.mConfirmProcess("คุณต้องการแบ่งยอดผลิต ใช่หรือไม่ ..", 2301311147, Me.Text) Then
    ...
    Private Function SaveDataCM() As Boolean
        Try
            Dim _Cmd As String = ""
            Dim _CmSeq As Integer = 0
            Dim _odt As DataTable
            With DirectCast(Me.ogdDivertDT.DataSource, DataTable)
                .AcceptChanges()
                _odt = .Copy
            ...
            _Cmd = "Select  max (FNCMSeq) as FNCMSeq  from   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].. TMERTOrderSub_CM_BreakDown   where    FTSubOrderNo = N'" & HI.UL.ULF.rpQuoted(Me.FTSubOrderNoSrc.Text) & "'"
            _CmSeq = HI.Conn.SQLConn.GetField(_Cmd, Conn.DB.DataBaseName.DB_MERCHAN, 0)
            _CmSeq += +1
                                ...
                                Try
                                    ...
                                    If Not DBNull.Value.Equals("" & Me.ogvDivertDT.GetRowCellValue(nLoopDivertDT, "FNAmnt" & tTextMatSizeTag).ToString) Then
                                        ...
                                        If numFNQuantityDivert > 0 Then
                                            ...
                                            tTextDivertFTMatSizeCode = Me.ogvDivertDT.Columns("FNAmnt" & tTextMatSizeTag).Caption.Trim()
                                            ...
                                            _Qry &= vbCrLf & "   INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "].[dbo].[TMERTOrderSub_CM_BreakDown] ([FTInsUser],[FDInsDate],[FTInsTime]"
                                            _Qry &= vbCrLf & ",FNCMSeq"
                                            ...
                                            _Qry &= vbCrLf & ",[FTColorwayNew] , FNHSysCmpIdTo, FNCMOperCutting, FNCMOperSew, FNCMOperPack   )"
                                            ...
                                            _Qry &= vbCrLf & "'" & HI.UL.ULF.rpQuoted(Me.FTSubOrderNoSrc.Text) & "',"
                                            ...
                                            _Qry &= vbCrLf & ", " & Val(Me.FNHSysCmpIdTo.Properties.Tag.ToString)
                                            _Qry &= vbCrLf & "," & IIf(Me.FTCutting.Checked, 1, 0) & "," & IIf(Me.FTSew.Checked, 1, 0) & "," & IIf(Me.FTPack.Checked, 1, 0)
            ...
            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) > 0 Then
                ...
                _Qry &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_CM_BreakDown  AS M INNER JOIN "
                ...
                _Qry &= vbCrLf & "  AND B.FTSubOrderNo='" & HI.UL.ULF.rpQuoted(Me.FTSubOrderNoSrc.Text) & "'"
                ...
                _Qry &= vbCrLf & "  AND M.FTSubOrderNo='" & HI.UL.ULF.rpQuoted(Me.FTSubOrderNoSrc.Text) & "'"
                _Qry &= vbCrLf & "  AND M.FNCMSeq=" & _CmSeq & ""
    ...
    Private Sub LoadDataDivertSeq(paramFTOrderNo As String, paramFTSubOrderNo As String)
        ...
        sSQL &= Environment.NewLine & " WHERE A.FTOrderNo = N'" & HI.UL.ULF.rpQuoted(paramFTOrderNo) & "'"
        sSQL &= Environment.NewLine & "      AND A.FTSubOrderNo = N'" & HI.UL.ULF.rpQuoted(paramFTSubOrderNo) & "'"
        sSQL &= Environment.NewLine & "      AND A.FNCMSeq =" & Val(SubOrderDivertSeq) & ""
        ...
        sSQL &= Environment.NewLine & " FROM [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_CM_BreakDown AS A (NOLOCK)"
