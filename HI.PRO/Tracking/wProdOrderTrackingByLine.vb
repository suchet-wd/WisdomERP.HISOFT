...
Public Class wProdOrderTrackingByLine
        ...
        Try
            ...
            With ogvdetailcolorsizelineg
                Select Case CType(e.Item, DevExpress.XtraGrid.GridSummaryItem).FieldName.ToString
                    ...
                    Case "FNCutQuantity", "FNSendSuplQuantity", "FNRcvSuplQuantity", "FNSPMKQuantity", "FNSPMKQuantityBal", "FNBalSuplQuantity", "FNCutBalQuantity"
...
#Region "Procedure"
    ...
    Private Sub LoadData()
        ...
        Try
            Call LoaddataDetailColorSizeByLineNew()
        Catch ex As Exception
        End Try
        ...
        Try
            ...
            Select Case True
                ...
                Case (FTSSendSuplDate.Text <> "" Or FTESendSuplDate.Text <> "")
                    ...
                    _Qry &= vbCrLf & " Left OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.V_TPRODTBundle_MainBarcode AS B WITH(NOLOCK) ON SC.FTBarcodeBundleNo = B.FTBarcodeBundleNo"
                    _Qry &= vbCrLf & " INNER Join  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd As P With(NOLOCK) On B.FTOrderProdNo = P.FTOrderProdNo"
                    _Qry &= vbCrLf & "  WHERE P.FTOrderNo <>'' "
            ...
            _Qry &= vbCrLf & "               [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle_Detail AS BDXXX WITH(NOLOCK)  ON BXXX.FTBarcodeBundleNo = BDXXX.FTBarcodeBundleNo"
            ...
            _Qry &= vbCrLf & "  SELECT SC.FTOrderNo,B.FTColorway,B.FTSizeBreakDown, SUM(SC.FNScanQuantity) AS FNQuantity , B.FTPOLineItemNo , SC.FTBarcodeNo"
            ...
            _Qry &= vbCrLf & "   LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitSect AS US WITH(NOLOCK) ON SC.FNHSysUnitSectId = US.FNHSysUnitSectId "
            _Qry &= vbCrLf & "    INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.V_TPRODTBundle_MainBarcode AS B WITH(NOLOCK) ON SC.FTBarcodeNo = B.FTBarcodeBundleNo " ' and SC.FTColorway = B.FTColorway and SC.FTSizeBreakDown = B.FTSizeBreakDown"

            _Qry &= vbCrLf & "     INNER JOIN #TabDataBD As X ON X.FTOrderNo = SC.FTOrderNo AND B.FTColorway=SC.FTColorway AND B.FTSizeBreakDown=SC.FTSizeBreakDown      "
            ...
            _Qry &= vbCrLf & "   GROUP BY SC.FTOrderNo,B.FTColorway,B.FTSizeBreakDown , B.FTPOLineItemNo , SC.FTBarcodeNo  "
    ...
    Private Sub LoaddataDetailColorSizeByLineNew()
        Dim _Qry As String
        Dim _dt As DataTable
        ogcdetailcolorsizelineg.DataSource = Nothing
        Try

            _Qry = "  DECLARE @TabOrder AS TABLE ([FTOrderNo] [nvarchar](30) NULL  Unique nonclustered (FTOrderNo)  ) "


            _Qry &= vbCrLf & " DECLARE @TabDataBD AS TABLE([FTOrderNo] [nvarchar](30) NULL,		"
            _Qry &= vbCrLf & "  [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "  [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "  [FNScanQuantity] [Int] NULL,  "
            _Qry &= vbCrLf & "  [FTPOLineItemNo] [nvarchar](30) NULL,"
            ...
            _Qry &= vbCrLf & " DECLARE @TabData AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNScanQuantity] [Int] NULL,               "
            _Qry &= vbCrLf & "  [FTPOLineItemNo] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & " [FTBarcodeNo] [nvarchar](30) NULL"
            _Qry &= vbCrLf & "   Unique nonclustered ([FTOrderNo],[FTUnitSectCode],[FTColorway],[FTSizeBreakDown],[FTPOLineItemNo],FTBarcodeNo)   "
            _Qry &= vbCrLf & "   ) "

            _Qry &= vbCrLf & " DECLARE @TabDataLC AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNQuantity] [Int] NULL,               "
            _Qry &= vbCrLf & "  [FTPOLineItemNo] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & " [FTBarcodeNo] [nvarchar](30) NULL"
            _Qry &= vbCrLf & "   Unique nonclustered ([FTOrderNo],[FTUnitSectCode],[FTColorway],[FTSizeBreakDown],[FTPOLineItemNo])   "
            _Qry &= vbCrLf & "   ) "

            _Qry &= vbCrLf & " DECLARE @TabDataCut AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNQuantity] [Int] NULL,               "
            ...
            _Qry &= vbCrLf & " DECLARE @TabDataSendSupl AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNQuantity] [Int] NULL,               "
            ...
            _Qry &= vbCrLf & " [FTBarcodeNo] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNSendSuplType] [Int] NULL  "
            _Qry &= vbCrLf & "  Unique nonclustered ([FTOrderNo],[FTUnitSectCode],[FTColorway],[FTSizeBreakDown],[FTPOLineItemNo],FNSendSuplType)   "
            _Qry &= vbCrLf & "   ) "

            _Qry &= vbCrLf & " DECLARE @TabDataRcvSupl AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNQuantity] [Int] NULL,               "
            ...
            _Qry &= vbCrLf & " DECLARE @TabDataSMK AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FNQuantity] [Int] NULL,               "
            ...
            _Qry &= vbCrLf & " DECLARE @TabDataSew AS TABLE([FTOrderNo] [nvarchar](30) NULL,				 "
            _Qry &= vbCrLf & "  [FTUnitSectCode] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "  [FTUnitSectCodeSew] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTColorway] [varchar](30) NULL,"
            _Qry &= vbCrLf & "   [FTSizeBreakDown] [varchar](30) NULL,"
            ...
            _Qry &= vbCrLf & "  [FTPOLineItemNo] [nvarchar](30) NULL,"
            _Qry &= vbCrLf & " [FTBarcodeNo] [nvarchar](30) NULL"
            _Qry &= vbCrLf & "   Unique nonclustered ([FTOrderNo],[FTUnitSectCode],[FTUnitSectCodeSew],[FTColorway],[FTSizeBreakDown],[FTPOLineItemNo],[FTBarcodeNo])   "
            _Qry &= vbCrLf & "   ) "
            ...
            Select Case True
                ...
                Case Else
                    ...
                    _Qry &= vbCrLf & " Select  FTOrderNo,'' FTUnitSectCode, SUX.FTColorway, FTSizeBreakDown ,0 AS  FNScanQuantity ,SUX.FTNikePOLineItem ,'' FTBarcodeNo    "

                    _Qry &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "]..V_OrderSub_BreakDown_ShipDestination As Z  "

                    _Qry &= vbCrLf & "  Outer Apply(select top 1 SUX. FTColorway , SUX.FTNikePOLineItem   "
                    _Qry &= vbCrLf & "  From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS SUX WITH(NOLOCK)"
                    _Qry &= vbCrLf & "Where SUX.FTOrderNo = Z.FTOrderNo"
                    _Qry &= vbCrLf & "And SUX.FTSubOrderNo =Z.FTSubOrderNo"
                    _Qry &= vbCrLf & "And SUX.FTColorway = z.FTColorwayOriginal "
                    ...
                    _Qry &= vbCrLf & "   GROUP BY  FTOrderNo, SUX.FTColorway, FTSizeBreakDown, SUX.FTNikePOLineItem "
            ...
            _Qry &= vbCrLf & "   SELECT P.FTOrderNo,US.FTUnitSectCode,B.FTColorway,B.FTSizeBreakDown ,SUM(BD.FNQuantity) AS FNQuantity,  B.FTPOLineItemNo,'',SS.FNSendSuplType  "
            _Qry &= vbCrLf & "   FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTSendSupl_Barcode AS A WITH(NOLOCK)   INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcode_SendSupl AS SS WITH(NOLOCK)   ON A.FTBarcodeSendSuplNo = SS.FTBarcodeSendSuplNo "

            '_Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.V_TPRODTBundle_MainBarcode AS B  WITH(NOLOCK)  ON SS.FTBarcodeBundleNo = B.FTBarcodeBundleNo INNER JOIN"

            _Qry &= vbCrLf & "       outer apply (select  BX1.FTBarcodeBundleNo,Sum(FNQuantity) AS FNQuantity,BX1.FTColorway,BX1.FTSizeBreakDown,BX1.FTPOLineItemNo "
            _Qry &= vbCrLf & "      		      from  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS BX1  WITH(NOLOCK) "
            _Qry &= vbCrLf & "      	           WHERE BX1.FTMainBarcodeBundleNo =  SS.FTBarcodeBundleNo"
            _Qry &= vbCrLf & "      	          GROUP BY BX1.FTBarcodeBundleNo,BX1.FTColorway,BX1.FTSizeBreakDown,BX1.FTPOLineItemNo"
            _Qry &= vbCrLf & "      	  ) AS B "

            _Qry &= vbCrLf & "          INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd AS P WITH(NOLOCK)   ON SS.FTOrderProdNo = P.FTOrderProdNo  INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle_Detail AS BD WITH(NOLOCK)  ON B.FTBarcodeBundleNo = BD.FTBarcodeBundleNo INNER JOIN"
            _Qry &= vbCrLf & "            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTLayCut AS LC WITH(NOLOCK)  ON BD.FTLayCutNo = LC.FTLayCutNo "
            _Qry &= vbCrLf & "         INNER JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd_MarkMain AS PM WITH(NOLOCK)  ON LC.FTOrderProdNo = PM.FTOrderProdNo AND LC.FNHSysMarkId = PM.FNHSysMarkId"
            ...
            _Qry &= vbCrLf & "       INNER JOIN @TabOrder AS Ozx ON   P.FTOrderNo = Ozx.FTOrderNo   "
            ...
            _Qry &= vbCrLf & "   	FROM     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail AS AXXX WITH(NOLOCK)  INNER JOIN"
            _Qry &= vbCrLf & "               [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitSect AS USXXX WITH(NOLOCK) ON AXXX.FNHSysUnitSectId = USXXX.FNHSysUnitSectId INNER JOIN"
            _Qry &= vbCrLf & "               [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS BXXX WITH(NOLOCK) ON AXXX.FTBarcodeNo = BXXX.FTBarcodeBundleNo  INNER JOIN "
            _Qry &= vbCrLf & "               [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle_Detail AS BDXXX WITH(NOLOCK)  ON BXXX.FTBarcodeBundleNo = BDXXX.FTBarcodeBundleNo"
            _Qry &= vbCrLf & "   WHERE   AXXX.FTBarcodeNo = A.FTBarcodeNo  AND  (USXXX.FTStateSew = '1')   AND (AXXX.FNHSysOperationId = 1405310010) "
            _Qry &= vbCrLf & "   ) As XXX "
            ...
            _Qry &= vbCrLf & "   SELECT SC.FTOrderNo,US.FTUnitSectCode,B.FTColorway,B.FTSizeBreakDown, SUM(SC.FNQuantity) AS FNScanQuantity , B.FTPOLineItemNo  , B.FTMainBarcodeBundleNo As FTBarcodeNo"
            '_Qry &= vbCrLf & "   FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail AS SC"
            '_Qry &= vbCrLf & "   LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitSect AS US WITH(NOLOCK) ON SC.FNHSysUnitSectId = US.FNHSysUnitSectId "
            ...
            _Qry &= vbCrLf & "   INNER JOIN @TabDataBD As X ON SC.FTOrderNo =X.FTOrderNo  AND B.FTColorway =X.FTColorway AND B.FTSizeBreakDown =X.FTSizeBreakDown    "
            _Qry &= vbCrLf & "   INNER JOIN @TabOrder AS Ozx ON   SC.FTOrderNo = Ozx.FTOrderNo "
            _Qry &= vbCrLf & "   WHERE SC.FTOrderNo <>''  "
            ...
            _Qry &= vbCrLf & "   GROUP BY SC.FTOrderNo,US.FTUnitSectCode,B.FTColorway,B.FTSizeBreakDown , B.FTPOLineItemNo , B.FTMainBarcodeBundleNo ) AS T "
            _Qry &= vbCrLf & "  GROUP BY FTOrderNo,FTUnitSectCode,FTColorway,FTSizeBreakDown , FTPOLineItemNo ,FTBarcodeNo "
        ...
        Try
            ...
            With Me.ogvdetailcolorsizelineg
                Select Case e.Column.FieldName
                    ...
                    Case "FNGarmentQtyTest", "FNCutBalQuantity", "FNBalCutQuantity", "FNSPMKQuantity", "FNSPMKQuantityBal"
                        If ("" & .GetRowCellValue(e.RowHandle1, "FTOrderNo").ToString = "" & .GetRowCellValue(e.RowHandle2, "FTOrderNo").ToString) _
