...
Public Class wThreadAutoGeneratePO

    Private _lstPo As wListThreadAutoPurchaseOrderNo
    Private _ListPurchaseOrder As New List(Of DataTable)
    Private _ListPurchaseOrderSupl As New List(Of DataTable)
    Private _wChangeUnitAndCurrency As wThreadAutoGeneratePOChangeUnit
    ...
    Sub New(Optional ListJobOrderNo As String = "")
        ...
        _lstPo = New wListThreadAutoPurchaseOrderNo
        ...
        _wChangeUnitAndCurrency = New wThreadAutoGeneratePOChangeUnit
...
#Region "Query"
    ...
    Private Function QueryJOb(ListJobOrderNo As String) As String
        Dim _Str As String = ""

        _Str = "  SELECT '1' AS FTStateSelect "
        _Str &= vbCrLf & " ,A.FTOrderNo"
        _Str &= vbCrLf & " ,C.FTStyleCode "
        _Str &= vbCrLf & " ,S.FTSeasonCode "
        _Str &= vbCrLf & "    FROM "
        _Str &= vbCrLf & "  (SELECT        FTOrderNo"
        _Str &= vbCrLf & "  FROM            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS S WITH(NOLOCK)"
        ...
        _Str &= vbCrLf & "  WHERE        (FTPurchaseNo = N'')   AND ISNULL(FNHSysUnitId,0) <> 0   "
        _Str &= vbCrLf & " AND FTOrderNo in ('" & ListJobOrderNo & "')"

        _Str &= vbCrLf & "  GROUP BY FTOrderNo"
        _Str &= vbCrLf & "  ) AS A INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS B  WITH(NOLOCK) ON A.FTOrderNo = B.FTOrderNo "
        _Str &= vbCrLf & "  LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTStyle AS C WITH(NOLOCK)"
        ...
        _Str &= vbCrLf & " WHERE  ISNULL(B.FNHSysStyleIdPull,0) <=0 "
        _Str &= vbCrLf & "  ORDER BY A.FTOrderNo "
        Return _Str

    End Function

    Private Function QuerySupplier(ListJobOrderNo As String) As String
        Dim _Str As String = ""
        ...
        _Str &= vbCrLf & "  INSERT INTO #TAB(FTOrderNo)	 SELECT FTOrderNo FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder WITH(NOLOCK) WHERE   ISNULL(FNHSysStyleIdPull,0) <=0    AND FTOrderNo in ('" & ListJobOrderNo & "') "
        ...
        _Str &= vbCrLf & " , A.FNHSysCurId AS FNHSysCurId_Hide"
        ...
        _Str &= vbCrLf & " ,0.00 AS FNDisCountPer"
        _Str &= vbCrLf & " ,ISNULL(("
        _Str &= vbCrLf & " SELECT      Count(DISTINCT S.FNHSysCurId) AS FNHSysCurId"
        _Str &= vbCrLf & "    FROM            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS S WITH (NOLOCK)"
        _Str &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON S.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON S.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "    WHERE        (S.FTPurchaseNo = N'') AND ISNULL(FNHSysUnitId,0) <> 0  AND S.FNHSysSuplId = A.FNHSysSuplId"
        ...
        _Str &= vbCrLf & "  FROM ("
        _Str &= vbCrLf & "   SELECT    S.FNHSysRawMatId,Count(DISTINCT FNHSysUnitId) AS FNHSysUnitIdPurchase"
        _Str &= vbCrLf & "  FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS S WITH (NOLOCK)"
        _Str &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON S.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON S.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "   WHERE  (FTPurchaseNo = N'')  AND ISNULL(FNHSysUnitId,0) <> 0    "
        ...
        _Str &= vbCrLf & "  FROM ("
        _Str &= vbCrLf & "   SELECT    S.FNHSysRawMatId,Count(DISTINCT FNPrice) AS FNPricePurchase"
        _Str &= vbCrLf & "   FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS S WITH (NOLOCK)"
        _Str &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON S.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON S.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "   WHERE  (FTPurchaseNo = N'')  AND ISNULL(FNHSysUnitId,0) <> 0      "
        ...
        _Str &= vbCrLf & "   LEFT JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TFINMCurrency AS CR WITH(NOLOCK) ON A.FNHSysCurId =CR.FNHSysCurId"
    ...
    Private Function QueryPOItem(ListJobOrderNo As String)
        ...
        _Str &= vbCrLf & "   SELECT  MPO.FNHSysSuplId"
        ...
        _Str &= vbCrLf & ",  MPO.FNHSysUnitId AS FNHSysUnitIdMRP"
        _Str &= vbCrLf & ", Convert(numeric(18,4),MPO.FNQuantity) AS FNQuantityMRP"
        ...
        _Str &= vbCrLf & "     FROM            (SELECT        M.FNHSysSuplId, SC.FNHSysRawMatId, SUM(SC.FNUsedQuantity +FNUsedPlusQuantity) AS FNQuantity, SC.FNHSysUnitId AS FNHSysUnitId, SC.FNPrice AS FNPrice, "
        ...
        _Str &= vbCrLf & "   INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON S.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON S.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "    WHERE        (FTPurchaseNo = N'')  AND ISNULL(FNHSysUnitId,0) <> 0     AND (S.FNHSysSuplId = A.FNHSysSuplId)"
        ...
        _Str &= vbCrLf & "   ((SELECT        MAX( FNHSysUnitIdPurchase) AS FNHSysUnitIdPurchase"
        _Str &= vbCrLf & "   FROM            (SELECT        FNHSysRawMatId, COUNT(DISTINCT FNHSysUnitId) AS FNHSysUnitIdPurchase"
        _Str &= vbCrLf & "   FROM            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS S WITH (NOLOCK)"
        _Str &= vbCrLf & "   INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON S.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON S.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "  WHERE        (FTPurchaseNo = N'')  AND ISNULL(FNHSysUnitId,0) <> 0   AND (S.FNHSysSuplId = A.FNHSysSuplId)"
        ...
        _Str &= vbCrLf & "   ((SELECT        Max( FNPricePurchase) AS FNPricePurchase"
        _Str &= vbCrLf & "   FROM            (SELECT        FNHSysRawMatId, COUNT(DISTINCT FNPrice) AS FNPricePurchase"
        _Str &= vbCrLf & "    FROM            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS S WITH (NOLOCK)"
        _Str &= vbCrLf & "   INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON S.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON S.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "   WHERE        (FTPurchaseNo = N'')  AND ISNULL(FNHSysUnitId,0) <> 0     AND (S.FNHSysSuplId = A.FNHSysSuplId)"
        ...
        _Str &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource AS SC WITH (NOLOCK) ON M.FNHSysSuplId = SC.FNHSysSuplId"
        ...
        _Str &= vbCrLf & "  INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON SC.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "   WHERE     (SC.FTPurchaseNo = N'')  "

        _Str &= vbCrLf & "   AND SC.FTOrderNo IN (SELECT  FTOrderNo FROM  #TAB  )"


        _Str &= vbCrLf & "   GROUP BY M.FNHSysSuplId, SC.FNHSysRawMatId, SC.FNHSysUnitId, SC.FNPrice) AS MPO) AS MPO"
    ...
    Private Function QueryOrderItem(ListJobOrderNo As String)
        ...
        _Str &= vbCrLf & ",FNHSysUnitIdPurchaseChk,FNPricePurchaseChk,FNHSysCurIdChk,CR.FTCurCode"
        ...
        _Str &= vbCrLf & "     FROM            (SELECT        SC.FTOrderNo,M.FNHSysSuplId, SC.FNHSysRawMatId, SUM(SC.FNUsedQuantity+ SC.FNUsedPlusQuantity) AS FNQuantity, SC.FNHSysUnitId AS FNHSysUnitId, SC.FNPrice AS FNPrice, "
        _Str &= vbCrLf & "        MAX(SC.FTFabricFrontSize) AS FTFabricFrontSize"
        _Str &= vbCrLf & " ,SC.FNHSysUnitId AS FNHSysUnitIdPurchaseChk"
        _Str &= vbCrLf & " ,SC.FNPrice AS FNPricePurchaseChk "
        ...
        _Str &= vbCrLf & "   INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMSupplier AS SU WITH (NOLOCK) ON SC.FNHSysSuplId = SU.FNHSysSuplId"
        _Str &= vbCrLf & "   INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder  AS AAA WITH(NOLOCK) ON SC.FTOrderNo=AAA.FTOrderNo "
        _Str &= vbCrLf & "   WHERE    ISNULL(FNHSysUnitId,0) <> 0   AND (SC.FTPurchaseNo = N'')    "
        ...
        _Str &= vbCrLf & "   AND SC.FTOrderNo IN (SELECT  FTOrderNo FROM  #TAB  )"
            ...
            _Str &= vbCrLf & "   GROUP BY SC.FTOrderNo,M.FNHSysSuplId, SC.FNHSysRawMatId, SC.FNHSysUnitId, SC.FNPrice ,SC.FNHSysCurId ) AS MPO) AS MPO"
        _Str &= vbCrLf & "   INNER Join"
        _Str &= vbCrLf & "    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMaterial AS IM WITH (NOLOCK) ON MPO.FNHSysRawMatId = IM.FNHSysRawMatId INNER JOIN"
        _Str &= vbCrLf & "    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnit AS U WITH(NOLOCK) ON MPO.FNHSysUnitId = U.FNHSysUnitId LEFT OUTER JOIN"
        _Str &= vbCrLf & "    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TINVENMMatColor AS C WITH (NOLOCK) ON IM.FNHSysRawMatColorId = C.FNHSysRawMatColorId LEFT OUTER JOIN"
...
#Region "Procedure"
    ...
    Public Sub PrepareDataGenerate(Optional ListJobOrderNo As String = "")
        ...
        Try
            _DtJob = HI.Conn.SQLConn.GetDataTable(Me.QueryJOb(ListJobOrderNo), Conn.DB.DataBaseName.DB_MERCHAN)
            ogclistorder.DataSource = _DtJob
        Catch ex As Exception
        End Try

        Try
            _DtSupl = HI.Conn.SQLConn.GetDataTable(Me.QuerySupplier(ListJobOrderNo), Conn.DB.DataBaseName.DB_MERCHAN)
        ...
        Try
            ...
            _DtItem = HI.Conn.SQLConn.GetDataTable(Me.QueryPOItem(ListJobOrderNo), Conn.DB.DataBaseName.DB_MERCHAN)
        ...
        _DtItemSub = HI.Conn.SQLConn.GetDataTable(Me.QueryOrderItem(ListJobOrderNo), Conn.DB.DataBaseName.DB_MERCHAN)
    ...
    Private Sub GeneratePOAuto(_SplashScreen As HI.TL.SplashScreen)
        ...
        For Each Row As DataRow In _DtSupl.Select("FTSelect='1' AND FTStateComplete='1'  AND FNStateCurP=1 AND FNStatePriceP=1 AND FNStateUnitP=1 ")
            ...
            _Complete = GenerateDataPO(_Supl, _PoDocNo, Val(Row!FNPoState_Hide.ToString), Row!FNHSysPurGrpId.ToString.Trim(), Val(Row!FNHSysPurGrpId_Hide.ToString),
                                        Row!FDDeliveryDate.ToString, Val(Row!FNLeadtime),
                                       Val(Row!FNHSysCurId_Hide.ToString), Val(Row!FNExchangeRate), Val(Row!FNDisCountPer),
    ...
    Private Function GenerateDataPO(SuplKey As String, ByRef PoNoKey As String, POTypeKey As Integer, POGrpText As String, POGrpKey As Integer, DeliDateKey As String,
        ...
        Dim _CmpH As String = HI.Conn.SQLConn.GetField("SELECT TOP 1 FTDocRun FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp WITH(NOLOCK) WHERE FNHSysCmpId=" & Val(HI.ST.SysInfo.CmpID) & " ", Conn.DB.DataBaseName.DB_SYSTEM, "")
        ...
        Select Case Me.GENAUTOPO.SelectedIndex
            Case 0 'Generate By Supplier All Item Code / PO
                ...
                _PoDocNo = HI.TL.Document.GetDocumentNo(HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR), "TPURTPurchase", "", False, _CmpH & FNHSysCmpRunId.Text & _Year & POGrpText & HI.TL.CboList.GetListRefer("FNPoState", POTypeKey) & _Month).ToString

                If _PoDocNo <> "" Then
                    _Complete = GeneratePO(SuplKey, _PoDocNo, POTypeKey, POGrpText, POGrpKey, DeliDateKey,
                                                                      LeadTimeKey, CurrCodeKey, Exchng, DiscountPer,
                                                                      Vat, SurCharge, Credit, CreditDay, PaymentTerm, ContactPerson, _MaxItem, _Filter, SuplCode, SuplName)
    ...
    Private Function GeneratePO(SuplKey As String, PoNoKey As String, POTypeKey As Integer, POGrpText As String, POGrpKey As Integer, DeliDateKey As String,
                                                               LeadTimeKey As Integer, CurrCodeKey As Integer, Exchng As Double, DiscountPer As Double,
                                                               Vat As Double, SurCharge As Double, Credit As Integer, CreditDay As Integer, PaymentTerm As Integer, ContactPerson As String,
                                                              _MaxItem As Integer, Filter As String, SuplCode As String, SuplName As String) As Boolean
                ...
                For Each StrFO As String In grp
                    ...
                    If StrFO <> "" Then
                        If StrFO.Length >= 10 Then
                            _OGACDate = HI.UL.ULDate.ConvertEN(StrFO)
                        Else
                            _OGACDate = Microsoft.VisualBasic.Right(StrFO, 2) + "/" + Microsoft.VisualBasic.Left(StrFO, 4)
                        End If
        ...
        Try
            ...
            _Str &= vbCrLf & " , FTStateColorBox,FTStateSendApp,FNPoType)"
            ...
            _Str &= vbCrLf & "," & Val(FNHSysDeliveryId.Properties.Tag.ToString) & ",'" & HI.UL.ULF.rpQuoted(ContactPerson) & "'"
            _Str &= vbCrLf & ",'','','','','' "
            _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(_Remark) & "'"
            _Str &= vbCrLf & "," & _Amt & "  "
            _Str &= vbCrLf & "," & DiscountPer & "," & _DisAmt & "," & (_Amt - _DisAmt) & "," & Vat & "," & _VatAmt & "," & SurCharge & "," & _NetAmt & ""
            _Str &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(_AmtTH) & "','" & HI.UL.ULF.rpQuoted(_AmtEN) & "','0','0','0','0','0','0',0 "
            ...
            For Each Row As DataRow In _DtItem.Select(Filter & "  AND FTSelect='1'  ", "FTRawMatCode,FTRawMatColorCode,FTRawMatSizeCode")
                ...
                For Each R As DataRow In _DtItemSub.Select(_Filter, "FTShipDate") 'DRaw
                    Application.DoEvents()
                    Count = Count + 1
                    _Seq = _Seq + 1
                    ...
                    _Str = " UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TMERTOrderThread_Resource SET "
...
#End Region
            ...
            With ogvdetail
                If Not (ogcdetail.DataSource Is Nothing) And ogvlistsupplier.RowCount > 0 Then
                    ...
                    With ogvdetail
                        ...
                        Dim _FNHSysUnitIdMRP As Integer = Integer.Parse(Val("" & .GetRowCellValue(.FocusedRowHandle, "FNHSysUnitIdMRP").ToString))
                        Dim _FNQuantityMRPP As Double = Double.Parse(Val("" & .GetRowCellValue(.FocusedRowHandle, "FNQuantityMRP").ToString))
                        Dim _FNConvertRaito As Double = 1.0
                        ...
                        If _FTUnitCode <> "" And _FNHSysUnitId > 0 Then
                            ...
                            With _wChangeUnitAndCurrency
                                ...
                                If .ProcOK = True Then
                                    ...
                                    If _FNHSysUnitIdMRP <> _FNHSysUnitIdNew Then
                                        Dim _Str As String
                                        ...
                                        _Str &= vbCrLf & "  WHERE FNHSysUnitId =" & Integer.Parse(_FNHSysUnitIdMRP) & " "
                                        _Str &= vbCrLf & "  AND FNHSysUnitIdTo =" & Integer.Parse(_FNHSysUnitIdNew) & " "

                                        _FNConvertRaito = Val(HI.Conn.SQLConn.GetFieldOnBeginTrans(_Str, Conn.DB.DataBaseName.DB_INVEN, "1"))
                                    End If
                                    ...
                                    With ogvdetail
                                        ...
                                        For Each R As DataRow In _DtItemSub.Select("FNHSysSuplId=" & _SysSuplID & " AND FNHSysRawMatId=" & _SysMatID & "")
                                            ...
                                            If _FNHSysUnitIdMRP <> _FNHSysUnitIdNew Then
                                                _FNQuantityMRPP = Val(R!FNQuantityMRPP.ToString)
                                                R!FNQuantity = CDbl(Format(_FNQuantityMRPP * _FNConvertRaito, "0.0000"))
                                            End If

                                            GQty = GQty + Val(R!FNQuantity.ToString)

                                            _Qry = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TMERTOrderThread_Resource "
                                            _Qry &= vbCrLf & " SET FNHSysUnitId =" & _FNHSysUnitIdNew & " "
                                            ...
                                            _Qry &= vbCrLf & " AND   FNHSysUnitId<>" & Integer.Parse(_FNHSysUnitIdNew) & ""
                                            _Qry &= vbCrLf & " UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PUR) & "].dbo.TMERTOrderThread_Resource "
    ...
    Private Sub ockselectall_CheckedChanged(sender As Object, e As EventArgs)
        Try
    ...
    Private Sub ockselectdelatilall_CheckedChanged(sender As Object, e As EventArgs)
        Try
                    ...
                    With ogvdetail
                        ...
                        If _FNPrice <> _FNPriceNew Or _DtItemSub.Select("FNHSysSuplId=" & _SysSuplID & " AND FNHSysRawMatId=" & _SysMatID & " AND FNPricePurchaseChk<>" & _FNPriceNew & "").Length > 0 Then
                            ...
                            For Each R As DataRow In _DtItemSub.Select("FNHSysSuplId=" & _SysSuplID & " AND FNHSysRawMatId=" & _SysMatID & "")
                                ...
                                _Qry &= vbCrLf & " SET FNPrice =" & _FNPrice & " "
                                _Qry &= vbCrLf & " WHERE FTOrderNo='" & HI.UL.ULF.rpQuoted(R!FTOrderNo.ToString) & "' "
                                _Qry &= vbCrLf & " AND FNHSysRawMatId=" & Integer.Parse(Val(_SysMatID)) & " "
                                _Qry &= vbCrLf & " AND   FNHSysSuplId=" & Integer.Parse(Val(_SysSuplID)) & ""
                                _Qry &= vbCrLf & " AND ISNULL(FTPurchaseNo,'')='' "
                                _Qry &= vbCrLf & " AND   FNPrice<>" & Double.Parse(_FNPrice) & ""
