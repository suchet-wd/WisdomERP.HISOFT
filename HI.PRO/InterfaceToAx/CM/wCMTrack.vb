...
#Region "Procedure"
    ...
    Private Sub LoadData(OrderKey As String, OrderSubKey As String)
        ...
        Qry &= vbCrLf & ",sum(S.FNQuantity)-isnull(DD.Di,0) As FNQuantityOriginal ,0 As QtyDivert , 0 As Bl,SB.FTSubOrderNoDivertRef,Case When SB.FTPORef<>'' then '' else O.FTPORef end as FTPORef,O.FTOrderNo,S.FTColorway,'' AS FTColorwayNew , o.FNHSysCmpId , 0 as FNHSysCmpIdFrom  "
        Qry &= vbCrLf & "from"
        ...
        Qry &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_CM_BreakDown AS S WITH(NOLOCK)"
        ...
        If Me.FTPORef.Text <> "" Then
            Qry &= vbCrLf & "and O.FTPORef='" & Me.FTPORef.Text & "'"
        End If
        If OrderKey <> "" Then
        ...
        End If
        Qry &= vbCrLf & "group by S.FTSubOrderNo,S.FTSizeBreakDown,SB.FTSubOrderNoDivertRef,O.FTPORef,O.FTOrderNo,S.FTColorway,SB.FTPORef,DD.Di, o.FNHSysCmpId) AS A"
        ...
        Qry &= vbCrLf & ",S.FTOrderNo,SB.FTColorway,'' As FTColorwayNew , o.FNHSysCmpId   , 0 as FNHSysCmpIdFrom   "
        ...
        If Me.FTPORef.Text <> "" Then
            Qry &= vbCrLf & "and S.FTPORef='" & Me.FTPORef.Text & "'"
        End If
        ...
        Qry &= vbCrLf & "group by S.FTSubOrderNo,SB.FTSizeBreakDown,S.FTSubOrderNoDivertRef,S.FTPORef,S.FTOrderNo,SB.FTColorway,O.FTPORef , o.FNHSysCmpId  )AS B"
        Qry &= vbCrLf & "where B.FTPORef<>''"
        'end modify
        ' modify by joker 2016/10/24 11.06
        Qry &= vbCrLf & "union"
        Qry &= vbCrLf & "select * from"
        Qry &= vbCrLf & "(select S.FTSubOrderNo+'-C' AS FTSubOrderNo,S.FTSizeBreakDown"
        Qry &= vbCrLf & ",0 AS FNQuantityOriginal  ,sum(S.FNQuantity) AS QtyDivert ,(A.Origin-B.Di) AS Bl,'' AS FTSubOrderNoDivertRef"
        Qry &= vbCrLf & ",case when isnull(D.FTPORef,'')='' then O.FTPORef else D.FTPORef end as FTPORef,S.FTOrderNo,S.FTColorway, ISNULL(S.FTColorwayNew,'') AS FTColorwayNew ,S.FNHSysCmpIdTo , O.FNHSysCmpId as FNHSysCmpIdFrom "
        Qry &= vbCrLf & "from "
        'end modify
        ...
        Qry &= vbCrLf & "from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub  AS D with(nolock)"
        Qry &= vbCrLf & "where D.FTOrderNo<>''"
        If Me.FTOrderNo.Text <> "" Then
            Qry &= vbCrLf & "and D.FTOrderNo='" & Me.FTOrderNo.Text & "'"
        End If
        If Me.FTPORef.Text <> "" Then
            Qry &= vbCrLf & "and D.FTPORef='" & Me.FTPORef.Text & "'"
        End If
        If OrderKey <> "" Then
        ...
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_CM_BreakDown AS S WITH(NOLOCK)ON AA.FTOrderNo=S.FTOrderNo inner join"
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub  AS D WITH(NOLOCK) ON S.FTOrderNo=D.FTOrderNo and S.FTSubOrderNo=D.FTSubOrderNo  inner join"
        ...
        Qry &= vbCrLf & ",sum(S.FNQuantity) AS Di"
        Qry &= vbCrLf & "from"
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_CM_BreakDown AS S WITH(NOLOCK)"
        ...
        Qry &= vbCrLf & "group by S.FTSubOrderNo,S.FTSizeBreakDown,a.Origin,b.Di,D.FTPORef,O.FTPORef,S.FTOrderNo,S.FTColorway, ISNULL(S.FTColorwayNew,'')  ,S.FNHSysCmpIdTo , O.FNHSysCmpId "
        ...
        End If
        If Me.FTPORef.Text <> "" Then
            Qry &= vbCrLf & "and X.FTPORef='" & Me.FTPORef.Text & "'"
        End If
        If OrderKey <> "" Then
        ...
        Qry &= vbCrLf & " Outer apply (select top 1 FTCmpCode from   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp c  where  "
        Qry &= vbCrLf & " c.FNHSysCmpId  = z.FNHSysCmpId  ) c   "


        Qry &= vbCrLf & " Outer apply (select top 1 FTCmpCode as FTCmpCodeFrom from   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp c  where  "
        Qry &= vbCrLf & " c.FNHSysCmpId  = z.FNHSysCmpIdFrom  ) cc   "
        ...
        Qry = "select CC.FTCmpCode as FTCmpCodeFrom  ,  C.FTCmpCode,Cust.FTCustCode,S.FTStyleCode,convert(varchar(10),convert(datetime,O.FDOrderDate),103) AS OrderDate,Bgrp.FTBuyGrpCode,Plnt.FTPlantCode"
        ...
        Qry &= vbCrLf & ",OS.FNHSysContinentId,OS.FNHSysCountryId,OS.FNHSysProvinceId,OS.FNHSysShipModeId,OS.FNHSysShipPortId,OS.FNHSysBuyGrpId,OS.FNHSysPlantId , O.FNHSysCmpID , 0 as FNHSysCmpIdFrom "
        Qry &= vbCrLf & "from"
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITh(NOLOCK) INNER JOIN"
        ...
        Qry &= vbCrLf & ",S.FNHSysContinentId,S.FNHSysCountryId,S.FNHSysProvinceId,S.FNHSysShipModeId,S.FNHSysShipPortId,S.FNHSysBuyGrpId,S.FNHSysPlantId , O.FNHSysCmpId , 0 as FNHSysCmpIdFrom   "
        Qry &= vbCrLf & "from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub aS S with(nolock) inner join"
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS SB with(NOLOCK) ON S.FTOrderNo=SB.FTOrderNo and S.FTSubOrderNo=SB.FTSubOrderNo"
        Qry &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH(NOLOCK) ON S.FTOrderNo=O.FTOrderNo"
        Qry &= vbCrLf & "where S.FTOrderNo<>''"
        ...
        Qry &= vbCrLf & ",OSDD.FTSubOrderNo+'-C'+convert(varchar(5),OSDD.FNCMSeq) AS FTSubOrderNoRef"
        ...
        Qry &= vbCrLf & ",SD.FNHSysContinentId,SD.FNHSysCountryId,SD.FNHSysProvinceId,SD.FNHSysShipModeId,SD.FNHSysShipPortId,SD.FNHSysBuyGrpId,SD.FNHSysPlantId , OSDD.FNHSysCmpIdTo   , o.FNHSysCmpId as FNHSysCmpIdFrom    "
        Qry &= vbCrLf & "from"
        Qry &= vbCrLf & "(select  O.FTOrderNo from"
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WItH(NOLOCK)"
        Qry &= vbCrLf & "where O.FTOrderNo<>''"
        ...
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_CM_BreakDown AS OSDD WITH(NOLOCK) ON OSD.FTOrderNo=OSDD.FTOrderNo and OSD.FTSubOrderNo=OSDD.FTSubOrderNo and OSD.FTColorway=OSDD.FTColorway"
        Qry &= vbCrLf & "and OSD.FTSizeBreakDown=OSDD.FTSizeBreakDown LEFT OUTER JOIN"
        Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub  AS SD WItH(NOLOCK) ON OSDD.FTOrderNo=Sd.FTOrderNo and OSDD.FTSubOrderNo=SD.FTSubOrderNo  "
        ...
        End If
        If Me.FTPORef.Text <> "" Then
            Qry &= vbCrLf & "and A.FTPOref='" & Me.FTPORef.Text & "'"
        End If
        If OrderKey <> "" Then
        ...
        Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp AS C with(nolock) ON T.FNHSysCmpId=C.FNHSysCmpId"
        Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMCmp AS CC with(nolock) ON T.FNHSysCmpIdFrom =CC.FNHSysCmpId"
...
#End Region
    ...
    Public Function Verify() As Boolean
        If Me.FTOrderNo.Text <> "" Or Me.FTPORef.Text <> "" Then
            Return True
        Else
            Return False
        End If
    ...
    Private Sub ogvDetail_RowStyle(sender As Object, e As RowStyleEventArgs) Handles ogvDetail.RowStyle
        ...
        If (e.RowHandle >= 0) Then
            Dim cmfacfrom As String = View.GetRowCellDisplayText(e.RowHandle, View.Columns("FTCmpCodeFrom"))
            If cmfacfrom = "" Then Exit Sub
            Dim cmfacto As String = View.GetRowCellDisplayText(e.RowHandle, View.Columns("FTCmpCode"))

            If cmfacfrom <> cmfacto Then
