...
Public Class wPurchaseSendSupl
...
#End Region
    ...
    Private Sub LoadDetail(ByVal _DocRefNo As String)
        Try
            ...
            _Qry &= vbCrLf & "  WHERE        (FTDocumentRef = A.FTSendSuplNo) AND (ISNULL(FTStateFromSupl, '0') = '1')), 0) ) -isnull( Dx.FNQuantity,0)  AS FNQuantity"
            ...
            _Qry &= vbCrLf & "	,Convert(numeric(18,2),(A.FNQuantity -  isnull( Dx.FNQuantity,0)) * ISNULL(D.FNPrice,0.0000))  AS FNAmount "

            _Qry &= vbCrLf & ", Convert(int, ((A.FNSendSupQty * A.FNDefectAcceptPer) /100)) as   FNDefectAcceptPer"
            _Qry &= vbCrLf & ",Tx.CPC  FNFarbicAmt"
            ...
            _Qry &= vbCrLf & "  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "]..TPROSendSuplDefect AS XQA WITH (NOLOCK)"
            _Qry &= vbCrLf & "WHERE        (FTDocumentRef = A.FTSendSuplNo) AND (ISNULL(FTStateFromSupl, '0') = '1')), 0) + A.FNDefectQtyFac ) -Convert(int, ((A.FNSendSupQty * A.FNDefectAcceptPer) /100))) * Tx.CPC) +"
            _Qry &= vbCrLf & " (((ISNULL ((SELECT        SUM(FNFacDefectQty) AS FNDefectQty"
            _Qry &= vbCrLf & "   FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "]..TPROSendSuplDefect AS XQA WITH (NOLOCK)"
            ...
            _Qry &= vbCrLf & ",(CASE WHEN A.FNDefectQty > Convert(int, ((A.FNSendSupQty * A.FNDefectAcceptPer) /100)) THEN Convert(numeric(18,2),(A.FNQuantity - Dx.FNQuantity) * ISNULL(D.FNPrice,0.0000) - "
            _Qry &= "(((A.FNDefectQty -Convert(int, ((A.FNSendSupQty * A.FNDefectAcceptPer) /100))) * Tx.CPC) + ((A.FNDefectQty - Convert(int, ((A.FNSendSupQty * A.FNDefectAcceptPer) /100)) ) * A.FNCutBaht))) Else Convert(numeric(18,2),A.FNQuantity * ISNULL(D.FNPrice,0.0000)) END  "
            ...
            _Qry &= vbCrLf & " , O.FNHSysStyleId  AS FNFarbicAmt"
            ...
            _Qry &= vbCrLf & "	          [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.V_TPRODTBundle_MainBarcode AS BD WITH(NOLOCK)   ON BSS.FTBarcodeBundleNo = BD.FTBarcodeBundleNo"
            ...
            _Qry &= vbCrLf & " outer apply (  Select TOP 1 Isnull(M.FNConSmp,0) * Isnull(C.FNPrice,0) AS CPC"
            _Qry &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTStyle_Mat AS M WITH(NOLOCK) LEFT OUTER JOIN "
            _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TMERMMainMat AS Az WITH(NOLOCK) ON M.FNHSysMerMatId = Az.FNHSysMainMatId"
            _Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TINVENMMaterial AS B WITH(NOLOCK) ON Az.FTMainMatCode = B.FTRawMatCode"
            _Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENBarcode AS C WITH(NOLOCK) ON B.FNHSysRawMatId = C.FNHSysRawMatId"
            _Qry &= vbCrLf & "where M.FTStateMainMaterial = '1'"
            _Qry &= vbCrLf & "and M.FNHSysStyleId = O.FNHSysStyleId   "
            '_Qry &= vbCrLf & "order by isnull(C.FNPrice ,0) desc "
            _Qry &= vbCrLf & "  ) as Tx"
            ...
            _Qry &= vbCrLf & "	and exists (select  * "
            _Qry &= vbCrLf & "	from   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSupl_Barcode rcv with(nolock)"
            _Qry &= vbCrLf & "	where psb.FTBarcodeSendSuplNo = rcv.FTBarcodeSendSuplNo"
            _Qry &= vbCrLf & "	)"
            _Qry &= vbCrLf & "	"
            ...
            _Qry &= vbCrLf & "	 ("
            _Qry &= vbCrLf & "	 SELECT x.FTPurchaseNo, x.FNHSysPartId, x.FNSendSuplType, x.FNQuantity,x.FNPrice, x.FNAmount,x.FTNote "
            _Qry &= vbCrLf & "	 FROM     [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTPurchaseSendSupl_Detail x   WITH(NOLOCK)"
            _Qry &= vbCrLf & "   inner join  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTPurchaseSendSupl_DocSendRef  d  WITH(NOLOCK) on x.FTPurchaseNo = d.FTPurchaseNo"
            _Qry &= vbCrLf & "	 WHERE d.FTSendSuplNo in ('" & _DocNo & "') and x.FTPurchaseNo <> '" & HI.UL.ULF.rpQuoted(FTPurchaseNo.Text) & "'"
            _Qry &= vbCrLf & "	 ) AS Dx ON A.FNHSysPartId = Dx.FNHSysPartId AND A.FNSendSuplType=Dx.FNSendSuplType  AND A.FTNote = Dx.FTNote "

            _Qry &= vbCrLf & " outer apply (  Select TOP 1 Isnull(M.FNConSmp,0) * Isnull(C.FNPrice,0) AS CPC"

            _Qry &= vbCrLf & "From ("
            _Qry &= vbCrLf & "  Select az.FTMainMatCode , m.FNConSmp   "
            _Qry &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTStyle_Mat AS M WITH(NOLOCK)  INNER JOIN    "
            _Qry &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TMERMMainMat AS Az WITH(NOLOCK) ON M.FNHSysMerMatId = Az.FNHSysMainMatId"
            _Qry &= vbCrLf & "where M.FTStateMainMaterial = '1'"
            _Qry &= vbCrLf & "and M.FNHSysStyleId = A.FNFarbicAmt   "
            _Qry &= vbCrLf & " ) AS M "

            _Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "]..TINVENMMaterial AS B WITH(NOLOCK) ON M.FTMainMatCode = B.FTRawMatCode"
            _Qry &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_INVEN) & "]..TINVENBarcode AS C WITH(NOLOCK) ON B.FNHSysRawMatId = C.FNHSysRawMatId"
            ...
            Me.ogcsenddetail.DataSource = _oDt.Select("FNQuantity > 0 ").CopyToDataTable
    ...
    Private Sub LoadDetail_bak(ByVal _DocRefNo As String)   ' เปิด po ตามยอดใบรับ
        Try
            Dim _Qry As String = ""
            Dim _oDt As DataTable
            Dim _DocNo As String = ""
    ...
    Private Sub RepositoryItemCalcQty_KeyDown(sender As Object, e As KeyEventArgs) Handles RepGFNPrice.KeyDown
        Try
                            ...
                            Select Case .Columns(I).FieldName.ToString.ToUpper
                                Case "FNPrice".ToUpper
                                    For _i As Integer = 0 To .DataRowCount Step 1
                                        ...
                                        _TotalAmt = .GetRowCellValue(_i, "FNQuantity") * _Value

                                        .SetRowCellValue(_i, "FNAmount", _TotalAmt)
                                        .SetRowCellValue(_i, "FNPaySupl", _TotalAmt - Double.Parse(Val(.GetFocusedRowCellValue("FNAmtDeductSupl"))))
                                    Next
                                Case Else
    ...
    Private Function CheckQC(docno As String, purchaseNo As String) As Boolean
        Try
            Dim _Cmd As String = ""
            _Cmd = " Select   FTStateQC  from  dbo.FN_Check_SendSuplQC('" & docno & "' , '" & HI.ST.Lang.Language & "' ,'" & purchaseNo & "') as t"
        ...
        Try
            Select Case e.KeyCode
                Case Keys.Enter
                    ...
                    If Not CheckQC(Me.FTSendSuplNo.Text, Me.FTPurchaseNo.Text) Then
                        HI.MG.ShowMsg.mInfo("รายการนี้ไม่ได้ทำการ QC กรุณาตรวจสอบ !!!", 2203141729, Me.Text)
                        Exit Sub
                    End If
