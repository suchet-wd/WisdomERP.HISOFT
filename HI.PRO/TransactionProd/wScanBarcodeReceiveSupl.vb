...
Public Class wScanBarcodeReceiveSupl
...
#Region "Procedure"
    ...
    Public Sub LoadDataInfo(Key As Object)
        ...
        _Str &= vbCrLf & "FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSuplToBranch As M WITH(NOLOCK) "
    ...
    Private Function GetCmpSupl() As Integer
        Try
            Dim _Cmd As String = ""
            _Cmd = "Select A.FNHSysCmpId  From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSupl_Barcode AS B  WITH (NOLOCK) "
            _Cmd &= vbCrLf & "  INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTSendSuplToBranch_Barcode AS SSB  WITH (NOLOCK)   ON  B.FTBarcodeSendSuplNo = SSB.FTBarcodeSendSuplNo"
            _Cmd &= vbCrLf & "  LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTSendSuplToBranch AS A WITH(NOLOCK) ON SSB.FTSendSuplNo = A.FTSendSuplNo"
            _Cmd &= vbCrLf & " Where B.FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(Me.FTRcvSuplNo.Text) & "' "
            Return HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_PROD, "0").Rows.Count
        Catch ex As Exception
            Return 0
        End Try
    End Function
    ...
    Private Function SaveDataSendBranch(ByVal _Key As String) As Boolean
        Try
            ...
            _Cmd = " SELECT  TOP (1) S.FNHSysCmpId  FROM   [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTSendSuplToBranch AS S WITH (NOLOCK) INNER JOIN"
            _Cmd &= vbCrLf & " [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTSendSuplToBranch_Barcode AS B WITH (NOLOCK) ON S.FTSendSuplNo = B.FTSendSuplNo"
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            _Qry = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_CHECK_SuplType '" & HI.UL.ULF.rpQuoted(FTBarcodeNo.Text) & "'"
            dt_chk = (HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_PROD))
            ...
            If Me.FNRcvSuplType.SelectedIndex = 0 Then
                ...
                _Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODMOperationByStyle AS OPS WITH (NOLOCK)  ON O.FNHSysStyleId = OPS.FNHSysStyleId AND SD.FNHSysOperationIdTo = OPS.FNHSysOperationId INNER JOIN"
                ...
                _Qry &= vbCrLf & " 	        LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSupl_Barcode AS RCVB  WITH (NOLOCK) ON B.FTBarcodeSendSuplNo = RCVB.FTBarcodeSendSuplNo "
            ...
            Else
                ...
                _Qry &= vbCrLf & "	LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMPart  AS MP WITH (NOLOCK) ON A.FNHSysPartId = MP.FNHSysPartId"
                _Qry &= vbCrLf & "	LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TPRODMOperation AS MPP WITH (NOLOCK)  ON  A.FNHSysOperationId  = MPP.FNHSysOperationId"
                _Qry &= vbCrLf & "	LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSupl_Barcode AS RCVB  WITH (NOLOCK) ON A.FTBarcodeSendSuplNo = RCVB.FTBarcodeSendSuplNo "
...
#Region "MAIN PROC"
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTRcvSuplNo.Text <> "" Then
            ...
            Dim cmd As String = ""
            cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.[SP_CREATE_TEMPRECEIVESUPL] '" & HI.UL.ULF.rpQuoted(Me.FTRcvSuplNo.Text) & "'"
...
#Region " Proc "
    ...
    Private Function SaveBarcode(Key As String) As Boolean
        ...
        Try
            ...
            If _StateNew Then
                ...
                _Str = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSupl_Barcode(FTInsUser, FDInsDate, FTInsTime, FTRcvSuplNo, FTBarcodeSendSuplNo,FNHSysUnitSectId)  "
    ...
    Private Function DeleteBarcode(BarcodeKey As String) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTReceiveSuplToBranch_Barcode  WHERE FTRcvSuplNo='" & HI.UL.ULF.rpQuoted(FTRcvSuplNo.Text) & "' AND FTBarcodeSendSuplNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
            HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
