...
Public Class wScanBarcodeSingle
...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        Try
            ...
            _Qry = " SELECT   BSS.FTBarcodeHeatNo AS FTBarcodeBundleNo"
            ...
            _Qry &= vbCrLf & "   INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcode_SingleCon AS BSS  WITH(NOLOCK)  ON  BB.FTBarcodeBundleNo = BSS.FTBarcodeBundleNo"
            _Qry &= vbCrLf & " 	INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail  AS B  WITH(NOLOCK) "
            _Qry &= vbCrLf & "  ON BSS.FTBarcodeHeatNo = B.FTBarcodeNo"
            ...
            _Qry &= vbCrLf & " 	 LEFT OUTER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee AS M WITH (NOLOCK)  ON  B.FNHSysEmpID  = M.FNHSysEmpID"
            _Qry &= vbCrLf & " LEFT OUTER JOIN    [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_MASTER) & "].dbo.THRMPrename AS PR WITH (NOLOCK) ON M.FNHSysPreNameId = PR.FNHSysPreNameId"
            _Qry &= vbCrLf & "   WHERE B.FTDocScanNo='" & HI.UL.ULF.rpQuoted(Key) & "' "
            _Qry &= vbCrLf & "  ORDER BY BB.FTBarcodeBundleNo "
    ...
    Private Function DeleteData() As Boolean
        Try
            ...
            Dim _Str As String
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single WHERE FTDocScanNo='" & HI.UL.ULF.rpQuoted(Me.FTDocScanNo.Text) & "'"
            If HI.Conn.SQLConn.Execute_Tran(_Str, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                HI.Conn.SQLConn.Tran.Rollback()
                HI.Conn.SQLConn.DisposeSqlTransaction(HI.Conn.SQLConn.Tran)
                HI.Conn.SQLConn.DisposeSqlConnection(HI.Conn.SQLConn.Cmd)
            ...
            _Str = "Delete From  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail WHERE FTDocScanNo='" & HI.UL.ULF.rpQuoted(Me.FTDocScanNo.Text) & "'"
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTDocScanNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .ReportName = "ScanBarcodeSingleSlip.rpt"
                .Formular = "{TPRODBarcodeScan_Single.FTDocScanNo}='" & HI.UL.ULF.rpQuoted(FTDocScanNo.Text) & "' "
                .Preview()
            End With
        Else
...
#Region " Proc "
    ...
    Private Function SaveBarcode(Key As String) As Boolean
        ...
        Try
            ...
            _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail WITH(NOLOCK)"
            ...
            If _StateNew Then
                ...
                _Str = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail("
                _Str &= vbCrLf & "FTInsUser, FDInsDate, FTInsTime"
                _Str &= vbCrLf & ", FTDocScanNo, FTBarcodeNo,FNHSysOperationId,FNHSysEmpID"
                _Str &= vbCrLf & ")  "
                ...
                _Str &= vbCrLf & "," & Integer.Parse(Val(FNHSysEmpID.Properties.Tag.ToString)) & ""

            Else

                _Str = " UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail "
                ...
                _Str &= vbCrLf & ",FNHSysEmpID=" & Integer.Parse(Val(FNHSysEmpID.Properties.Tag.ToString)) & ""
                _Str &= vbCrLf & "  WHERE FTDocScanNo='" & HI.UL.ULF.rpQuoted(FTDocScanNo.Text) & "'"
                _Str &= vbCrLf & "  AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
                _Str &= vbCrLf & "  AND FNHSysOperationId=" & Integer.Parse(Val(FNHSysOperationId.EditValue.ToString)) & " "
    ...
    Private Function DeleteBarcode(BarcodeKey As String, _Operation As Integer) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail"
            _Str &= vbCrLf & "   WHERE FTDocScanNo='" & HI.UL.ULF.rpQuoted(FTDocScanNo.Text) & "' "
            _Str &= vbCrLf & "  AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            _Qry = " SELECT  TOP 1 BSB.FTBarcodeHeatNo AS FTBarcodeBundleNo"
            _Qry &= vbCrLf & " 	, O.FTOrderNo "
            _Qry &= vbCrLf & " 	, BB.FTOrderProdNo"
            _Qry &= vbCrLf & " 	, BB.FNHSysCmpId"
            _Qry &= vbCrLf & " 	, B.FTDocScanNo"
            ...
            _Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd AS ODP WITH (NOLOCK)  ON O.FTOrderNo = ODP.FTOrderNo RIGHT OUTER JOIN"
            _Qry &= vbCrLf & " 	        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS BB WITH (NOLOCK)  ON ODP.FTOrderProdNo = BB.FTOrderProdNo "
            _Qry &= vbCrLf & " 	INNER JOIN        [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcode_SingleCon AS BSB WITH (NOLOCK)  ON BB.FTBarcodeBundleNo = BSB.FTBarcodeBundleNo "
            _Qry &= vbCrLf & " 	LEFT OUTER JOIN   ( SELECT TOP 1  FTDocScanNo,FTBarcodeNo,FNHSysOperationId,FNHSysEmpID"
            _Qry &= vbCrLf & "  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Single_Detail WITH (NOLOCK)  "
            ...
            _Qry &= vbCrLf & "  ) AS B  "
            _Qry &= vbCrLf & "  ON BSB.FTBarcodeHeatNo = B.FTBarcodeNo"
            _Qry &= vbCrLf & "   WHERE BSB.FTBarcodeHeatNo='" & HI.UL.ULF.rpQuoted(Key) & "' AND ISNULL(BB.FTStateGenBarcode,'') ='1' "
            ...
            If dt.Rows.Count > 0 Then
                ...
                If dt.Rows(0)!FTDocScanNo.ToString = "" Or dt.Rows(0)!FTDocScanNo.ToString = Me.FTDocScanNo.Text Then
                    ...
                    If Integer.Parse(_CheckOperBefore) > 0 Then
                        ...
                        _Qry = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_CheckOperationBefore '" & HI.UL.ULF.rpQuoted(FTOrderProdNo.Text) & "'," & Integer.Parse(_CheckOperBefore) & ",'" & HI.UL.ULF.rpQuoted(FTBarcodeNo.Text) & "'"
                        ...
                        Else
                            ...
                            'olberror.Text = HI.MG.ShowMsg.GetMessage("ไม่พบข้อมูลขั้นตอนก่อนหน้า หรือยังไม่ได้ทำการ Scan กรุณาทำการตรวจสอบ !!!", 1407311102)
                        End If
                ...
                Else
                    ...
                    olberror.Text = HI.MG.ShowMsg.GetMessage("Barcode ถูก Scan ด้วยหมายเลขอื่นไปแล้ว", 1407311101) & " (" & dt.Rows(0)!FTDocScanNo.ToString & ")"
            ...
            If (_StatePass) Then
                ...
                If Me.FNHSysEmpID.Text = "" Or Integer.Parse(Val(Me.FNHSysEmpID.Properties.Tag.ToString)) = 0 Then
                    HI.MG.ShowMsg.mInvalidData(MG.ShowMsg.InvalidType.SelectData, Me.FNHSysEmpID_lbl.Text, Me.Text)
                    Me.FNHSysEmpID.Focus()
                    Me.FNHSysEmpID.SelectAll()
                    Exit Sub
                End If
        ...
        Try
            ...
            Else
                ...
                If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_PROD, "") = "" Then
                    ...
                    _Qry &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODMOperationByStyle AS A WITH(NOLOCK) INNER JOIN"
                    _Qry &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TPRODMOperation AS B WITH(NOLOCK) ON A.FNHSysOperationId = B.FNHSysOperationId"
                    _Qry &= vbCrLf & "  WHERE  A.FNHSysStyleId=ISNULL((SELECT TOP 1 FNHSysStyleId  FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TMERMStyle WITH(NOLOCK) WHERE FTStylecode='" & HI.UL.ULF.rpQuoted(_FTStyleCode) & "'),0) AND FNOperationState =1 "
                    _Qry &= vbCrLf & "  ) AS A ORDER BY A.FNSeq "
                Else
                    _Qry = "SELECT FNHSysOperationId,FTOperationName  FROM ( SELECT -1 AS FNHSysOperationId ,'' AS  FTOperationName,-1 AS FNSeq  UNION "
                    _Qry &= vbCrLf & "  SELECT     A.FNHSysOperationId"
                    ...
                    _Qry &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOperationByOrderProd AS A WITH(NOLOCK) INNER JOIN"
                    _Qry &= vbCrLf & "   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TPRODMOperation AS B WITH(NOLOCK) ON A.FNHSysOperationId = B.FNHSysOperationId"
                    _Qry &= vbCrLf & "  WHERE  A.FTOrderProdNo='" & HI.UL.ULF.rpQuoted(_FTOrderProdNo) & "' AND FNOperationState =1  "
                    _Qry &= vbCrLf & "  ) AS A ORDER BY A.FNSeq "
