...
Public Class wScanBarcodeOperation
...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail(Key As String)
        ...
        Try
            ...
            _Qry &= vbCrLf & " 	INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail  AS B  WITH(NOLOCK) "
    ...
    Private Sub Proc_Preview(sender As System.Object, e As System.EventArgs) Handles ocmpreview.Click
        If Me.FTDocScanNo.Text <> "" Then
            With New HI.RP.Report
                ...
                .Formular = "{TPRODBarcodeScan_Detail.FTDocScanNo}='" & HI.UL.ULF.rpQuoted(FTDocScanNo.Text) & "' "
                .Preview()
            End With
        Else
...
#Region " Proc "
    ...
    Private Function SaveBarcode(Key As String) As Boolean
        ...
        Try
            ...
            _Str &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail WITH(NOLOCK)"
            ...
            If _StateNew Then
                ...
                _Str = " INSERT INTO [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail("
            ...
            Else
                ...
                _Str = " UPDATE [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail "
    ...
    Private Function DeleteBarcode(BarcodeKey As String, _Operation As Integer) As Boolean
        ...
        Try
            ...
            _Str = " DELETE  FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail"
            _Str &= vbCrLf & "   WHERE FTDocScanNo='" & HI.UL.ULF.rpQuoted(FTDocScanNo.Text) & "' "
            _Str &= vbCrLf & "  AND FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarCode) & "' "
    ...
    Private Sub LoadBarcodeInfo(Key As String)
        ...
        Try
            ...
            If (_StatePass) Then
                ...
                '_Qry &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(HI.Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail WITH(NOLOCK)"
                '_Qry &= vbCrLf & "  WHERE   FTBarcodeNo='" & HI.UL.ULF.rpQuoted(FTBarcodeNo.Text) & "' "
                '_Qry &= vbCrLf & "  AND FNHSysOperationId=" & Integer.Parse(Val(FNHSysOperationId.EditValue.ToString)) & " "

                'If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_PROD, "") <> "" Then
                '    olberror.ForeColor = Drawing.Color.Red
                '    olberror.Text = HI.MG.ShowMsg.GetMessage("Barcode ถูก Scan ขั้นตอนนี้ไปแล้ว กรุณาตรวจสอบ", 2008311403) & " (" & dt.Rows(0)!FTDocScanNo.ToString & ")"
                ...
                If HI.Conn.SQLConn.GetField(_Qry, Conn.DB.DataBaseName.DB_PROD, "") <> "" Then
                    olberror.ForeColor = Drawing.Color.Red
                    olberror.Text = HI.MG.ShowMsg.GetMessage("Barcode ถูก Scan ออกไลน์ไปแล้ว กรุณาตรวจสอบ", 1610261133) & " (" & dt.Rows(0)!FTDocScanNo.ToString & ")"
                    Me.FTBarcodeNo.Focus()
                    Me.FTBarcodeNo.SelectAll()
                    Exit Sub
                End If
...
#End Region
    ...
    Private Sub ocmdeletebarcode_Click(sender As Object, e As EventArgs) Handles ocmdeletebarcode.Click
        ...
        With ogvdetail
            ...
            _Qry = " Select Top 1   FTBarcodeNo"
            _Qry &= vbCrLf & " From  [" & HI.Conn.DB.GetServerName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline WITH(NOLOCK)"
            _Qry &= vbCrLf & " where FTBarcodeNo = '" & HI.UL.ULF.rpQuoted(_BarKey) & "'"
            _Qry &= vbCrLf & " UNION  "
            _Qry &= vbCrLf & " Select Top 1 FTBarcodeNo"
            _Qry &= vbCrLf & " From [" & HI.Conn.DB.GetServerName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail WITH(NOLOCK) "
            _Qry &= vbCrLf & " where FTBarcodeNo = '" & HI.UL.ULF.rpQuoted(_BarKey) & "'"
