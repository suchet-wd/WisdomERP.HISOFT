...
Public Class wScanBarcodeCustOutlineHyper
    ...
    Private _defualsewoper As Integer = 0
    ...
    Sub New()
        ...
        FTHyperBoxNo.Enabled = _HyperActive
...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail()
        Try
            ...
            _Qry = "SELECT  B.FTBarcodeNo , B.FTBarcodeCustRef, B.FNHSysUnitSectId, sum(B.FNQuantity) AS FNQuantity , U.FTUnitSectCode,   D.FTSizeBreakDown, P.FTOrderNo , ODRS.FTPORef , D.FNBunbleSeq "
            ...
            _Qry &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle As D With (NOLOCK) On B.FTBarcodeNo = D.FTBarcodeBundleNo "
    ...
    Private Sub FormRefresh()
        ...
        For Each Obj As Object In Me.Controls.Find(Me.MainKey, True)
            ...
            End Select
        Next
        Me.FNStateSewPack.SelectedIndex = _defualsewoper
    End Sub
...
#End Region
    ...
    Private Sub wScanBarcodeSendSupl_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        _defualsewoper = HI.Conn.SQLConn.GetField("Select   Top(1) FTCfgData   FROM     TSESystemConfig   WHERE  (FTCfgName = N'CfgDefualSew') ", Conn.DB.DataBaseName.DB_SECURITY, 0)
        Me.FNStateSewPack.SelectedIndex = _defualsewoper

        If HI.ST.SysInfo.Admin Then
            Me.FTStateDeleteBarcode.Enabled = True
        Else
            Me.FTStateDeleteBarcode.Enabled = ocmdelete.Enabled
        End If
            ...
            With DirectCast(Me.ogcdetail.DataSource, DataTable)
                ...
                For Each R As DataRow In .Select("FTBarcodeNo ='" & HI.UL.ULF.rpQuoted(_BundleNo) & "'")
                    ...
                    If HI.Conn.SQLConn.GetDataTable(_Cmd, Conn.DB.DataBaseName.DB_PROD).Rows.Count <= 0 Then
                        ...
                        _Cmd &= vbCrLf
                        _Cmd &= vbCrLf & "FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Barcode AS C   "
                        _Cmd &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Detail AS B WITH(NOLOCK) ON C.FTPackNo = B.FTPackNo AND C.FNCartonNo = B.FNCartonNo "
    ...
    'Private Function GetBarcodeBundle() As String
    '    Try
    '        Dim _Cmd As String = ""
    '        _Cmd = "Select Top 1   A.FTBarcodeNo, A.FTOrderNo, A.FTUnitSectCodeSew, A.FTColorway, A.FTSizeBreakDown, A.FNQuantity, A.FTCustBarcodeNo, A.FDUpdDate, A.FDInsDate, A.FTUpdTime, A.FTInsTime, A.FNHSysUnitSectId, "
    '        _Cmd &= vbCrLf & "  ISNULL(O.FNQuantity, 0) AS OutQty"
    '        _Cmd &= vbCrLf & "From (SELECT     A.FTBarcodeNo, P.FTOrderNo, US.FTUnitSectCode AS FTUnitSectCodeSew, B.FTColorway, B.FTSizeBreakDown, SUM(BD.FNQuantity) AS FNQuantity , C.FTCustBarcodeNo "
    '        _Cmd &= vbCrLf & ", A.FDUpdDate, A.FDInsDate  ,A.FTUpdTime, A.FTInsTime ,A.FNHSysUnitSectId"
    ...
    '        _Cmd &= vbCrLf & "INNER  JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..TPRODTOrder_CustBarcode AS C WITH(NOLOCK) ON P.FTOrderNo = C.FTOrderNo and B.FTColorway = C.FTColorway and B.FTSizeBreakDown = C.FTSizeBreakDown"
    ...
    '        _Cmd &= vbCrLf & "AND c.FTCustBarcodeNo = '" & HI.UL.ULF.rpQuoted(Me.FTBarcodeNo.Text) & "'"

    '        _Cmd &= vbCrLf & "GROUP BY A.FTBarcodeNo, P.FTOrderNo, US.FTUnitSectCode, B.FTColorway, B.FTSizeBreakDown, C.FTCustBarcodeNo , A.FDUpdDate, A.FDInsDate  ,A.FTUpdTime, A.FTInsTime ,A.FNHSysUnitSectId"
    '        _Cmd &= vbCrLf & ") AS A"
    '        _Cmd &= vbCrLf & "LEFT OUTER JOIN (SELECT        FTBarcodeNo, FNHSysUnitSectId, sum(FNQuantity) AS FNQuantity"
    '        _Cmd &= vbCrLf & "FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..TPRODBarcodeScanOutline  WITH (NOLOCK)"
    '        _Cmd &= vbCrLf & "group by    FTBarcodeNo, FNHSysUnitSectId) AS O   ON A.FTBarcodeNo = O.FTBarcodeNo  and A.FNHSysUnitSectId = O.FNHSysUnitSectId"
    '        _Cmd &= vbCrLf & "where A.FNQuantity >   ISNULL(O.FNQuantity, 0) "
    '        _Cmd &= vbCrLf & "Order by Isnull(A.FDUpdDate, A.FDInsDate ), Isnull(A.FTUpdTime, A.FTInsTime) ASC "
    ...
    Private Function ChkBarCode() As Boolean
        Try
            ...
            If Not (Me.FTStateDeleteBarcode.Checked) Then
                ...
                _Cmd = "Select Top 1 A.FTSubOrderNo,  A.FTBarcodeNo, A.FTOrderNo, A.FTUnitSectCodeSew, A.FTColorway, A.FTSizeBreakDown, A.FNQuantity, A.FDUpdDate, A.FDInsDate, A.FTUpdTime, A.FTInsTime, A.FNHSysUnitSectId, "
                _Cmd &= vbCrLf & "  ISNULL(O.FNQuantity, 0) AS OutQty , A.FTStyleCode ,A.FTOrderNo , A.FTPORef , A.FTOrderProdNo , A.FNBunbleSeq,A.FTPOLineItemNo "
                _Cmd &= vbCrLf & "From (SELECT    SB.FTSubOrderNo,  A.FTBarcodeNo, P.FTOrderNo, US.FTUnitSectCode AS FTUnitSectCodeSew, B.FTColorway, B.FTSizeBreakDown, SUM(B.FNQuantity) AS FNQuantity "
                _Cmd &= vbCrLf & ", A.FDUpdDate, A.FDInsDate  ,A.FTUpdTime, A.FTInsTime ,A.FNHSysUnitSectId , T.FTStyleCode ,  O.FTPORef , B.FTOrderProdNo , B.FNBunbleSeq,B.FTPOLineItemNo "
                _Cmd &= vbCrLf
                _Cmd &= vbCrLf & "FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail AS A WITH (NOLOCK) "
                ...
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS B WITH (NOLOCK) ON A.FTBarcodeNo = B.FTBarcodeBundleNo "
                '  _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle_Detail AS BD WITH (NOLOCK) ON B.FTBarcodeBundleNo = BD.FTBarcodeBundleNo INNER JOIN"
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd_Detail AS P WITH (NOLOCK) ON B.FTOrderProdNo = P.FTOrderProdNo "
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan AS HA WITH (NOLOCK) ON A.FTDocScanNo = HA.FTDocScanNo"
                '_Cmd &= vbCrLf & "INNER  JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..TPRODTOrder_CustBarcode AS C WITH(NOLOCK) ON P.FTOrderNo = C.FTOrderNo and B.FTColorway = C.FTColorway and B.FTSizeBreakDown = C.FTSizeBreakDown"
                _Cmd &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS O WITH(NOLOCK) ON  P.FTOrderNo = O.FTOrderNo"
                ...
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown AS SB WITH(NOLOCK) ON "
                _Cmd &= vbCrLf & "  isnull(B.FTPOLineItemNo,'') = isnull(SB.FTNikePOLineItem,'')  AND B.FTSizeBreakDown = SB.FTSizeBreakDown AND B.FTColorway = SB.FTColorway  and P.FTOrderNo = SB.FTOrderNo and P.FTSubOrderNo = SB.FTSubOrderNo "
                _Cmd &= vbCrLf & "    AND P.FTSizeBreakDown = SB.FTSizeBreakDown AND P.FTColorway = SB.FTColorway  "
                ...
                _Cmd &= vbCrLf & "          AND A.FTBarcodeNo in (SELECT    distinct A.FTBarcodeNo"
                _Cmd &= vbCrLf & "FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail AS A WITH (NOLOCK) "
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TCNMUnitSect AS US WITH (NOLOCK) ON A.FNHSysUnitSectId = US.FNHSysUnitSectId "
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS B WITH (NOLOCK) ON A.FTBarcodeNo = B.FTBarcodeBundleNo "
                ...
                '_Cmd &= vbCrLf & "       WHERE     (US.FTStateSew = '1') "
                '_Cmd &= vbCrLf & "                 AND (A.FNHSysUnitSectId=" & Integer.Parse(Val(FNHSysUnitSectId.Properties.Tag.ToString)) & ")"
                '_Cmd &= vbCrLf & "                 AND c.FTCustBarcodeNo = '" & HI.UL.ULF.rpQuoted(Me.FTCustBarcodeNo.Text) & "'   "
                ...
                _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan AS HA WITH (NOLOCK) ON A.FTDocScanNo = HA.FTDocScanNo "
                _Cmd &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.TPRODMOperation AS OP WITH(NOLOCK) ON A.FNHSysOperationId = OP.FNHSysOperationId "
                _Cmd &= vbCrLf & "INNER  JOIN   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "]..TMERTOrderSub_BreakDown  AS BC WITH(NOLOCK) ON P.FTOrderNo = BC.FTOrderNo  and B.FTColorway = BC.FTColorway and B.FTSizeBreakDown = BC.FTSizeBreakDown  and B.FTPOLineItemNo = BC.FTNikePOLineItem "
                ...
                If Me.FTOrderNo.Text <> "" Then
                    ...
                    If Me.FTSubOrderNo.Text <> "" Then
                        _Cmd &= vbCrLf & " And BC.FTSubOrderNo ='" & HI.UL.ULF.rpQuoted(Me.FTSubOrderNo.Text) & "'"
                ...
                _Cmd &= vbCrLf & "GROUP BY SB.FTSubOrderNo, A.FTBarcodeNo, P.FTOrderNo, US.FTUnitSectCode, B.FTColorway, B.FTSizeBreakDown,  A.FDUpdDate, A.FDInsDate  ,A.FTUpdTime, A.FTInsTime "
                _Cmd &= vbCrLf & ",A.FNHSysUnitSectId , T.FTStyleCode ,  O.FTPORef, B.FTOrderProdNo, B.FNBunbleSeq,B.FTPOLineItemNo"
                _Cmd &= vbCrLf & ") AS A"
                _Cmd &= vbCrLf & "LEFT OUTER JOIN (Select FTBarcodeNo , SUM(FNQuantity) AS FNQuantity ,FNHSysUnitSectId  From (Select FTBarcodeNo , SUM(FNQuantity) AS FNQuantity ,FNHSysUnitSectId"
                ...
                '_Cmd &= vbCrLf & "Select   FTBarcodeNo ,   SUM(FNScanQuantity) AS FNQuantity ,FNHSysUnitSectId"
                '_Cmd &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan   WITH(NOLOCK) "
                '_Cmd &= vbCrLf & " Where   isnull(FTStateScanAuto,'') <> '1' "
                '_Cmd &= vbCrLf & " Group by FTBarcodeNo,FNHSysUnitSectId   "
                ...
                If Me.FTOrderNo.Text <> "" Then
                    ...
                    If Me.FTSubOrderNo.Text <> "" Then
                        _Cmd &= vbCrLf & " And A.FTSubOrderNo ='" & HI.UL.ULF.rpQuoted(Me.FTSubOrderNo.Text) & "'"
                    End If
                End If

                _Cmd &= vbCrLf & "Order by Isnull(A.FDUpdDate, A.FDInsDate ) ASC, Isnull(A.FTUpdTime, A.FTInsTime) ASC "
            ...
            Else
                ...
                _Cmd &= vbCrLf & " WHERE  ( O.FTBarcodeCustRef='" & HI.UL.ULF.rpQuoted(Me.FTCustBarcodeNo.Text) & "' OR  B.FTBarcodeBundleNo='" & HI.UL.ULF.rpQuoted(Me.FTCustBarcodeNo.Text) & "' )"
                ...
                _Cmd &= vbCrLf & "And O.FNHSysUnitSectId=" & Integer.Parse(Me.FNHSysUnitSectId.Properties.Tag)
            ...
            For Each R As DataRow In _oDt.Rows
                ...
                If FTStateDeleteBarcode.Checked = False Then
                    ...
                    _Cmd &= vbCrLf & "From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle As B With(NOLOCK) "
                    _Cmd &= vbCrLf
                    _Cmd &= vbCrLf & "INNER Join [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd_Detail As C With(NOLOCK) On B.FTOrderProdNo = C.FTOrderProdNo "
                    _Cmd &= vbCrLf
                    _Cmd &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrderSub_BreakDown As C2 With (NOLOCK) "
                    _Cmd &= vbCrLf & "On C.FTOrderNo = C2.FTOrderNo And C.FTSubOrderNo = C2.FTSubOrderNo And C.FTColorway = C2.FTColorway And C.FTSizeBreakDown = C2.FTSizeBreakDown"
                    _Cmd &= vbCrLf
                    _Cmd &= vbCrLf & "OUTER APPLY(Select SUM(X.FNQuantity)  As FNQuantity"
                    _Cmd &= vbCrLf & "From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline As X With(NOLOCK) "
                    _Cmd &= vbCrLf
    ...
    Private Function ScansBarcode(BarcodeKey As String) As Boolean
        Try
            ...
            If Not (Me.FTStateDeleteBarcode.Checked) Then
                ...
                '    _Cmd &= vbCrLf & ",FTBarcodeCustRef=''"
                'Else
                '    _Cmd &= vbCrLf & ",FTBarcodeCustRef='" & HI.UL.ULF.rpQuoted(Me.FTCustBarcodeNo.Text) & "'"
                'End If
                ...
                If HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    _Cmd &= vbCrLf & "(FTInsUser, FDInsDate, FTInsTime , FTBarcodeNo, FNHSysUnitSectId, FDDate, FTTime "
                    _Cmd &= vbCrLf & ", FNQuantity,FTBarcodeCustRef , FTOrderNo , FTSubOrderNo, FNStateSewPack "
                    _Cmd &= vbCrLf & If(_HyperActive, ", FTBoxHyperBarcode, FTStateExport ", "")
                    _Cmd &= vbCrLf & ") "
                    _Cmd &= vbCrLf
            ...
            Else
                ...
                If HI.Conn.SQLConn.GetDataTableOnbeginTrans(_Cmd).Rows.Count <= 0 Then
                    If Not checkcalsinc(BarcodeKey, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) Then
                        HI.MG.ShowMsg.mInfo("ไม่สามารถลบข้อมูลได้ เนื่องจากมีการคำนวณ Incentive แล้ว !!! ", 2211011619, Me.Text, "", MessageBoxIcon.Stop)
                    ...
                    If HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                        ...
                        Return False
                    End If
                    CreateLogDelete(HI.ST.SysInfo.MenuName, Me.Name, _Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran)
                    ...
                    _Cmd = "Select Top 1 FNQuantity From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline"
                    _Cmd &= vbCrLf & "WHERE FTBarcodeNo='" & HI.UL.ULF.rpQuoted(BarcodeKey) & "'"
    ...
    Private Function checkcalsinc(BarcodeKey As String, sqlcmd As System.Data.SqlClient.SqlCommand, Tr As System.Data.SqlClient.SqlTransaction) As Boolean
        Try
            Dim _Cmd As String = ""
            Dim _oDt As DataTable
            Dim _dtcals As DataTable
            _Cmd = "Select top 1 FDDate From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline "
            _Cmd &= vbCrLf & "WHERE FTBarcodeNo='" & HI.UL.ULF.rpQuoted(BarcodeKey) & "'"
            _Cmd &= vbCrLf & "And FNHSysUnitSectId=" & Integer.Parse(Me.FNHSysUnitSectId.Properties.Tag)
            _Cmd &= vbCrLf & "And isnull(FNStateSewPack,0)=" & Integer.Parse(Me.FNStateSewPack.SelectedIndex)
            If Me.FTOrderNo.Text <> "" Then
            ...
            _Cmd = "select top 1   FTCalDate, FNHSysUnitSectId "
            _Cmd &= vbCrLf & "From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRTIncentive_Emp with(nolock) "
            _Cmd &= vbCrLf & "where  FNHSysUnitSectId=" & Integer.Parse(Me.FNHSysUnitSectId.Properties.Tag)
            _Cmd &= vbCrLf & "and FTCalDate='" & _oDt.Rows(0).Item("FDDate").ToString & "'"
            _dtcals = HI.Conn.SQLConn.GetDataTableOnbeginTrans(_Cmd)
            If _dtcals.Rows.Count > 0 Then
    ...
    Public Function CreateLogDelete(pMenuName As String, pFormName As String, pQuery As String, sqlcmd As System.Data.SqlClient.SqlCommand, Tr As System.Data.SqlClient.SqlTransaction) As Boolean
        Try
            Dim StrSql As String = ""
            StrSql = "INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_LOG) & "].dbo.HSysDeleteLog "
            StrSql &= vbCrLf & " (FTDeleteUser, FDDeleteDate, FTDeleteTime, FTMnuName, FTFormName, FTQuery) "
            ...
            StrSql &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(pMenuName) & "'"
            StrSql &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(pFormName) & "'"
            StrSql &= vbCrLf & " ,'" & HI.UL.ULF.rpQuoted(pQuery) & "'"

            If HI.Conn.SQLConn.Execute_Tran(StrSql, sqlcmd, Tr) <= 0 Then
    ...
    Private Function ChkBarcodeInlineBal(BarCode As String, OrderNo As String, UnitSectId As Integer) As Boolean
        Try
            ...
            _Cmd &= vbCrLf & "FROM    "
            _Cmd &= vbCrLf & " (Select FTBarcodeNo , SUM(FNQuantity) AS FNQuantity ,FNHSysUnitSectId"
            _Cmd &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline   WITH(NOLOCK) "
            _Cmd &= vbCrLf & " Group by FTBarcodeNo,FNHSysUnitSectId "
            '_Cmd &= vbCrLf & "UNION ALL "
            '_Cmd &= vbCrLf & "Select FTBarcodeNo ,   SUM(FNScanQuantity) AS FNQuantity ,FNHSysUnitSectId"
            ...
            _Cmd &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS B WITH(NOLOCK) ON A.FTBarcodeNo = B.FTBarcodeBundleNo  "
            _Cmd &= vbCrLf & "INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd AS P WITH (NOLOCK) ON B.FTOrderProdNo = P.FTOrderProdNo"
    ...
    Private Sub FNStateSewPack_SelectedIndexChanged(sender As Object, e As EventArgs) Handles FNStateSewPack.SelectedIndexChanged
