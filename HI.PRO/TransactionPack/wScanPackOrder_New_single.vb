...
Public Class wScanPackOrder_New_single
...
#Region " Proc "
        ...
        Try
            ...
            If (_Node Is Nothing) Then
                ...
                Try
                    ...
                    _Qry &= vbCrLf & "       LEFT OUTER JOIN ( SELECT     FTPackNo, FNCartonNo, FTOrderNo, FTSubOrderNo, FTColorway, FTSizeBreakDown, FNHSysEmpId"
                    _Qry &= vbCrLf & ",   sum(FNScanQuantity) AS FNScanQuantity"
                    _Qry &= vbCrLf & "FROM         TPACKOrderPack_Carton_Scan_Single WITH(NOLOCK) "
                    _Qry &= vbCrLf & "group by  FTPackNo, FNCartonNo, FTOrderNo, FTSubOrderNo, FTColorway, FTSizeBreakDown, FNHSysEmpId ) AS Z ON A.FTPackNo = Z.FTPackNo"
                    ...
                    For Each R As DataRow In dt.Rows
                        nodeChild = _Lst.AppendNode(New Object() {Me.FNCartonNo2_lbl.Text & "" & R!FNCartonNo.ToString & " (" & R!FTCartonInfo.ToString & ")", R!FNCartonNo.ToString,
                                                                  R!FNQuantity.ToString, R!FNNetWeight.ToString, R!FNHSysCartonId.ToString, R!FTCartonCode.ToString, R!FNWeight.ToString,
    ...
    Private Function DeleteBarcode(ByVal _PackNo As String, ByVal _CartonNo As Integer) As Boolean
        Try
            ...
            _Cmd &= vbCrLf & " WHERE FTPackNo  in(Select Top 1 C.FTPackNo From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKCarton AS C INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single AS S ON C.FTPackNo = S.FTPackNo"
            ...
            _Cmd &= vbCrLf & "  )"
            _Cmd &= vbCrLf & " and FNCartonNo in (Select Top 1 C.FNCartonNo From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKCarton AS C INNER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single AS S ON C.FTPackNo = S.FTPackNo"
            _Cmd &= vbCrLf & " and C.FNCartonNo = S.FNCartonNo"
            _Cmd &= vbCrLf & " WHERE C.FTPackNo = '" & HI.UL.ULF.rpQuoted(_PackNo) & "'"
            _Cmd &= vbCrLf & " and C.FNCartonNo =" & CInt(_CartonNo)
...
#End Region
                ...
                If (_hifo.Node IsNot Nothing) Then
                    With _hifo.Node
                        ...
                        If Convert.ToBoolean(.Tag) = False Then
                            ...
                            _Qry = " Select TOP 1  A.FTEmpCode"
                            _Qry &= vbCrLf & "  FROM    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single AS P WITH(NOLOCK) INNER JOIN"
                            _Qry &= vbCrLf & "          [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_HR) & "].dbo.THRMEmployee AS A WITH(NOLOCK) ON P.FNHSysEmpId = A.FNHSysEmpId"
    ...
    Private Function Save_Scan(ByVal _OrderNo As String, ByVal _SubOrderNo As String, ByVal _Colorway As String,
                               ByVal _SizeBreakDown As String, ByVal _BarcodeNo As String, ByVal _EmpId As Integer, ByVal _ScanQty As Double) As Boolean
        Try
            Dim _Qry As String = ""
            Dim _oDt As DataTable
            ...
            _Qry = " UPDATE  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single"
            ...
            _Qry &= vbCrLf & "and FNHSysEmpId=" & CInt(_EmpId)
            _Qry &= vbCrLf & "and FTBarcodeNo='" & HI.UL.ULF.rpQuoted(_BarcodeNo) & "'"

            If HI.Conn.SQLConn.Execute_Tran(_Qry, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then

                _Qry = "INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single"
                _Qry &= "(FTInsUser, FDInsDate, FTInsTime,   FTPackNo, FNCartonNo, FTOrderNo, FTSubOrderNo, FTColorway, FTSizeBreakDown, FNHSysEmpId,FTBarcodeNo, FNScanQuantity)"
            ...
            _Qry &= vbCrLf & " Update  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single "
            ...
            _Qry &= vbCrLf & "  BEGIN"
            _Qry &= vbCrLf & "  INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single (FTInsUser, FDInsDate, FTInsTime,"
            _Qry &= "  FTPackNo, FNCartonNo, FTOrderNo, FTSubOrderNo, FTColorway, FTSizeBreakDown, FNHSysEmpId, FTBarcodeNo,  FDScanDate, FDScanTime, FNScanQuantity)"
            _Qry &= vbCrLf & "Select '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
            _Qry &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB
            _Qry &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB
            _Qry &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
        ...
        Try
            ...
            If _dt.Rows.Count > 0 Then
                For Each R As DataRow In _dt.Rows
                    ...
                    _Qry &= vbCrLf & "  FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo. TPACKOrderPack_Carton_Scan_Single (NOLOCK)"
...
L0:
            ...
            For Each R As DataRow In _dt.Rows
                ...
                Else
                    If CartonNo > 0 Then
                        ...
                        _Qry &= vbCrLf & "  FROM   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single AS A WITH(NOLOCK)"
                        _Qry &= vbCrLf & " WHERE  (FTPackNo = '" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "') "
    ...
    Private Function CheckBarcodeCust(BarcodeKey As String, CartonNo As String) As String
        ...
        If _dt.Rows.Count > 0 Then
            For Each R As DataRow In _dt.Rows
                ...
                _Qry &= vbCrLf & "  FROM      [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan_Detail WITH (NOLOCK)"
                _Qry &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS B (NOLOCK)  ON A.FTBarcodeNo = B.FTBarcodeBundleNo"
                _Qry &= vbCrLf & "INNER JOIN  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single AS CS  WITH (NOLOCK) ON"
    ...
    Private Function ChkBreakTime() As Boolean
        Try
            If Me.FNHSysEmpId.Text = "" Then
                HI.MG.ShowMsg.mInfo("กรุณาระบุ emp code ......", 2209291603, Me.Text, "", MessageBoxIcon.Stop)
                ...
                Else
                    ...
                    If BarCodeSet = False Then
                        ...
                        If Me.FTStateDeleteBarcode.Checked = False Then
                            cmdstring = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_SCAN_DATA_PACK_Single '" & HI.UL.ULF.rpQuoted(FTPackNo.Text) & "','" & HI.UL.ULF.rpQuoted(FTCartonBarcodeNo.Text) & "'," & _PFNCartonNo & ",'" & HI.UL.ULF.rpQuoted(FTProductBarcodeNo.Text) & "'," & Val(Me.FNHSysEmpId.Properties.Tag.ToString) & ",'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                        Else
                            cmdstring = " EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_SCAN_DATA_PACK_DELETE_Single '" & HI.UL.ULF.rpQuoted(FTPackNo.Text) & "','" & HI.UL.ULF.rpQuoted(FTCartonBarcodeNo.Text) & "'," & _PFNCartonNo & ",'" & HI.UL.ULF.rpQuoted(FTProductBarcodeNo.Text) & "'," & Val(Me.FNHSysEmpId.Properties.Tag.ToString) & ",'" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                        End If
        ...
        If BarCodeSet = False Then
                        ...
                        If Check_Barcode(HI.UL.ULF.rpQuoted(_FTProductBarcodeNo), _FTOrderNo, _FTColorWay, _FTSizeBreakDown, _FTSubOrderNo, _CartonQty) Then
                            ...
                            If _TotalCarton >= _PFNCartonNo Then
                                If _getScanQty < _GetCartonQty Then
                                    ' If CheckScanCloseCarton(Me.FTPackNo.Text, _PFNCartonNo) Then Exit Sub
                                    If Save_Scan(_FTOrderNo, _FTSubOrderNo, _FTColorWay, _FTSizeBreakDown, _FTProductBarcodeNo, Integer.Parse("0" & Me.FNHSysEmpId.Properties.Tag.ToString), 1) Then
                                        Me._LoadDataScan()
                                        Me.FTProductBarcodeNo.Focus()
                                        Me.FTProductBarcodeNo.SelectAll()
                                    Else
            ...
            Else
                ...
                If Check_Barcode(HI.UL.ULF.rpQuoted(_FTProductBarcodeNo), _FTOrderNo, _FTColorWay, _FTSizeBreakDown, _FTSubOrderNo, _CartonQty) Then
                    ...
                    If DeleteBarcode(Me.FTPackNo.Text, _PFNCartonNo, _PFTOrderNo, _PFTSubOrderNo, _PFTColorway, _PFTSizeBreakDown, Integer.Parse("0" & Me.FNHSysEmpId.Properties.Tag.ToString),
    ...
    Private Sub ProcessBarcodeSet(packno As String, barcodesetno As String)
        ...
        If (CheckBarcodeSet(packno, barcodesetno, dtbarset)) Then
            ...
            If FTStateDeleteBarcode.Checked = False Then
                ...
                cmdstring &= vbCrLf & "    FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single AS C  WITH(NOLOCK) "
                    ...
                    If checkmatching And dtbarset.Select("FNPass=1").Length <= 0 Then
                        ...
                        Try
                            ...
                            For Each R As DataRow In dtbarcodeoutline.Select("FNQtyCreatePack>0")
                                ...
                                cmdstring = " UPDATE  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single"
                                ...
                                cmdstring &= vbCrLf & "and FNHSysEmpId=" & Integer.Parse("0" & Me.FNHSysEmpId.Properties.Tag.ToString)
                                ...
                                If HI.Conn.SQLConn.Execute_Tran(cmdstring, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                                    ...
                                    cmdstring = "INSERT INTO  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single"
                                    cmdstring &= "(FTInsUser, FDInsDate, FTInsTime,   FTPackNo, FNCartonNo, FTOrderNo, FTSubOrderNo, FTColorway, FTSizeBreakDown, FNHSysEmpId,FTBarcodeNo, FNScanQuantity)"
                                    ...
                                    cmdstring &= vbCrLf & "," & Integer.Parse("0" & Me.FNHSysEmpId.Properties.Tag.ToString)
                                ...
                                cmdstring &= vbCrLf & " Update  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single "
                                ...
                                cmdstring &= vbCrLf & "  INSERT INTO [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single (FTInsUser, FDInsDate, FTInsTime,"
                                cmdstring &= "  FTPackNo, FNCartonNo, FTOrderNo, FTSubOrderNo, FTColorway, FTSizeBreakDown, FNHSysEmpId, FTBarcodeNo,  FDScanDate, FDScanTime, FNScanQuantity,FTBarcodeSetNo)"
                                cmdstring &= vbCrLf & "Select '" & HI.UL.ULF.rpQuoted(HI.ST.UserInfo.UserName) & "'"
                                cmdstring &= vbCrLf & "," & HI.UL.ULDate.FormatDateDB
                                cmdstring &= vbCrLf & "," & HI.UL.ULDate.FormatTimeDB
                                cmdstring &= vbCrLf & ",'" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
            ...
            Else
                ...
                For Each R As DataRow In dtbarset.Rows
                    ...
                    cmdstring = " SELECT FTColorway,FTSizeBreakDown,FNHSysEmpId,FDScanDate,FDScanTime,FNScanQuantity, 0 AS FNDelete "
                    cmdstring &= vbCrLf & " FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single  "
                    cmdstring &= vbCrLf & " WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
                    cmdstring &= vbCrLf & " AND FNCartonNo=" & CInt("0" & _PFNCartonNo)
                    cmdstring &= vbCrLf & " AND FTColorway='" & HI.UL.ULF.rpQuoted(R!FTColorway.ToString) & "'"
                    cmdstring &= vbCrLf & " AND FTSizeBreakDown='" & HI.UL.ULF.rpQuoted(R!FTSizeBreakDown.ToString) & "'"
                ...
                Try
                    ...
                    For Each R As DataRow In dtdelate.Select("FNDelete>0")
                        ...
                        cmdstring = " UPDATE   [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single Set FNScanQuantity=FNScanQuantity-" & Val(R!FNDelete.ToString) & " "
                        cmdstring &= vbCrLf & "WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
                        cmdstring &= vbCrLf & " AND FNCartonNo=" & CInt("0" & _PFNCartonNo)
                        cmdstring &= vbCrLf & " AND FTColorway='" & HI.UL.ULF.rpQuoted(R!FTColorway.ToString) & "'"
                        cmdstring &= vbCrLf & " AND FTSizeBreakDown='" & HI.UL.ULF.rpQuoted(R!FTSizeBreakDown.ToString) & "'"
                    ...
                    cmdstring &= vbCrLf & " AND FNHSysEmpId=" & Integer.Parse("0" & Me.FNHSysEmpId.Properties.Tag.ToString)
                    cmdstring &= vbCrLf & " ),0) "
                    cmdstring &= vbCrLf & " FROM [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single AS A"
                    cmdstring &= vbCrLf & " WHERE  FTPackNo='" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
                    cmdstring &= vbCrLf & " and FNCartonNo=" & CInt("0" & _PFNCartonNo)
                    cmdstring &= vbCrLf & " AND FNHSysEmpId=" & Integer.Parse("0" & Me.FNHSysEmpId.Properties.Tag.ToString)
                    ...
                    cmdstring = " DELETE  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single  "
                    cmdstring &= vbCrLf & " WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
                    cmdstring &= vbCrLf & "       AND FNCartonNo=" & CInt("0" & _PFNCartonNo)
                    cmdstring &= vbCrLf & "       AND FTBarcodeSetNo='" & HI.UL.ULF.rpQuoted(barcodesetno) & "'"
                    cmdstring &= vbCrLf & "       AND FNScanQuantity<=0"
    ...
    Private Function GetScanQty(ByVal _OrderNo As String, ByVal _SubOrderNo As String, ByVal _Colorway As String,
                               ByVal _SizeBreakDown As String, ByVal _BarcodeNo As String, ByVal _UnitSectId As Integer) As Integer
        Try
            Dim _Cmd As String = ""
            _Cmd = "Select  Sum(FNScanQuantity) AS FNScanQuantity   From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single  WITH(NOLOCK) "
            _Cmd &= vbCrLf & "WHERE  FTPackNo='" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "'"
    ...
    Private Sub _LoadDataScan()
        Try
            ...
            _Cmd = "EXEC [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.SP_Get_ScanQty_single '" & HI.UL.ULF.rpQuoted(Me.FTPackNo.Text) & "','" & CInt("0" & _PFNCartonNo) & "','" & HI.UL.ULF.rpQuoted(_PFTOrderNo) & "'"
    ...
    Private Function DeleteBarcode(ByVal _PackNo As String, ByVal _CartonNo As Integer, ByVal _OrderNo As String, ByVal _SubOrderNo As String, ByVal _Colorway As String, ByVal _SizeBreakDown As String _
        ...
        Try
            ...
            _Cmd = "Select top 1  FNScanQuantity From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single WITH(NOLOCK) "
            ...
            _Cmd &= vbCrLf & " AND FTColorway='" & HI.UL.ULF.rpQuoted(_Colorway) & "'"
            _Cmd &= vbCrLf & " AND FTSizeBreakDown='" & HI.UL.ULF.rpQuoted(_SizeBreakDown) & "'"
            _Cmd &= vbCrLf & " AND FNHSysEmpId=" & CInt(_UnitSectId)
            ...
            If _ScanQty <= 0 Then
                ...
                _Cmd = "Delete From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single "
                _Cmd &= vbCrLf & "WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(_PackNo) & "'"
                _Cmd &= vbCrLf & " AND FNCartonNo=" & CInt(_CartonNo)
                _Cmd &= vbCrLf & " AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_OrderNo) & "'"
                _Cmd &= vbCrLf & " AND FTSubOrderNo='" & HI.UL.ULF.rpQuoted(_SubOrderNo) & "'"
            ...
            Else
                ...
                _Cmd = "UPDATE [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single"
            ...
            ''New
            _Cmd = "Select top 1  FNScanQuantity From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single WITH(NOLOCK) "
            _Cmd &= vbCrLf & "WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(_PackNo) & "'"
            _Cmd &= vbCrLf & " AND FNCartonNo=" & CInt(_CartonNo)
            _Cmd &= vbCrLf & " AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_OrderNo) & "'"
            _Cmd &= vbCrLf & " AND FTSubOrderNo='" & HI.UL.ULF.rpQuoted(_SubOrderNo) & "'"
            ...
            _Cmd &= vbCrLf & "Select top 1 FDScanDate+'|'+FDScanTime  From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single "
            _Cmd &= vbCrLf & "WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(_PackNo) & "'"
            _Cmd &= vbCrLf & " AND FNCartonNo=" & CInt(_CartonNo)
            _Cmd &= vbCrLf & " AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_OrderNo) & "'"
            _Cmd &= vbCrLf & " AND FTSubOrderNo='" & HI.UL.ULF.rpQuoted(_SubOrderNo) & "'"
            ...
            If _ScanQty <= 0 Then
                ...
                _Cmd = "Delete  From [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single "
                _Cmd &= vbCrLf & "WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(_PackNo) & "'"
                _Cmd &= vbCrLf & " AND FNCartonNo=" & CInt(_CartonNo)
                _Cmd &= vbCrLf & " AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_OrderNo) & "'"
                _Cmd &= vbCrLf & " AND FTSubOrderNo='" & HI.UL.ULF.rpQuoted(_SubOrderNo) & "'"
            ...
            Else
                ...
                _Cmd = "UPDATE     [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Detail_Single"
                _Cmd &= vbCrLf & "SET FNScanQuantity=FNScanQuantity-1"
                _Cmd &= vbCrLf & "WHERE FTPackNo='" & HI.UL.ULF.rpQuoted(_PackNo) & "'"
                _Cmd &= vbCrLf & " AND FNCartonNo=" & CInt(_CartonNo)
                _Cmd &= vbCrLf & " AND FTOrderNo='" & HI.UL.ULF.rpQuoted(_OrderNo) & "'"
    ...
    Private Function GetPackNoByCarton(ByVal BarcodeKey As String, ByRef _CartonNo As String) As String
        Try
            ...
            If Me.FTPackNo.Text = "" Then
                ...
                _Cmd &= vbCrLf & "LEFT OUTER JOIN [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..TPACKOrderPack_Carton_Scan_Detail_Single AS SD WITH(NOLOCK) ON P.FTPackNo = SD.FTPackNo and P.FNCartonNo = SD.FNCartonNo  "
        ...
        Select Case e.KeyCode
            Case Keys.Enter
                ...
                Else
                    ...
                    _Cmd &= vbCrLf & "  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPACKOrderPack_Carton_Scan_Single As PD With(NOLOCK) On P.FTSizeBreakDown = PD.FTSizeBreakDown And P.FTColorway = PD.FTColorway And P.FTSubOrderNo = PD.FTSubOrderNo And P.FTOrderNo = PD.FTOrderNo And "
