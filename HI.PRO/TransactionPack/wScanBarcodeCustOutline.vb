...
#Region "Procedure"
    ...
    Private Sub LoadDucumentDetail()
        Try
            ...
            _Qry &= vbCrLf & ",isnull((Select Sum(X.FNQuantity) AS FNQuantity From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScanOutline AS X WITH(NOLOCK)"
    ...
    Private Function ChkBarCode() As Boolean
        Try
            ...
            If Not (Me.FTStateDeleteBarcode.Checked) Then
                ...
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle AS B WITH (NOLOCK) ON A.FTBarcodeNo = B.FTBarcodeBundleNo INNER JOIN"
                '  _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTBundle_Detail AS BD WITH (NOLOCK) ON B.FTBarcodeBundleNo = BD.FTBarcodeBundleNo INNER JOIN"
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODTOrderProd_Detail AS P WITH (NOLOCK) ON B.FTOrderProdNo = P.FTOrderProdNo INNER JOIN"
                _Cmd &= vbCrLf & "[" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "].dbo.TPRODBarcodeScan AS HA WITH (NOLOCK) ON A.FTDocScanNo = HA.FTDocScanNo"
                '_Cmd &= vbCrLf & "INNER  JOIN    [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_PROD) & "]..TPRODTOrder_CustBarcode AS C WITH(NOLOCK) ON P.FTOrderNo = C.FTOrderNo and B.FTColorway = C.FTColorway and B.FTSizeBreakDown = C.FTSizeBreakDown"
                ...
                _Cmd &= vbCrLf & " (Select FTBarcodeNo , SUM(FNQuantity) AS FNQuantity ,FNHSysUnitSectId  From (Select FTBarcodeNo , SUM(FNQuantity) AS FNQuantity ,FNHSysUnitSectId"
        ...
        Try
            ...
            If Not (Me.FTStateDeleteBarcode.Checked) Then
                ...
                If HI.Conn.SQLConn.Execute_Tran(_Cmd, HI.Conn.SQLConn.Cmd, HI.Conn.SQLConn.Tran) <= 0 Then
                    ...
                    _Cmd &= vbCrLf & "(FTInsUser, FDInsDate, FTInsTime , FTBarcodeNo, FNHSysUnitSectId, FDDate, FTTime, FNQuantity,FTBarcodeCustRef , FTOrderNo , FTSubOrderNo,FNStateSewPack)"
