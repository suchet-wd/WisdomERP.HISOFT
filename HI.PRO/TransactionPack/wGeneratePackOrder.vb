...
Public Class wGeneratePackOrder
...
#Region "Procedure"
    ...
    Private Sub InitData()
        ...
        _Qry &= vbCrLf & ", Isnull((Select Top 1 Z.FTPORef  From  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS Z WITH(NOLOCK) Where Z.FTOrderNo = M.FTOrderNo ),'') AS FTCustomerPO "
        ...
        _Qry &= vbCrLf & "	, Convert(numeric(18,0),ISNULL(M.FNSubOrderQty,0)) AS FNTotalQty"
        ...
        _Qry &= vbCrLf & "	 FROM            [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.V_TMERTOrderSub AS M WITH(NOLOCK) INNER JOIN"
        ...
        _Qry &= vbCrLf & "	 WHERE  M.FTOrderNo IN (SELECT FTOrderNo FROM  [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.TMERTOrder AS MO WITH(NOLOCK) WHERE FNHSysStyleId=" & FNHSysStyleId & ")"
        ...
        For Each R As DataRow In _dt.Rows
            ...
            _Qry = " Exec [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.SP_Get_SubOrderBreakDown_Pack '" & HI.UL.ULF.rpQuoted(R!FTOrderNo.ToString) & "','" & HI.UL.ULF.rpQuoted(R!FTSubOrderNo.ToString) & "' "
            _dtSub = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN)

            _Qry = "SELECT   FTPackNo AS FTOrderProdNo, FTColorway, FTSizeBreakDown, FNQuantity"
