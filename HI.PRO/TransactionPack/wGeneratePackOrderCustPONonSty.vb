...
Public Class wGeneratePackOrderCustPONonSty
    ...
    Private _ListDataSubOrderOverShip As New List(Of DataTable)
...
#Region "Property"
    ...
    Private _PackSetType As Integer = 0
    Property PackSetType As Integer
        Get
            Return _PackSetType
        End Get
    ...
    Private _PackSetValue As Integer = 0
    Property packSetValue As Integer
        Get
            Return _PackSetValue
        End Get
        Set(value As Integer)
            _PackSetValue = value
        End Set
...
#Region "Procedure"
    ...
    Private Sub InitData()
        ...
        _ListDataSubOrderOverShip.Clear()
        Dim _Qry As String = ""
        ...
        _Qry &= vbCrLf & "),'0') AS FTStatePack , m.FTStyleCode as FNHSysStyleId ,  M.FNHSysStyleId as FNHSysStyleId_Hide "
        _Qry &= vbCrLf & "	 FROM   "
        _Qry &= vbCrLf & "  ( "
        ...
        _Qry &= vbCrLf & "	, SUM(t.FNGrandQuantity) As FNTotalQty , s.FTStyleCode , t.FNHSysStyleId"
        ...
        _Qry &= vbCrLf & "left outer join " & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & ".dbo.TMERMStyle s  on t.FNHSysStyleId = s.FNHSysStyleId"
        _Qry &= vbCrLf & " WHERE     (t.FTPOref = N'" & HI.UL.ULF.rpQuoted(CustPONo) & "')  and o.FNHSysCmpId=" & Val(HI.ST.SysInfo.CmpID)
        _Qry &= vbCrLf & " GROUP BY t.FTOrderNo "
        _Qry &= vbCrLf & " 	 , t.FTSubOrderNo"
        _Qry &= vbCrLf & " 	 , t.FTPOref"
        _Qry &= vbCrLf & " 	 , t.FNHSysCountryId"
        _Qry &= vbCrLf & " 	 , t.FDShipDate , s.FTStyleCode , t.FNHSysStyleId"
        ...
        For Each R As DataRow In _dt.Rows
            ...
            _Qry = " Exec [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.SP_Get_SubOrderBreakDown_Pack_ByCustPO_Partial '" & HI.UL.ULF.rpQuoted(CustPONo) & "','" & HI.UL.ULF.rpQuoted(R!FTOrderNo.ToString) & "','" & HI.UL.ULF.rpQuoted(R!FTSubOrderNo.ToString) & "','0','" & HI.UL.ULF.rpQuoted(PackOrderNo.ToString) & "'"
            _dtSub = HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN)

            _Qry = " Exec [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MERCHAN) & "].dbo.SP_Get_SubOrderBreakDown_Pack_ByCustPO_Partial '" & HI.UL.ULF.rpQuoted(CustPONo) & "','" & HI.UL.ULF.rpQuoted(R!FTOrderNo.ToString) & "','" & HI.UL.ULF.rpQuoted(R!FTSubOrderNo.ToString) & "' ,'1','" & HI.UL.ULF.rpQuoted(PackOrderNo.ToString) & "'"
            _ListDataSubOrderOverShip.Add(HI.Conn.SQLConn.GetDataTable(_Qry, Conn.DB.DataBaseName.DB_MERCHAN))
            ...
            _Qry &= vbCrLf & "  and  H.FTPackNo='" & HI.UL.ULF.rpQuoted(Me.PackOrderNo) & "'"
            ...
            For Each Rx As DataRow In _dtProd.Select("FTOrderProdNo<>'" & HI.UL.ULF.rpQuoted(Me.PackOrderNo) & "'")
                ...
                For Each Rx2 As DataRow In M1.Select("FTColorway='" & HI.UL.ULF.rpQuoted(Rx!FTColorway.ToString) & "' and FTPOLine ='" & HI.UL.ULF.rpQuoted(Rx!FTPOLine.ToString) & "'")
                    ...
                    If M1.Columns.IndexOf(Rx!FTSizeBreakDown.ToString) >= 0 Then
                        Rx2.Item(Rx!FTSizeBreakDown.ToString) = IIf((Rx2.Item(Rx!FTSizeBreakDown.ToString) - Val(Rx!FNQuantity)) < 0, 0, (Rx2.Item(Rx!FTSizeBreakDown.ToString) - Val(Rx!FNQuantity)))
            ...
            For Each Rx As DataRow In _dtProd.Select("FTOrderProdNo='" & HI.UL.ULF.rpQuoted(Me.PackOrderNo) & "'")
                For Each Rx2 As DataRow In M3.Select("FTColorway='" & HI.UL.ULF.rpQuoted(Rx!FTColorway.ToString) & "'  and FTPOLine ='" & HI.UL.ULF.rpQuoted(Rx!FTPOLine.ToString) & "'")
...
#End Region
        ...
        Try
            ...
            Else
                With CType(sender.Parent.MainView, DevExpress.XtraGrid.Views.Grid.GridView)
                    ...
                    _LineItem = .GetFocusedRowCellValue("FTPOLine")
                    For Each R As DataRow In _ListDataSubOrderOverShip(Me.ogvsub.FocusedRowHandle).Select("FTColorway='" & HI.UL.ULF.rpQuoted(_FTColorway) & "' and FTPOLine ='" & HI.UL.ULF.rpQuoted(_LineItem) & "' ")
                        Try
    ...
    Private Sub ocmcreate_Click(sender As Object, e As EventArgs) Handles ocmcreate.Click
        Try
            ...
            If Not VerifyData() Then
                HI.MG.ShowMsg.mInfo("กรุณาเลือก sub order ให้ครบ Set  !!!", 2206021647, Me.Text)
                Exit Sub
            End If
            ...
            End If
        Catch ex As Exception
            HI.MG.ShowMsg.mInfo("คีย์ยอด สร้างใบแพ็คไม่ถูกต้อง", 2005271652, Me.Text)
        End Try
    End Sub
    ...
    Private Function VerifyData() As Boolean
        Try
            If PackSetType = 1 Then
                Dim _cmd As String = ""
                With DirectCast(Me.ogcsub.DataSource, DataTable)
                    .AcceptChanges()
                    For Each R As DataRow In .Select("FTStateSelect='1'")
                        _cmd = "Select  isnull( FTStateStyleSet ,'0') as FTStateStyleSet  from [" & HI.Conn.DB.GetDataBaseName(Conn.DB.DataBaseName.DB_MASTER) & "].dbo.tmermstyle with(nolock) where FTStyleCode = '" & R!FNHSysStyleId.ToString & "'"
                ...
                If HI.Conn.SQLConn.GetField(_cmd, Conn.DB.DataBaseName.DB_MASTER, "").ToString <> "0" Then
                    ...
                    Return CType(Me.ogcsub.DataSource, DataTable).Select("FTStateSelect='1'").Length >= Me.packSetValue
